...
//--------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_print_detalle_asignaciones($la_data,$ai_total,$as_totalletras,&$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('titulo'=>$ls_titulo.'</b>              ','total'=>$ai_total));
	...
	function uf_print_detalle_presupuestario($la_data,&$io_pdf)
	{
		...
		$la_columna=array('ano'=>'<b>Año</b>',
   						  'proyecto'=>$_SESSION["la_empresa"]["nomestpro1"].'</b>',
						  'especifica'=>$_SESSION["la_empresa"]["nomestpro2"].'</b>',
	...
	else // Imprimimos el reporte
	{
		...
		for($li_z=1;$li_z<=$li_totrowsol;$li_z++)
		{
			...
			$lb_valido=$io_report->uf_select_solicitudpago_personal($ls_codemp,$ls_codsoldes,$ls_codsolhas,
			                                                        $ld_desde,$ld_hasta,$li_orden,$ls_codsolvia,
																	$li_rdnomina); // Cargar el DS con los datos de la cabecera del reporte
