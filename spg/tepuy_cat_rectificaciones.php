...
{
		$ls_sql="SELECT procede,comprobante,descripcion,fecha,cod_pro,ced_bene,estapro, codfuefin, coduac FROM tepuy_cmp_md 
		WHERE codemp='".$ls_codemp."' AND comprobante like '".$ls_comprobante."' AND procede='SPGREC'";
	...
	if($row=$SQL_cmp->fetch_row($rs_cta))
	{
		...
		for($z=1;$z<=$totrow;$z++)
		{
			...
			$ls_codfuefin = $data["codfuefin"][$z];
				...
				print "<td><a href=\"javascript: uf_aceptar('$ls_comprobante','$ls_descripcion','$ls_procedencia','$ls_fecha','$ls_tip','$ls_prov','$ls_bene','$li_estapro','$ls_codfuefin','$ls_coduniadm');\">".$ls_comprobante."</a></td>";
...
<script language="JavaScript">
  ...
  function uf_aceptar(comprobante,descripcion,procede,fecha,tipo,prov,bene,estapro,codfuefin,coduniadm)
  {
  ...
  function ue_search()
  {
	  ...
	  f.action="tepuy_cat_rectificaciones.php";
	  f.submit();
  }
