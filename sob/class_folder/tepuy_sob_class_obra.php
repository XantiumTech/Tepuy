...
function tepuy_sob_class_obra()
{
	...
	function uf_guardar_obra($as_codobr,$as_codten,$as_codtipest,$as_codest,$as_codmun,$as_codpar,$as_codcom,$as_codsiscon,$as_codpro,$as_codtob,$as_desobr,$as_dirobr,$as_obsobr,$as_resobr,$ad_feciniobr,$ad_fecfinobr,$ai_monto,$ad_feccreobr,$aa_seguridad)
	{
		//////////////////////////////////////////////////////////////////////////////
...
function uf_update_cantidadpartida ($as_codobr,$as_codpar,$ad_cantidad,$aa_seguridad)
	...
	function uf_select_fuentesfinanciamiento ($as_codobr,&$aa_data,&$ai_rows)
	{
		...
		$ls_sql="SELECT  ffo.codemp,ffo.codobr,ffo.codfuefin, ff.denfuefin,ffo.monto
				 FROM sob_fuentefinanciamientoobra ffo, tepuy_fuentefinanciamiento ff
	...
	function uf_guardar_dtfuentesfinanciamiento($as_codobr,$as_codfuefin,$ad_monto,$aa_seguridad)
	{
		...
		if($li_row===false)
		{			
			print "Error en metodo uf_guardar_dtfuentes".$this->io_function->uf_convertirmsg($this->io_sql->message);
			$this->io_sql->rollback();	
		}
		else
		{
...
// FUNCION QUE ME LLEVA EL MONTO DEL ANTICIPO
function uf_montoanticipo ($as_sql,$quellevo)
{
	//////////////////////////////////////////////////////////////////////////////
	...
	else
	{
		if($la_row=$this->io_sql->fetch_row($rs_data))
		{
			if($quellevo==='M')
			{
				$li_estado=$la_row["monto"];
			}
			else
			{
				if($quellevo==='F')
				{
					$li_estado=$la_row["fecintant"];
				}
				else
				{
					$li_estado=$la_row["numrecdoc"];
				}
			}
...
function uf_codcontrato ($as_sql,$quellevo)
{
	//////////////////////////////////////////////////////////////////////////////
	//	Function:	    uf_contabilizado
	// Access:			public
	...
	if($rs_data===false)
	{
		print "Error en uf_codcontrato".$this->io_function->uf_convertirmsg($this->io_sql->message);
	}
	else
	{
		if($la_row=$this->io_sql->fetch_row($rs_data))
		{
			if($quellevo==='C')
			{
				$li_estado=$la_row["precon"];
			}
			else
			{
				$li_estado=$la_row["codcon"];
			}
...
////////////////////////// FIN DE LA BUSQUEDA DEL CONTRATO ///////////////////////

//////// FUNCION QUE ME LLEVA EL CODIGO DEL CONTRATO ////////////
function uf_obtengodatos ($as_sql,$quetraigo)
{
	//////////////////////////////////////////////////////////////////////////////
	//	Function:	    uf_contabilizado
	// Access:			public
