...
<input name="txtfechas" type="text" id="txtfechas" value="<?php print $ld_fechas ?>" size="13" maxlength="10" datepicker="true" onkeypress="currencyDate(this);" style="text-align:left"></td>
    ...
    <tr>
      ...
      <td height="22"><label>
        ...
        <img src="../shared/imagebank/tools15/buscar.png" name="buscar" width="15" height="15" id="buscar" style="visibility:hidden" onclick="javascript:uf_catalogo_proveedores();" /></label></td>
    ...
    <tr>
      ...
      <td height="22">&nbsp;</td>
      <td height="22">&nbsp;</td>
      <td height="22"><div align="right"><a href="javascript: ue_search();"><img src="../shared/imagebank/tools20/buscar.png" alt="Buscar" width="20" height="20" border="0" onclick="ue_search()" />Buscar Solicitud</a></div></td>
    </tr>
  </table>
	<p>
...
{
   window.open('tepuy_soc_cat_unidad_ejecutora.php?tipo=BUSCAR_CAT_SEP',"_blank","menubar=no,toolbar=no,scrollbars=yes,width=550,height=400,location=no,resizable=yes");
}
...
function ue_aceptar(as_numsol,as_codtipsol,as_coduniadm,as_codfuefin,as_estsoa,as_consol,as_tipo_destino,as_codpro,as_denuniadm,
					as_denfuefin,as_nompro,as_estapro,ad_fecregsoa,ai_monto,ai_monbasinm,ai_montotcar,as_estatus,as_estope,
					as_modsep,as_codestpro1,as_codestpro2,as_codestpro3,as_codestpro4,as_codestpro5)
{
    ...
    if (ls_coduniadm=="")
	   {
	     opener.document.formulario.txtcoduniadm.value = as_coduniadm;
		 opener.document.formulario.txtdenuniadm.value = as_denuniadm;
	   }	
	else
	   {
	     if (ls_coduniadm!='----------' && ls_coduniadm!=as_coduniadm)
		    {
	          opener.document.formulario.txtcoduniadm.value = '----------';
		      opener.document.formulario.txtdenuniadm.value = 'NINGUNA';
			}
	   }
	...
	if(valido)
	{
	     ...
	     ls_uniejeaso = ls_uniejeaso+" "+"Nro. SEP:"+as_numsol+". Unidad Ejecutora: "+as_coduniadm+" - "+as_denuniadm+";";
	...
	if(as_modsep=="B") // Bienes
	{
		...
		for(j=1;(j<=rowbienes)&&(valido);j++)
		{
			...
			parametros=parametros+"&txtcodart"+j+"="+codart+"&txtdenart"+j+"="+denart+""+
					   ...
					   "&txtdenuniadmsep"+j+"="+denuniadmsep+"&txtestpreunieje"+j+"="+ls_estpreunieje+""+
					   "&txtdenunimed"+j+"="+ls_denunimed;
	...
	for(j=1;(j<rowcuentas)&&(valido);j++)
	{
		...
		parametros=parametros+"&txtprogramaticagas"+j+"="+programaticagas+"&txtcodprogas"+j+"="+codprogas+
		           "&txtcuentagas"+j+"="+cuentagas+"&txtmoncuegas"+j+"="+moncuegas;
	}
	...
	for(j=1;(j<rowcuentas)&&(valido);j++)
	{
		...
		parametros=parametros+"&txtprogramaticacar"+j+"="+programaticacar+"&txtcodcargo"+j+"="+cargo+"&txtcodprocar"+j+"="+codpro+
		           "&txtcuentacar"+j+"="+cuenta+"&txtmoncuecar"+j+"="+moncue;
	}
...
function ue_search()
{
	...
	if((ld_fecregdes!="")&&(ld_fecreghas!=""))
	{
		...
		ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
		// Enviar todos los campos a la pagina para que haga el procesamiento
		ajax.send("catalogo=SOLICITUD-PRESUPUESTARIA&numsol="+ls_numsol+"&coduniadm="+ls_coduniadm+"&fecregdes="+ld_fecregdes+
				  "&fecreghas="+ld_fecreghas+"&tipo="+ls_tipo+"&tipord="+ls_tipord+"&orden="+ls_orden+"&campoorden="+ls_campoorden);
	}
	else
	{
