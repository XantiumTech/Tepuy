...
require_once("../../class_folder/dao/tepuy_srh_c_personal.php");
$io_personal=new tepuy_srh_c_personal('../../../');
require_once("../../class_folder/dao/tepuy_srh_c_pais.php");
$io_pais=new tepuy_srh_c_pais('../../../');
require_once("../../class_folder/dao/tepuy_srh_c_estado.php");
$io_estado=new tepuy_srh_c_estado('../../../');
require_once("../../class_folder/dao/tepuy_srh_c_municipio.php");
$io_municipio=new tepuy_srh_c_municipio('../../../');
require_once("../../class_folder/dao/tepuy_srh_c_parroquia.php");
$io_parroquia=new tepuy_srh_c_parroquia('../../../');
...
{  
    $valido= $io_personal->uf_srh_eliminar_beneficiario($_GET["codben"],$_GET["codper"],$la_seguridad);  
	if ($valido)
	{
...
{  
  ...
  else 
  {
	   if ($valido)
	   {
			if ($_POST["insmod"]=='modificar')
			{
				$ls_salida = 'No puede actualizar la Deduccion de Personal. Si desea cambiar la asignacion debe eliminarla y asignar otra.';	
			}
			else
			{
...
{  
    ...
    $valido_ded= $io_personal->uf_srh_guardar_deducion_familiar($lo_deduccion,$_POST["insmod"],$la_seguridad);
	if ($valido_ded)
	{
		 if ($_POST["insmod"]=='modificar')
		  {$ls_salida = 'La Deduccion del Familiar fue Actualizada';	}
		 else { $ls_salida = 'La Deduccion del Familiar fue Registrada';}
	 }
	 else {$ls_salida = 'Error al Guardar la Deducción del Familiar';} 
  }  
...
{  
    $ls_salida =$io_personal->uf_srh_calcular_monto_deduccion ($_GET["codper"], $_GET["codtipded"]);

}

elseif ($ls_operacion == "calcular_monto_deduccion_fam")
{  
    $ls_salida =$io_personal->uf_srh_calcular_monto_deduccion_fam ($_GET["codper"], $_GET["codtipded"], $_GET["cedfam"]);
