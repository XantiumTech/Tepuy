<?php
	...
	$oi_fun_integrador->uf_load_seguridad("SCB","tepuy_mis_p_reverso_scbcol.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		$lo_title[5]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Movimientos de Colocaciones Contabilizados";
		$ls_nametable="grid";
...
<html>
<head>
<title>Reverso de Colocaciones</title>
...
<?php
	...
	require_once("class_folder/class_tepuy_scbcol_integracion.php");  	
	$in_class = new class_tepuy_scbcol_integracion();  
	uf_limpiarvariables();
	switch ($ls_operacion) 
	{
		case "NUEVO":
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_reverso_colocacion($ls_codban,$ls_ctaban,$ls_numcol,$ls_numdoc,
																		 $ls_codope,$ls_estcol,$ls_fecmov,$ld_fechaconta,
																		 $la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("El movimiento de colocación ".$ls_numdoc." fue reversado.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo reversar el movimiento de colocacion ".$ls_numdoc);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_colocacion_contabilizar($ls_numdoc,$ls_fecdoc,$ls_tipo_operacion,$lo_object,$li_totrows,"C");
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
<table width="762" border="0" align="center" cellpadding="0" cellspacing="0" class="contorno">
  ...
  <tr>
    <td width="432" height="20" colspan="11" bgcolor="#E7E7E7"><table width="762" border="0" align="center" cellpadding="0" cellspacing="0">
      <td width="432" height="20" bgcolor="#E7E7E7" class="descripcion_sistema">Caja y Banco -><i>Reversa Contabilizacion de Colocaciones</i></td>
...
?>	
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Reverso  de Contabilizaci&oacute;n de Colocaciones </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="tepuy_mis_p_reverso_scbcol.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(numdoc,codban,ctaban,codope,numcol)
{
	...
	window.open("tepuy_mis_pdt_scbcol.php?numdoc="+numdoc+"&codban="+codban+"&ctaban="+ctaban+"&codope="+codope+"&numcol="+numcol+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
