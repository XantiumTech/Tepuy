...
$io_fun_banco->uf_load_seguridad("SCB","tepuy_scb_r_conciliacion.php",$ls_permisos,$la_seguridad,$la_permisos);
$ls_reporte = $io_fun_banco->uf_select_config("SCB","REPORTE","FORMATO_CONCILIACION","tepuy_scb_rpp_conciliacion_pdf.php","C");
$li_diasem = date('w');
switch ($li_diasem){
...
$title[7] = "Saldo Banco";    
	...
	function uf_cargar_conciliacion($li_row)
	{
		...
		$li_row = 0;
		$ls_sql = "SELECT a.codban,a.nomban,b.ctaban,b.dencta,c.mesano,c.salseglib,c.salsegbco,c.estcon
				     FROM scb_banco a,scb_ctabanco b,scb_conciliacion c
				    WHERE c.estcon=1
					  AND a.codban=b.codban 
					  AND a.codemp=b.codemp 
					  AND a.codban=c.codban 
					  AND b.ctaban=c.ctaban";
...
?>
  <table width="439" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    ...
    <tr class="titulo-ventana">
      <td  height="22" colspan="2" align="center">Conciliacion Bancaria <span style="text-align:left">
		       ...
		       if (f.chksel[i].checked)	
				  {
					...
					if (ls_mesano!="")
					   {
						 pagina="reportes/"+ls_reporte+"?codban="+ls_codban+"&ctaban="+ls_ctaban+"&mesano="+ls_mesano+"&salseglib="+ldec_salseglib+"&salsegbco="+ldec_salsegbco+"&nomban="+ls_nomban+"&tiporeporte="+ls_tiporeporte;
