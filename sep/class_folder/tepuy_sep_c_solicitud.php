...
class tepuy_sep_c_solicitud
 {
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function tepuy_sep_c_solicitud($as_path)
	{	
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: tepuy_sep_c_solicitud
		//		   Access: public 
	...
	}// end function tepuy_sep_c_solicitud
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_load_tiposolicitudayuda($as_seleccionado)
	{
		//////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql="SELECT codtipsol, dentipsol, estope, modsep ".
				...
				" ORDER BY codtipsol  ASC LIMIT 8";	
		//print $ls_sql;
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Solicitud MÉTODO->uf_load_tiposolicitudayuda ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_valido=false;
		}
		else
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_numero_ayudas_previas($as_codprovben)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_numero_ayudas_previas($as_codprovben)
		//		   Access: public
		...
		$this->io_sql->free_result($rs_data);
		return $totalayudaprevia;
	}// end function uf_numero_ayudas_previas

	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_buscar_ayudas_previas($as_codprovben,&$totalayudaprevia)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_buscar_ayudas_previas($as_codprovben)
		//		   Access: public
		//	    Arguments: as_codprovben  // cedula del beneficiario 
		//				   aa_seguridad  // arreglo de las variables de seguridad
		//	      Returns: lb_valido True si se ejecuto el insert ó False si hubo error en el insert
		...
		{
			//$this->io_mensajes->message("CLASE->Solicitud MÉTODO->uf_buscar_ayudas_previas->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_valido=true;
		}
		else
		{
		...
		return $lb_valido;
	}// end function uf_buscar_ayudas_previas
...
//-------------------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_insert_solicitud($ad_fecregsol,&$as_numsol,$as_coduniadm,$as_codfuefin,$as_tipodestino,$as_codprov,$as_cedben,$as_consol,
								 ...
								 $as_campo,$ai_totrowservicios,$ai_totrowconceptos,$aa_seguridad,$as_ayuda)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($lb_valido)
		{
			$ls_sql="INSERT INTO sep_solicitud (codemp,numsol,codtipsol,coduniadm,fecregsol,estsol,consol,monto,".
					" 							monbasinm,montotcar,cod_pro,ced_bene,tipo_destino,codfuefin,estapro,ayuda)".
					"	  VALUES ('".$this->ls_codemp."','".$as_numsol."','".$as_codtipsol."','".$as_coduniadm."',".
					" 			  '".$ad_fecregsol."','R','".$as_consol."',".$ai_total.",".$ai_subtotal.",".$ai_cargos.",".
					"			  '".$as_codprov."','".$as_cedben."','".$as_tipodestino."','".$as_codfuefin."',0,".$as_ayuda.")";       // print $ls_sql;
			$this->io_sql->begin_transaction();				
			 ...
			 for ($li_i=1;$li_i<=$li_totrowcre;$li_i++)
			     {
				   ...
				   if ($li_row===false)
				      {
						$lb_valido = false;
				        $this->io_mensajes->message("CLASE->tepuy_sep_c_solicitud.php;MÉTODO->uf_insert_cargos (IVA Contable) ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
				      }
				   else
					  {
					    /////////////////////////////////         SEGURIDAD               /////////////////////////////		
...
//				"	    (SELECT (asignado-(comprometido+precomprometido)+aumento-disminucion) ".
	...
	function uf_guardar($as_existe,$ad_fecregsol,&$as_numsol,$as_coduniadm,$as_codfuefin,$as_tipodestino,$as_codprovben,$as_consol,
						...
						$ai_totrowconceptos,$aa_seguridad,&$as_estsol,$as_permisosadministrador,$es_ayuda)
	{		
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_guardar
		//		   Access: public (tepuy_sep_p_solicitud.php)
		...
		switch ($as_existe)
		{
			case "FALSE":
					...
					$lb_valido=$this->uf_insert_solicitud($ad_fecregsol,&$as_numsol,$as_coduniadm,$as_codfuefin,$as_tipodestino,
														  ...
														  $ai_totrowconceptos,$aa_seguridad,$es_ayuda);
				break;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_delete_solicitud($as_numsol,$as_cargos,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
