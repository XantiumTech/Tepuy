...
$io_fun_banco->uf_load_seguridad("SCB","tepuy_scb_r_comp_ret_mun.php",$ls_permisos,$la_seguridad,$la_permisos);
$ls_reporte= $io_fun_banco->uf_select_config("SCB","REPORTE","COMPROBANTE_MUN","tepuy_scb_rpp_comp_ret_mun_pdf.php","C");
$li_diasem = date('w');
switch ($li_diasem){
	 ...
	 if ($ls_fuente!=false)
	    {
		  if ($ls_fuente=="C")
		     {
			   print "<script>";
			   print "alert('Los comprobantes deben ser visualizados a través del módulo de Cuentas por Pagar');";
  ...
  <table width="524" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    ...
    <tr>
      <td colspan="2" align="center">
        <p>&nbsp;</p>
        <?php $io_grid->makegrid($li_total,$title,$object,400,'Comprobantes de Retención Municipal',$grid);?></td>
    </tr>
  </table>
  ...
  if (li_imprimir=='1')
	 {
	   for (li_i=1;li_i<=li_total;li_i++)
		...
		if (ls_documentos.length>0)
		   {
			 pagina="reportes/"+ls_reporte+"?documentos="+ls_documentos+"&mes="+ls_mes+"&agno="+ls_agno+"&tiporeporte="+ls_tiporeporte;  	
