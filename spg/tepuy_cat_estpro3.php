...
<form name="form1" method="post" action="">
  <p align="center">
	...
	if($ls_operacion=="BUSCAR")
	{
		...
		{
		 $ls_sql_seguridad = " AND CONCAT('".$ls_codemp."','SPG','".$ls_logusr."',codestpro1,codestpro2,codestpro3) IN (SELECT CONCAT(codemp,codsis,codusu,substr(codintper,1,29)) 
		                       FROM sss_permisos_internos WHERE codusu = '".$ls_logusr."' AND codsis = 'SPG')";
		}
		else
		{
		 $ls_sql_seguridad = " AND '".$ls_codemp."'||'SPG'||'".$ls_logusr."'||codestpro1||codestpro2||codestpro3 IN (SELECT codemp||codsis||codusu||substr(codintper,1,29)
		                       FROM sss_permisos_internos WHERE codusu = '".$ls_logusr."' AND codsis = 'SPG')";
		}
