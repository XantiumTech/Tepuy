<?php
	...
	// proceso a ejecutar
	$ls_proceso=$io_fun_compra->uf_obtenervalor("proceso","");
	// total de filas de Bienes	
	switch($ls_proceso)
	{
		case "AGREGARCOTIZACIONES":
			$li_totalcotizaciones=$io_fun_compra->uf_obtenervalor("totalcotizaciones","1");			
		...
		case "ACTUALIZARITEMS":
			$li_totalitems=$io_fun_compra->uf_obtenervalor("total","0");
	...
	{	
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		global $io_grid, $io_fun_compra, $io_funciones;
		$lo_object=array();
		...
		for($li_fila=1;$li_fila<=$ai_total;$li_fila++)
		{
			...
			{
				$ls_numcot	  = $io_fun_compra->uf_obtenervalor("txtnumcot".$li_fila,"");
				$ls_numsol	  = $io_fun_compra->uf_obtenervalor("txtnumsol".$li_fila,"");
				$ls_nompro	  = utf8_decode($io_fun_compra->uf_obtenervalor("txtnompro".$li_fila,""));
				$ls_codpro	  = $io_fun_compra->uf_obtenervalor("txtcodpro".$li_fila,"");
				$ld_fecha	  = $io_fun_compra->uf_obtenervalor("txtfecha".$li_fila,"");
				$li_monto	  = $io_fun_compra->uf_obtenervalor("txtmonto".$li_fila,"");
				$li_iva		  = $io_fun_compra->uf_obtenervalor("txtiva".$li_fila,"");
				$ls_tipsolcot = $io_fun_compra->uf_obtenervalor("txttipsolcot".$li_fila,"");						
			}
			...
			else
			{
				...
				$lo_object[$li_fila][8]="<a href=javascript:ue_delete_cotizacion(".$li_fila.")><div align=center><img src=../shared/imagebank/tools15/eliminar.png title=Eliminar Cotización width=15 height=15 border=0></div></a>";
			}			
		}			
	...
	{	
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		global $io_analisis_cotizacion;
		global $io_grid, $io_fun_compra;
		$lo_object=array();
		//Se determita si el analisis es de bienes o servicios
		$ls_tipsolcot=$io_fun_compra->uf_obtenervalor("txttipsolcot1","");
		if($ls_tipsolcot=="B")
			$ls_titulo="Bienes";
		else
			$ls_titulo="Servicios";
		...
		$li_totrows = $io_fun_compra->uf_obtenervalor("hidtotrows","0");
		for ($li_fila=1;$li_fila<=$li_totrows;$li_fila++)
		    {
			  $ls_coditem     = $io_fun_compra->uf_obtenervalor("txtcoditem".$li_fila,"");
			  $ls_nomitem     = $io_fun_compra->uf_obtenervalor("txtnomitem".$li_fila,"");
			  $ls_nompro      = $io_fun_compra->uf_obtenervalor("txtnomproitem".$li_fila,"");
			  $ls_codpro      = $io_fun_compra->uf_obtenervalor("txtcodproselec".$li_fila,""); 
			  $ld_cantidad    = $io_fun_compra->uf_obtenervalor("txtcanselec".$li_fila,"");
			  $ld_precio      = $io_fun_compra->uf_obtenervalor("txtpreuniselec".$li_fila,"");
			  $ld_moniva      = $io_fun_compra->uf_obtenervalor("txtivaselec".$li_fila,"");
			  $ld_monto       = $io_fun_compra->uf_obtenervalor("txtmonselec".$li_fila,"");
			  $ls_observacion =	$io_fun_compra->uf_obtenervalor("txtobservacion".$li_fila,"");		  
			  $ls_numcot      = $io_fun_compra->uf_obtenervalor("txtnumcotsele".$li_fila,"");
		...
		$ls_numsolcot = $io_fun_compra->uf_obtenervalor("hidnumcot","");
		$ls_codpro    = $io_fun_compra->uf_obtenervalor("hidcodpro","");
		if($ab_catalogo)//si el cargar biene de seleccionar cotizaciones una a una
			uf_cargar_items($la_items);			
		else
			uf_select_items_cotizacion($ls_numsolcot,$ls_codpro,$ls_tipsolcot,$la_items);
	...
	{	
		...
		// Titulos del Grid de Cotizaciones
		//Se determita si el analisis es de bienes o servicios
		$ls_tipsolcot=$io_fun_compra->uf_obtenervalor("tipsolcot1","");
		$li_total=$io_fun_compra->uf_obtenervalor("total","");
		if($ls_tipsolcot=="B")
			$ls_titulo="Bienes";
		else
			$ls_titulo="Servicios";
		...
		{
			$ls_coditem		= $io_fun_compra->uf_obtenervalor("txtcoditem".$li_fila,"");
			$ls_nomitem		= utf8_decode($io_fun_compra->uf_obtenervalor("txtnomitem".$li_fila,""));
			$ls_nomproitem	= utf8_decode($io_fun_compra->uf_obtenervalor("txtnomproitem".$li_fila,""));
			$ls_canselec	= $io_fun_compra->uf_obtenervalor("txtcanselec".$li_fila,"");
			$ls_preuniselec = $io_fun_compra->uf_obtenervalor("txtpreuniselec".$li_fila,"");
			$ls_ivaselec	= $io_fun_compra->uf_obtenervalor("txtivaselec".$li_fila,"");
			$ls_monselec	= $io_fun_compra->uf_obtenervalor("txtmonselec".$li_fila,"");
			$ls_observacion = utf8_decode($io_fun_compra->uf_obtenervalor("txtobservacion".$li_fila,""));
			$ls_numcot		= $io_fun_compra->uf_obtenervalor("txtnumcotsele".$li_fila,"");
			$ls_codpro		= $io_fun_compra->uf_obtenervalor("txtcodproselec".$li_fila,"");
	...
	function uf_select_items_cotizacion($as_numcot,$as_codpro,$as_tipcot,&$aa_items)
	{
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		global $ls_empresa,$io_sql,$io_mensajes,$io_funciones,$io_fun_compra;
		$aa_items  = array();
		$lb_valido = false;
		$ls_sqlaux = $ls_strsql = "";
		$ls_strsql = $io_fun_compra->uf_obtenervalor("hidsqlaux","");
		...
		if ($rs_data===false)
		   {
		     $io_mensajes->message("CLASS->tepuy_soc_c_analisis_cotizacion_ajax.php;Metodo->;ERROR->uf_select_items_cotizacion".$io_funciones->uf_convertirmsg($io_sql->message)); 
			 $lb_valido=false;	
		   }
		else
		   {
	...
	function uf_cargar_cotizaciones()
	{
		...
		// 		    Fecha: 10/06/2007								Fecha Última Modificación : 15/04/2008.
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$aa_items=array();
		$lb_valido=false;
		$ls_numanacot=$io_fun_compra->uf_obtenervalor("numanacot","");				
		$ls_sql= "SELECT soc_analisicotizacion.numanacot, soc_analisicotizacion.fecanacot, soc_analisicotizacion.obsana,
						 soc_analisicotizacion.tipsolcot, soc_analisicotizacion.numsolcot, soc_cotxanalisis.numcot,
						 soc_cotxanalisis.cod_pro, soc_cotizacion.feccot, soc_cotizacion.montotcot, soc_cotizacion.poriva,
						 rpc_proveedor.nompro
				    FROM soc_analisicotizacion, soc_cotizacion, rpc_proveedor, soc_cotxanalisis
				   WHERE soc_analisicotizacion.codemp='".$ls_empresa."' 
				     AND soc_analisicotizacion.numanacot='".$ls_numanacot."'
	...
	function uf_cargar_items(&$aa_items)
	{
		...
		{				
			$ls_sql="SELECT soc_dtac_bienes.codart as codigo, siv_articulo.denart as denominacion, rpc_proveedor.nompro, 
							soc_dtcot_bienes.canart as cantidad, soc_dtcot_bienes.preuniart as precio, soc_dtcot_bienes.moniva,
							soc_dtcot_bienes.montotart as monto, soc_dtac_bienes.obsanacot, soc_dtac_bienes.numcot, soc_dtac_bienes.cod_pro
					   FROM soc_dtac_bienes, siv_articulo, rpc_proveedor, soc_dtcot_bienes
					  WHERE soc_dtac_bienes.codemp='".$ls_empresa."' 
					    AND soc_dtac_bienes.numanacot='".$ls_numanacot."' 
						AND soc_dtac_bienes.codemp=siv_articulo.codemp 
						AND siv_articulo.codemp=rpc_proveedor.codemp 
						AND rpc_proveedor.codemp=soc_dtcot_bienes.codemp 
						AND	soc_dtac_bienes.codart=siv_articulo.codart 
						AND soc_dtac_bienes.cod_pro=rpc_proveedor.cod_pro 
						AND soc_dtac_bienes.numcot=soc_dtcot_bienes.numcot 
						AND soc_dtac_bienes.cod_pro=soc_dtcot_bienes.cod_pro 
						AND soc_dtac_bienes.codart=soc_dtcot_bienes.codart";				
		}
		elseif($ls_tipsolcot=="Servicios" || $ls_tipsolcot=="S")
		{
				$ls_sql="SELECT soc_dtac_servicios.codser as codigo,
				                soc_servicios.denser as denominacion,
								rpc_proveedor.nompro,
								soc_dtcot_servicio.canser as cantidad,
								soc_dtcot_servicio.monuniser as precio,
								soc_dtcot_servicio.moniva,
								soc_dtcot_servicio.montotser as monto,
					            soc_dtac_servicios.obsanacot,
								soc_dtac_servicios.numcot,
								soc_dtac_servicios.cod_pro
					       FROM soc_dtac_servicios, soc_servicios, rpc_proveedor, soc_dtcot_servicio
					      WHERE soc_dtac_servicios.codemp='".$ls_empresa."'
						    AND soc_dtac_servicios.numanacot='".$ls_numanacot."'
							AND soc_dtac_servicios.codemp=soc_servicios.codemp
							AND soc_servicios.codemp=rpc_proveedor.codemp
							AND rpc_proveedor.codemp=soc_dtcot_servicio.codemp
							AND	soc_dtac_servicios.codser=soc_servicios.codser
							AND soc_dtac_servicios.cod_pro=rpc_proveedor.cod_pro
							AND soc_dtac_servicios.numcot=soc_dtcot_servicio.numcot
							AND soc_dtac_servicios.cod_pro=soc_dtcot_servicio.cod_pro
							AND soc_dtac_servicios.codser=soc_dtcot_servicio.codser";				
		}
