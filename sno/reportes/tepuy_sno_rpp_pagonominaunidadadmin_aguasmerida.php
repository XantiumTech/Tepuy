<?php
	...
	function uf_insert_seguridad($as_titulo,$as_desnom,$as_periodo,$ai_tipo)
	{
		...
		if($ai_tipo==1)
		{
			$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte_nomina("SNO","tepuy_sno_r_pagonominaunidadadmin.php",$ls_descripcion,$ls_codnom);
		}
		else
		{
			$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte_nomina("SNO","tepuy_sno_r_hpagonominaunidadadmin.php",$ls_descripcion,$ls_codnom);
		}
		return $lb_valido;
	}
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	else // Imprimimos el reporte
	{
		...
		$li_totrowuni=$io_report->DS->getRowCount("minorguniadm");
		for($li_k=1;(($li_k<=$li_totrowuni)&&($lb_valido));$li_k++)
		{
			$li_totasi=0;
			$li_totded=0;
			$li_totapo=0;
			$li_totgeneral=0;
			$ls_minorguniadm=$io_report->DS->data["minorguniadm"][$li_k];
			$ls_ofiuniadm=$io_report->DS->data["ofiuniadm"][$li_k];
			$ls_uniuniadm=$io_report->DS->data["uniuniadm"][$li_k];
			$ls_depuniadm=$io_report->DS->data["depuniadm"][$li_k];
			$ls_prouniadm=$io_report->DS->data["prouniadm"][$li_k];
			$ls_desuniadm=$io_report->DS->data["desuniadm"][$li_k];
			...
			$li_totrow=$io_report->DS_detalle->getRowCount("codper");
			for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
			{
				...
				$li_totalaporte=0;
				$li_total_neto=0;
				$ls_codper=$io_report->DS_detalle->data["codper"][$li_i];
				$ls_cedper=$io_report->DS_detalle->data["cedper"][$li_i];
				$ls_apenomper=$io_report->DS_detalle->data["apeper"][$li_i].", ". $io_report->DS_detalle->data["nomper"][$li_i];
				$ls_descar=$io_report->DS_detalle->data["descar"][$li_i];
				$ls_desuniadm=$io_report->DS_detalle->data["desuniadm"][$li_i];
				$ld_fecingper=$io_funciones->uf_convertirfecmostrar($io_report->DS_detalle->data["fecingper"][$li_i]);
				$ls_codcueban=$io_report->DS_detalle->data["codcueban"][$li_i];
				...
				if($lb_valido)
				{
					$li_totrow_res=$io_report->DS_detalle2->getRowCount("codconc");
					for($li_s=1;$li_s<=$li_totrow_res;$li_s++)
					{
						$ls_codconc=$io_report->DS_detalle2->data["codconc"][$li_s];
						$ls_nomcon=$io_report->DS_detalle2->data["nomcon"][$li_s];
						$ls_tipsal=rtrim($io_report->DS_detalle2->data["tipsal"][$li_s]);
						$li_valsal=abs($io_report->DS_detalle2->data["valsal"][$li_s]);
						switch($ls_tipsal)
						{
							case "A":
