<?php
class tepuy_ins_c_reprocesar_spg
{
	var $io_sql;
	var $io_message;
	var $io_function;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function tepuy_ins_c_reprocesar_spg()
	{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_reprocesar_saldos($ls_codemp,$as_sin_validacion,$aa_seguridad)
	{
		////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		for($li_i=1;($li_i<=$li_total_codigos)&&($lb_valido);$li_i++)		
		{
			...
			$ls_sql="SELECT codestpro1,codestpro2,codestpro3,codestpro4,codestpro5,fecha, spg_cuenta, sum(monto) AS monto ". 
					"  FROM spg_dt_cmp  ".
					" WHERE operacion='".$ls_codigo."' ".
					" GROUP BY codestpro1,codestpro2,codestpro3,codestpro4,codestpro5,spg_cuenta,fecha ".
					" ORDER BY codestpro1,codestpro2,codestpro3,codestpro4,codestpro5,spg_cuenta,fecha ";
			...
			else
			{
				...
				{
					$_SESSION["fechacomprobante"]=$row["fecha"];
					...
					if(!$lb_valido)
					{
						$this->io_message->message("Error al actualizar la cuenta ".$ls_cuenta." con mensaje ".$ls_mensaje." de programatica ".$la_estprog[0]."-".$la_estprog[1]."-".$la_estprog[2]."-".$la_estprog[3]."-".$la_estprog[4]); 			
						//$lb_valido=true;
					}					
				}		
			}									
								  ...
								  &$adec_causado,&$adec_pagado,$as_sin_validacion)
	{
	    ...
	    if($ls_vali_nivel==5)
	    {
			...
			$ls_vali_nivel = $this->io_int_spg->uf_spg_obtener_nivel($ls_formpre);
	    }
	    $lb_valido =true;
	    ...
	    $li_nivel = $this->io_int_spg->uf_spg_obtener_nivel($as_cuenta);
	    $as_mensaje = trim(strtoupper($as_mensaje));
	   
		/*if(trim($as_cuenta)=='4030901000000')
		{
			print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><br>";
	   ...
	   if($as_sin_validacion==0)
	   {
		   ...
		   if (!($li_pos_o===false))
		   {
			 if ($li_nivel < $ls_vali_nivel) 
			 {
				 ...
				 {
					 $lb_valido = false;
// ESTO SALTA EL PROCESO DE CONTROL DEL SALDO COMPROMETIDO
					 $lb_valido = true;
				 $adec_comprometido = $adec_comprometido - $adec_monto_anterior + $adec_monto_actual;
					$this->io_message->message( "Error de disponibilidad Presupuestaria.. en la estructura: ".$ls_estpro." y cuenta ".$as_cuenta);	
				 }			
		  ...
		  if (!($li_pos_c===false))
		  {
			...
			// ME OLVIDO DEL CAUSADO REAL E INCRUSTO EL SALDO DEL COMPROMETIDO
				$li_monto = $adec_comprometido;
...
//					$lb_valido = false;
					$lb_valido = true;
					$this->io_message->message("Intenta Causar mas que lo Comprometido ".$ls_estpro." - ".$as_cuenta." Monto: ".$adec_comprometido  );
				}
			}
			else
			{
		 ...
		 if (!($li_pos_p===false))
		 {
		  ...
		  if (!($li_pos_p===false))
		  {
			...
			// ME OLVIDO DEL PAGADO REAL E INCRUSTO EL SALDO DEL CAUSADO}
				$li_monto = $adec_causado;
...
//					$lb_valido = false;
					$lb_valido = true;
					$this->io_message->message(" Intenta Pagar mas que lo Causado ".$ls_estpro." - ".$as_cuenta);
				}
			}	
			else
			{
