<?php
	...
	//Modificado por: Ing. Miguel Palencia  08/08/2007	
	//-----------------------------------------------------------------------------------------------------------------------------------
    session_start();   
...
//--------------------------------------------------------------------------------------------------------------------------------
							  ...
							  $as_codclavia,$ad_fecsalvia,$ad_fecregvia,$ai_numdiavia,$as_denmis,$ai_acompanante,$as_cargo,&$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('mision'=>'<b>Conceptos del Viatico:</b> '.$as_denmis));
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('titulo'=>'<b>DETALES DEL VIATICOS</b>'));
		$la_columna=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('numero'=>'<b>Número de Días</b>','porcentaje'=>'<b>Porcentaje</b>','diario'=>'<b>Viatico Diario (Bs.)</b>',
							 'total'=>'<b>Total Bolivares</b>'));
		$la_columna=array('numero'=>'','porcentaje'=>'','diario'=>'','total'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
						 'shaded'=>2	, // Sombra entre líneas
						 ...
						 'width'=>540, // Ancho de la tabla
						 'maxWidth'=>540,
						 'cols'=>array('numero'=>array('justification'=>'center','width'=>120),
						               'porcentaje'=>array('justification'=>'center','width'=>220),
									   'diario'=>array('justification'=>'center','width'=>100),
									   'total'=>array('justification'=>'center','width'=>100))); // Ancho Máximo de la tabla
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);	
		unset($la_data);
		unset($la_columna);
		unset($la_config);
	...
	function uf_print_encabezado_detalle_asignaciones(&$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);
	}// end function uf_print_encabezado_detalle_asignaciones
//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_detalle_viatico_alojamiento($la_data,$ai_total,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($ls_tiporeporte==1)
		{
			$ls_titulo="Total Viatico Bs.F.:";
		}
		else
		{
			$ls_titulo="Total Viatico Bs.:";
		}
		$la_columna=array('numero'=>'','porcentaje'=>'','diario'=>'','total'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 8, // Tamaño de Letras
						 ...
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('numero'=>array('justification'=>'center','width'=>120),
						               'porcentaje'=>array('justification'=>'left','width'=>220),
		...
		unset($la_columna);
		unset($la_config);
	}// end function uf_print_detalle_viatico_alojamiento
	//--------------------------------------------------------------------------------------------------------------------------------
	...
	$li_rdnomina=$io_fun_viaticos->uf_obtenervalor_get("rdnomina",0);
	global $ls_tiporeporte;
	require_once("../../shared/ezpdf/class.ezpdf.php");
	if($ls_tiporeporte==1)
	{
		require_once("tepuy_scv_class_reportbsf.php");
		$io_report=new tepuy_scv_class_reportbsf();
	}
	else
	{
		require_once("tepuy_scv_class_report.php");
		$io_report=new tepuy_scv_class_report();
	}	
	//--------------------------------------------------------------------------------------------------------------------------------
	...
	else // Imprimimos el reporte
	{
		...
		for($li_z=1;$li_z<=$li_totrowsol;$li_z++)
		{
			...
			$lb_valido=$io_report->uf_select_solicitudpago_personal($ls_codemp,$ls_codsoldes,$ls_codsolhas,$ld_desde,
																	$ld_hasta,$li_orden,$ls_codsolvia,$li_rdnomina); // Cargar el DS con los datos de la cabecera del reporte
			if($lb_valido==false) // Existe algún error ó no hay registros
			{
				print("<script language=JavaScript>");
				print(" alert('No hay nada que Reportar');"); 
			...
			else // Imprimimos el reporte
			{
				...
				for($li_i=1;$li_i<=$li_totrow;$li_i++)
				{
					...
					$ls_coduniadm= $io_report->ds_solicitud->data["coduniadm"][$li_i];
					$io_report->uf_select_unidadsolicitante($ls_codemp,$ls_coduniadm,&$ls_denuniadm);
					...
					$lb_monto_viatico_alojamiento=$io_report->uf_select_solicitud_personal_spg($ls_codemp,$ls_codsolvia,$ls_cedper);
					///////////////// VIATICO Y ALOJAMIENTO ////////////////
					$ls_porcentaje=0;
					$ls_montodiario=0;
					$ls_numero="";
					for($li_s=1;$li_s<=4;$li_s++)
					{
						$ls_porcentaje=($ls_porcentaje+25);
						if($li_s==4)
						{
							$ls_numero=$li_numdiavia;
							$ls_montodiario=($lb_monto_viatico_alojamiento/$li_numdiavia);
						}
						$li_montodiario=number_format($ls_montodiario,2,",",".");
						$li_subtotal=number_format($lb_monto_viatico_alojamiento,2,",",".");
						$la_data1[$li_s]=array('numero'=>$ls_numero,'porcentaje'=>$ls_porcentaje."%",'diario'=>$ls_montodiario,'total'=>$li_subtotal);
					}
					uf_print_detalle_viatico_alojamiento($la_data1,$li_subtotal,&$io_pdf);
					/////////////// FIN DE VIATICO Y ALOJAMIENTO //////////////

					/////////////// INICIO DE ASIGNACIONES DEL VIATICO //////////////
					uf_print_encabezado_detalle_asignaciones(&$io_pdf);
