...
require_once("class_folder/tepuy_rpc_c_estado.php");
...
$io_estado    = new tepuy_rpc_c_estado($conn);//Instanciando la Clase tepuy Definiciones.
...
//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
	...
	$ls_sistema  = "CFG";
	$ls_ventanas = "tepuy_rpc_d_estado.php";
...
if ($ls_operacion=="NUEVO") 
   {
	 $ls_codigo=$io_estado->uf_generar_codigo($lb_empresa,$ls_empresa,'tepuy_estados','codest',$ls_codpais);
	 if(empty($ls_codigo))
	   {
      ...
      if ($lb_existe)
	     {
	       ...
	       $lb_tiene     = $io_chkrel->uf_check_relaciones($ls_empresa,$ls_condicion,'tepuy_estados',$ls_codigo,$ls_mensaje);//Verifica los movimientos asociados a la cuenta
           if (!$lb_tiene)
		      {
...
function ue_nuevo()
{
  ...
  if (li_incluir==1)
	 {	
	   ...
	   f.action="tepuy_rpc_d_estado.php";
	   f.submit();
	 }
  else
     {
	...
	if (li_leer==1)
	   {
	     if (f.cmbpais.value=="s1")
	        ...
	        {    
		      ...
		      pagina="tepuy_cfg_cat_estados.php?hidpais="+ls_pais;
