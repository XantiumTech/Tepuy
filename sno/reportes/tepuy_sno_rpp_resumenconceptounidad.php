<?PHP
	...
	function uf_insert_seguridad($as_titulo,$as_desnom,$as_periodo,$ai_tipo)
	{
		...
		if($ai_tipo==1)
		{
			$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte_nomina("SNO","tepuy_sno_r_resumenconceptounidad.php",$ls_descripcion,$ls_codnom);
		}
		else
		{
			$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte_nomina("SNO","tepuy_sno_r_hresumenconceptounidad.php",$ls_descripcion,$ls_codnom);
		}
		return $lb_valido;
	}
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_piedetalle($ai_totalasignacion,$ai_totaldeduccion,$ai_totalaporte,$ai_total_neto,&$io_pdf,$li_total_sueldo)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		// Fecha Creación: 28/04/2006 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$la_data=array(array('totales'=>'<b>Totales Unidad</b>','asignacion'=>$ai_totalasignacion,'deduccion'=>$ai_totaldeduccion,
							 'aporte'=>$ai_totalaporte));
		$ai_total_sueldo_neto=$li_total_sueldo;
		...
		$la_data=array(array('name 1'=>'<b>Neto Unidad</b> '.$ai_total_neto,'name 2'=>'<b>SUELDO-Deducciones </b> '.$ai_total_sueldo_neto));
		$la_columna=array('name 1'=>'','name 2'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'showLines'=>0, // Mostrar Líneas
						 'shaded'=>0, // Sombra entre líneas
						 ...
						 'xOrientation'=>'center', // Orientación de la tabla
						 'cols'=>array('name 1'=>array('justification'=>'center'),'name 2'=>array('justification'=>'center'))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		$la_data=array(array('name 1'=>'','name 2'=>''));
		$la_columna=array('name 1'=>'','name 2'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'showLines'=>0, // Mostrar Líneas
						 'shaded'=>0, // Sombra entre líneas
	...
	function uf_print_piecabecera($ai_totasi,$ai_totded,$ai_totapo,$ai_totgeneral,&$io_pdf)
	{
		...
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);	
		$la_data=array(array('name'=>'<b>Neto a Pagar: </b>','total'=>$ai_totgeneral));
	...
	if($lb_valido)
	{
		//$lb_valido=$io_report->uf_resumenconceptounidad_unidad($ls_codconcdes,$ls_codconchas,$ls_coduniadm,$ls_conceptocero); // Cargar el DS con los datos de la cabecera del reporte
	...
	else // Imprimimos el reporte
	{
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			$lb_valido=$io_report->uf_resumenconceptounidad_concepto($ls_codconcdes,$ls_codconchas,$ls_coduniadm,$ls_conceptocero,$ls_orden); // Obtenemos el detalle del reporte
			$li_totasi=0;
			$li_total_sueldo=0;
			$li_totded=0;
			$li_totapo=0;
			if($lb_valido)
			{
				...
				for($li_s=1;$li_s<=$li_totrow_res;$li_s++)
				{
					...
					switch($ls_tipsal)
					{
						case "A": // Asignación
							...
							if($li_total_sueldo==0)
							{
							$li_total_sueldo=$li_totasi;
							}
				...
				$li_total_sueldo=$li_total_sueldo-$li_totded;
				$li_total_sueldo=$io_fun_nomina->uf_formatonumerico($li_total_sueldo);
				$li_totded=$io_fun_nomina->uf_formatonumerico($li_totded);
				$li_totapo=$io_fun_nomina->uf_formatonumerico($li_totapo);
				$li_totnet=$io_fun_nomina->uf_formatonumerico($li_totnet);
				uf_print_piedetalle($li_totasi,$li_totded,$li_totapo,$li_totnet,$io_pdf,$li_total_sueldo); // Imprimimos el pie de la cabecera
				...
				{// Hacemos un rollback de los registros, agregamos una nueva página y volvemos a imprimir
					...
					uf_print_cabecera($ls_coduniadm,$ls_desuniadm,$io_pdf); // Imprimimos la cabecera del registro
					uf_print_detalle($la_data,$io_pdf); // Imprimimos el detalle 
					uf_print_piedetalle($li_totasi,$li_totded,$li_totapo,$li_totnet,$io_pdf,$li_total_sueldo); // Imprimimos el pie del detalle
				}
			}
			unset($la_data);
		}
