...
// pie de pagina articulo de retencion de IVA
	...
	$as_pie ='<b>G.O. 38.632 de fecha 26/02/2007,Ley de I.V.A. Art. 11:</b>';
	$as_pie1='"La Administracion Tributaria podra designar como responsable del pago del impuesto, en calidad';
	$as_pie2=' de agentes de retencion a quienes por sus funciones publicas o por razon de sus actividades';
	$as_pie3=' privadas intervengan en operaciones gravadas con el impuesto establecido en esta Ley."';
	//$io_pdf->Rectangle(50,50,892,28);	
		$io_pdf->addText(236,585,9,$as_pie); // Agregar el t�ulo
		$io_pdf->addText(246,575,9,$as_pie1); // Agregar el t�ulo
		$io_pdf->addText(246,565,9,$as_pie2); // Agregar el t�ulo
		$io_pdf->addText(246,555,9,$as_pie3); // Agregar el t�ulo
		$io_pdf->restoreState();
		$io_pdf->closeObject();
...
//$io_pdf->line(50,40,960,40);
		...
		unset($la_columna);
		unset($la_config);								 
		$io_pdf->Rectangle(400,454,370,30);	
		$io_pdf->addText(405,473,9,"<b>REGISTRO DE INFORMACION FISCAL DEL AGENTE DE RETENCION</b>"); // Agregar el titulo
		$io_pdf->addText(405,458,9,$as_rifagenteret); // Agregar el t�ulo									 
        //---------------------------------------------------------------------------------------------------
		$la_data[1]=array('titulo'=>'');
		$la_columna=array('titulo'=>'');		
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
		...
		unset($la_columna);
		unset($la_config);								
		$io_pdf->Rectangle(400,370,370,30);	
		$io_pdf->addText(405,390,9,"<b>REGISTRO DE INFORMACION FISCAL DEL SUJETO RETENIDO (R.I.F)</b>"); // Agregar el titulo
		$io_pdf->addText(405,375,9,$as_rif); // Agregar el titulo									 
	...
	$ls_tiporeten=$io_report->uf_obtengo_retencion("SELECT codcmp FROM cxp_contador WHERE codemp='$ls_codemp' AND tipo='I'",'C');
	...
	if($lb_valido)
	{
		...
		//$la_datos=explode("-",$la_comprobantes);
		...
		else
		{
			...
			for ($li_z=0;($li_z<$li_totrow)&&($lb_valido);$li_z++)
			{
				...
				//uf_print_encabezadopagina($ls_titulo,$ls_numcom,&$io_pdf); 
				$lb_valido=$io_report->uf_retencionesiva_proveedor($ls_numcom,$ls_tiporeten);
				if($lb_valido)
				{
					...
					$lb_valido=$io_report->uf_retencionesiva_detalle_unificadas($ls_numcom,$ls_tiporeten,2);
					if($lb_valido)
					{
						$li_totalconiva = 0;
						$li_totalsiniva = 0;
						...
						$ld_fecfac=$io_funciones->uf_convertirfecmostrar($io_report->ds_detalle->data["fecfac"][$li_i]);
							...
							$procede=$io_report->ds_detalle->data["procede"][$li_i];
							//print "Procede: ".$procede;
							//print "Tot imp: ".$li_totimp." base: ".$li_baseimp;die();
							if($procede=="SOBCON")
							{
								//if($li_"SOBCON")
								//print "con iva: ".$li_coniva." tot imp: ".$li_totimp; die();
								$aux_base=$li_coniva-$li_baseimp;
								$base_real=$li_coniva-$aux_base;
						//print "base nueva: ".$aux_base."base real".$base_real;die();//." imp.".($li_coniva-$aux_base); die();
								if($aux_base>$base_real)
								{
									//print "base nueva: ".$aux_base; die();
									$li_totimp=$li_baseimp;
									$li_baseimp=$aux_base;
									$li_siniva=0;
								}
								else
								{
									//print "base nueva: ".$aux_base; die();
									$li_totimp=$aux_base;//$li_baseimp;
									$li_baseimp=$li_coniva-$aux_base;
									$li_siniva=0;
								}
							}
							else
							...
							$la_data[$li_i]=array('numope'=>(int)$ls_numope,'fecfac'=>$ld_fecfac,'numfac'=>$ls_numfac,'numref'=>$ls_numref,
