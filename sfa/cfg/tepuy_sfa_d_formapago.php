...
require_once("tepuy_sfa_c_formapago.php");
...
$io_forpag    = new tepuy_sfa_c_formapago($conn);//Instanciando la Clase tepuy Definiciones.
...
//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
	...
	$ls_sistema  = "SFA";
	$ls_ventanas = "tepuy_sfa_d_formapago.php";
...
if (array_key_exists("operacion",$_POST))
{
	...
	$denabr		= $_POST["txtdenominacionabr"];
	$ls_contable=$_POST["txtcontable"];
	$ls_dencontable=$_POST["txtdencontable"];
}
else
{
	$ls_operacion	= "NUEVO";
...
if ($ls_operacion=="NUEVO")
   {
     $ls_codigo=$io_funciondb->uf_generar_codigo($lb_empresa,$ls_codemp,'sfa_formapago','codfor');
	 if(empty($ls_codigo))
	   {
...
if ($ls_operacion=="ue_guardar")
   {      
	 $lb_existe=$io_forpag->uf_select_formapago($ls_codigo);     
	 if ($lb_existe)
        {
		$ls_codemp=$_SESSION["la_empresa"]["codemp"];
	      if ($ls_estatus=="NUEVO")
		     {
			   $io_msg->message("Este Código de Forma de Pago ya existe !!!");  
			   $lb_valido=false;
			 }
		  elseif($ls_estatus=="GRABADO")
		     {
			
               $lb_valido=$io_forpag->uf_update_formapago($ls_codemp,$ls_codigo,$ls_denominacion,$denabr,$ls_contable,$la_seguridad);
	           if ($lb_valido)
		          {
	    ...
	    {  
		 $lb_valido=$io_forpag->uf_insert_formapago($ls_codemp,$ls_codigo,$ls_denominacion,$denabr,$ls_contable,$la_seguridad);
	     if ($lb_valido)
		    {
...
if ($ls_operacion=="ELIMINAR")
   {
	 ...
	 if ($lb_existe)
	    {
		  $ls_condicion = " AND (column_name='codfor')";//Nombre del o los campos que deseamos buscar.
	      $ls_mensaje   = "";                              //Mensaje que será enviado al usuario si se encuentran relaciones a asociadas al campo.
	      $lb_tiene     = $io_chkrel->uf_check_relaciones($ls_codemp,$ls_condicion,'sfa_formapago',$ls_codigo,$ls_mensaje);//Verifica los movimientos asociados a la cuenta  
		  if (!$lb_tiene)
		     {
			   $lb_valido=$io_forpag->uf_delete_formapago($ls_codemp,$ls_codigo,$ls_denominacion,$denabr,$ls_contable,$la_seguridad);
			   if ($lb_valido)
				  {
				    $io_sql->commit();
				  ...
				  {
				    $io_sql->rollback();
				    $io_msg->message($io_forpag->is_msg_error);
				  }
	         }
		  else
		     {
    ...
    <td width="578" height="173"><div align="center">
      <table width="554" height="133"  border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
        <tr class="titulo-ventana">
          <td height="22" colspan="3" class="titulo-ventana"><p>Formas de Pago </p></td>
        </tr>
        <tr>
          <td height="22" >&nbsp;</td>
        ...
        <tr>
         ...
         <tr>
          ...
          <td height="22" align="right">Denominaci&oacute;n Abreviada</td>
          <td height="22" colspan="2"><p>
              <input name="txtdenominacionabr" id="txtdenominacionabr" value="<?php print $denabr ?>" type="text" size="2" maxlength="2" onKeyPress="return keyRestrict(event,'abcdefghijklmnñopqrstuvwxyz '+'.,-');">
          </p></td>
        </tr>
          <tr class="formato-blanco">
            <td height="22" align="right"><span class="sin-borde"><strong>(*)</strong></span>&nbsp;C&oacute;digo Contable (Activo)&nbsp;</td>
            <td height="22" colspan="3"><input name="txtcontable" type="text" id="txtcontable" onKeyPress="return keyRestrict(event,'1234567890');" value="<?php print $ls_contable ?>" size="25" maxlength="25" readonly  style="text-align:center ">
              <a href="javascript:catalogo_cuentas();"><img src="../../shared/imagebank/tools15/buscar.png" width="15" height="15" border="0"></a> <input name="txtdencontable" type="text" class="sin-borde" id="txtdencontable" value="<?php print $ls_dencontable ?>" size="70" maxlength="100" readonly  onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmn&ntilde;opqrstuvwxyz '+'.,-');"></td>
          </tr>
         <tr>
          <td height="22">&nbsp;</td>
...
function ue_nuevo()
{
	...
	if (li_incluir==1)
	   {	
	     ...
	     f.action="tepuy_sfa_d_formapago.php";
	     f.submit();
       }
	else
	   {
...
function ue_buscar()
{
	...
	if (li_leer==1)
	   {
	     f.operacion.value="";			
	     pagina="tepuy_sfa_cat_formapago.php";
