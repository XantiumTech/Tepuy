<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","tepuy_mis_p_reverso_siv.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
...
<?php
	...
	require_once("class_folder/class_tepuy_siv_integracion.php");  	
	$in_class = new class_tepuy_siv_integracion();  
	uf_limpiarvariables();
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido = $in_class->uf_reversar_contabilizacion_despacho($ls_comprobante,$ld_fechaconta,$la_seguridad);
					if($lb_valido)
					{
		...
		case "BUSCAR":
			uf_load_variables();
		    $in_class_contabiliza->uf_select_comprobantes_despacho($ls_comprobante,$ld_fecdes,1,$lo_object,$li_totrows);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
?>		  
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    ...
    <tr>
      <td width="116"  height="23"><div align="right">Despacho </div>
          <div align="right"></div></td>
      <td width="632" ><input name="txtnumorddes" type="text" id="txtnumorddes" onKeyUp="javascript: ue_validarcomillas(this);" maxlength="15"></td>
    </tr>
    <tr>
    ...
    <tr>
      <td  height="23"><div align="right">Fecha Contabilizaci&oacute;n </div></td>
      <td ><div align="left">
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="tepuy_mis_p_reverso_siv.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(numorddes)
{
	...
	window.open("tepuy_mis_pdt_siv.php?numorddes="+numorddes+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
