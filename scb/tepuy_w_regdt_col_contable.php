...
if($ls_operacion=="GUARDAR")
{
	...
	if($lb_valido)
	{
		$lb_valido=$in_classmovcol->uf_procesar_dt_contable($arr_movbco,$ls_cuenta_scg_col,$ls_operacioncon,'00000',$ls_descripcion,$ldec_monto,true);	
	}	
	...
	if($lb_valido)
	{
		$lb_valido=$in_classmovcol->uf_procesar_dt_contable($arr_movbco,$ls_sc_cuenta,$ls_debhab,'00000',$ls_descripcion,$ldec_monto,false);	
...
<script language="JavaScript">
  function aceptar_presupuestario()
  {
	...
	if((f.txtdescripcion.value!="") && (f.txtcuenta.value!="") && (f.txtmonto.value!="") && (f.txtmonto.value!="0") && (f.txtmonto.value!=",00") && (f.txtmonto.value!="0,00"))
	{
		...
		f.action="tepuy_w_regdt_col_contable.php";
		f.submit();	
	}
	else
	{
