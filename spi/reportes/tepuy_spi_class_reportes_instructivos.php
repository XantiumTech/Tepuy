<?php
class tepuy_spi_class_reportes_instructivos
{
	var $io_msg;
	var $io_fecha;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function  tepuy_spi_class_reportes_instructivos()
    {
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	//////////////////////////////////////////////////////////////////////////////////
	//   CLASE REPORTES SPI  INSTRUCTIVOS " CONSOLIDADO DE EJECUCION TRIMESTRAL "  //
	/////////////////////////////////////////////////////////////////////////////////
    ...
    {///////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 // Fecha Creación :    18/05/2008          Fecha última Modificacion :      Hora :
  	 ////////////////////////////////////////////////////////////////////////////////////////////////////////
	 $lb_valido = true;	 
	 $ls_sql=" SELECT spi_cuenta, MAX(denominacion) as denominacion, MAX(status) as status, sum(previsto) as previsto,    ".
    ...
    {///////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 {		
		...
		if($li_numrows>=0)
		{
			...
			$this->dts_reporte->insertRow("ejecutado_acumulado",$ld_ejecutadoacum);  
	    }//if	
	 $this->io_sql->free_result($rs_data);
	 }//else
     return $lb_valido;
...
//		  if($li_cobrado_anticipado)
//		  { 
//		    $ad_cobrado_anticipado_acumulado=$ad_cobrado_anticipado_acumulado+$ld_monto;
//		  }//if
		  if($li_cobrado)
		  { 
  ...
  return $lb_valido;
    ...
    {////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 // Fecha Creación : 18/06/2008                       Fecha última Modificacion :      Hora :
  	 ///////////////////////////////////////////////////////////////////////////////////////////////////////
	  $lb_valido=true;
	  $ls_sql=" SELECT spi_cuenta, denominacion, max(status) as status,                  ".
			  ...
			  " GROUP BY spi_cuenta,denominacion ";  	  	  
	  $rs_data=$this->io_sql->select($ls_sql);
	  if($rs_data===false)
	  { // error interno sql
		$this->io_mensajes->message("CLASE->tepuy_spg_reporte_pres_caja_07 ". 
	  ...
	  else
	  {
	   ...
	   $ls_sql=" SELECT spg_cuenta, max(denominacion) as denominacion, max(status) as status, sum(asignado) as asignado, ".
              "        sum(precomprometido) as precomprometido, sum(comprometido) as comprometido,       ".
              "        sum(causado) as causado, sum(pagado) as pagado, sum(aumento) as aumento,          ".
              "        sum(disminucion) as disminucion, sum(enero) as enero , sum(febrero) as febrero ,  ".
              "        sum(marzo) as marzo, sum(abril) as abril, sum(mayo) as mayo, sum(junio) as junio, ".
...
function uf_spi_reportes_gastos_corrientes($adt_fecdes,$adt_fechas,$adts_datastore,$as_mesdes,$as_meshas)
    ...
    {//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 // Fecha Creación :    18/08/2008               Fecha última Modificacion :      Hora :
	  ...
	  if($rs_data===false)
	  {   // error interno sql
		  $this->io_msg->message("CLASE->tepuy_spi_class_reportes_instructivos  MÉTODO->uf_spg_reporte_select_saldo_empresa  ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));
		  $lb_valido = false;
	  }
	  else
	  {
