...
//-----------------------------------------------------------------------------------------------------------------------------
	...
	//--------------------------------------------------  Parámetros para Filtar el Reporte  -----------------------------------------
	 ...
	 $io_function_report->uf_load_seguridad_reporte("SPG","tepuy_spg_r_comprobante_formato2.php",$ls_desc_event);
	////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////////////////////////////////////////////////////////
	//----------------------------------------------------  Parámetros del encabezado  -------------------------------------------------------------------------------
		...
		$ls_periodo_comp="<b>Comprobantes desde la Cuenta Nro.  ".trim($ls_cuentades)." al ".trim($ls_cuentahas)."</b>";
		...
		for($li_i=1;$li_i<=$li_tot;$li_i++)
		{
            ...
            if($lb_valido)
			{
				...
				for($li_s=1;$li_s<=$li_totrow_det;$li_s++)
				{
					...
					$ls_programatica=$io_report->dts_reporte->data["programatica"][$li_s]; //print $ls_programatica."<br>";
					...
					$ls_denestpro=$ls_denestpro1." , ".$ls_denestpro2." , ".$ls_denestpro3; // print $ls_denestpro."<br>";
					if($li_estmodest==2)
					{
			   ...
			   // uf_print_total_programatica($ld_totalprogramatica,$io_pdf); // Imprimimos el total programatica//quitar
				...
				if($ls_tipoformato==1)
				{
					//uf_print_total_comprobante($ld_totalcomprobante,'Bs.F.',$io_pdf); // Imprimimos el total comprobante
				}
				else
				{
					uf_print_total_comprobante($ld_totalcomprobante,'Bs.',$io_pdf); // Imprimimos el total comprobante
					$ld_totalcomprobante_bsf=number_format($ld_totalcomprobante_bsf,2,",",".");
					//uf_print_total_comprobante($ld_totalcomprobante_bsf,'Bs.F.',$io_pdf); // Imprimimos el total comprobante
				}	
				$ld_totalcomprobante=0;
				$ld_totalprogramatica=0;
			}
			...
			{// Hacemos un rollback de los registros, agregamos una nueva página y volvemos a imprimir
				...
				//uf_print_total_comprobante($ld_totalcomprob,'Bs.',$io_pdf); // Imprimimos el total comprobante
				if($ls_tipoformato==1)
				{
					//uf_print_total_comprobante($ld_totalcomprob,'Bs.F.',$io_pdf); // Imprimimos el total comprobante
				}
				else
				{
					uf_print_total_comprobante($ld_totalcomprob,'Bs.',$io_pdf); // Imprimimos el total comprobante
					$ld_total_bsf=$io_rcbsf->uf_convertir_monedabsf($ld_total, $li_candeccon,$li_tipconmon,1000,$li_redconmon);	
					//$ld_totalcomprob_bsf=number_format($ld_totalcomprob_bsf,2,",",".");
				//	uf_print_total_comprobante($ld_totalcomprob_bsf,'Bs.F.',$io_pdf); // Imprimimos el total comprobante
				}	
