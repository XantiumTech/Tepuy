<?php
	...
	{	
		...
		$lo_object[0]="";
		require_once("tepuy_soc_c_aprobacion_orden_compra.php");
		$io_aprobacion = new tepuy_soc_c_aprobacion_orden_compra("../../");
		...
		while ($row=$io_aprobacion->io_sql->fetch_row($rs_data))	  
		      {
			    ...
			    $ls_codpro    = $row["cod_pro"];/////modificado 12/12/2007
				...
				$ld_fecaprcom = $io_funcion->uf_convertirfecmostrar($row["fecaprord"]);
				...
				$lo_object[$li_fila][3]="<input name=txtnompro".$li_fila."    id=txtnompro".$li_fila."     type=text class=sin-borde  size=40   style=text-align:left     value='".$ls_nompro."'    readonly>";
		 	    $lo_object[$li_fila][4]="<input name=txtobsordcom".$li_fila." id=txtobsordcom".$li_fila."  type=text class=sin-borde  size=50   style=text-align:left     value='".$ls_obsordcom."' readonly>";
