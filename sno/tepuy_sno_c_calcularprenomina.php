<?PHP
class tepuy_sno_c_calcularprenomina
{
	var $io_sql;
	var $io_mensajes;
	var $io_funciones;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function tepuy_sno_c_calcularprenomina()
	{	
		//////////////////////////////////////////////////////////////////////////////
		//	     Function: tepuy_sno_c_calcularnomina
		//		   Access: public (tepuy_sno_p_calcularnomina)
	...
	}// end function tepuy_sno_c_calcularnomina
	//-----------------------------------------------------------------------------------------------------------------------------------
										 ...
										 &$ai_totdednomant,&$ai_totapoempnomant,&$ai_totapopatnomant,&$ai_totnomant)
	{
		...
		//		   Access: public (tepuy_sno_p_calcularprenomina.php)
		...
		else
		{
			...
			while(($row=$this->io_sql->fetch_row($rs_data))&&($lb_valido))
			{
				...
				if($lb_valido)
				{
					...
					$ls_glocon=$_SESSION["la_conceptopersonal"]["glocon"];
					$ls_aplcon=$_SESSION["la_conceptopersonal"]["aplcon"];
					$ls_forcon=$_SESSION["la_conceptopersonal"]["forcon"];
					$ls_sigcon=$_SESSION["la_conceptopersonal"]["sigcon"];
					$ls_forpatcon=$_SESSION["la_conceptopersonal"]["forpatcon"];
					$ls_acuemp=$_SESSION["la_conceptopersonal"]["acuemp"];
					$ls_acupat=$_SESSION["la_conceptopersonal"]["acupat"];
					$li_valmincon=$_SESSION["la_conceptopersonal"]["valmincon"];
					$li_valmaxcon=$_SESSION["la_conceptopersonal"]["valmaxcon"];
					$li_valminpatcon=$_SESSION["la_conceptopersonal"]["valminpatcon"];
					$li_valmaxpatcon=$_SESSION["la_conceptopersonal"]["valmaxpatcon"];
					$li_valcon=0;
					$li_valconhis=0;					
					$lb_filtro=true;
					$lb_aplica=true;
				...
				if(array_key_exists("la_concetopersonal",$_SESSION))
				{
	...
	function uf_evaluar_prestamospersonal($as_codper,$as_perhis,$as_anocurhis)
	{
		...
		$ls_sql=" SELECT sno_prestamos.codconc, COALESCE(SUM(sno_prestamosperiodo.moncuo),0) AS total ".
