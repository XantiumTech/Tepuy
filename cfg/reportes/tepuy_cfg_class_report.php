...
class tepuy_cfg_class_report
{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function tepuy_cfg_class_report()
	{
	...
	}// end function tepuy_cfg_class_report
	//-----------------------------------------------------------------------------------------------------------------------------------
function uf_select_unidad_tributaria()
{	
	
	$lb_valido=true;
	$ls_sql = "SELECT * FROM tepuy_unidad_tributaria ";	
    $li_select =$this->SQL->select($ls_sql);	                                                                                                                                                                                
	if($li_select===false)
	 {
	   $lb_valido=false;
	   $this->io_msg->message("CLASE->tepuy_cfg_class_report MÉTODO->uf_select_unidad_tributaria ERROR->".$this->fun->uf_convertirmsg($this->SQL->message));
   	 }
      else
		{
