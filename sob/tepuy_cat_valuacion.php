	 ...
	 else
	 {
	  if($ls_codasi!="%%")   
	   ...
	   else
	   {
		...
		{
		  $ls_sql="SELECT v.*,a.codasi, d.desobr"; 
		  $ls_sql=$ls_sql." FROM sob_valuacion v,sob_asignacion a,sob_contrato c, sob_obra d
	                        WHERE v.codemp='".$ls_codemp."'  AND c.codasi=a.codasi  AND c.codcon=v.codcon AND a.codobr=d.codobr";
		}
	   }
	   }
	 }
	...
	}else
	{
		if($row=$io_sql->fetch_row($rs_data))
		{
			...
			for($li_z=1;$li_z<=$li_totrow;$li_z++)
			{
				...
				$ls_numfactura=$data["numfac"][$li_z];
				...
				print "<td><a href=\"javascript: aceptar('$ls_codval','$ls_codasi','$ls_codcon','$ls_numrecdoc','$ls_numref','$ls_numfactura','$ls_fecha','$ls_fecinival','$ls_fecfinval','$ls_obsval','$ls_amoval','$ls_obsamoval','$ls_amoantval','$ls_amototval','$ls_amoresval','$ls_basimpval','$ls_montotval','$ls_subtotpar','$ls_totreten','$ls_subtot','$ls_nomestval');\">".$ls_codcon."</a></td>";
				print "<td>".$ls_codval."</td>";
				print "<td>".$ls_numrecdoc."</td>";
				print "<td>".$ls_desobr."</td>";
				print "<td>".$ls_fecha."</td>";
				print "<td>".$ls_nomestval."</td>";			
			// muestra bien el total retenido almacenado en la tabla
				//print "<td>".$ls_totreten."</td>";			
				print "</tr>";			
			}
			print "</table>";
		}
...
function aceptar(ls_codval,ls_codasi,ls_codcon,ls_numrecdoc,ls_numref,ls_numfactura,ls_fecha,ls_fecinival,ls_fecfinval,ls_obsval,ls_amoval,ls_obsamoval,ls_amoantval,ls_amototval,ls_amoresval,ls_basimpval,ls_montotval,ls_subtotpar,$ls_totreten,ls_subtot,ls_nomestval)
  {
  	//alert(ls_numfactura);
    opener.ue_cargarvaluacion(ls_codval,ls_codasi,ls_codcon,ls_numrecdoc,ls_numref,ls_numfactura,ls_fecha,ls_fecinival,ls_fecfinval,ls_obsval,ls_amoval,ls_obsamoval,ls_amoantval,ls_amototval,ls_amoresval,ls_basimpval,ls_montotval,ls_subtotpar,$ls_totreten,ls_subtot,ls_nomestval);
	close();
  }
  ...
  f.action="tepuy_cat_valuacion.php";
  f.submit();
  }
</script>
