<?php
	...
	function uf_insert_seguridad($as_titulo,$as_desnom,$as_periodo,$ai_tipo)
	{
		...
		if($ai_tipo==1)
		{
			$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte_nomina("SNO","tepuy_sno_r_resumenconcepto.php",$ls_descripcion,$ls_codnom);
		}
		else
		{
			$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte_nomina("SNO","tepuy_sno_r_hresumenconcepto.php",$ls_descripcion,$ls_codnom);
		}
		return $lb_valido;
	}
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	if($lb_valido)
	{
		$lb_valido=$io_report->uf_resumenconcepto_conceptos($ls_codconcdes,$ls_codconchas,$ls_aportepatronal,$ls_conceptocero,$ls_subnomdes,$ls_subnomhas); // Cargar el DS con los datos de la cabecera del reporte
	}
	...
	else  // Imprimimos el reporte
	{
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			$li_monto=$io_fun_nomina->uf_formatonumerico(abs($io_report->DS->data["monto"][$li_i]));
			$ls_tipsal=rtrim($io_report->DS->data["tipsal"][$li_i]);
			$li_totalpersonal=number_format($io_report->DS->data["total"][$li_i],0,"",".");
			...
			switch($ls_tipsal)
			{
				...
				case "D": // Deducción
					$li_totded=$li_totded+abs($io_report->DS->data["monto"][$li_i]);
				...
				case "P2": // Aporte Patrón
					$li_totapo=$li_totapo+abs($io_report->DS->data["monto"][$li_i]);
