...
// ESTAS LINEAS SON NECESARIAS PARA QUE SE PUEDA MOSTRAR EL PDF //
	...
	function uf_insert_seguridad()
	{
	    ...
	    $ls_evento      = "IMPRIMIR";
	    $ls_descripcion = "Imprimio Portada de Distribucion y Ordenanza";
...
//-----------------------------------------------------------------------------------------------------------------------------
	...
	//--------------------------------------------------  Parámetros para Filtar el Reporte  -----------------------------------------
		...
		$ls_titulo =" <b> ORDENANZA DE PRESUPUESTO </b>";
		$ls_titulo2=" <b> DE INGRESOS Y GASTOS </b>";
		$ls_ejercicio="<b> Ejercicio Económico Financiero 2.010 </b>";
		$ls_titulo1=" <b> DISTRIBUCION DEL PRESUPUESTO </b>";
		if($cmbnivel=="s1")
		{
          		$desde=1;
			$hasta=2;
			  ...
			  for ($z=$desde;$z<=$hasta;$z++)
				...
				for ($l=1;$l<=20;$l++)
				{
					...
					if($l==1)
					{
						if($z==1)
						{
							$la_data[$ubica]=array('titulo'=>$ls_titulo);
							$ubica=$ubica+1;
							$la_data[$ubica]=array('titulo'=>$ls_titulo2);
							$ubica=$ubica+1;
							$la_data[$ubica]=array('titulo'=>$ls_ejercicio);
						}
						if($z==2)
						{
							$la_data[$ubica]=array('titulo'=>$ls_titulo1);
							$ubica=$ubica+1;
							$la_data[$ubica]=array('titulo'=>$ls_titulo2);
							$ubica=$ubica+1;
							$la_data[$ubica]=array('titulo'=>$ls_ejercicio);
