...
var $ds_sol_t;
var $ds_retenciones;
    function tepuy_scb_c_cmpret_op($conn)
	{
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if ($rs_data===false)
     {
	    $lb_valido=false;
 	    $this->io_msg->message("CLASE->tepuy_CXP_CLASS_REPORT; METODO->uf_load_recepciones; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	 }
   else
      {
   ...
   function uf_select_nombre_proveedor($as_codemp,$as_codpro)
	{
     ...
     //////////////////////////////////////////////////////////////////////////////
       ...
       if ($rs===false)
		  {
	        $lb_valido=false;
 	        $this->io_msg->message("CLASE->tepuy_CXP_RC_LIBROCOMPRA; METODO->uf_load_libro_compras; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
  	      }
       else
          {
             return $rs;
...
//////////////////////////////////////////////////////////////////////////////
  ...
  if ($rs_detpresupuesto===false)
     {
	    $lb_valido=false;
 	    $this->io_msg->message("CLASE->tepuy_CXP_CLASS_REPORT; METODO->uf_select_detallespresupuestarios; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	 }
  else
	 {
...
//////////////////////////////////////////////////////////////////////////////
  ...
  if ($rs_detcontable===false)
     {
 	   $lb_valido=false;
	   $this->io_msg->message("CLASE->tepuy_CXP_CLASS_REPORT; METODO->uf_select_detallescontables; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	 }
  else
	 {
...
//        $lb_valido:  Variable booleana que devuelve true si la sentencia sql fue ejecutada
//                     sin problemas y el resulset obtuvo registros de la consulta.           
//       Description:  Función que se encarga de extraer todos los registros de la Tabla tepuy_deducciones.      
//     Elaborado Por:  Ing. Néstor Falcón.
// Fecha de Creación:  08/05/2006        Fecha Última Actualización:08/05/2006.	 
////////////////////////////////////////////////////////////////////////////////////////////////////////////
  
  $this->ds_deducciones = new class_datastore();
  $ls_sql= " SELECT * FROM tepuy_deducciones WHERE codemp='".$as_codemp."' ORDER BY codded ASC";
  $rs_data=$this->io_sql->select($ls_sql);		    
  if ($rs_data===false)
     {
	   $lb_valido=false;
 	   $this->io_msg->message("CLASE->tepuy_CXP_CLASS_REPORT; METODO->uf_load_deducciones; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	 }
  else
	 {
...
function uf_selec_rel_prov_factrs($as_codemp,$as_tipo,$as_codigo1,$as_codigo2,&$lb_valido)
	{
		...
		if($rs_result===false)
		{
		       $this->io_msg->message("CLASE->tepuy_CXP_CLASS_REPORT; METODO->uf_selec_rel_prov_factrs; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
...
//        $lb_valido:  Variable booleana que devuelve true si la sentencia sql fue ejecutada
//                     sin problemas y el resulset obtuvo registros de la consulta.           
//       Description:  Función que se encarga de extraer todos los registros de la Tabla tepuy_cargos.      
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////  
  ...
  if ($rs_data===false)
     {
	   $lb_valido=false;
 	   $this->io_msg->message("CLASE->tepuy_CXP_CLASS_REPORT; METODO->uf_selec_rel_facturas; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	 }
  else
	 {
	   if ($row=$this->io_sql->fetch_row($rs_data))
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  $this->ds_otroscreditos = new class_datastore();
  $ls_sql= " SELECT * FROM tepuy_cargos WHERE codemp='".$as_codemp."' ORDER BY codcar ASC";
  $rs_data=$this->io_sql->select($ls_sql);		    
  if ($rs_data===false)
     {
	   $lb_valido=false;
 	   $this->io_msg->message("CLASE->tepuy_CXP_CLASS_REPORT; METODO->uf_load_otros_creditos; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	 }
  else
	 {
	   if ($row=$this->io_sql->fetch_row($rs_data))
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if ($rs_data===false)
     {
	   $lb_valido=false;
 	   $this->io_msg->message("CLASE->tepuy_CXP_CLASS_REPORT; METODO->uf_load_documentos; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	 }
  else
	 {
	   if ($row=$this->io_sql->fetch_row($rs_data))
...
function uf_load_recepcion($as_codemp,$as_numrecdoc,$as_codtipdoc,$as_categoria,$as_codigo,&$lb_valido)
{
  ...
  if ($rs_data===false)
     {
       $lb_valido=false;
 	   $this->io_msg->message("CLASE->tepuy_CXP_CLASS_REPORT; METODO->uf_load_recepcion; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));	 
	 }  
  else
     {
	   $li_numrows=$this->io_sql->num_rows($rs_data); 
...
return $lb_valido;		   	
	...
	function uf_select_ret_iva_cab($as_codemp,$as_numcom)
	{
	 ...
	 if ($rs_data===false)
	   {
               $this->io_msg->message("CLASE->tepuy_CXP_CLASS_REPORT; METODO->uf_select_ret_iva_cab; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		 $lb_valido=false;
	        print ($this->io_sql->message);
          }
	 else
	...
	function uf_select_ret_iva_det($as_codemp,$as_numcom)
	{
	 ...
	 if ($rs===false)
	   {
		 $this->io_msg->message("CLASE->tepuy_CXP_CLASS_REPORT; METODO->uf_select_ret_iva_det; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		 $lb_valido=false;
	        print ($this->io_sql->message);
          }
	 else
		...
		if ($rs_data===false)
		   {
			 $lb_valido=false;
			 $ls_dirpro="";
                      $this->io_msg->message("CLASE->tepuy_CXP_CLASS_REPORT; METODO->uf_select_monfac; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
...
*/  //   print $ls_sql."<br>";
	...
	function uf_generar_islr($ls_numdoc,$ls_codban,$ls_ctaban,$ls_codpro,$ls_cedbene,$ls_tipodestino,$ls_numdocres,$ls_nrocontrol,$ld_fecdocres)
	{
		...
		$ls_sql="SELECT a.monobjret,b.porded,a.monto,b.codded,b.islr,c.cod_pro,c.ced_bene,c.tipo_destino".$ls_campos."
				 FROM scb_movbco_scg a,tepuy_deducciones b,scb_movbco c".$ls_from."
