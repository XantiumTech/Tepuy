<?php
	...
	$oi_fun_integrador->uf_load_seguridad("SOB","tepuy_mis_p_anula_asignacion_sob.php",$ls_permisos,$la_seguridad,$la_permisos);
...
<html>
<head>
<title>Anulaci&oacute;n de la Contabilizaci&oacute;n de las Asignaciones</title>
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_anulacion_asignacion($ls_codasi,$ls_fecha,$ld_fechaconta,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("Asignación  ".$ls_codasi." fue Anulada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo contabilizar la Anulación ".$ls_codasi);
					}		
				}
			}
			$li_totrows=1;
...
<p>&nbsp;</p>
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Anulaci&oacute;n de la Contabilizaci&oacute;n de Asignaci&oacute;n </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="tepuy_mis_p_anula_asignacion_sob.php";
			f.submit();
		}
		else
		{
