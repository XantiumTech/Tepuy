...
{
	$ls_sql="SELECT a.ced_bene,a.nombene,a.apebene,a.codban,b.nomban,a.ctaban,a.codbansig 
			 FROM   rpc_beneficiario a,scb_banco b
			 WHERE  a.codemp='".$arr["codemp"]."' AND a.codemp=b.codemp AND a.ced_bene like '".$ls_codigo."' 
			 AND    a.nombene like '".$ls_nombre."' AND a.ced_bene<>'----------' AND a.codban=b.codban ";
	...
	if($row=$SQL->fetch_row($rs_cta))
	{
		...
		for($z=1;$z<=$totrow;$z++)
		{
			...
			$ls_nombre=$data["nombene"][$z]."  ".$data["apebene"][$z];
			$ls_codban=$data["codban"][$z];
			$ls_nomban=$data["nomban"][$z];
			$ls_ctaban=$data["ctaban"][$z];
			$ls_codbansig=$data["codbansig"][$z];
			print "<td><a href=\"javascript: aceptar('$ls_cedula','$ls_nombre','$ls_codban','$ls_nomban','$ls_ctaban','$ls_codbansig');\">".$ls_cedula."</a></td>";
...
<script language="JavaScript">
  ...
  f.action="tepuy_cat_bene_op.php";
  f.submit();
  }
