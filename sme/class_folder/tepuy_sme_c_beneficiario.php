<?php
class tepuy_sme_c_beneficiario
 {
var $ls_sql="";
var $la_emp;
var $is_msg_error;
 	
	function tepuy_sme_c_beneficiario()
	{
...
{  
 ...
 //////////////////////////////////////////////////////////////////////////////
  ...
  $ls_sql=" INSERT INTO rpc_beneficiario(codemp,ced_bene,nombene,apebene,dirbene,telbene,celbene,email,sc_cuenta,sc_cuenta_comp,ctaban,                  ".
		  "  codban,codtipcta,codpai,codest,codmun,codpar,rifben,codbansig,fecregben,nacben,numpasben,tipconben,tipcuebanben)                          ". 
		  "  VALUES('".$as_codemp."','".$ls_cedula."','".$ls_nombre."','".$ls_apellido."','".$ls_direccion."','".$ls_telefono."',         ".
		  " '".$ls_celular."','".$ls_email."','".$ls_contable."','".$ls_contablecomp."','".$ls_cuenta."','".$ls_banco."','".$ls_tipocuenta."',                   ".
		  " '".$ls_pais."','".$ls_estado."','".$ls_municipio."','".$ls_parroquia."','".$ls_rif."','".$ls_codbansig."','".$ls_fecregben."',".
  ...
  if ($rs_data===false)
   {
	 ...
	 $this->is_msg_error="CLASE->tepuy_sme_c_beneficiario; METODO->uf_insert_beneficiario; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message);
	 $lb_valido=false;
   }
  else
   {   
...
//////////////////////////////////////////////////////////////////////////////
  ...
  $ls_sql=" UPDATE rpc_beneficiario ". 
		  ...
		  "        rifben='".$ls_rif."',         codbansig='".$ls_codbansig."', ".
		  "        nacben='".$ls_nacben."',      numpasben='".$ls_numpasben."', ".
 		  "        tipconben='".$ls_tipconben."'                              , ". 
 ...
 if ($rs_data===false)
   {                              
	 $this->io_sql->rollback();
	 $this->is_msg_error="CLASE->tepuy_sme_c_beneficiario; METODO->uf_update_beneficiario; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message);
	 $lb_valido=false;
   }
 else
   {   
...
//////////////////////////////////////////////////////////////////////////////
	...
	$lb_valido = false;
	$ls_sql    = "SELECT * FROM rpc_beneficiario WHERE codemp='".$as_codemp."' AND ced_bene='" .$as_cedben."'";
	$rs_data   = $this->io_sql->select($ls_sql);
	if ($rs_data===false)
	   {
		 $this->io_msg_message("CLASE->tepuy_sme_c_beneficiario; METODO->uf_update_beneficiario; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		 $lb_valido=false;
	   }
	else
	   {			
	...
	if(($lb_existe)&&(!$lb_relacion))
	  {
	    ...
	    if ($rs_data===false)
	       { 
		     $this->io_sql->rollback();
		     $this->is_msg_error="CLASE->tepuy_sme_c_beneficiario; METODO->uf_delete_beneficiario; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message);
...
function uf_select_estado($as_codpai)
{
 ...
 //              en específico de la tabla tepuy_estados.	
 //////////////////////////////////////////////////////////////////////////////
...
{
	$ls_sql  = "SELECT numsol FROM sme_solicitud WHERE codemp='".$as_codemp."' AND trim(ced_bene)='".trim($as_cedben)."'";
	$rs_data = $this->io_sql->select($ls_sql);
	if ($rs_data===false)
	   {
		 $lb_valido=false;
	...
	else
	   {
	     if ($row=$this->io_sql->fetch_row($rs_data))
		    ...
		    {
			  $ls_sql  = "SELECT ced_bene FROM cxp_rd WHERE codemp='".$as_codemp."' AND trim(ced_bene)='".trim($as_cedben)."'";
	          $rs_data = $this->io_sql->select($ls_sql);
			  if ($rs_data===false)
	             {
				   $lb_valido=false;
			  ...
			  else
			     {
	               if ($row=$this->io_sql->fetch_row($rs_data))
					  ...
					  {				   
				        $ls_sql  = "SELECT ced_bene FROM scb_movbco WHERE codemp='".$as_codemp."' AND trim(ced_bene)='".trim($as_cedben)."'";
	                    $rs_data = $this->io_sql->select($ls_sql);
			            if ($rs_data===false)
	                       {
							 $lb_valido=false;
					    ...
					    else
						   {
				  	         if ($row=$this->io_sql->fetch_row($rs_data))
							    ...
							    {				   
								  $ls_sql  = "SELECT codper FROM scv_dt_personal WHERE codemp='".$as_codemp."' AND trim(codper)='".trim($as_cedben)."'";
								  $rs_data = $this->io_sql->select($ls_sql);
								  if ($rs_data===false)
								     {
									   $lb_valido=false;
								  ...
								  else
									 {
									   if ($row=$this->io_sql->fetch_row($rs_data))
										  ...
										  {				   
										    $ls_sql  = "SELECT cedben FROM sno_concepto WHERE codemp='".$as_codemp."' AND trim(cedben)='".trim($as_cedben)."'";
											$rs_data = $this->io_sql->select($ls_sql);
											if ($rs_data===false)
											   {
											     $lb_valido=false;
											...
											else
											   {
											     if ($row=$this->io_sql->fetch_row($rs_data))
												    ...
												    {				   
													  $ls_sql  = "SELECT ced_bene FROM sno_dt_spg WHERE codemp='".$as_codemp."' AND trim(ced_bene)='".trim($as_cedben)."'";
													  $rs_data = $this->io_sql->select($ls_sql);
													  if ($rs_data===false)
														 { 
														   $lb_valido=false;
													  ...
													  else
														 {
														   if ($row=$this->io_sql->fetch_row($rs_data))
															  ...
															  {	  
															    $ls_sql  = "SELECT ced_bene FROM sno_dt_scg WHERE codemp='".$as_codemp."' AND trim(ced_bene)='".trim($as_cedben)."'";
															    $rs_data = $this->io_sql->select($ls_sql);
															    if ($rs_data===false)
																   { 
																	 $lb_valido=false;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
     ...
     if ($rs_data===false)
	    {
		  $lb_valido = false;
		  $this->io_msg->message("CLASE->tepuy_sme_c_beneficiario; METODO->uf_load_personal; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
     else
	    {
		  $li_numrows = $this->io_sql->num_rows($rs_data);
...
//print $ls_sql;
  ...
  if ($rs_data===false)
     {
	   $lb_valido = false;
	   $this->is_msg_error="CLASE->tepuy_sme_c_beneficiario; METODO->uf_load_datos_personal; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message);
	 }
  return $rs_data;
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  $ls_sql=" INSERT INTO rpc_beneficiario(codemp,ced_bene,nombene,apebene,dirbene,telbene,celbene,email,sc_cuenta,sc_cuenta_comp,                ".
          "             codpai,codest,codmun,codpar,nacben,tipconben,codbansig,codban,ctaban)                                                            ". 
		  "  VALUES('".$as_codemp."','".$ls_cedula."','".$ls_nombre."','".$ls_apellido."','".$ls_direccion."','".$ls_telefono."',".
		  " '".$ls_celular."','".$ls_email."','".$ls_contable."','".$ls_contablecomp."','".$ls_pais."','".$ls_estado."','".$ls_municipio."',            ".
		  "'".$ls_parroquia."','".$ls_nacben."','".$ls_tipconben."','".$ls_bansigcof."','".$ls_codban."','".$ls_ctaban."')                                                             ";
  $rs_data=$this->io_sql->execute($ls_sql);
  if ($rs_data===false)
     {
	   $this->is_msg_error="CLASE->tepuy_sme_c_beneficiario; METODO->uf_insert_personal; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message);
	   $lb_valido=false;
     }
  else
     {   
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if ($rs_data===false)
     {
	   $this->io_msg->message("CLASE->tepuy_sme_c_beneficiario; METODO->uf_load_and_insert_codbansig(SELECT); ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	   $lb_valido=false;
	 }
  else
     {
	   ...
	   if ($li_numrows<=0)
	      {
			...
			if ($rs_datos===false)
			   {
	             $this->io_msg->message("CLASE->tepuy_sme_c_beneficiario; METODO->uf_load_and_insert_codbansig(INSERT); ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	             $lb_valido=false;
			   }
		  }
	 } 
