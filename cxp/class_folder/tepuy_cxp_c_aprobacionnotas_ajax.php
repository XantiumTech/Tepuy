<?php
	...
	require_once("tepuy_cxp_c_aprobacionnotas.php");
	$io_aprobacion=new tepuy_cxp_c_aprobacionnotas('../../');
	...
	{	
		...
		$ls_sql="SELECT cxp_sol_dc.*,(CASE cxp_sol_dc.cod_pro WHEN '----------' THEN ".$ls_aux." ELSE rpc_proveedor.nompro END) as nombre
				 FROM cxp_sol_dc,rpc_proveedor,rpc_beneficiario 
				 WHERE cxp_sol_dc.codemp='".$ls_codemp."' AND cxp_sol_dc.fecope BETWEEN '".$ad_fecregdes."' AND '".$ad_fecreghas."' ".$ls_aux_sql."
				 AND cxp_sol_dc.cod_pro=rpc_proveedor.cod_pro AND cxp_sol_dc.ced_bene=rpc_beneficiario.ced_bene 
				 AND cxp_sol_dc.codemp=rpc_proveedor.codemp AND cxp_sol_dc.codemp=rpc_beneficiario.codemp AND cxp_sol_dc.estapr='".$as_tipooperacion."' ".$as_tipo;
