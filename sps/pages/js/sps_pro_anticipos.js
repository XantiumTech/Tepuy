...
function ue_buscar()
{
   pagina="sps_cat_anticipos.html.php";
   catalogo = popupWin(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=440,height=250,resizable=yes,location=no,top=0,left=0");
}
...
{  
  ...
  var cajas = new Array('txtcodper','txtnomper','txtapeper','txtcodnom','txtdennom','txtfecantper','txtanoserper','txtmesserper','txtdiaserper','txtmotant','txtmondeulab','txtmonporant','txtmonant','txtestant','txtobsant' );
  for (i=0; i<cajas.length; i++)
  {
	  $(cajas[i]).value = arr_datos[i];
  }
  if (arr_datos[13] == 0)
  { $(cajas[13]).value = 'Solicitud'; }
  if (arr_datos[13] == 1)
  { $(cajas[13]).value = 'Aprobado'; }
  if (arr_datos[13] == 2)
  { $(cajas[13]).value = 'Rechazado'; }
  if (arr_datos[13] == 3)
  { $(cajas[13]).value = 'Pagado'; }
