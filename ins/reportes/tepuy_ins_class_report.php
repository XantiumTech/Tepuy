...
class tepuy_ins_class_report
{
	...
	function tepuy_ins_class_report()
	{
	...
	function uf_select_comprobantes($as_procede)	
	{
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		$ls_sql="DELETE FROM `scg_dt_cmp` WHERE (procede='SCBBTR' OR procede='SCBBCH') and codban='---' and ctaban='-------------------------'";
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===true)
		{
			$this->io_sql->commit();
		...
		if($as_procede=="CXP")
		{
			$ls_sql="SELECT scg_dt_cmp.*, cxp_dt_solicitudes.numrecdoc ". //codemp,procede,comprobante,fecha,codban,ctaban".
				"  FROM scg_dt_cmp, cxp_dt_solicitudes".
				" WHERE scg_dt_cmp.codemp='".$this->ls_codemp."'".
				"   AND scg_dt_cmp.procede like '%".$as_procede."%'".
				"   AND scg_dt_cmp.codemp=cxp_dt_solicitudes.codemp ".
				"   AND scg_dt_cmp.comprobante=cxp_dt_solicitudes.numsol ".
				...
				" ORDER BY scg_dt_cmp.comprobante,scg_dt_cmp.procede";	
		}
		else
		{	
...
} //fin  class tepuy_ins_class_report
?>
