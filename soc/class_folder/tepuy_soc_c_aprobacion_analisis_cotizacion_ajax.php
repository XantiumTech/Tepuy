<?php
	...
	{	
		...
		$lo_object[0]="";
		require_once("tepuy_soc_c_aprobacion_analisis_cotizacion.php");
		$io_aprobacion = new tepuy_soc_c_aprobacion_analisis_cotizacion("../../");
		...
		while ($row=$io_aprobacion->io_sql->fetch_row($rs_data))	  
		      {
			    ...
			    $lo_object[$li_fila][3]="<input name=txtnompro".$li_fila." id=txtnompro".$li_fila."  type=text class=sin-borde  size=50   style=text-align:left     value='".$ls_nompro."' readonly>";
		 	    $lo_object[$li_fila][4]="<input name=txtobsanacot".$li_fila." id=txtobsanacot".$li_fila."  type=text class=sin-borde  size=50   style=text-align:left     value='".$ls_obsana."' readonly>";
				$lo_object[$li_fila][5]="<input name=txtfecanacot".$li_fila." id=txtfecanacot".$li_fila."  type=text class=sin-borde  size=10   style=text-align:center   value='".$ld_fecanacot."' readonly>";
				$lo_object[$li_fila][6]="<input name=txttipanacot".$li_fila." id=txttipanacot".$li_fila."  type=text class=sin-borde  size=10   style=text-align:center   value='".$ls_tipanacot."' readonly>";
				if ($as_tipope=='R')
				   {
