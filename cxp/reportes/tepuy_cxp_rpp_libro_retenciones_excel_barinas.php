<?php
	...
	$ls_titulo     = "SERVICIO AUTÓNOMO MUNICIPAL DE ADMINISTRACIÓN TRIBUTARIA - SAMAT MUNICIPIO BARINAS";
	$ls_titulo1    = "FORMATO ÚNICO PARA REPORTE DE RETENCIONES DEL IMPUESTO SOBRE ACTIVIDADES ECONÓMICAS";
	...
	$ls_periodo    = "MES: ".$ls_mesletras." ".$ls_agno."";
	...
	else // Imprimimos el reporte
	{
		...
		$lo_hoja->set_column(2,3,40);
		$lo_hoja->set_column(3,4,25);
		//$lo_hoja->set_column(5,5,15);
		$lo_hoja->set_column(4,5,20);	
		//$lo_hoja->set_column(7,7,20);	
		//$lo_hoja->set_column(8,8,15);	
		$lo_hoja->set_column(5,6,15);
		$lo_hoja->set_column(6,7,15);	
		$lo_hoja->set_column(7,9,15);	
		$lo_hoja->set_column(8,10,15);	
		$lo_hoja->set_column(9,11,15);	
		$lo_hoja->set_column(10,12,15);
		$lo_hoja->set_column(11,13,15);	
		$lo_hoja->set_column(12,14,10);	
		$lo_hoja->set_column(13,15,15);	
		$lo_hoja->set_column(14,16,15);		
		$lo_hoja->set_column(15,17,12);	
		$lo_hoja->set_column(16,18,18);	
		$lo_hoja->set_column(17,19,12);	
		$lo_hoja->write(0,3,$ls_titulo,$lo_encabezado);
		$lo_hoja->write(1,3,$ls_titulo1,$lo_encabezado);
		$lo_hoja->write(2,1,"AGENTE DE RETENCIÓN",$lo_encabezado);
		$lo_hoja->write(2,2,"ALCALDIA DEL MUNICIPIO BARINAS",$lo_encabezado);

		$lo_hoja->write(3,1,"RIF DEL AGENTE DE RETENCIÓN",$lo_encabezado);
		$lo_hoja->write(3,2,"G-200002859",$lo_encabezado);
		$lo_hoja->write(3,3,"MES ENTERADO",$lo_encabezado);
		$lo_hoja->write(3,4, $ls_periodo,$lo_encabezado);
		$lo_hoja->write(4,1,"CORREO ELECTRONICO",$lo_encabezado);
		$lo_hoja->write(4,2,"alcaldiaretencion@gmail.com",$lo_encabezado);
		$lo_hoja->write(5,1,"ELABORADO POR",$lo_encabezado);
		$lo_hoja->write(5,2,"",$lo_encabezado);
		$lo_hoja->write(5,3,"TELÉFONO DE CONTACTO",$lo_encabezado);
		$lo_hoja->write(5,4,"0414-",$lo_encabezado);
		$lo_hoja->write(6,0, "NRO.",$lo_titulo);	
		$lo_hoja->write(6,1, "LICENCIA ISAE",$lo_titulo);
		$lo_hoja->write(6,2, "NOMBRE O RAZON SOCIAL",$lo_titulo);	
		$lo_hoja->write(6,3, "RIF",$lo_titulo);	
		$lo_hoja->write(6,4, "NRO. DE LA FACTURA",$lo_titulo);
		$lo_hoja->write(6,5, "FECHA DE PAGO DE LA FACTURA",$lo_titulo);
		$lo_hoja->write(6,6, "MONTO DE LA FACTURA",$lo_titulo);
		$lo_hoja->write(6,7, "ALÍCUOTA",$lo_titulo);
		$lo_hoja->write(6,8, "MONTO RETENIDO",$lo_titulo);
		...
		{	
		    ...
		    while($row=$io_report->io_sql->fetch_row($rs_resultado))	
			{
				...
				if($ls_tiporeporte==1)
				{
					$la_cmpret=$io_report->uf_select_rowdata($io_sql,"SELECT DISTINCT max(a.numrecdoc) as numrecdoc,
																					  ...
																					  iva_ret,max(tiptrans) as tiptrans, max(c.fecrep) as fecharet".
																    "   FROM cxp_rd_deducciones a,scb_dt_cmp_ret b, scb_cmp_ret c     ".
																    "  WHERE a.codemp=b.codemp AND a.numrecdoc=b.numfac AND b.numcom=c.numcom AND 
				...
				if(count($la_cmpret)>0)
				{
					...
					if ($ls_codret=='01')
					{
					 	...
					 	$li_fecharet    = $la_cmpret["fecharet"];
					 	$ls_fecharet=substr($li_fecharet,8,2).'-'.substr($li_fecharet,5,2).'-'.substr($li_fecharet,0,4);
					}
					else
					{
						 $ldec_monret    = 0; 
				...
				// $lo_hoja->write($li_row, 0, $ls_cheque, $lo_datacenter);
				 $lo_hoja->write($li_row, 0, $li, $lo_datacenter);
				 $lo_hoja->write($li_row, 1, $fecha, $lo_datacenter);
				 $lo_hoja->write($li_row, 2, $ls_nombre, $lo_dataleft);
				...
				// $lo_hoja->write($li_row, 8, '', $lo_datacenter);
				 //$lo_hoja->write($li_row, 6, trim($ls_numrecdoc), $lo_datacenter);
				 $lo_hoja->write($li_row, 4, $ls_factura, $lo_datacenter);
				 $lo_hoja->write($li_row, 6, $ldec_baseimp, $lo_dataright);
				 $lo_hoja->write($li_row, 7, $ldec_porded, $lo_datacenter);
				 $lo_hoja->write($li_row, 8,$ls_numnd, $lo_datacenter);
				/*  $lo_hoja->write($li_row, 9,$ls_numnc, $lo_datacenter);
				 $lo_hoja->write($li_row, 10,$ls_tiptrans, $lo_datacenter);
				 $lo_hoja->write($li_row, 11,trim($row["numrecdoc"]), $lo_datacenter);
				 $lo_hoja->write($li_row, 11, $ls_factura, $lo_datacenter);
				 $lo_hoja->write($li_row, 12,$ldec_montodoc, $lo_dataright);
				 $lo_hoja->write($li_row, 13,$ldec_sinderiva, $lo_dataright);
				 $lo_hoja->write($li_row, 14,$ldec_baseimp, $lo_dataright);
				 $lo_hoja->write($li_row, 15,$ldec_porcar, $lo_datacenter);
				 $lo_hoja->write($li_row, 16,$ldec_monimp, $lo_dataright);
				 $lo_hoja->write($li_row, 17,$ldec_montoret, $lo_dataright);
				 $lo_hoja->write($li_row, 18,$ls_cmpret, $lo_datacenter);
				 $lo_hoja->write($li_row, 19,$ls_fecharet, $lo_dataright);*/
		    ...
		    $li_row12=$li_row11+1;			
			$lo_hoja->write($li_row12, 0,"Total del 12% de IVA Retenido:",$lo_titulo);
			$lo_hoja->write($li_row12, 1,$ldec_totimpret9, $lo_dataright);

            $li_row13=$li_row12+1;			
			$lo_hoja->write($li_row13, 0,"Total del 100% de IVA Retenido:",$lo_titulo);
