<?php
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//    REPORTE: Listado de Solicitudes de (Ayudas) Ejecucion Presupuestaria
//  ORGANISMO: ALCALDIA DE BARINAS
//  MODIFICADO POR: ING.Miguel Palencia         FECHA DE MODIFICACION : 10/04/2015
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	session_start();
	...
	function uf_insert_seguridad($as_titulo)
	{
		...
		$lb_valido=$io_fun_sep->uf_load_seguridad_reporte("AYU","tepuy_sep_r_solicitudes_ayuda.php",$ls_descripcion);
		return $lb_valido;
	}
	//-----------------------------------------------------------------------------------------------------------------------------------

	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina($as_titulo,$as_parroquia,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->rectangle(115,710,460,40);
		$io_pdf->line(430,750,430,710);
		$io_pdf->line(430,730,575,730);
		...
		{
			$io_pdf->addText(155,732,11,$as_titulo." <b>".substr($as_parroquia,0,5)."</b>"); // Agregar el título
			$li_tm=$io_pdf->getTextWidth(11,$as_parroquia);
			$tm=288-($li_tm/2);
			$io_pdf->addText($tm,718,11,"<b>".substr($as_parroquia,6,strlen($as_parroquia))."</b>"); // Agregar el título
		}
		else
		{
			$io_pdf->addText(170,725,11,$as_titulo); // Agregar el título
		}
		$io_pdf->addText(460,735,10,"Fecha: ".date("d/m/Y")); // Agregar la Fecha
		$io_pdf->addText(460,715,10,"Hora: ".date("h:i a")); // Agregar la hora
		$io_pdf->restoreState();
		$io_pdf->closeObject();
	...
	function uf_print_piecabecera($ai_total,$ai_totrows,$io_pdf)
	{
		...
		// Fecha Creación: 30/03/2015
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$la_data=array(array('name'=>'<b>Total Ayudas</b>','totrows'=>$ai_totrows,'total'=>$ai_total));
	...
	$ls_titulo="<b>RELACION DE AYUDAS A PERSONAS</b>";
	...
	$ls_parroquia=$io_fun_sep->uf_obtenervalor_get("cmbparroquia","");
	//--------------------------------------------------------------------------------------------------------------------------------
	global $ls_tipoformato;
	if($ls_tipoformato==1)
	{
	...
	if($lb_valido)
	{
		//$ls_parroquia = str_pad($ls_parroquia, 3, '0', STR_PAD_LEFT);

		$lb_valido=$io_report->uf_select_solicitudes_ayuda($ls_numsoldes,$ls_numsolhas,$ls_tipproben,$ls_codprobendes,$ls_codprobenhas,
													 ...
													 $li_despachada,$ls_orden,$ls_parroquia); // Cargar el DS con los datos del reporte
		...
		else  // Imprimimos el reporte
		{
			...
			if(!empty($ls_parroquia))
			{
				$parroquia=$io_report->uf_select_parroquia($ls_parroquia);
				$parroquia="DE LA PARROQUIA ".$parroquia;
				//$ls_parroquia="001";
				$existen=0;
			// recorre el Array para certificar que existen registros de la parroquia //
				for($i=1;$i<=$li_totrow;$i++)
				{
					$ls_codparroquia=$io_report->DS->data["codpar"][$i];
					//print ("alert($parroquia);");
					if($ls_parroquia==$ls_codparroquia)
					{
						$existen++;
					}
					else
			...
			{
				print("<script language=JavaScript>");
				print(" alert('No existen registros para esta parroquia');"); 
				//print ("alert($existen);");
				//print(" close();");
				print("</script>");
			}
			...
			uf_print_encabezado_pagina($ls_titulo,$parroquia,$io_pdf); // Imprimimos el encabezado de la página
			...
			for($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				$ls_codparroquia=$io_report->DS->data["codpar"][$li_i];
