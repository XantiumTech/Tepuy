	...
	if($ls_operacion=="BUSCAR")
	{
		...
		if($li_row>0)
		{
			while($row=$io_sql->fetch_row($rs_cta))
			{
				...
				print "<td><a href=\"javascript: aceptar('$ls_numfactura','$ls_cedcli','$ls_nombre','$ls_forpag','$ls_obsfac','$ld_fecfac','$ls_tipo');\">".$ls_numfactura."</a></td>";
...
<script language="JavaScript">
	function aceptar(ls_numfactura,ls_cedcli,ls_nomcli,ls_forpag,ls_obsfac,ld_fecfac,tipo)
	{
		if(tipo=="DESDE")
		{
	...
	function ue_search()
  	{
		...
		f.action="tepuy_sfa_cat_facturas.php";
		f.submit();
	}
</script>
