...
class tepuy_siv_class_report
{
	...
	function tepuy_siv_class_report()
	{
	...
	function uf_select_almacen($as_codemp,$as_codalm,$as_codart,$ai_orden)	
	{
		...
		$ls_sql="SELECT siv_articuloalmacen.codalm,".
				"       (SELECT nombre FROM tepuy_empresa".
				"         WHERE siv_articuloalmacen.codemp=tepuy_empresa.codemp) AS nombre,".
	...
	function uf_select_articulos($as_codemp,$as_codalm,$as_codart,$ad_desde,$ad_hasta,&$ai_total,$ai_ordenart)	
	{
		...
		$ls_sql="SELECT siv_dt_movimiento.codart,".
				"       (SELECT nombre FROM tepuy_empresa".
				"         WHERE siv_dt_movimiento.codemp=tepuy_empresa.codemp) as nombre,".
				"       (SELECT denart FROM siv_articulo".
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql="SELECT siv_despacho.numorddes,siv_despacho.numsol,siv_despacho.coduniadm,siv_despacho.obsdes,siv_despacho.fecdes,".
				"       (SELECT nombre FROM tepuy_empresa".
				"         WHERE siv_despacho.codemp=tepuy_empresa.codemp) AS nombre,".
				...
				"         WHERE spg_unidadadministrativa.coduniadm=siv_despacho.codunides) AS denuniadm".
				"  FROM siv_despacho".
				" WHERE codemp='".$as_codemp."' ".
				//"   AND estrevdes = 1".
				"   AND estdes = 1".
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql= "SELECT siv_dt_despacho.*,".
				 ...
				 "       (SELECT spg_cuenta FROM siv_articulo".
				 "         WHERE siv_articulo.codart=siv_dt_despacho.codart) AS spg_cuenta".
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql="SELECT siv_recepcion.numconrec,siv_recepcion.numordcom,siv_recepcion.cod_pro,".
				...
				"       (SELECT nombre FROM tepuy_empresa".
				"         WHERE siv_recepcion.codemp=tepuy_empresa.codemp) AS nombre,".
				"       (SELECT nomfisalm FROM siv_almacen".
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql="SELECT siv_transferencia.numtra,siv_transferencia.codalmori,siv_transferencia.codalmdes,".
				...
				"      (SELECT nombre FROM tepuy_empresa".
				"        WHERE siv_transferencia.codemp=tepuy_empresa.codemp) AS nombre,".
				"      (SELECT nomfisalm FROM siv_almacen".
						...
						if($lb_valido)
						{
							...
							if($lb_valido)
							{
								...
								if($lb_valido)
								{
									...
									$lb_valido=$this->uf_select_fecha_entrega($as_codemp,$ls_numordcom,$fecha_entrega);	
									if ($lb_valido)
									{
									 $this->dts_reporte->insertRow("fecentr",$fecha_entrega);										
									}
									else 
									{
									  $this->dts_reporte->insertRow("fecentr","");
									}								
...
//---------------------------------------------------------------------------------------------------------------------------------	
//---------------------------------------------------------------------------------------------------------------------------------	
   function uf_select_fecha_entrega($as_codemp,$as_numordcom,&$fecha_entrega)	
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	         Function: uf_select_fecha_entrega
		//	           Access: public
		//  		Arguments: $as_codemp      //  codigo de empresa
		//  			       $as_numordcom     //  numero de la orden de  compra
		//                     $fecha_entrega     //  monto total del articulo (referencia)
		//	         Returns : $lb_valido True si se creo el Data stored correctamente ó False si no se creo
		//	      Description: Función que se encarga de realizar la busqueda de la fecha de entrega del bien en la orden de compra
		//         Creado por: Ing. Rivero Jennifer.
		//   Fecha de Cracion: 08/02/2007							Fecha de Ultima Modificación:  
		///////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		$ls_sql=" SELECT fecent ".
		        " FROM   soc_ordencompra ".
				" WHERE  codemp='".$as_codemp."' AND numordcom='".$as_numordcom."' AND  estcondat='B' "; 
		$rs_data=$this->io_sql->select($ls_sql);
   ...
   function uf_select_listadoimpu_presupuestaria($as_codemp,$as_coddesde,$as_codhasta,$ai_orden)	
	{
		...
		$ls_sql=" SELECT distinct siv_articulo.codart,siv_articulo.denart,siv_articulo.prearta,siv_articulo.spg_cuenta,spg_cuentas.denominacion ".
