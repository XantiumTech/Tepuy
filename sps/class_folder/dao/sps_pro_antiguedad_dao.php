  ...
  {       
  	$lb_valido = false;
		...
		elseif($row=$this->io_sql->fetch_row($rs_data))
		{
			...
			{
				$ls_sql    = "SELECT operador,canmes,tiempo,diasal,condicion,estacu,diaacu,numcon FROM sps_articulos WHERE numart='".$ps_numart."' and fecvig='".$ld_fecha."' ORDER BY operador,canmes,tiempo,diasal,condicion,estacu,diaacu,numcon DESC ";				  
				$rs_data   = $this->io_sql->select($ls_sql);
				if($rs_data==false)
				{
					$this->io_function_sb->message("Error en getConsultaArticulo ");
