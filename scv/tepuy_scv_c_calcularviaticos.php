...
class tepuy_scv_c_calcularviaticos
{
	...
	function uf_scv_load_cuentas_presupuestos($as_codemp,$as_codsolvia,&$aa_object)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->solicitud_viaticos MÉTODO->uf_scv_load_cuentas_presupuestarias ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_scv_load_cuentas_contables($as_codemp,$as_codsolvia,&$aa_object)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->solicitud_viaticos MÉTODO->uf_scv_load_cuentas_contables ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
								   ...
								   $as_tipodestino,$as_descripcion,$ai_monto,$aa_seguridad,$ai_orden,$as_fecha,$as_cedper)
	{
		...
		else
		{
			...
			if($li_result===false)
			{
				...
				{   
					$this->is_msg_error="CLASE->tepuy_int_scg MÉTODO->uf_scv_insert_dt_scg ERROR->Fallo alguna clave foranea";
				}
				else
				{
					$this->is_msg_error="CLASE->tepuy_int_scg MÉTODO->uf_scv_insert_dt_scg ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
				}
														 ...
														 $as_descripcion,$ad_fecha,$ai_monto,$as_codper,$aa_seguridad)
	{
		...
		if($li_row===false)
		{  
           	$this->io_msg->message("CLASE->tepuy_scv_c_calcularviaticos MÉTODO->uf_scv_procesar_recepcion_documento_viatico ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));			
			$lb_valido=false;
		}
		if($lb_valido)
		{
	...
	function uf_insert_recepcion_documento_gasto($as_comprobante,$as_codtipdoc,$as_cedbene,$as_codpro,$ai_monto)
	{
		...
		if($rs_data===false)
		{   
           	$this->io_msg->message("CLASE->tepuy_scv_c_calcularviaticos MÉTODO->uf_insert_recepcion_documento_gasto ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{           
	...
	function uf_insert_recepcion_documento_contable($as_comprobante,$as_codtipdoc,$as_cedbene,$as_codpro,$ai_monto)
	{
		...
		if($rs_data===false)
		{   
           	$this->io_msg->message("CLASE->tepuy_scv_c_calcularviaticos MÉTODO->uf_insert_recepcion_documento_contable ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{           
