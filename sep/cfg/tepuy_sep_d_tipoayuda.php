...
<html>
<head>
<title>Registro de Tipo de Ayudas</title>
...
<table width="780" border="0" align="center" cellpadding="0" cellspacing="0" class="contorno">
  ...
  <tr>
	...
	<table width="776" border="0" align="center" cellpadding="0" cellspacing="0">
		...
		<td width="423" height="20" bgcolor="#E7E7E7" class="descripcion_sistema">Sistema Control de Ayudas -><b> Tipos </b></td>
...
$io_tipoayuda  = new tepuy_sep_c_tipo($conn);//Instanciando la Clase tepuy Definiciones.
...
//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
	...
	$ls_sistema  = "AYU";
	$ls_ventanas = "tepuy_sep_d_tipoayuda.php";
...
if (array_key_exists("operacion",$_POST))
   {
     ...
     $ls_codayuda    = $_POST["txtcodigo"];
     $ls_dentipoayuda= $_POST["txtdenominacion"];
...
if ($ls_operacion=="NUEVO")
   {
	 $ls_codayuda=$io_funciondb->uf_generar_codigo($ls_empresa,$ls_codemp,'sep_tipoayuda','codtipayuda');
	 if (empty($ls_codayuda))
	    {
...
if ($ls_operacion=="ue_guardar")
   { 
	 $lb_existe=$io_tipoayuda->uf_select_tipoayuda($ls_codemp,$ls_codayuda);
	 if ($lb_existe)
        { 
	      if ($ls_estatus=="NUEVO")
		     {
			   $io_msg->message("Este Código de Tipo de Ayudas Ya Existe !!!");  
			   $lb_valido=false;
			 }
		  elseif($ls_estatus=="GRABADO")
		     {
		       $lb_valido=$io_tipoayuda->uf_update_tipoayuda($ls_codemp,$ls_codayuda,$ls_dentipoayuda,$la_seguridad);
  	           if ($lb_valido)
		          {
				    $io_sql->commit();
				    $io_msg->message("Registro Actualizado !!!");
				    $ls_codayuda=$io_funciondb->uf_generar_codigo($lb_empresa,$ls_codemp,'sep_tipoayuda','codtipayuda');
				    $ls_dentipoayuda="";
					$ls_estatus="NUEVO";
			      }
		       else
	    ...
	    {  
		  $lb_valido=$io_tipoayuda->uf_insert_tipoayuda($ls_codayuda,$ls_dentipoayuda,$la_seguridad);
	      if($lb_valido)
		    {
		      $io_sql->commit();
...
if ($ls_operacion=="ELIMINAR")
   {
	  ...
	  if ($lb_existe)
	     {
		   $ls_condicion = " AND (column_name='codtipayuda')";//Nombre del o los campos que deseamos buscar.
	       $ls_mensaje   = "";                              //Mensaje que será enviado al usuario si se encuentran relaciones a asociadas al campo.
	       $lb_tiene     = $io_chkrel->uf_check_relaciones($ls_codemp,$ls_condicion,'sep_tipoayuda',$ls_codayuda,$ls_mensaje);//Verifica los movimientos asociados a la cuenta  
		   if (!$lb_tiene)
		      {
			    $lb_valido=$io_tipoayuda->uf_delete_tipoayuda($ls_codemp,$ls_codayuda,$la_seguridad);
			    if ($lb_valido)
				   {
					 $io_sql->commit();
				   ...
				   {
					 $io_sql->rollback();
					 $io_msg->message($io_tipoayuda->is_msg_error);
				   }	 
		      }
		   else
		      {
    ...
    <tr>
      <td width="517" height="170"><div align="center">
        <table  border="0" cellspacing="0" cellpadding="0" class="formato-blanco" align="center">
          <tr>
            <td height="22" colspan="2" class="titulo-ventana">Registro de Tipo de Ayudas</td>
          </tr>
          <tr>
            <td height="22" >&nbsp;</td>
          ...
          <tr>
            ...
            <td width="334" height="22" ><input name="txtcodigo" type="text" id="txtcodigo" value="<?php print  $ls_codayuda ?>" size="2" maxlength="2" onKeyPress="return keyRestrict(event,'1234567890');" style="text-align:center "  onBlur="javascript:rellenar_cadena(this.value,2);">
                <input name="operacion" type="hidden" id="operacion"  value="<?php print $ls_operacion ?>"></td>
          </tr>
          <tr>
            <td height="22"><div align="right">Denominaci&oacute;n</div></td>
            <td height="22"><input name="txtdenominacion" id="txtdenominacion" value="<?php print $ls_dentipoayuda ?>" type="text" size="60" maxlength="60" onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnñopqrstuvwxyz '+'.,-');"></td>
          </tr>
          <tr>
            <td height="22">&nbsp;</td>
            <td height="22">&nbsp;</td>
...
function ue_nuevo()
{
	...
	if (li_incluir==1)
	   {	
	     ...
	     f.action="tepuy_sep_d_tipoayuda.php";
	     f.submit();
	   }
    else
	   {
...
if (((lb_status=="GRABADO")&&(li_cambiar==1))||(lb_status=="NUEVO")&&(li_incluir==1))
   {
     with (document.form1)
	      {
	        if (campo_requerido(txtcodigo,"El codigo del Tipo de Ayuda debe estar lleno !!!")==false)
			   {
		 	     txtcodigo.focus();
			   }
			else
			   {
			     ...
			     if (campo_requerido(txtdenominacion,"La denominacion del Tipo de Ayuda debe estar llena !!")==false)
				    {
...
function ue_buscar()
{
	...
	if (li_leer==1)
	   {
	     f.operacion.value="";			
	     pagina="tepuy_sep_cat_tipoayuda.php";
