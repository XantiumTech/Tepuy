<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","tepuy_mis_p_reverso_asignacion_sob.php",$ls_permisos,$la_seguridad,$la_permisos);
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					if($lb_valido)
					{
						$in_class->io_msg->message("La Contabilización de la Asignación  ".$ls_codasi." fue reversada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo reversar la Contabilización de la asignacion ".$ls_codasi);
					}		
				}
			}
			$li_totrows=1;
  ...
  {  
		...
		for($li_i=1;$li_i<=$li_total_record;$li_i++)
		{
			if(array_key_exists("chksel".$li_i,$_POST))
			{
				...
				if ($lb_valido) { $in_class_int->io_msg->message("La Asignación fue reversada"); }
				else { $in_class_int->io_msg->message("Error en el reverso de la Asignación."); }		
			}
		}
  }
