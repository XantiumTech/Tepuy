...
class tepuy_srh_c_personal 
{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	public function tepuy_srh_c_personal($path)
	{	
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function uf_srh_guardarPersonal ($ao_personal,$as_operacion="insertar", $aa_seguridad)
  { 
		...
		//         access: public (tepuy_sno_personal)														                    //
      ...
      $ao_personal->fecegrper=$this->io_funcion->uf_convertirdatetobd($ao_personal->fecegrper);
	...
	if ($ao_personal->fecegrper =="" )
	{$ao_personal->fecegrper = '1900-01-01';}
	...
	{ $this->io_sql->begin_transaction();
	  ...
	  $ls_sql = "INSERT INTO sno_personal (codper, cedper, nomper, apeper, dirper, fecnacper, edocivper, nacper, codpai, codest, codmun, codpar, telhabper, telmovper, coreleper, sexper, estaper, pesper, codpro, nivacaper, codcom, codran, cedbenper, numhijper, contraper, cenmedper ,turper, horper, hcmper, tipsanper, numexpper, tipvivper, tenvivper, monpagvivper, cuecajahoper, cajahoper, cuelphper, cuefidper, fecingadmpubper,anoservpreper,fecingper, obsegrper,  obsper, estper,   codpainac , codestnac, enviorec, fotper, situacion, fecsitu, talcamper, talpanper, talzapper, anoservprecont,anoservprefijo, codemp) ".	  
	            "VALUES ('$ao_personal->codper', '$ao_personal->cedper', '$ao_personal->nomper', '$ao_personal->apeper', '$ao_personal->dirper', '$ao_personal->fecnacper', '$ao_personal->edocivper', '$ao_personal->nacper', '$ao_personal->codpai', '$ao_personal->codest', '$ao_personal->codmun', '$ao_personal->codpar', '$ao_personal->telhabper', '$ao_personal->telmovper', '$ao_personal->coreleper', '$ao_personal->sexper', '$ao_personal->estaper', '$ao_personal->pesper', '$ao_personal->codpro', '$ao_personal->nivacaper', '$ao_personal->codcom', '$ao_personal->codran',  '$ao_personal->cedbenper', '$ao_personal->numhijper', '$ao_personal->contraper', '$ao_personal->cenmedper' ,'$ao_personal->turper', '$ao_personal->horper', '$ao_personal->hcmper', '$ao_personal->tipsanper', '$ao_personal->numexpper', '$ao_personal->tipvivper', '$ao_personal->tenvivper', '$ao_personal->monpagvivper', '$ao_personal->cuecajahoper', '$ao_personal->cajahoper', '$ao_personal->cuelphper', '$ao_personal->cuefidper', '$ao_personal->fecingadmpubper', '$ao_personal->anoservpreper' , '$ao_personal->fecingper', '$ao_personal->obsegrper',  '$ao_personal->obsper', '0',''$ao_personal->codpainac','$ao_personal->codestnac', '$ao_personal->enviorec', '$ao_personal->fotper', '$ao_personal->talcamper', '$ao_personal->talpanper', '$ao_personal->talzapper','$ao_personal->anoservprecont', '$ao_personal->anoservprefijo','".$this->ls_codemp."')";
...
function uf_srh_buscar_personal($as_codper,$as_cedper,$as_apeper,$as_nomper,$as_tipo)
	...
	case '1' :
		...
		$ls_sql= " SELECT sno_personal.*, tepuy_pais.despai, tepuy_estados.desest, tepuy_municipio.denmun, ".
				 " tepuy_parroquia.denpar, sno_profesion.despro,  ".
				 ...
				 " AND sno_rango.codran = sno_personal.codran) AS desran, ".
				  " (SELECT codpai FROM tepuy_pais ".
				 "	 WHERE tepuy_pais.codpai = sno_personal.codpainac ) AS codpainac, ".
				 "	(SELECT codest FROM tepuy_estados ".
				 "	  WHERE tepuy_estados.codpai = sno_personal.codpainac ".
				 "		AND tepuy_estados.codest = sno_personal.codestnac ) AS codestnac ".
		 		 " FROM tepuy_pais, tepuy_estados, tepuy_municipio, tepuy_parroquia, sno_personal		          
		...
		{	
			...
			while ($row=$this->io_sql->fetch_row($rs_data)) 
			{
					...
					$ls_dirper=htmlentities ($row["dirper"]);
					...
					$ls_estaper=$row["estaper"];
					$ls_pesper=$row["pesper"];
					...
					$ls_tenvivper=htmlentities($row["tenvivper"]);
					$ls_monpagvivper=$row["monpagvivper"];
					$ls_cuecajahoper=$row["cuecajahoper"];
					$ls_cajahoper=$row["cajahoper"];
					$ls_cuelphper=$row["cuelphper"];
					$ls_cuefidper=$row["cuefidper"];
					$ls_fecingadmpubper=$this->io_funcion->uf_formatovalidofecha($row["fecingadmpubper"]);
				    $ls_fecingadmpubper=$this->io_funcion->uf_convertirfecmostrar($ls_fecingadmpubper);
					
					$ls_anoservpreper=$row["anoservpreper"];
					$ls_fecingper=$this->io_funcion->uf_formatovalidofecha($row["fecingper"]);
				    $ls_fecingper=$this->io_funcion->uf_convertirfecmostrar($ls_fecingper);
					
					$ls_fecegrper=$this->io_funcion->uf_formatovalidofecha($row["fecegrper"]);
				    $ls_fecegrper=$this->io_funcion->uf_convertirfecmostrar($ls_fecegrper);
					
				/*	$ls_fecjubper=$this->io_funcion->uf_formatovalidofecha($row["fecjubper"]);
				    $ls_fecjubper=$this->io_funcion->uf_convertirfecmostrar($ls_fecjubper);
					
					$ls_fecreingper=$this->io_funcion->uf_formatovalidofecha($row["fecreingper"]);
				    $ls_fecreingper=$this->io_funcion->uf_convertirfecmostrar($ls_fecreingper);
					
					$ls_fecfevid=$this->io_funcion->uf_formatovalidofecha($row["fecfevid"]);
				    $ls_fecfevid=$this->io_funcion->uf_convertirfecmostrar($ls_fecfevid);
					
					$ls_feclossfan=$this->io_funcion->uf_formatovalidofecha($row["fecleypen"]);
				    $ls_feclossfan=$this->io_funcion->uf_convertirfecmostrar($ls_feclossfan);*/
					
					$ls_cauegrper=$row["cauegrper"];
					$ls_obsegrper=htmlentities  ($row["obsegrper"]);
					...
					//$ls_codunivi=$row["codunivipladin"];
					//$ls_denunivi=htmlentities ($row["denunivipladin"]);	
					$ls_enviorec=$row["enviorec"];	
					$ls_descom=htmlentities ($row["descom"]);	
					$ls_desran=htmlentities ($row["desran"]);
					
					/*$ls_situacion=$row["situacion"];
					$ls_fecsitu=$this->io_funcion->uf_convertirfecmostrar($row["fecsitu"]);*/
					$ls_talcamper=htmlentities($row["talcamper"]);
					$ls_talpanper=htmlentities($row["talpanper"]);
					$ls_talzapper=$row["talzapper"];
					
					//$ls_codcausa= $row["codcausa"];	
					//$ls_dencausa= htmlentities  ($row["dencausa"]);
					$ls_anoservprefijo=$row["anoservprefijo"];	
					$ls_anoservprecont=$row["anoservprecont"];
	...
	switch ($_SESSION["ls_gestor"])
		{
			case "MYSQL":
				$ls_sql=" SELECT DISTINCT (sno_personalnomina.codper),sno_personal.cedper,sno_personal.nomper, ".
				  		" sno_personal.apeper, sno_personal.fecingper, MAX(sno_asignacioncargo.denasicar) as denasicar,   
						  MAX(sno_cargo.descar) as descar".
						" FROM sno_nomina, sno_personal, sno_personalnomina   ".
		...
		{	
			...
			while ($row=$this->io_sql->fetch_row($rs_data)) 
			{
				   ...
				   if ($ls_cargo2!="Sin Cargo")
					...
					$cell->appendChild($dom->createTextNode($row['codper']." ^javascript:aceptar_personal2(\"$ls_codper\", \"$ls_nomper\", \"$ls_apeper\",\"$ls_cargosup\",\"$ls_codperdestino\",  \"$ls_nomperdestino\",\"$ls_carsupdestino\");^_self"));
	...
	$lb_valido=true;				
		...
		{	
			...
			while ($row=$this->io_sql->fetch_row($rs_data)) 
			{
					...
					$ls_nomper=trim (htmlentities ($row["nomper"]));
					$ls_apeper=trim (htmlentities ($row["apeper"]));					
					$ls_fechaing=$this->io_funcion->uf_formatovalidofecha($row["fecingper"]);
				   ...
				   if ($ls_cargo2!="Sin Cargo")
					...
					$cell->appendChild($dom->createTextNode($row['codper']." ^javascript:aceptar_personal_ascenso(\"$ls_codper\",\"$ls_nomper\", \"$ls_apeper\",\"$ls_fechaing\", \"$ls_cargo\",\"$ls_codperdestino\", \"$ls_nomperdestino\",\"$ls_fechadestino\",\"$ls_cargodestino\");^_self"));
   ...
   case '6' :
	...
	$lb_valido=true;
		...
		$ls_sql= "SELECT sno_personal.codper,cedper, nomper, apeper  FROM sno_personal ".
				" WHERE sno_personal.codper like '$as_codper' ".
				"   AND cedper like '$as_cedper' ".
				"   AND nomper like '$as_nomper' ".
				"   AND apeper like '$as_apeper' ".			
				"   ORDER BY sno_personal.codper LIMIT 500";
		...
		{	
			...
			while ($row=$this->io_sql->fetch_row($rs_data)) 
			{
					...
					$cell->appendChild($dom->createTextNode($row['codper']." ^javascript:aceptar_personal3(\"$ls_codper\",\"$ls_codperdestino\");^_self"));
	...
	$lb_valido=true;
		...
		{	
			...
			while ($row=$this->io_sql->fetch_row($rs_data)) 
			{
					...
					$cell->appendChild($dom->createTextNode($row['codper']." ^javascript:aceptar_tutor(\"$ls_codper\",			 \"$ls_nomper\", \"$ls_apeper\",\"$ls_codperdestino\",  \"$ls_nomperdestino\");^_self"));
		...
		{	
			...
			while ($row=$this->io_sql->fetch_row($rs_data)) 
			{
				   ...
				   if ($ls_cargo2!="Sin Cargo")
					...
					$cell->appendChild($dom->createTextNode($row['codper']."^javascript:aceptar_persona_cargo(\"$ls_codper\",\"$ls_nomper\", \"$ls_apeper\",\"$ls_cargo\",\"$ls_codperdestino\",\"$ls_nomperdestino\",\"$ls_cargodestino\");^_self"));
		...
		{	
			...
			while ($row=$this->io_sql->fetch_row($rs_data)) 
			{
				   ...
				   if ($ls_cargo2!="Sin Cargo")
					...
					$cell->appendChild($dom->createTextNode($row['codper']."^javascript:aceptar_persona_nivaca(\"$ls_codper\",\"$ls_nomper\", \"$ls_apeper\",\"$ls_cargo\",\"$ls_codperdestino\",\"$ls_nomperdestino\",\"$ls_cargodestino\",\"$ls_nivaca\",\"$ls_nivacadestino\" );^_self"));
	...
	$lb_valido=true;
		...
		switch ($_SESSION["ls_gestor"])
		{
			...
			case "POSTGRE":
				...
				$ls_sql=" SELECT DISTINCT (sno_personalnomina.codper),sno_personal.cedper,sno_personal.nomper, ".
				  		" sno_personal.apeper, sno_personal.fecingper,sno_asignacioncargo.denasicar, 
						  sno_cargo.descar, ". 
						" sno_unidadadmin.desuniadm, sno_personalnomina.sueper, MIN(sno_personalnomina.codnom) AS codnom, ".
				        ...
				        " sno_personal.apeper, sno_personal.fecingper,  ". 
						" sno_unidadadmin.desuniadm, sno_personalnomina.sueper, sno_personalnomina.codgra, 
						  sno_personalnomina.codpas, sno_asignacioncargo.denasicar, sno_cargo.descar,".
						" sno_asignacioncargo.codasicar, sno_cargo.codcar, ".
						" sno_unidadadmin.minorguniadm, sno_unidadadmin.ofiuniadm, sno_unidadadmin.uniuniadm, 
						  sno_unidadadmin.depuniadm , sno_unidadadmin.prouniadm".
						" ORDER BY sno_personalnomina.codper LIMIT 500"; 						
		...
		{	
			...
			while ($row=$this->io_sql->fetch_row($rs_data)) 
			{
					...
					$ls_paso=trim (htmlentities ($row["codpas"]));
					
					$ls_grado=trim (htmlentities ($row["codgra"]));
				   ...
				   if ($ls_cargo2!="Sin Cargo")
					...
					$ls_uniadm=trim (htmlentities ($row["desuniadm"]));
					$ls_coduniadm= ($row["minorguniadm"].'-'.$row["ofiuniadm"].'-'.$row["uniuniadm"].'-'.$row["depuniadm"].'-'.$row["prouniadm"]);
					$ls_sueact=trim ($row["sueper"]);						
			
					$row_ = $team->appendChild($dom->createElement('row'));
					$row_->setAttribute("id",$row['codper']);
					$cell = $row_->appendChild($dom->createElement('cell'));   
					
					$cell->appendChild($dom->createTextNode($row['codper']."^javascript:aceptar_persona_movimiento(\"$ls_codper\",\"$ls_nomper\", \"$ls_apeper\",\"$ls_cargo\",\"$ls_uniadm\",\"$ls_sueact\", \"$ls_codperdestino\",\"$ls_nomperdestino\", \"$ls_caractdestino\", \"$ls_uniadmdestino\", \"$ls_sueactdestino\",\"$ls_codcar\",\"$ls_codcardestino\",\"$ls_coduniadm\",\"$ls_coduniadmdestino\",\"$ls_codnom\",\"$ls_codnomdestino\",\"$ls_paso\",\"$ls_pasodestino\",\"$ls_grado\",\"$ls_gradodestino\" );^_self"));
	...
	$lb_valido=true;
		...
		{	
			...
			while ($row=$this->io_sql->fetch_row($rs_data)) 
			{
					...
					$ls_codper=$row["cedper"];
					$ls_nomper=trim (htmlentities ($row["nomper"]));
					$ls_apeper=trim (htmlentities ($row["apeper"]));
					$ls_codpro=trim ($row["codpro"]);
					$ls_despro=trim (htmlentities ($row["despro"]));
					$ls_nacper=trim ($row["nacper"]);
			
					$row_ = $team->appendChild($dom->createElement('row'));
					$row_->setAttribute("id",$row['codper']);
					$cell = $row_->appendChild($dom->createElement('cell'));   
					
					$cell->appendChild($dom->createTextNode($row['codper']." ^javascript:aceptar_persona_contrato(\"$ls_codper\",			 \"$ls_nomper\", \"$ls_apeper\",\"$ls_codperdestino\",  \"$ls_nomperdestino\", \"$ls_apeperdestino\",\"$ls_codpro\",\"$ls_codprodestino\",\"$ls_despro\",\"$ls_desprodestino\",\"$ls_nacper\",\"$ls_nacperdestino\");^_self"));
	...
	$lb_valido=true;
		...
		{	
			...
			while ($row=$this->io_sql->fetch_row($rs_data)) 
			{
					...
					$cell->appendChild($dom->createTextNode($row['codper']." ^javascript:aceptar_bono_merito(\"$ls_codper\",			 \"$ls_nomper\", \"$ls_apeper\",\"$ls_codperdestino\",  \"$ls_nomperdestino\", \"$ls_dentippersss\",\"$ls_tipperdestino\" );^_self"));
   ...
   case '14' :
		...
		$ls_sql= " SELECT sno_personal.*, tepuy_pais.despai, tepuy_estados.desest, ".
				 " sno_profesion.despro ".			
		 		 " FROM tepuy_pais, tepuy_estados,  sno_personal		          
		           LEFT JOIN sno_profesion ON (sno_profesion.codpro = sno_personal.codpro) ".			  
				 " WHERE codper like '$as_codper' ".
				 " AND cedper like '$as_cedper' ".
			 	 " AND nomper like '$as_nomper' ".
...
public function getCodPersonal($as_codper,&$ao_datos="")
	   ...
	   /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if($rs_data===false)
			{
					$this->io_msg->message("CLASE->tepuy_srh_c_personal MÉTODO->getCodPersonal ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message)); 
...
public function getCedPersonal($as_cedper,&$ao_datos="")
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if($rs_data===false)
			{
					$this->io_msg->message("CLASE->tepuy_srh_c_personal MÉTODO->getCedPersonal ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message)); 
...
//----------------------------------------------------------------------------------------------------------------------------
	function uf_srh_getProximoCodigo_estudio($as_codper)
    {
		...
		//         Access: public (tepuy_srh_d_personal)
		//      Argumento: 
...
function uf_srh_insert_familiar($as_codper,$as_cedfam,$as_nomfam,$as_apefam,$as_sexfam,$ad_fecnacfam,$as_nexfam,$ai_estfam,
								$ai_hcfam,$ai_hcmfam,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql="INSERT INTO sno_familiar".
				"(codemp,codper,cedfam,nomfam,apefam,sexfam,fecnacfam,nexfam,estfam,hcfam,hcmfam)VALUES".
				"('".$this->ls_codemp."','".$as_codper."','".$as_cedfam."','".$as_nomfam."','".$as_apefam."',".
				"'".$as_sexfam."','".$ad_fecnacfam."','".$as_nexfam."','".$ai_estfam."','".$ai_hcfam."','".$ai_hcmfam."')";				
		$this->io_sql->begin_transaction()	;
		$li_row=$this->io_sql->execute($ls_sql);
		if($li_row===false)
		{
...
function uf_srh_guardar_familiar($ao_familiar,$as_operacion="insertar", $aa_seguridad)
	{	
		...
		//		   Access: public (tepuy_snorh_d_familiar)
		//	    Arguments: as_codper  // Código del Personal
		//				   as_cedfam  // Cedula
		//				   as_nomfam  // Nombre
		//				   as_apefam  // Apellido
			...
			case "insertar":
				if($this->uf_srh_select_familiar($ao_familiar->codper,$ao_familiar->cedfam)===false)
				{
					$lb_valido=$this->uf_srh_insert_familiar($ao_familiar->codper,$ao_familiar->cedfam,
														 ...
														 $ao_familiar->hcfam,$ao_familiar->hcmfam,$aa_seguridad);
				}
				else
				{
			...
			case "modificar":
				if(($this->uf_srh_select_familiar($ao_familiar->codper,$ao_familiar->cedfam)))
				{
					$lb_valido=$this->uf_srh_update_familiar($ao_familiar->codper,$ao_familiar->cedfam,$ao_familiar->nomfam,
														 ...
														 $ao_familiar->hcmfam,$aa_seguridad);
				}
				else
				{
	...
	{		
		...
		{	
			...
			{	
				...
				\"$ls_cedfamdestino\",\"$ls_nomfamdestino\",\"$ls_apefamdestino\",\"$ls_sexfamdestino\",\"$ls_fecnacfamdestino\",\"$ls_nexfamdestino\",\"$ls_hcfamdestino\",\"$ls_hcmfamdestino\",\"$ls_estfamdestino\");^_self"));
...
function uf_srh_guardar_permiso($ao_permiso,$as_operacion="insertar", $aa_seguridad)
	{	
		...
		//		   Access: public (tepuy_snorh_d_permiso)
		//	    Arguments: as_codper  // Código de Personal
		//				   ai_numper  // Número del Permiso
		//				   ad_feciniper  // fecha inicio
		//				   ad_fecfinper  // fecha fin
  ...
  { 
  	...
  	if ($as_operacion == "modificar")
	{
	 $this->io_sql->begin_transaction();
	  ...
	  "codtipded= '$ao_deduccion->codtipded' ".
	  "WHERE codtipded= '$ao_deduccion->codtipded' AND codper = '$ao_deduccion->codper' AND codemp='".$this->ls_codemp."'" ;


			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="UPDATE";
			$ls_descripcion ="Modificó la deduccion ".$ao_deduccion->codtipded." del personal  ".$ao_deduccion->codper;
			$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////	
	...
	{ $this->io_sql->begin_transaction();
	  ...
	  $ls_sql = "INSERT INTO sno_personaldeduccion (codper,codtipded,  codemp) ".	  
	            "VALUES ('$ao_deduccion->codper',  '$ao_deduccion->codtipded'  ,'".$this->ls_codemp."')";
...
//-------------------------------------------------------------------------------------------------------------------------------
	...
	{		
		...
		$ls_sql= "SELECT * FROM sno_personaldeduccion, srh_tipodeduccion   WHERE codper='".$as_codper."' 
		...
		{	
			...
			while ($row=$this->io_sql->fetch_row($rs_data)) 
			{
				...
				$cell->appendChild($dom->createTextNode($row['codtipded']." ^javascript:aceptar(\"$ls_codtipded\",\"$ls_dentipded\", \"$ls_codtipdeddestino\",\"$ls_dentipdeddestino\");^_self"));
...
function uf_srh_calcular_monto_deduccion ($as_codper, $as_codtipded)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        $lb_valido=true;
		 $ls_valor=0; 
		$ls_sql=" SELECT  srh_dt_tipodeduccion.*, sno_personal.hcmper, sno_personal.sexper, 
		          sno_personal.fecnacper, sno_personalnomina.sueper ". 
				" FROM srh_dt_tipodeduccion, sno_personal, sno_personalnomina, sno_nomina ".
				" WHERE srh_dt_tipodeduccion.codtipded='".$as_codtipded."'  ".
				" AND srh_dt_tipodeduccion.codemp='".$this->ls_codemp."'  ".				
		  ...
		  if ($num!=0) 
		  {
			...
			{   
				...
				{  
				   ...
				   if (($ls_sueldoper >= $ls_sueldo)&&($ls_edadper >= $ls_edadmin)&&($ls_edadper <= $ls_edadmax)&&($ls_sexper==$ls_sexo)&& ($ls_hcm==$ls_hcmper))
				 ...
				 {  
					...
					if (($ls_sueldoper >= $ls_sueldo)&&($ls_edadper >= $ls_edadmin)&&($ls_edadper <= $ls_edadmax)&&($ls_sexper==$ls_sexo))
					{   
...
function uf_srh_guardar_deducion_familiar ($ao_deduccion,$as_operacion="insertar", $aa_seguridad)
  { 
	...
	{ $this->io_sql->begin_transaction();
	  ...
	  $ls_sql = "INSERT INTO sno_familiardeduccion (codper,codtipded,cedfam,codemp) ".	  
	            "VALUES ('$ao_deduccion->codper','$ao_deduccion->codtipded','$ao_deduccion->cedfam','".$this->ls_codemp."')";
...
//-------------------------------------------------------------------------------------------------------------------------------
	...
	{		
		...
		$ls_sql= "SELECT * FROM sno_familiardeduccion, srh_tipodeduccion, sno_familiar".
		...
		{	
			...
			while ($row=$this->io_sql->fetch_row($rs_data)) 
			{
				...
				$cell->appendChild($dom->createTextNode($row['codtipded']." ^javascript:aceptar(\"$ls_codtipded\",\"$ls_dentipded\",\"$ls_nomfam\",\"$ls_cedfam\", \"$ls_codtipdeddestino\",\"$ls_dentipdeddestino\",\"$ls_nomfamdestino\",\"$ls_cedfamdestino\");^_self"));
...
function uf_srh_calcular_monto_deduccion_fam ($as_codper, $as_codtipded, $as_cedfam)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        $lb_valido=true;
		 $ls_valor=0; 
		$ls_sql=" SELECT srh_dt_tipodeduccion.*, sno_personalnomina.sueper,  ".
					"      (SELECT sno_familiar.nexfam from sno_familiar             ".
					"        WHERE sno_familiar.codemp= '".$this->ls_codemp."' ".
					"		   AND sno_familiar.codper='$as_codper'  ".
					"		   AND sno_familiar.cedfam='$as_cedfam') as nexofam, ".
					"      (SELECT sno_familiar.fecnacfam from sno_familiar          ".
					"        WHERE sno_familiar.codemp= '".$this->ls_codemp."' ".
					"		   AND sno_familiar.codper='$as_codper'  ".
					"		   AND sno_familiar.cedfam='$as_cedfam') as fecha_nac, ".
					"      (SELECT sno_familiar.hcfam from sno_familiar              ".
					"        WHERE  sno_familiar.codemp= '".$this->ls_codemp."' ".
					"		   AND sno_familiar.codper='$as_codper'  ".
					"		   AND sno_familiar.cedfam='$as_cedfam') as hcfam, ".
					"      (SELECT sno_familiar.hcmfam from sno_familiar              ".
					"        WHERE sno_familiar.codemp= '".$this->ls_codemp."' ".
					"		   AND sno_familiar.codper='$as_codper'   ".
					"		   AND sno_familiar.cedfam='$as_cedfam') as hcmfam, ".
					"      (SELECT sno_familiar.sexfam from sno_familiar              ".
					"        WHERE sno_familiar.codemp= '".$this->ls_codemp."'  ".
					"		   AND sno_familiar.codper='$as_codper'   ".
					"		   AND sno_familiar.cedfam='$as_cedfam') as sexofam ".
				    "         FROM srh_dt_tipodeduccion, sno_personalnomina, sno_nomina      ".
				    "        WHERE srh_dt_tipodeduccion.titular='N' ".
					"        AND   sno_personalnomina.codemp= '".$this->ls_codemp."'  ".
					"		 AND   sno_personalnomina.codper='$as_codper'   ". 
					"		 AND   sno_personalnomina.codnom=sno_nomina.codnom   ". 
					"        AND   sno_nomina.espnom='0' ".
		 ...
		 $num=$this->io_sql->num_rows($rs_data); 
		  ...
		  if ($num!=0) 
		  {
			...
			{   
						...
						$hc_familiar=$row["hcfam"];
						$hcm_familiar=$row["hcmfam"];
						$nexo_familiar=$row["nexofam"];
						$sexo_familiar=$row["sexofam"];
				      ...
				      if ($ls_hcm!="")
				      {	
						if (($ls_sueldoper>=$ls_sueldobene)&&($edad_familiar>=$ls_edadmin)&&($edad_familiar<=$ls_edadmax)&&($ls_sexoben==$sexo_familiar)&&($ls_nexo==$nexo_familiar)&&($ls_sexoben==$sexo_familiar)&&($ls_hcm==$hcm_familiar)&&($ls_codded==$as_codtipded))
						{
					   ...
					   {
					     if (($ls_sueldoper>=$ls_sueldobene)&&($edad_familiar>=$ls_edadmin)&&($edad_familiar<=$ls_edadmax)&&($ls_sexoben==$sexo_familiar)&&($ls_nexo==$nexo_familiar)&&($ls_sexoben==$sexo_familiar)&&($ls_codded==$as_codtipded))
						  {
  ...
  { 
  	...
  	if ($as_operacion == "modificar")
	{
	 $this->io_sql->begin_transaction();
	  ...
	  "WHERE codben= '$ao_beneficiario->codben' AND codper = '$ao_beneficiario->codper' AND codemp='".$this->ls_codemp."'" ;
...
//-------------------------------------------------------------------------------------------------------------------------------
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql= "SELECT sno_beneficiario.*, scb_banco.nomban FROM sno_beneficiario , scb_banco
...
//------------------------------------------------------------------------------------------------------------------------------- 	
function uf_srh_eliminar_beneficiario ($as_codben, $as_codper, $aa_seguridad)
  {
    ...
    $ls_sql = "DELETE FROM sno_beneficiario ".
	          "WHERE codben= '$as_codben' AND codper = '$as_codper'   AND codemp='".$this->ls_codemp."'";
