...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    // Cargar el dts_cab con los datos de la cabecera del reporte( Selecciono todos comprobantes )	
     $lb_valido=$io_report->uf_spg_reportes_unidades_ejecutoras($ls_codestpro1_min,$ls_codestpro2_min,$ls_codestpro3_min,$ls_codestpro4_min,
																...
																$ls_ckq_unidad,$ls_codfuefindes,$ls_codfuefinhas);
	/* if(($ls_coduniadm_des=="")&&($ls_coduniadm_has==""))
	 ...
	 $io_function_report->uf_load_seguridad_reporte("SPG","tepuy_spg_r_unidades_ejecutoras.php",$ls_desc_event);
	 ////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//	print(" close();");
	 ...
	 else // Imprimimos el reporte
	 {
		...
		for($z=1;$z<=$li_tot;$z++)
		{
			...
			{
				$a++;
				$ls_actual=substr($ls_actual,18,2)."-".substr($ls_actual,24,2)."-".substr($ls_actual,27,2);
				$la_data[$a]=array('coduniadm'=>" \n ",'denuniadm'=>"\n <b>Estructura Presupuestara: ".$ls_actual."</b>");				
			}
			$a++;
			$ls_coduniadm=trim($io_report->dts_reporte->data["coduniadm"][$z]);
			$ls_denominacion=trim($io_report->dts_reporte->data["denuniadm"][$z]);
	
			$la_data[$a]=array('coduniadm'=>$ls_coduniadm,'denuniadm'=>$ls_denominacion);
