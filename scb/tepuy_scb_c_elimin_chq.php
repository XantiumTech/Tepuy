...
class tepuy_scb_c_elimin_chq
{
	...
	var $ds_solbanco;
	var $la_security;
	function tepuy_scb_c_elimin_chq($aa_security)
	{
	...
	{		
		...
		//  Modificado Por: Ing. Miguel Palencia.     Fecha Última Modificación: 19/01/2016.
		////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql = "SELECT numdoc, cod_pro, ced_bene, nomproben, fecmov, conmov, estmov, monto, conmov,codban,ctaban,estcon
				     FROM scb_movbco 
				    WHERE codemp='".$ls_codemp."' 
					  AND (estmov='N' OR estmov='L') 
					  AND (codope='CH'OR codope='TR') $ls_straux
					ORDER BY numdoc ASC";
		//print $ls_sql;
		...
		if ($rs_data===false)
		   {
			 $lb_valido=false;
			 $this->is_msg_error="CLASS->tepuy_scb_c_elim_chq.php;Método->uf_cargar_cheques_filtrados".$this->fun->uf_convertirmsg($this->SQL->message);
		   }
		return $rs_data;
	}//Fin uf_cargar_solicitudes
			...
			if ($li_numrows>0)
			     ...
			     while($row=$this->SQL->fetch_row($rs_solbanco))
				      {
					    ...
					    if ($lb_valido)
						   {
						     $ls_sql = "DELETE FROM cxp_sol_banco WHERE codemp='".$ls_codemp."' AND codban='".$ls_codban."' AND ctaban='".$ls_ctaban."' AND (codope='CH' OR codope='TR') AND numdoc='".$ls_numdoc."'";
			 ...
			 if ($lb_valido)//Eliminacion de lo detalles de gasto
				...
				if($lb_valido)//Eliminacion de lo detalles de contabilidad
				{
					...
					AND codban='".$ls_codban."' AND ctaban='".$ls_ctaban."' AND (codope='CH' OR codope='TR') AND numdoc='".$ls_numdoc."' AND estmov='N'";
				...
				if($lb_valido)//Eliminacion de lo detalles de contabilidad
				{
					...
					AND codban='".$ls_codban."' AND ctaban='".$ls_ctaban."' AND (codope='CH' OR codope='TR') 
	...
	function uf_liberar_cheque($as_codemp,$as_codban,$as_ctaban,$as_numche)
	{
		...
		if ($rs_data===false)
		   {
			 $lb_valido=false;
			 $this->is_msg_error="Error: CLASS->tepuy_scb_c_elimin_chq.php; Método->uf_liberar_cheque();, ".$this->fun->uf_convertirmsg($this->SQL->message);
		   }
		return $lb_valido;
	}//function uf_liberar_cheque.
}
