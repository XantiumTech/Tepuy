...
// ESTAS LINEAS SON NECESARIAS PARA QUE SE PUEDA MOSTRAR EL PDF //
	...
	function uf_print_cabecera($as_etiqueta,&$io_pdf)
	{
		...
		$la_data=array(array('cuenta'=>'<b>Partida</b>','denominacion'=>'<b>Denominación</b>','pres_anual'=>'<b>Asignación</b>'));
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('cuenta'=>array('justification'=>'center','width'=>60), // Justificación y ancho de la
						 			   'denominacion'=>array('justification'=>'center','width'=>365), // Justificación y ancho de la
	...
	function uf_print_detalle($la_data,&$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('cuenta'=>array('justification'=>'center','width'=>60), // Justificación y ancho de la columna
				 			   'denominacion'=>array('justification'=>'left','width'=>365), // Justificación y ancho de la 
						 			   'pres_anual'=>array('justification'=>'right','width'=>125))); // Justificación y ancho de
		$la_columnas=array('cuenta'=>'<b>Cuenta</b>','denominacion'=>'<b>Denominación</b>',
...
//-----------------------------------------------------------------------------------------------------------------------------
	...
	//--------------------------------------------------  Parámetros para Filtar el Reporte  -----------------------------------------
		...
		{
          		$li_cmbnivel="16";
			$ls_titulo=" <b>RESUMEN DEL PRESUPUESTO DE GASTOS A NIVEL DE SECTORES Y PARTIDAS </b>";
			$ls_titulo1=" <b></b>"; 
		}
          	else
		{
	...
	$li_cantidad_sector=0;
         $lb_valido=$io_report->uf_spg_reportes_distribucion_sector_partida($ls_combo,$li_cmbnivel,$li_cantidad_sector);
	     if ($lb_valido==false) // Existe algún error ó no hay registros
			...
			// MAS CANTIDAD DE SECTORES //
			$li_total1=($li_total+$li_cantidad_sector);
			$ld_acumulado_sector=0;	
			$z=0;
			$registro=0;
			//print $li_cantidad_sector;
			//print $li_total1;
			$paso=0;
			while($z<$li_total1)
			  //for ($z=1;$z<=$li_total;$z++)
			      {
					$z=$z+1;
					$registro=$registro+1;
					if($registro>$li_total){$registro=$li_total;$paso=1;}	
				    	$thisPageNum=$io_pdf->ezPageCount;

				    	$ls_spg_cuenta=$io_report->dts_reporte->data["spg_cuenta"][$registro];
				    	$ls_codestpro1=$io_report->dts_reporte->data["codestpro1"][$registro];

					$h=$registro;
					$adelante=$registro+1;
					if($adelante>$li_total)
					{
						$h=$li_total;
					}
					else
					...
					$ls_codestpro1_next = $io_report->dts_reporte->getValue("codestpro1",$h);


					$ls_denominacion=$io_report->dts_reporte->data["denestpro1"][$registro];
					$ls_denominacion_cuenta=$io_report->dts_reporte->data["denominacioncuenta"][$registro];
					$li_nivel=$io_report->dts_reporte->data["nivel"][$registro];
					$ld_asignado=$io_report->dts_reporte->data["asignado"][$registro];
					...
					if($paso==0)
					{	
						$la_data[$z]=array('cuenta'=>$ls_spg_cuenta,'denominacion'=>$ls_denominacion_cuenta,'pres_anual'=>$ld_pres_anual);
					}
					else
					{
						$ld_total_pres_anual=$ld_total_pres_anual-$ld_asignado;
						$ld_acumulado_sector=number_format($ld_acumulado_sector,2,",",".");
						$la_data[$z]=array('cuenta'=>'<b></b>','denominacion'=>'<b> Total Sector '.$ls_codestpro1.' '.$ls_denominacion.'</b>','pres_anual'=>'<b>'.$ld_acumulado_sector.'</b>');
					}					  
					$ld_pres_anual=str_replace('.','',$ld_pres_anual);
					$ld_acumulado_sector=$ld_acumulado_sector+$ld_pres_anual;
					if($ls_codestpro1_next!=$ls_codestpro1)
					{
						...
						$la_data[$z]=array('cuenta'=>'<b></b>','denominacion'=>'<b> Total Sector '.$ls_codestpro1.' '.$ls_denominacion.'</b>','pres_anual'=>'<b>'.$ld_acumulado_sector.'</b>');
						//$z=$z+1;
						//$la_data[$z]=array('cuenta'=>'<b></b>','denominacion'=>'<b> </b>','pres_anual'=>'<b></b>');
						$ld_acumulado_sector=str_replace('.','',$ld_acumulado_sector);
						$ld_acumulado_sector=0;
					}//if
