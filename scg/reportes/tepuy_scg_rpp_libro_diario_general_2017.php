...
*/
	...
	$ls_titulo=" <b> LIBRO DIARIO GENERAL </b> ";
	//--------------------------------------------------------------------------------------------------------------------------------
    // Cargar el dts_cab con los datos de la cabecera del reporte( Selecciono todos comprobantes )	
	$lb_valido=uf_insert_seguridad("<b>Libro Diario General en PDF</b>"); // Seguridad de Reporte
	if($lb_valido)
	{
		//print "Procede desde ".$ld_fecdesde."Procede Hasta ".$ld_fechasta;die();
	//	 $lb_valido=$io_report->uf_scg_reporte_select_libro_diario_general_2017($ls_procdes,$ls_prochas,$ls_compdes,$ls_comphas,
	//	                                                                   $ldt_fecdes,$ldt_fechas,$ls_orden,&$rs_data);
		 $lb_valido=$io_report->uf_scg_reporte_select_libro_diario_general_2017($ld_fecdesde,$ld_fechasta,$li_nivel,&$rs_data);
	}
	...
	else // Imprimimos el reporte
	{
		...
		$ld_totalh=0;
		$li_i=0;
		$li_tot=$io_sql->num_rows($rs_data);
	    while($row=$io_sql->fetch_row($rs_data))
		{
			...
			//------------------------------------------------------------------------------------------------------
			$rs_detalle="";
			$lb_valido=$io_report->uf_scg_reporte_libro_diario_general($ls_procede,$ls_comprobante,$ldt_fecha,$ls_orden,&$rs_detalle);
			if($lb_valido)
			{
		        $li_totrow_det=$io_sql->num_rows($rs_detalle);
				$li_s=0;
	            while($row=$io_sql->fetch_row($rs_detalle))
				{  
				    $li_s++;
				    $ls_comprobante=$row["comprobante"];
