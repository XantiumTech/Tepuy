<?php
	...
	require_once("class_folder/tepuy_scf_c_comprobante.php");
	$io_scf=new tepuy_scf_c_comprobante("../");	
...
function ue_cambiardestino()
{
	...
	if(tipdes!="-")
	{
		if(tipdes=="P")
		{
			window.open("tepuy_scf_cat_proveedor.php?tipo=catcomp","_blank","menubar=no,toolbar=no,scrollbars=yes,width=530,height=400,left=50,top=50,location=no,resizable=no");
		}
		else
		{
			window.open("tepuy_scf_cat_beneficiario.php?tipo=catcomp","_blank","menubar=no,toolbar=no,scrollbars=yes,width=530,height=400,left=50,top=50,location=no,resizable=no");
		}	
	}
}
...
function ue_search()
{
	...
	if((fecdes!="")&&(fechas!=""))
	{
		...
		ajax=objetoAjax();
		// Pagina donde están los métodos para buscar y pintar los resultados
		ajax.open("POST","class_folder/tepuy_scf_c_catalogo_ajax.php",true);
 		ajax.onreadystatechange=function(){
			if(ajax.readyState==1)
			{
