...
class tepuy_sfa_c_tipo_producto
{
	...
	function uf_sfa_select_tipo_producto($as_codtippro)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->tipo_producto MÉTODO->uf_sfa_select_tipo_producto ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function  uf_sfa_insert_tipo_producto($as_codtippro,$as_dentippro,$as_obstippro,$aa_seguridad)
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->tipo_producto MÉTODO->uf_sfa_insert_tipo_producto ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		...
		else
		{
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insertó el Tipo de Producto ".$as_codtippro;
	...
	function uf_sfa_update_tipo_producto($as_codtippro,$as_dentippro,$as_obstippro,$aa_seguridad) 
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->tipo_producto MÉTODO->uf_sfa_update_tipo_producto ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
			$this->io_sql->rollback();
		}
		else
		{
			...
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="UPDATE";
			$ls_descripcion ="Modificó el Tipo de Producto ".$as_codtippro;
			$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_sfa_delete_tipo_producto($as_codtippro,$aa_seguridad)
	{
		...
		else
		{
			...
			if($li_row===false)
			{
				$this->io_msg->message("CLASE->tipo_producto MÉTODO->uf_sfa_delete_tipo_producto ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
				$lb_valido=false;
				$this->io_sql->rollback();
			}
			else
			{
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="DELETE";
				$ls_descripcion ="Eliminó el Tipo de Producto ".$as_codtippro;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////			
	...
	function uf_sfa_select_productoproducto($as_codtippro)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->tipoproducto MÉTODO->uf_sfa_select_tipoproductoproducto ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
