<?php
  ////////////////////////////////////////////////////////////////////////////////////////////////////////
  //       Class : class_tepuy_scb_integracion_php                                                     //    
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////
class class_tepuy_scbmov_integracion
{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function class_tepuy_scbmov_integracion()
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: class_tepuy_scbmov_integracion
		//		   Access: public 
		...
		// Modificado Por: Ing. Miguel Palencia								Fecha Última Modificación : 01/11/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		require_once("class_tepuy_cxp_integracion.php");  
		...
		$this->io_tepuy_int_cxp = new class_tepuy_cxp_integracion();				
		...
		$this->as_codban="";
		$this->as_ctaban="";
	}// end function class_tepuy_scbmov_integracion
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_destroy_objects()
	{
		...
		//	   Creado Por: Ing.Juniors Fraga
		// Modificado Por: Ing. Miguel Palencia								Fecha Última Modificación : 25/10/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	   // if( is_object($this->io_fecha) ) { unset($this->io_fecha);  }
        ...
        if( is_object($this->io_tepuy_int_cxp) ) { unset($this->io_tepuy_int_cxp);  }	   	   
											   ...
											   $aa_seguridad)
	{
		...
		//		   Access: public (tepuy_mis_p_contabiliza_scb.php)
		...
		$lb_valido=$this->io_tepuy_int->uf_int_init($this->is_codemp,$this->is_procede,$this->is_comprobante,$ldt_fecha,
	    ...
	    if($lb_valido)
	    {
	        ...
	        $lb_valido=$this->io_tepuy_int->uf_init_end_transaccion_integracion($aa_seguridad,$ldt_fecha_cont); 
	        if($lb_valido===false)
		    {
				$this->io_msg->message("ERROR-> ".$this->io_tepuy_int->is_msg_error);
	...
	function uf_obtener_data_movimento_banco()
	{
		...
		//	   Creado Por: Ing.Juniors Fraga
		// Modificado Por: Ing. Miguel Palencia								Fecha Última Modificación : 02/11/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    $lb_existe=false;		
	...
	function uf_procesar_gastos($as_descripcion)
	{
		...
		{                 
			while($row=$this->io_sql->fetch_row($rs_data) and ($lb_valido))
		    {
				...
				$ls_mensaje=$this->io_tepuy_int_spg->uf_operacion_codigo_mensaje($row["operacion"]);
				$ls_spg_cuenta=$this->io_tepuy_int_spg->uf_spg_pad_cuenta($ls_spg_cuenta);
				$lb_valido=$this->io_tepuy_int->uf_spg_insert_datastore($this->is_codemp,$ls_codestpro1,$ls_codestpro2,$ls_codestpro3,
	...
	function uf_procesar_contable($as_descripcion,$as_procede)
	{
		...
		{                 
			while($row=$this->io_sql->fetch_row($rs_data) and ($lb_valido))
		    {
				...
				$ls_documento=$row["documento"];
			    $ls_procede=$row["procede_doc"];				
				$ls_scg_cuenta=$this->io_tepuy_int_scg->uf_pad_scg_cuenta( $this->dts_empresa["formcont"],$ls_scg_cuenta);
				$lb_valido=$this->io_tepuy_int->uf_scg_insert_datastore($this->is_codemp,$ls_scg_cuenta,$ls_debhab,$ldec_monto,$ls_documento,$ls_procede,$as_descripcion);
				if($lb_valido===false)
				{  
					$this->io_msg->message("ERROR-> ".$this->io_tepuy_int->is_msg_error);
					break;
	...
	function uf_procesar_ingresos($as_descripcion)
	{
		...
		//	   Creado Por: Ing.Juniors Fraga
		// Modificado Por: Ing. Miguel Palencia								Fecha Última Modificación : 03/11/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql="SELECT * ".
                "  FROM scb_movbco_spi ".
		...
		{    
			while($row=$this->io_sql->fetch_row($rs_data) and ($lb_valido))
		    {
				...
				$ls_mensaje=$this->io_tepuy_int_spi->uf_operacion_codigo_mensaje($row["operacion"]);
				$ls_spi_cuenta=$this->io_tepuy_int_spi->uf_spi_pad_cuenta($ls_spi_cuenta);
				$lb_valido=$this->io_tepuy_int->uf_spi_insert_datastore($this->is_codemp,$ls_spi_cuenta,$ls_operacion,
																		 $ldec_monto,$ls_documento,$ls_procede,$as_descripcion);
				if ($lb_valido===false)
				{  
					$this->io_msg->message("ERROR ->".$this->io_tepuy_int->is_msg_error);
	...
	function uf_procesar_programacion_pagos($adt_fecha)
	{
		...
		if(($this->ii_datasource==4)&&(!$this->ii_compromiso_previo))
		{
			...
			if($rs_data===false)
			{   
	            $this->io_msg->message("CLASE->Integración SCB MÉTODO->uf_procesar_programacion_pagos ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
				return false;
			}
			else
			{                 
				while($row=$this->io_sql->fetch_row($rs_data) and ($lb_valido))
				{
					...
					$ls_sql_tmp="SELECT * ".
                                "  FROM (SELECT SUM(b.monto) as monto_bco ".
                                "          FROM cxp_sol_banco b,cxp_solicitudes s ".
                                "         WHERE b.codemp=s.codemp AND b.numsol=s.numsol AND ".
							    "               b.codemp='".$this->is_codemp."' AND b.numsol='".$ls_numsol."' AND b.estmov='C') TA,".
                                "       (SELECT s.monsol as monto_sol ".
                                "          FROM cxp_sol_banco b,cxp_solicitudes s ".
                                "         WHERE b.codemp=s.codemp AND b.numsol=s.numsol AND ".
							    "               b.codemp='".$this->is_codemp."' AND b.numsol='".$ls_numsol."' AND numdoc ='".$this->is_numdoc."') TB, ".
                                "       (SELECT b.monto as monto_actual ".
                                "          FROM cxp_sol_banco b,cxp_solicitudes s ".
                                "         WHERE b.codemp=s.codemp AND b.numsol=s.numsol AND ".
							    "               b.codemp='".$this->is_codemp."' AND b.numsol='".$ls_numsol."' AND numdoc ='".$this->is_numdoc."') TC";
					...
					{    
						...
						{
							$ldec_monto_bco=$row_tmp["monto_bco"];
							$ldec_monto_actual=$row_tmp["monto_actual"];
							$ldec_monto_sol=$row_tmp["monto_sol"];	
							$ldec_total=($ldec_monto_bco+$ldec_monto_actual);
							if($ldec_total==$ldec_monto_sol) 
							{ 
								$lb_valido=$this->uf_update_estatus_solicitud_cancelado($ls_numsol,"P");
							   	if ($lb_valido)
							   	{
							    	$lb_valido=$this->io_tepuy_int_cxp->uf_delete_historico_pagado($this->is_codemp,$ls_numsol);
									if($lb_valido)
									{
							      		$lb_valido=$this->io_tepuy_int_cxp->uf_insert_historico_contabilizacion($this->is_codemp,$ls_numsol,$adt_fecha,"P"); 
									}
							   	}
							   	if($lb_valido)
								{
										...
										$as_procede,$as_comprobante,$adt_fecmov,$adt_fecha)
	{
		...
		$ls_sql = "INSERT INTO scb_movbco (codemp,codban,ctaban,numdoc,codope,estmov,cod_pro,ced_bene,".
				  ...
				  "						   fechaconta, fechaanula,nrocontrolop) ".
				  ...
				  "        codusu, codopeidb, aliidb, feccon, estreglib, numcarord, ".
				  "        numpolcon,coduniadmsig,codbansig,fecordpagsig,tipdocressig,".
				  "        numdocressig,estmodordpag,codfuefin,forpagsig,medpagsig,codestprosig,fechaconta,fechaanula,nrocontrolop ".
				  "  FROM scb_movbco ".
		...
		$ls_sql = " INSERT INTO scb_movbco_spgop (codemp,codban,ctaban,numdoc,codope,estmov,codestpro,".
		          "                             spg_cuenta,operacion,documento,coduniadm,desmov,procede_doc,monto,baseimp,codcar) ".
				  "SELECT codemp,codban,ctaban,numdoc,codope,'".$as_estmov_new."',codestpro,spg_cuenta,".
				  "        operacion,documento,coduniadm,desmov,procede_doc,monto,baseimp,codcar ".
	...
	function uf_reversar_contabilizacion_banco($as_codban,$as_ctaban,$as_numdoc,$as_codope,$as_estmov,$ad_fechaconta,$aa_seguridad)
	{
		...
		//		   Access: public (tepuy_mis_reverso_scb)
		//	    Arguments: as_codban // Código de Banco
		//	    		   as_ctaban // Cuenta Banco
		//	    		   as_numdoc // Número de Documento
		//	    		   as_codope // Código de Operación
		...
		$lb_valido=$this->io_tepuy_int->uf_init_delete($this->is_codemp,$ls_procede_scb,$ls_comprobante_scb,$ad_fechaconta,
										 ...
										 $ad_fechaconta,$as_conanu,$aa_seguridad)
	{
		...
		//		   Access: public (tepuy_mis_anula_scb.php)
		//	    Arguments: as_codban // Código de Banco
		//	    		   as_ctaban // Cuenta Banco
		//	    		   as_numdoc // Número de Documento
		//	    		   as_codope // Código de Operación
		...
		$ldt_fecha=$this->dts_banco->getValue("fecha",1);
		$ls_descripcion=$this->dts_banco->getValue("conmov",1);
		$ldt_fechamov=$this->dts_banco->getValue("fecmov",1);
		...
		$lb_valido = $this->io_tepuy_int->uf_int_anular($this->is_codemp,$this->is_procede,$this->is_comprobante,
	...
	function uf_reversar_anulado($as_codban,$as_ctaban,$as_numdoc,$as_codope,$as_estmov,$ad_fechaconta,$ad_fechaanula,$aa_seguridad)
	{
		...
		//		   Access: public (tepuy_mis_reverso_anula_scb.php)
		//	    Arguments: as_codban // Código de Banco
		//	    		   as_ctaban // Cuenta Banco
		//	    		   as_numdoc // Número de Documento
		//	    		   as_codope // Código de Operación
										   ...
										   $ad_fechaanula,$aa_seguridad)
	{
		...
		//	   Creado Por: Ing.Juniors Fraga
		// Modificado Por: Ing. Miguel Palencia								Fecha Última Modificación : 06/11/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$this->is_codemp=$this->io_codemp;
	    $this->is_codban=$as_codban;
	    $this->is_ctaban=$as_ctaban;
		...
		$ls_ced_bene="";
		$ls_cod_pro="";
	    $lb_valido = $this->io_tepuy_int->uf_obtener_comprobante($this->is_codemp,$ls_procede,$ls_comprobante,$adt_fecha,
        ...
        $lb_check_close=false;
		$lb_valido=$this->io_tepuy_int->uf_init_delete($this->is_codemp,$ls_procede,$ls_comprobante,$adt_fecha,$ls_tipo_destino,
	...
	function uf_procesar_contabilizacion_orden_pago($as_codban,$as_ctaban,$as_numdoc,$as_codope,$as_estmov,$adt_fecha,$aa_seguridad)
	{
		...
		//		   Access: public (tepuy_mis_p_contabiliza_scbop.php)
		//	    Arguments: as_codban // Código de Banco
		//	    		   as_ctaban // Cuenta Banco
		//	    		   as_numdoc // Número de Documento
		//	    		   as_codope // Código de Operación
	    ...
	    $this->is_comprobante=$this->io_tepuy_int->uf_fill_comprobante(trim($as_numdoc));
		$this->is_documento="";
		// Verifico si el comprobante existe y de ser así lo cargo en el datastored		
        $lb_valido=$this->uf_obtener_data_movimento_banco();
        if(!$lb_valido)
	...
	function uf_procesar_contable_pago_directo($as_descripcion)
	{
		...
		{                 
			while($row=$this->io_sql->fetch_row($rs_data) and ($lb_valido))
		    {
				...
				$lb_valido=$this->io_tepuy_int->uf_scg_insert_datastore($this->is_codemp,$ls_scg_cuenta,$ls_debhab,$ldec_monto,
															...
															$aa_seguridad)
	{
		...
		//		   Access: public (tepuy_mis_p_reverso_scbop.php)
		//	    Arguments: as_codban // Código de Banco
		//	    		   as_ctaban // Cuenta Banco
		//	    		   as_numdoc // Número de Documento
		//	    		   as_codope // Código de Operación
...
/*	function uf_update_estatus_solicitud_banco($as_codemp,$as_codban,$as_ctaban,$as_numdoc,$as_codope,$as_estmov,$as_estdoc)
	{	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	   Function :	uf_update_estatus_solicitud_banco
		//       Access :	public
		//   Argumentos :   $as_codemp->codigo empresa :: $as_codban->codigo banco :: $as_ctaban->cuenta banco 
		//                  $as_numdoc->numero de documento :: $as_codope->codigo operacion :: $as_estmov-> estatus movimiento
		//	    Returns :	Retorna un booleano
		//	Description :	Método que actualiza el estatus del movimiento generado por medio dela emision de cheque
		//                  con estatus sin contabilizar      
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    $lb_valido=true;
		$ls_sql  = " UPDATE cxp_sol_banco ".
		           " SET estdoc='".$as_estdoc."'".
                   " WHERE codemp='".$this->is_codemp."' AND codban='".$this->is_codban."' AND ctaban='".$this->is_ctaban."' AND ".
				   "       numdoc='".$this->is_numdoc."' AND codope='".$this->is_codope."' AND estmov='".$this->is_estmov."'";
		...
		return $lb_valido;
	}  // fin function uf_update_estatus_solicitud_banco()
*/
}
?>
