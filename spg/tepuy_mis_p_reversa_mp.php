<?php
	...
	$oi_fun_integrador->uf_load_seguridad("SPG","tepuy_mis_p_reversa_mp.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_load_variables()
   {
		...
		//////////////////////////////////////////////////////////////////////////////
   		global $ls_comprobante,$ls_procede,$ld_fecha;
...
<?php
	...
	require_once("class_folder/class_tepuy_spg_integracion.php");  	
	$in_class = new class_tepuy_spg_integracion();  
	uf_limpiarvariables();
	switch ($ls_operacion) 
	{
		case "NUEVO":
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_reversar_contabilizacion_modpre($ls_comprobante,$ld_fecha,$ls_procede,$ls_descripcion,
																		     $ld_fechaconta,$la_seguridad);

					if($lb_valido)
					{
						$in_class->io_msg->message("Modificación Presupuestaria ".$ls_comprobante." fue Reversada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo reversar la Modificación Presupuestaria ".$ls_comprobante);
					}		
				}
			}
			uf_limpiarvariables();
		...
		case "BUSCAR":
			uf_load_variables();
			$in_class_contabiliza->uf_select_comprobantes_spg($ls_comprobante,$ls_procede,$ld_fecha,"1",$lo_object,$li_totrows);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
<table width="762" border="0" align="center" cellpadding="0" cellspacing="0" class="contorno">
  ...
  <tr>
    <td width="432" height="20" colspan="11" bgcolor="#E7E7E7"><table width="762" border="0" align="center" cellpadding="0" cellspacing="0">
      <td width="432" height="20" bgcolor="#E7E7E7" class="descripcion_sistema">Contabilidad Presupuestaria de Gastos -> <i>M&oacute;dulo Integrador</i></td>
...
?>		  
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Reverso de Aprobaci&oacute;n de Modificaciones Presupuestarias de Gasto </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="tepuy_mis_p_reversa_mp.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(codcom,procede)
{
	...
	window.open("tepuy_mis_pdt_spg.php?codcom="+codcom+"&procede="+procede+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
</script>
