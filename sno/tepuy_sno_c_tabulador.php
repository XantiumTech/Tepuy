...
class tepuy_sno_c_tabulador
{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function tepuy_sno_c_tabulador()
	{	
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: tepuy_sno_c_tabulador
		//		   Access: public (tepuy_sno_d_tabla)
		...
		$this->io_asignacioncargo= new tepuy_sno_c_asignacioncargo();
        $this->ls_codemp=$_SESSION["la_empresa"]["codemp"];
        $this->ls_codnom=$_SESSION["la_nomina"]["codnom"];
		
	}// end function tepuy_sno_c_tabulador
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_load_grado($as_codtab,$as_comauto,$ai_maxpasgra,&$ai_totrows,&$ao_object)
	{
		...
		$ls_sql="SELECT codemp, codnom, codtab, codpas, codgra, monsalgra, moncomgra ".
				...
				"   AND codnom='".$this->ls_codnom."'".
				"   AND codtab='".$as_codtab."'".
				" ORDER BY codemp, codnom, codtab, ".$ls_cadena;
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		{
			$lb_valido=false;
		...
		else
		{
			...
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ao_object[$ai_totrows][1]="<input class=".$ls_estilo." name=txtcodgra".$ai_totrows." type=text id=txtcodgra".$ai_totrows." size=18 maxlength=15 onKeyUp='javascript: ue_validarnumero(this);' value='".$ls_codgra."' readOnly>";
				$ao_object[$ai_totrows][2]="<input class=".$ls_estilo." name=txtcodpas".$ai_totrows." type=text id=txtcodpas".$ai_totrows." size=18 maxlength=15 onKeyUp='javascript: ue_validarnumero(this);' value='".$ls_codpas."' readOnly><input name='existe".$ai_totrows."' type='hidden' id='existe".$ai_totrows."' value='1'>";
				...
				$ao_object[$ai_totrows][7]="<a href=javascript:uf_delete_dt(".$ai_totrows.");><img src=../shared/imagebank/tools15/deshacer.png alt=Aceptar width=15 height=15 border=0></a>";			
	...
	function uf_load_primagrado($as_codtab,$as_codpas,$as_codgra,&$ai_totrows,&$ao_object)
	{
		...
		//		   Access: public (tepuy_sno_pdt_primagrado)
		//	    Arguments: as_codtab  // código de tabla
		//				   as_codpas  // Código de paso
		//				   as_codgra  // Código de grado
		//				   ai_totrows  // total de fila
		...
		else
		{
			...
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ao_object[$ai_totrows][4]="<a href=javascript:uf_agregar_dt(".$ai_totrows.");><img src=../shared/imagebank/tools15/aprobado.png alt=Aceptar width=15 height=15 border=0></a>";
				$ao_object[$ai_totrows][5]="<a href=javascript:uf_delete_dt(".$ai_totrows.");><img src=../shared/imagebank/tools15/deshacer.png alt=Deshacer width=15 height=15 border=0></a>";			
			}
			$ai_totrows=$ai_totrows+1;
