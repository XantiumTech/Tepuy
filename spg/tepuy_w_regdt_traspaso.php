...
if($ls_operacion=="GUARDARPRE")
{
	...
	else
	{
		...
		$lb_valido=$in_classcmp->uf_guardar_automatico($ls_comprobante,$ld_fecha,$ls_proccomp,$ls_desccomp,&$ls_prov,&$ls_bene,$ls_tipo,2,0,$ls_codfuefin,$ls_coduniadm);
		if(!$lb_valido)
		{
...
/*			$lb_valido=$in_classcmp->uf_update_bsf_tepuycmpmd(0,$ls_codemp,$ls_proccomp,$ls_comprobante,
		...
		if($lb_valido)
		{
			...
			$in_classcmp->io_sql->begin_transaction();
			$lb_valido=$in_classcmp->uf_guardar_movimientos($arr_cmp,$ls_est1,$ls_est2,$ls_est3,$ls_est4,$ls_est5,$ls_cuenta,$ls_procede,$ls_descripcion,$ls_documento,$ls_operacionpre,0,$ldec_monto,"P");
//print "pase";
			if($lb_valido)
			{
...
<table width="583" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
  ...
  <tr>
    ...
    <a href="javascript:catalogo_cuentasSPG();"><img src="../shared/imagebank/tools15/buscar.png" width="15" height="15" border="0" alt="Catálogo de Cuentas de Presupuestarias de Gasto"></a>	 <input name="txtdenominacion" type="text" class="sin-borde" id="txtdenominacion3" style="text-align:left" value="<?php print $ls_denominacion ?>" size="53" maxlength="254"></td>
  </tr>
  <tr>
...
<script language="JavaScript">
  function aceptar_presupuestario()
  {
	...
	ldec_monto_di=fop.txttotspg.value;
	ldec_monto_au=fop.txttotspg2.value;
	...
	if(ls_operacion=='AU')
	{
		ldec_monto_di=uf_convertir_monto(ldec_monto_di);
		ldec_monto_au=uf_convertir_monto(ldec_monto_au);
		ldec_monto=uf_convertir_monto(ldec_monto);
		ldec_total=parseFloat(ldec_monto_au)+parseFloat(ldec_monto);
		ldec_total=redondear(ldec_total,2);
		ldec_monto_di=redondear(ldec_monto_di,2);
		
		if(parseFloat(ldec_total)>parseFloat(ldec_monto_di))
		{
			alert("Monto a aumentar no puede ser mayor al monto a disminuir");
			lb_valido=false;
		}
	}
	if(uf_valida_campos()&&(lb_valido))
	{
		f.operacion.value="GUARDARPRE";
		f.action="tepuy_w_regdt_traspaso.php";
		f.submit();
	}
	else
	{
