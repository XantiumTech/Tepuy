...
//-------------------------------------------------------------------------------------------------------------------------------
	...
	// Fin filtro por categoria programatica //
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report MÉTODO->uf_select_solicitudesF1A4 ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
   ...
   //---------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_retencionesiva_detalle_unificadas($as_numcom,$as_tiporeten,$agrupo)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report MÉTODO->uf_retencionesiva_detalle_unificadas ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_select_contribuyentes_libro_iva($as_codemp,$as_mes,$as_anio,$as_codreten,$as_periodofiscal,$ad_fecdes,$ad_fechas)
	{
		...
		//	    		as_anio     // Año del cual se van a generar el archivo txt
		//	    		as_codreten // Codigo de la retencion para filtrar
		//	    		as_criterio // Criterio de filtro de rango de fechas
		//	      Returns: lb_valido True si se creo el Data stored correctamente ó False si no se creo
		//    Description: Función que filtra las retenciones de IVA ue posteriorente genera el txt para la declaración informativa
		//	   Creado Por: Ing. Miguel Palencia
	...
	/*	$ls_sql="SELECT *  FROM scb_cmp_ret ".
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report MÉTODO->uf_select_contribuyentes_libro_iva ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_retenciones_del_proveedor_unificadas($as_rif,$as_mes,$as_anio,$as_numsol,$as_fecsol)
	{
		...
		//    Description: Función que busca la información de los comprobantes municipales
		//	   Creado Por: Ing. Miguel Palencia / Ing. Juniors Fraga
		// Fecha Creación: 15/07/2007									Fecha Última Modificación :  22/09/2016
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ld_fechadesde=$as_anio."-".$as_mes."-01";
		$ld_fechahasta=$as_anio."-".$as_mes."-".substr($this->io_fecha->uf_last_day($as_mes,$as_anio),0,2);
...
/*		$ls_sql="SELECT max(scb_dt_cmp_ret.codret) as codret, max(scb_dt_cmp_ret.numsop) as numsop, max(scb_dt_cmp_ret.fecfac) as fecfac,  max(scb_dt_cmp_ret.numfac) as numficha, max(scb_dt_cmp_ret.numcon) as numcon, ".
	...
	function uf_retencionesotros_detalles($as_numcom,$as_tiporeten,$agrupo)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report MÉTODO->uf_retencionesotros_detalles ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
...
//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_select_montobruto($as_numsol)
	{
		...
		if($rs_data1===false)
		{
			$this->io_mensajes->message("CLASE->Report MÉTODO->uf_select_montobruto ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
