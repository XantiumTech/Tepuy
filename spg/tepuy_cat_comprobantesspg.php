...
if($ls_operacion=="BUSCAR")
{
		...
		$ls_order=" ORDER BY tepuy_cmp.fecha ASC , tepuy_cmp.comprobante ASC , tepuy_cmp.procede ASC";
		$ls_sql=" SELECT DISTINCT tepuy_cmp.procede, MAX(tepuy_cmp.codemp) AS codemp, tepuy_cmp.comprobante, ".
		        "                 tepuy_cmp.fecha as feccmp, tepuy_cmp.ced_bene, tepuy_cmp.cod_pro,  ".
                "                 MAX(tepuy_cmp.fecha) AS fecha, MAX(tepuy_cmp.ced_bene) AS ced_bene, ".
                "                 MAX(tepuy_cmp.descripcion) AS descripcion, tepuy_cmp.tipo_destino, ".
				"                 SUM(spg_dt_cmp.monto) as total, tepuy_cmp.codban, tepuy_cmp.ctaban ".
				" FROM  tepuy_cmp, spg_dt_cmp   ".
				" WHERE tepuy_cmp.codemp='".$as_codemp."' AND tepuy_cmp.comprobante like '%".$ls_comprobante."%' AND ".
				"       tepuy_cmp.procede like '%".$ls_procedencia."%' AND tepuy_cmp.fecha>='".$ls_fecdesde."' AND ".
				"	    tepuy_cmp.fecha<='".$ls_fechasta."'  ".$ls_cadena."  AND tepuy_cmp.codemp=spg_dt_cmp.codemp AND ".
				" 	    tepuy_cmp.comprobante=spg_dt_cmp.comprobante AND ".
				"       tepuy_cmp.procede=spg_dt_cmp.procede AND     ".
				" 	    tepuy_cmp.fecha=spg_dt_cmp.fecha     and        ".
				" 	    tepuy_cmp.codban=spg_dt_cmp.codban   and         ".
				" 	    tepuy_cmp.ctaban=spg_dt_cmp.ctaban            ".
				" GROUP BY tepuy_cmp.procede, tepuy_cmp.comprobante, tepuy_cmp.fecha,  tepuy_cmp.codban, tepuy_cmp.ctaban,".
				"  tepuy_cmp.ced_bene, ".
				"  tepuy_cmp.cod_pro, tepuy_cmp.tipo_destino ";	
		...
		if($row=$SQL_cmp->fetch_row($rs_cmp))
		{
			...
			for($z=1;$z<=$totrow;$z++)
			{
				...
				$ls_descripcion= str_replace("'","",$ls_descripcion);
				$ls_descripcion= str_replace('"',"",$ls_descripcion);
				$ls_descripcion= str_replace("\\","",$ls_descripcion);
				$ls_descripcion= str_replace("\n"," ",$ls_descripcion);
				$ls_descripcion= str_replace("\r"," ",$ls_descripcion);
				$ls_procedencia=$data["procede"][$z];
				$ls_fecha=$data["feccmp"][$z];
				...
				else
				{
					...
					if($ls_tipocat=="")
					{
						...
						print "<td><a href=\"javascript: uf_aceptar('$ls_comprobante','$ls_descripcion','$ls_procedencia','$ls_fecha','$ls_tip','$ls_prov','$ls_bene','$ls_provbene','$ls_codban','$ls_ctaban');\">".$ls_comprobante."</a></td>";
...
<script language="JavaScript">
  ...
  function uf_aceptar(comprobante,descripcion,procede,fecha,tipo,prov,bene,ls_provbene,ls_codban,ls_ctaban)
  {
  ...
  function ue_search()
  {
	  ...
	  f.action="tepuy_cat_comprobantesspg.php?tipocat=<?php print $ls_tipocat; ?>";
	  f.submit();
  }
	function catprovbene(provbene)
	{
		...
		if(provbene=="P")
		{
			...
			window.open("tepuy_catdinamic_prov.php","_blank","menubar=no,toolbar=no,scrollbars=yes,width=502,height=350,left=50,top=50,location=no,resizable=yes");
		}
		else if(provbene=="B")
		{
			f.txtprovbene.disabled=false;	
			window.open("tepuy_catdinamic_bene.php","_blank","menubar=no,toolbar=no,scrollbars=yes,width=502,height=350,left=50,top=50,location=no,resizable=yes");
