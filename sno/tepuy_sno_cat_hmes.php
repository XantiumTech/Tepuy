<?php
   ...
   function uf_print($as_tipo,$as_codnom,$as_codnomhas)
   {
		...
		print "</tr>";
		$ls_sql="SELECT DISTINCT sno_hperiodo.anocur, substr(sno_hperiodo.fechasper,6,2) as mes ".
		...
		{
			$ls_sql=$ls_sql."	AND (substr(sno_hperiodo.fecdesper,6,2) IN (SELECT LPAD(mescurper,2,'0') ".
		...
		{
			$ls_sql=$ls_sql."	AND (substr(sno_hperiodo.fecdesper,6,2) NOT IN (SELECT LPAD(mescurper,2,'0') ".
							"											     FROM sno_fideiperiodo ".
							"									 		    WHERE sno_hperiodo.codemp=sno_fideiperiodo.codemp ".
							"   								 		  	  AND sno_hperiodo.codnom=sno_fideiperiodo.codnom ".
							"   								 		   	  AND sno_hperiodo.anocur=sno_fideiperiodo.anocurper)) ".
		...
		if(($as_tipo=="repapopat")||($as_tipo=="repipaspat")||($as_tipo=="repipascob")||($as_tipo=="repretislr")
		   ||($as_tipo=="repapopatdes")||($as_tipo=="repapopathas"))
		{
			$ls_sql="SELECT DISTINCT sno_hperiodo.anocur, substr(sno_hperiodo.fechasper,6,2) as mes  ".
					"  FROM sno_hperiodo, sno_hnomina, sno_periodo  ".
		...
		$ls_sql=$ls_sql." GROUP BY sno_hperiodo.anocur, substr(sno_hperiodo.fechasper,6,2) ".
						" ORDER BY sno_hperiodo.anocur, substr(sno_hperiodo.fechasper,6,2) ";
		if($as_tipo=="repcestic")
		{
			$ls_sql="SELECT DISTINCT sno_hperiodo.anocur, substr(sno_hperiodo.fechasper,6,2) as mes, sno_hperiodo.codperi ".
					"  FROM sno_hperiodo, sno_hnomina, sno_periodo ".
					...
					"   AND sno_hperiodo.codnom=sno_periodo.codnom ".
					"   AND sno_hperiodo.codperi=sno_periodo.codperi ".
					" GROUP BY sno_hperiodo.anocur, substr(sno_hperiodo.fechasper,6,2), sno_hperiodo.codperi ".
					" ORDER BY sno_hperiodo.anocur, substr(sno_hperiodo.fechasper,6,2), sno_hperiodo.codperi ";
		}
		...
		else
		{
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				switch ($as_tipo)
				{
					case "": // tepuy_snorh_p_fideicomiso
