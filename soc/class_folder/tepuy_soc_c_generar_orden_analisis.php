...
class tepuy_soc_c_generar_orden_analisis
{
  function tepuy_soc_c_generar_orden_analisis($as_path)
  {
			...
			if (array_key_exists("chk".$i,$_POST))
			   {
					...
					/////////////////////////////////////////////////////////////
					// BUSCA ESTOS DATOS DESDE EL ANALISIS DE LA COTIZACION
						$li_subtotal=$la_ganadores[$li_i]["subtotal"];
						$li_totaliva=$la_ganadores[$li_i]["iva"];
						$li_total=$la_ganadores[$li_i]["montotal"];;	
					/////////////////////////////////////////////////////////////
					...
					// ANTES SUMABA EL SUB-TOTAL, EL IVA Y ARROJABA EL TOTAL AHORA LOS BUSCO DESDE EL ANALISIS DE LA COTIZACION //
	...
	function uf_select_cotizacion_analisis($as_numanacot, $ls_tipanacot,&$as_numanacot)
	{
		...
		$ls_sql= "SELECT cxa.numcot, cxa.cod_pro, p.tipconpro,s.consolcot, r.monsubtot as subtotal, r.monimpcot as iva, r.montotcot as montotal
	...
	function  uf_select_items($as_numcot,$as_codpro,$as_numanacot,$as_tipsolcot,&$aa_items,&$li_i)
	{
		...
		if($as_tipsolcot=="B")
		{				
			$ls_sql="SELECT d.codart as codigo, a.denart as denominacion, p.nompro, dts.canart as cantidad, dt.preuniart as precio, dt.moniva,dt.monsubart as monsubtotal, dt.montotart as monto,
					        d.obsanacot, d.numcot, d.cod_pro, dts.numsep, dts.codcar as codcar 
		...
		else
		{
				...
				$ls_sql="SELECT d.codser as codigo, a.denser as denominacion, p.nompro, dts.canser as cantidad, dt.monuniser as precio, dt.moniva,dt.monsubser as monsubtotal, dt.montotser as monto,
					        d.obsanacot, d.numcot, d.cod_pro, dts.numsep, dts.codcar as codcar
	...
	function uf_calcular_montos($ai_totrow,$aa_items,&$aa_totales,$as_tipo_proveedor)
	{
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			for($li_j=1;$li_j<=$ai_totrow;$li_j++)
		 	{
				// ANTES LO VOLVIA A CALCULAR AHORA ASIGNO DESDE soc_dtcot_(bienes o servicio)
				//$li_subtotal+=(($aa_items[$li_j]["precio"]) * ($aa_items[$li_j]["cantidad"]));
				$li_subtotal+=$aa_items[$li_j]["monto"];
			  ...
			  for ($li_j=0;$li_j<$li_totrows;$li_j++)
			      {
				    ...
				    $li_moncue    = $aa_items[$li_i]["monsubtotal"];
				   ...
				   // print "codestpro: ".$ls_codestpro." monto: ".$li_moncue;
			      }
			      //print "articulo:".$aa_items[$li_i]["codigo"];
		    }
		     ...
		     for ($li_i=1;($li_i<=$li_totrow)&&($lb_valido);$li_i++)
			     {
				   ...
				   if ($ab_vienesep)
					$la_cargos=$this->uf_select_cargos_sep($ls_codart,$ls_numsep,$as_estcondat);
				   		...
				   		$la_cargos=$this->uf_select_cargos_2016($ls_codart,$as_numordcom,$as_estcondat,$ls_codcar);
				  // print "IVA: ".count($la_cargos);
				   if (count($la_cargos)>0)
				      {
				      	//print "articulo:".$ls_codart." Orden: ".$as_numordcom."as_estcondat".$as_estcondat;
						...
						$ld_monto     = $aa_items[$li_i]["monsubtotal"];
						...
						$this->io_dscuentas->insertRow("moncue",$li_moncue);	
						$this->io_dscuentas->insertRow("coditem",$ls_codart);
						//print "codestpro: ".$ls_codestpro." monto: ".$ld_monto;
				      }
			     }
		   }
		...
		if (count($this->io_dscuentas->data)>0)
		   {
			 ...
			 for ($li_fila=1;$li_fila<=$li_total;$li_fila++)
				   ...
				   if(strlen(trim($ls_codestpro1))>0)
				   {
    ...
    function uf_insert_cuentas_cargos($as_numordcom,$as_estcondat,$aa_items,$li_totrow,$aa_seguridad,$ab_vienesep)
	{
		...
		//	   Creado Por: Ing. Miguel Palencia/ Ing. Juniors Fraga
		// Modificado Por: Ing. Juniors Fraga, Ing Miguel Palencia, Ing..
		...
		for ($li_i=1;($li_i<=$li_totrow)&&($lb_valido);$li_i++)
			...
			//print "numsep: ".$ls_numsep;
			  ...
			  if (count($la_cargos)>0)
				...
				//   $ld_monto   = $aa_items[$li_i]["cantidad"] * $aa_items[$li_i]["precio"];
				// MODIFICACIONES PARA QUE NO CALCULE SINO QUE BUSQUE
	   			$ld_bascar  = $aa_items[$li_i]["monsubtotal"];
				$ld_monto   = $aa_items[$li_i]["monsubtotal"];
				   $ls_formula = str_replace('$LD_MONTO',$ld_monto,$la_cargos["formula"]);
				   eval('$ld_moncar ='.$ls_formula.";");
				   $ld_moncar  = $aa_items[$li_i]["moniva"];
				   ...
				   $this->io_dscargos->insertRow("spg_cuenta",$ls_spg_cuenta);
					//print "cargo: ".$ls_codcar." bascar:".$ld_bascar." monto: ".$ld_monto;
			     }
		    }
		//print "que paso";
		$arr_group[0]="codestpro1";
	...
	//---------------------------------------------------------------------------------------------------------------------------------------
	function uf_select_cargos_2016($as_coditem,$ad_numordcom,$ls_tipsolcot,$as_codcar)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//print $ls_sql;
		...
		else
		   {
			/* $ls_sql= "SELECT s.codser, s.codcar, c.formula ,c.codestpro, c.spg_cuenta 
					    ...
					    WHERE s.codemp='$this->ls_codemp'
						  ...
						  AND s.codemp=c.codemp
					      AND s.codcar=c.codcar";*/
			$ls_sql= "SELECT MAX(s.codser) as codart, MAX(s.codcar) as codcar, MAX(c.formula) as formula ,MAX(c.codestpro) as codestpro, MAX(c.spg_cuenta) as spg_cuenta  
					    ...
					    WHERE s.codemp='$this->ls_codemp'
						  ...
						  AND s.numsolcot=a.numsolcot						  
						  AND s.codcar=c.codcar ";
						  //AND s.codcar='$as_codcar'  Se excluyo ya que se compara un valor vacio al no tener codcar en la funcion uf_insert_cargos
		   }
		//print $ls_sql;
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		...
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))//
			{
				...
				//print "cargo: ".strlen(trim($ls_codcar));
	...
	function  uf_select_items_cotizacion($as_numcot,$as_codpro,$as_numanacot,$as_tipsolcot,&$aa_items,&$li_i)
	{
		...
		{				
			$ls_sql="SELECT DISTINCT d.codart as codigo, a.denart as denominacion, p.nompro, dt.canart as cantidad, 
							dt.preuniart as precio, dt.moniva, dt.monsubart as monsubtotal, dt.montotart as monto, d.obsanacot, d.numcot, d.cod_pro,
		...
		{
				$ls_sql="SELECT DISTINCT d.codser as codigo, a.denser as denominacion, p.nompro, dt.canser as cantidad, dt.monuniser as precio, dt.moniva, dt.monsubser as monsubtotal, dt.montotser as monto,
