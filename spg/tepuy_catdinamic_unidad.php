...
//session_id('8675309');
session_start();
require_once("tepuy_spg_c_cmbestpro.php");
$dat=$_SESSION["la_empresa"];
$nvo_catestprog=new tepuy_spg_c_cmbestpro();
...
{
	$ls_sql="SELECT coduniadm,denuniadm,estemireq,codestpro1,codestpro2,codestpro3 FROM spg_unidadadministrativa WHERE codemp='".$ls_codemp."' AND coduniadm like '".$ls_codigo."' AND denuniadm like '".$ls_denominacion."'";
	if($ls_estpro1!="")
	{
	...
	if($row=$SQL->fetch_row($rs_unidad))
	{
		...
		for($z=1;$z<=$totrow;$z++)
		{
			...
			print "<td align=center><a href=\"javascript: aceptar('$codigo','$denominacion','$estreq','$codestpro1','$codestpro2','$codestpro3');\">".$codigo."</a></td>";
...
<script language="JavaScript">
  function aceptar(codigo,deno,estreq,codest1,codest2,codest3)
  {
  ...
  f.action="tepuy_catdinamic_unidad.php";
  f.submit();
  }
