...
<form name="form1" method="post" action="">
  <p align="center">
	...
	if($ls_operacion=="BUSCAR")
	{
		...
		{
		 $ls_sql_seguridad = " AND CONCAT('".$ls_codemp."','SPG','".$ls_logusr."',codestpro1) IN (SELECT CONCAT(codemp,codsis,codusu,substr(codintper,1,20)) 
		                       FROM sss_permisos_internos WHERE codusu = '".$ls_logusr."' AND codsis = 'SPG')";
		}
		else
		{
		 $ls_sql_seguridad = " AND '".$ls_codemp."'||'SPG'||'".$ls_logusr."'||codestpro1 IN (SELECT codemp||codsis||codusu||substr(codintper,1,20)
		                       FROM sss_permisos_internos WHERE codusu = '".$ls_logusr."' AND codsis = 'SPG')";
		}
		
		$ls_sql=" SELECT codestpro1,denestpro1 ".
		        " FROM   spg_ep1 ".
				" WHERE  codemp='".$ls_codemp."' AND codestpro1 like '".$ls_codigo."' AND denestpro1 like '".$ls_denominacion."' ".$ls_sql_seguridad." ".
				" ORDER BY codestpro1";
...
<script language="JavaScript">
  ...
  f.action="tepuy_catalogo_sector.php?tipo=<?php print $ls_tipo; ?>";
  f.submit();
  }
</script>
</html>
