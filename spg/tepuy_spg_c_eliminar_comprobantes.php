<?php
class tepuy_spg_c_eliminar_comprobantes
{
	var $is_msg_error;
	var $io_sql;
	var $io_include;	
	...
	var $is_descripcion;
	var $is_tipo;
	function tepuy_spg_c_eliminar_comprobantes()
	{
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if (!empty($as_fecdesde))
		{
		   if (!empty($as_fechasta))
		   {
				...
				$ls_criterio1=" AND tepuy_cmp.fecha between '".$as_fecdesde."' AND '".$as_fechasta."'";
				$ls_criterio2=" AND tepuy_cmp_md.fecha between '".$as_fecdesde."' AND '".$as_fechasta."'";
			}
		}
		$ls_sql=" SELECT tepuy_cmp.comprobante, tepuy_cmp.procede,                    ".
				        "        tepuy_cmp.fecha, tepuy_cmp.descripcion, tepuy_cmp.cod_pro,  ".
						"        tepuy_cmp.ced_bene, tepuy_cmp.total, tepuy_cmp.codban, tepuy_cmp.ctaban,  ".
						...
						"   FROM tepuy_cmp                                                     ".
						"  WHERE  tepuy_cmp.procede                                            ".
						"   LIKE '%SPGCMP%'                                                     ".
						"    AND  tepuy_cmp.comprobante NOT IN (SELECT  spg_dt_cmp.comprobante ".
						"                                          FROM spg_dt_cmp              ".
						"                                         WHERE  spg_dt_cmp.procede LIKE '%SPG%') ".
						"    AND tepuy_cmp.comprobante NOT IN (SELECT scg_dt_cmp.comprobante ".
						"                                        FROM scg_dt_cmp WHERE scg_dt_cmp.procede LIKE '%SPG%')".
						$ls_criterio1. 
						"   UNION                                                               ".
						"  SELECT tepuy_cmp_md.comprobante, tepuy_cmp_md.procede,             ".
						"         tepuy_cmp_md.fecha, tepuy_cmp_md.descripcion, tepuy_cmp_md.cod_pro, ".
						"         tepuy_cmp_md.ced_bene, tepuy_cmp_md.total,".
						"         '---' as codban, '-------------------------' as ctaban,                ".
						"	   (SELECT  rpc_proveedor.nompro                                             ".
						"		  FROM rpc_proveedor                                                     ".
						"		 WHERE rpc_proveedor.codemp='".$this->li_codemp."'              ".                                
						"		   AND rpc_proveedor.cod_pro=tepuy_cmp_md.cod_pro) as nompro,  ".
						"	   (SELECT rpc_beneficiario.nombene                                 ".
						"		  FROM rpc_beneficiario                                         ".
						"		 WHERE rpc_beneficiario.codemp='".$this->li_codemp."'           ". 
						"		   AND rpc_beneficiario.ced_bene=tepuy_cmp_md.ced_bene) as nombene  ".
						"  FROM tepuy_cmp_md                       ".
						" WHERE tepuy_cmp_md.procede LIKE '%SPG%'  ".
						"   AND tepuy_cmp_md.comprobante NOT IN (SELECT spg_dtmp_cmp.comprobante  ".
						"                                           FROM spg_dtmp_cmp WHERE spg_dtmp_cmp.procede LIKE '%SPG%')  ".
						"   AND tepuy_cmp_md.comprobante NOT IN (SELECT scg_dt_cmp.comprobante ".
						"                                        FROM scg_dt_cmp WHERE scg_dt_cmp.procede LIKE '%SPG%')".
						$ls_criterio2.
...
//-----------------------------------------------------------------------------------------------------------------------------------------
	...
	{	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		{
			$ls_sql=" DELETE FROM tepuy_cmp WHERE      codemp='".$this->li_codemp."'".
		...
		{
			$ls_sql=" DELETE FROM tepuy_cmp_md WHERE      codemp='".$this->li_codemp."'".
			        "                             AND comprobante='".$as_comprobante."'".
					"                             AND       fecha='".$as_fecha."';";					
		}	
...
}// fin de la calse tepuy_spg_c_eliminar_comprobantes
?>