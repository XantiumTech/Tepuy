<?PHP
class tepuy_sno_c_importardefiniciones
{
	var $io_sql;
	var $io_mensajes;
	var $io_funciones;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function tepuy_sno_c_importardefiniciones()
	{	
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: tepuy_sno_c_importardefiniciones
		//		   Access: public 
	...
	}// end function tepuy_sno_c_importardefiniciones
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	{	
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_destructor
		//		   Access: public (tepuy_sno_d_cargo)
	...
	function uf_load_nomina($as_codnom,&$aa_personaldisp,&$aa_conceptodisp)
	{
		...
		//		   Access: public (tepuy_sno_p_importardefiniciones.php)
	...
	function uf_importar_asignacioncargo($as_codnombus,$as_codper)
	{
		...
		else
		{
			$ls_sql="SELECT codasicar, denasicar, claasicar, minorguniadm, ofiuniadm, uniuniadm, depuniadm, prouniadm, codtab, ".
					"       codpas, codgra, codded, codtipper, numvacasicar, numocuasicar, codproasicar ".
			...
			while(($row=$this->io_sql->fetch_row($rs_data))&&($lb_valido))
			{
				...
				if(!$this->uf_existeregistro($ls_sql))
				{
					$ls_sql="INSERT INTO sno_asignacioncargo".
							...
							"codded,codtipper,numvacasicar,numocuasicar,codproasicar)VALUES('".$this->ls_codemp."','".$this->ls_codnom."',".
	...
	function uf_importar_personal($as_codnombus,$as_codper,$as_codcar,$as_codasicar,$as_codtab,$as_codpas,$as_codgra,$ai_sueper)
	{
		...
		$ls_sql="SELECT codper, codsubnom, codasicar, codtab, codgra, codpas, sueper, horper, minorguniadm, ofiuniadm, uniuniadm, ".
				...
				"		codunirac, fecascper, pagtaqper, grado ".
			...
			while(($row=$this->io_sql->fetch_row($rs_data))&&($lb_valido))
			{
				...
				if(!$this->uf_existeregistro($ls_sql))
				{
					$ls_sql="INSERT INTO sno_personalnomina(codemp,codnom,codper,codsubnom,codtab,codasicar,codgra,codpas,sueper,horper,".
							...
							"fecsusper,cauegrper,codescdoc,codcladoc,codubifis,tipcestic,conjub,catjub,codclavia,codunirac,fecascper, pagtaqper, grado)VALUES".
							...
							"'".$ls_tipcestic."','".$ls_conjub."','".$ls_catjub."','".$ls_codclavia."','".$ls_codunirac."','".$ld_fecascper."',".$li_pagtaqper.",'".$ls_grado."')";
					$li_row=$this->io_sql->execute($ls_sql);
					if($li_row===false)
					{
						$lb_valido=false;
	...
	function uf_importar_constantes($as_codnombus,$as_codcons)
	{
		...
		if($as_codcons<>"")
		{
			$ls_sql="SELECT codemp, codnom, codcons, nomcon, unicon, equcon, topcon, valcon, reicon, tipnumcon ".
					"  FROM sno_constante ".
					" WHERE codemp='".$this->ls_codemp."' ".
					"   AND codnom='".$as_codnombus."' ".
		...
		else
		{
			while(($row=$this->io_sql->fetch_row($rs_data))&&($lb_valido))
			{
				...
				if(!$this->uf_existeregistro($ls_sql))
				{
					$ls_sql="INSERT INTO sno_constante(codemp,codnom,codcons,nomcon,unicon,equcon,topcon,valcon,reicon,tipnumcon)VALUES(".
							"'".$this->ls_codemp."','".$this->ls_codnom."','".$ls_codcons."','".$ls_nomcon."','".$ls_unicon."',".$li_equcon.",".
							"".$li_topcon.",".$li_valcon.",".$li_reicon.",'".$ls_tipnumcon."')";
					$li_row=$this->io_sql->execute($ls_sql);
					if($li_row===false)
					{
						$lb_valido=false;
	...
	function uf_importar_constantespersonal($as_codnombus,$as_codcons)
	{
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql="SELECT codper, codcons, moncon ".
		...
		else
		{
			while(($row=$this->io_sql->fetch_row($rs_data))&&($lb_valido))
			{
				...
				if(!$this->uf_existeregistro($ls_sql))
				{
					$ls_sql="INSERT INTO sno_constantepersonal(codemp,codnom,codper,codcons,moncon)VALUES('".$this->ls_codemp."',".
							"'".$this->ls_codnom."','".$ls_codper."','".$as_codcons."',".$li_moncon.")";
					$li_row=$this->io_sql->execute($ls_sql);
					if($li_row===false)
					{
						$lb_valido=false;
			...
			while(($row=$this->io_sql->fetch_row($rs_data))&&($lb_valido))
			{
				...
				if(!$this->uf_existeregistro($ls_sql))
				{
					$ls_sql="INSERT INTO sno_concepto(codemp,codnom,codconc,nomcon,titcon,forcon,acumaxcon,valmincon,valmaxcon,concon,cueprecon,".
							...
							"titretpatcon,valminpatcon,valmaxpatcon,conprenom,sueintvaccon,codprov,cedben,conprocon)VALUES('".$this->ls_codemp."','".$this->ls_codnom."','".$ls_codconc."',".
							...
							"'".$ls_codprov."','".$ls_cedben."','".$ls_conprocon."')";
					$li_row=$this->io_sql->execute($ls_sql);
					if($li_row===false)
					{
						$lb_valido=false;
