<?php
	...
	else // Imprimimos el reporte
	{
		...
		$lo_hoja->set_column(14,14,10);	
		$lo_hoja->set_column(15,15,15);	
		$lo_hoja->set_column(16,16,15);		
		$lo_hoja->set_column(17,17,12);	
		$lo_hoja->set_column(18,18,18);	
		$lo_hoja->set_column(19,19,12);	
		...
		$lo_hoja->write(4,5, "Tipo Prov.",$lo_titulo);	
		//$lo_hoja->write(4,7, "Nro Planilla Importacin (C-80 o c-81)",$lo_titulo);
		//$lo_hoja->write(4,8, "Nro Expediente Importacin",$lo_titulo);
		$lo_hoja->write(4,6, "Nro de Factura",$lo_titulo);
		$lo_hoja->write(4,7, "Nro de Control",$lo_titulo);
		$lo_hoja->write(4,8, "Nro Nota Debito",$lo_titulo);
		$lo_hoja->write(4,9, "Nro Nota Credito",$lo_titulo);
		$lo_hoja->write(4,10, "Tipo de Transacc.",$lo_titulo);
		$lo_hoja->write(4,11, "Nro de Factura Afectada",$lo_titulo);
		$lo_hoja->write(4,12, "Total de Compra Incluyendo IVA",$lo_titulo);
		$lo_hoja->write(4,13, "Compra sin Derecho a Credito IVA",$lo_titulo);
		$lo_hoja->write(3,14, "Compras",$lo_titulo);
		$lo_hoja->write(3,15, "Internas o",$lo_titulo);
		$lo_hoja->write(3,16, "Importaciones",$lo_titulo);
		$lo_hoja->write(4,14, "Base Imponible",$lo_titulo);
		$lo_hoja->write(4,15, " % ",$lo_titulo);
		$lo_hoja->write(4,16, "Impuesto Iva",$lo_titulo);
		$lo_hoja->write(4,17, "IVA Retenido (Vendedor)",$lo_titulo);
		$lo_hoja->write(4,18, "Nro. Comprobante IVA Retenido",$lo_titulo);
		$lo_hoja->write(4,19, "Fecha de IVA Retenido",$lo_titulo);
		//------------------------------------------------------------------------------------------------------
		...
		{	
		    ...
		    while($row=$io_report->io_sql->fetch_row($rs_resultado))	
			{
				...
				// $lo_hoja->write($li_row, 8, '', $lo_datacenter);
				 ...
				 $lo_hoja->write($li_row, 6, $ls_factura, $lo_datacenter);
				 $lo_hoja->write($li_row, 7, $row["numref"], $lo_datacenter);
