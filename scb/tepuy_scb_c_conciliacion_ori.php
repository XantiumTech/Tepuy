	...
	function uf_cargar_movimientos_a_conciliar($as_codban,$as_ctaban,$object,$ad_fecha,$ad_fechasta,$li_rows, $ls_filtro='T',$as_orden)
	{
		...
		else
		{
			 while($row=$this->io_sql->fetch_row($rs_movimientos))
			 {
				...
				{
					$lb_concil="onClick=javascript:uf_selected()";
				}
				$object[$li_temp][1]  = "<input name=chk".$li_temp." type=checkbox id=chk".$li_temp." value=1 class=sin-borde ".$lb_concil."    ".$lb_checked.">";
...
//-----------------------HABER---------------------------------------//
	...
	{
		$ls_estmov=$ds_debe->getValue("estmov",$li_i);
		$ls_mondeb=$ds_debe->getValue("mondeb",$li_i);//print "Monto Debe =>".$ls_mondeb.'<br>';
		if($ls_estmov!='A')
		{
			$ldec_totdeb+=$ls_mondeb;
		}
