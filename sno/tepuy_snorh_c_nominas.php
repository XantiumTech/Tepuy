...
class tepuy_snorh_c_nominas
{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function tepuy_snorh_c_nominas()
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: tepuy_sno_c_cargo
		//		   Access: public (tepuy_sno_d_nominas)
		...
		require_once("tepuy_snorh_c_dedicacion.php");
		$this->io_dedicacion=new tepuy_snorh_c_dedicacion();		
		require_once("tepuy_snorh_c_tipopersonal.php");
		$this->io_tipopersonal=new tepuy_snorh_c_tipopersonal();		
		require_once("tepuy_snorh_c_escaladocente.php");
		$this->io_escaladocente=new tepuy_snorh_c_escaladocente();		
		require_once("tepuy_snorh_c_clasifidocente.php");
		$this->io_clasificaciondocente=new tepuy_snorh_c_clasifidocente();		
		require_once("tepuy_snorh_c_ubicacionfisica.php");
		$this->io_ubicacion=new tepuy_snorh_c_ubicacionfisica();		
		require_once("tepuy_snorh_c_clasificacionobreros.php");
		$this->io_obreros=new tepuy_snorh_c_clasificacionobreros();		
	...
	{	
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_destructor
		//		   Access: public (tepuy_snorh_d_nominas)
							  ...
							  $as_divcon,$aa_seguridad)
	{
		...
		$ls_sql="INSERT INTO sno_nomina(codemp,codnom,desnom,tippernom,despernom,anocurnom,fecininom,peractnom,numpernom,tipnom, ".
				...
				"codbenfid,cueconfid,divcon) VALUES ('".$this->ls_codemp."','".$as_codnom."','".$as_desnom."',".
	...
	function uf_insert_asignacioncargo_default($as_codnom)
	{
		...
		$ls_sql="INSERT INTO sno_asignacioncargo".
				...
				"codded,codtipper,numvacasicar,numocuasicar,codproasicar)VALUES('".$this->ls_codemp."','".$as_codnom."',".
	...
	function uf_insert_clasificacionobreros_default()
	{
		...
		{
			$ls_sql="INSERT INTO sno_clasificacionobrero(codemp,grado,suemin,suemax,tipcla,obscla) VALUES('".$this->ls_codemp."','0000',0,0,'01','')";
			$li_row=$this->io_sql->execute($ls_sql);
			if($li_row===false)
			{
				$lb_valido=false;
	...
	function uf_update_nomina($as_codnom,$as_consulnom,$as_descomnom,$as_codpronom,$as_codbennom,$as_conaponom,$as_cueconnom,
							  ...
							  $as_ctmetnom,$as_codorgcestic,$aa_seguridad)
	{
		///////////////////////////////////////////////////////////////////////////////////////////
			...
			case "FALSE":
				if($this->uf_select_nomina($as_codnom)===false)
				{
					$lb_valido=$this->uf_insert_nomina($as_codnom,$as_desnom,$as_tippernom,$as_despernom,$as_anocurnom,
													   ...
													   $as_cueconfid,$as_divcon,$aa_seguridad);
				}
				else
				{
			...
			case "TRUE":
				if(($this->uf_select_nomina($as_codnom)))
				{
					...
					$lb_valido=$this->uf_update_nomina($as_codnom,$as_consulnom,$as_descomnom,$as_codpronom,$as_codbennom,
													   ...
													   $as_divcon,$as_ctmetnom,$as_codorgcestic,$aa_seguridad);
				}
				else
				{
						 ...
						 &$as_titrepnom,&$as_codorgcestic,&$as_confidnom,&$as_recdocfid,&$as_tipdocfid,&$as_codbenfid,&$as_cueconfid,&$as_divcon)
	{
		...
		$ls_sql="SELECT codemp, codnom, desnom, tippernom, despernom, anocurnom, fecininom, peractnom, numpernom, tipnom, ".
				...
				"       confidnom, recdocfid, tipdocfid, codbenfid, cueconfid, divcon, ".
	...
	function uf_load_periodo_establecer($as_codnom,&$ai_totrows,&$ao_object)
	{
		...
		//		   Access: public (tepuy_snorh_d_establecer_periodo)
		//	    Arguments: as_codnom  //  código de la nómina
		//	    		   ai_totrows  // total de filas del detalle
		//				   ao_object  // objetos del detalle
		//	      Returns: lb_valido True si se ejecuto el buscar ó False si hubo error en el buscar
	...
	function load_nomina_historico($ai_conta_global,&$as_existe,&$as_codnom,&$as_desnom,&$as_consulnom,&$as_descomnom,						
								   ...
								   &$as_tipdocfid,&$as_codbenfid,&$as_cueconfid)
	{
		/////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql="SELECT codemp, codnom, desnom, tippernom, despernom, anocurnom, fecininom, peractnom, numpernom, tipnom, ".
			    "		subnom, racnom, adenom, espnom, ctnom, ctmetnom, diabonvacnom, diareivacnom, diainivacnom, diatopvacnom, ".
			    "		diaincvacnom, consulnom, descomnom, codpronom, codbennom, conaponom, cueconnom, notdebnom, numvounom, ".
			    "		perresnom, recdocnom, tipdocnom, recdocapo, tipdocapo, confidnom, recdocfid, tipdocfid, codbenfid, cueconfid, ".
				"		 (SELECT count(codperi) FROM sno_periodo ".
                " 		   WHERE codemp='".$this->ls_codemp."' ".
				"   		 AND codnom='".$as_codnom."' ".
				"   		 AND cerper=1) AS total ".
	...
	function uf_load_nominas_reportar(&$ai_totrows,&$ao_object)
	{
		...
		//		   Access: public (tepuy_snorh_r_retencion_arc)
