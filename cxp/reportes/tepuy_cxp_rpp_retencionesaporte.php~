<?PHP
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//    REPORTE: Retencion de Aporte Social
	...
	$ls_tiporeten=$io_report->uf_obtengo_retencion("SELECT codcmp FROM cxp_contador WHERE codemp='$ls_codemp' AND tipo='O'",'C');
	...
	if($lb_valido)
	{
		...
		else
		{
			...
			for ($li_z=0;($li_z<$li_totrow)&&($lb_valido);$li_z++)
			{
				...
				$lb_valido=$io_report->uf_retencionesmunicipales_proveedor($ls_numcom,$ls_tiporeten);
				if($lb_valido)
				{
					...
					$lb_valido=$io_report->uf_retencionesaporte_detalle($ls_numcom,$ls_tiporeten);
					if($lb_valido)
					{
						$li_totalconiva = 0;
						$li_totalsiniva = 0;
