...
if (array_key_exists("la_logusr",$_SESSION))
...
else
   {
	...
	$ls_sistema="SCB";
	$ls_ventanas="tepuy_scb_d_chequera.php";
	$la_security[1]=$ls_codemp;
...
$io_chequera  = new tepuy_scb_c_chequera($la_security);
...
if ($ls_operacion=="RANGO")
   {
	 ...
	 for ($li_i=$li_desde;$li_i<=$li_hasta;$li_i++)
		 {
		   $li_x++;
		   $ls_numcheque = str_pad(strval($li_i),15,0,0);
		   ...
		   $object[$li_x][4] = "<a href=javascript:uf_delete_dt('".$li_x."');><img src=../../shared/imagebank/tools15/eliminar.png alt=Eliminar Cheque width=15 height=15 border=0></a>";
		 }
	 $li_totrowche = $li_x; 
	 ...
	 $object_usu[1][4] = "<a href=javascript:uf_delete_dt_usu('1');><img src=../../shared/imagebank/tools15/eliminar.png alt=Eliminar Usuario width=15 height=15 border=0></a>";
     $li_totrowusu = 1;
   }
...
if ($ls_operacion=="DELETE_DT")
   {
	  ...
	  if (array_key_exists("chk".$li_fila_delete,$_POST))
		 {
		   $io_msg->message("No puede eliminar el cheque, ya esta asociado a un pago !!!");
		 }
	  else
		 {
		   $lb_valido=$io_chequera->uf_delete_cheques($ls_chequera,$ls_codban,$ls_ctaban,$ls_cheque);
		   if ($lb_valido)
			  {
			    $ls_operacion = "CARGAR";
				$io_msg->message("Cheque eliminado satisfactoriamente !!!");
			  }
		   else
			  {
			    $ls_operacion = "PINTAR";
				$io_msg->message("No puede eliminar el cheque,".$io_chequera->is_msg_error);
			  }
		 }
	}
...
if ($ls_operacion=="CARGAR")
   {
     require_once("tepuy_scb_c_chequera.php");
 	 $io_chequera=new tepuy_scb_c_chequera($la_security);
	 $io_chequera->uf_cargar_cheques($ls_codban,$ls_ctaban,$ls_chequera,$ls_codemp,$li_totrowche,$object,$li_totrowusu,$object_usu);
...
if ($ls_operacion=="GUARDAR")
   {
	 $li_totrowche = $_POST["total"];
	 $lb_existe    = $io_chequera->uf_select_chequera($ls_codemp,$ls_codban,$ls_ctaban,$ls_chequera);
	 if ($lb_existe)
        { 
	      if ($ls_status=="N")//Nuevo
		     {
			   $io_msg->message("Este Número de Chequera ya está Registrado para este Banco y Cuenta Bancaria !!!");  
			   $lb_valido = false;
			   $ls_operacion = "PINTAR";
			 }
			   ...
			   for ($li_i=1;($li_i<=$li_totrowche) && $lb_valido;$li_i++)
				   {
					 ...
					 $lb_valido = $io_chequera->uf_update_chequera($ls_codemp,$ls_codban,$ls_ctaban,$ls_chequera,$ls_numche,$ls_codusu,$li_i);
				   }
				if ($lb_valido)
				   {
     ...
     else
	    {
		  ...
		  for ($li_i=1;$li_i<=$li_totrowche;$li_i++)
		      {
			    $ls_numche = $_POST["txtnumrefche".$li_i];
				$lb_existe = $io_chequera->uf_validar_cheque($ls_codemp,$ls_codban,$ls_ctaban,$ls_numche);
			    if ($lb_existe)
				   {
				     break;
				   }
		  ...
		  if (!$lb_existe)
				...
				for ($li_x=1;($li_x<=$li_totrowche) && $lb_valido;$li_x++)
				    {
					  ...
					  $lb_valido = $io_chequera->uf_guardar_cheques($ls_codemp,$ls_codban,$ls_ctaban,$ls_tipcta,$ls_chequera,$ls_numche,$ls_codusu,0,$li_x);
					}
			    if ($lb_valido)
		           {
				     $io_chequera->io_sql->commit();
		     ...
		     {
			   $ls_operacion = "PINTAR";
			   $io_msg->message("Existen Números de Cheques Repetidos para esta Cuenta Bancaria, Por Favor Verifique !!!");
			 }
		}
   }
   ...
   {             
	 ...
	 for ($li_i=1;$li_i<=$li_totrowche;$li_i++)
		 { 
		   ...
		   $object[$li_i][4] = "<a href=javascript:uf_delete_dt('".$li_i."');><img src=../../shared/imagebank/tools15/eliminar.png alt=Eliminar Cheque width=15 height=15 border=0></a>";
		 }
     ...
     for ($li_i=1;$li_i<=$li_totrowusu;$li_i++)
         {
	       if ($li_i!=$li_rowdel)
	          {		
		        ...
		        $object_usu[$li_temp][4] = "<a href=javascript:uf_delete_dt_usu('".$li_temp."');><img src=../../shared/imagebank/tools15/eliminar.png alt=Eliminar Usuario width=15 height=15 border=0></a>";
			  }
	       else
	          {	
		        $li_rowdelete=0;
       ...
       for ($li_i=1;$li_i<$li_totrowusu;$li_i++)
           {
		     ...
		     $object_usu[$li_i][4] = "<a href=javascript:uf_delete_dt_usu('".$li_i."');><img src=../../shared/imagebank/tools15/eliminar.png alt=Eliminar Usuario width=15 height=15 border=0></a>";
		   }		  
		...
		$object_usu[$li_totrowusu][4] = "<a href=javascript:uf_delete_dt_usu('".$li_totrowusu."');><img src=../../shared/imagebank/tools15/eliminar.png alt=Eliminar Usuario width=15 height=15 border=0></a>";
	}
...
if ($ls_operacion=="NUEVO")
   {
		...
		$object[1][4] = "<a href=javascript:uf_delete_dt('1');><img src=../../shared/imagebank/tools15/eliminar.png alt=Eliminar Cheque width=15 height=15 border=0></a>";
...
if ($ls_operacion=="AGREGAR_CHEQUE")
   {
     $li_totrowche = $_POST["total"];
	 for ($li_i=1;$li_i<=$li_totrowche;$li_i++)
	     {
...
<div align="center">
  <table width="601" height="223" border="0" cellpadding="0" cellspacing="0" class="formato-blanco">
      <tr>
        <td width="601" height="221"><form name="form1" method="post" action="">
<?php 
//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
          ...
          <table width="574" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
              ...
              <tr class="formato-blanco">
                ...
                <td height="22" colspan="3" style="text-align:left"><label>
                  <input name="txtchequera" type="text" id="txtchequera" value="<?php echo $ls_chequera ?>" size="20" maxlength="10" style="text-align:center" onBlur="javascript:rellenar_cad(this.value,10,this)" onKeyPress="return keyRestrict(event,'1234567890'); ">
                  <input name="hidcontrol" type="hidden" id="hidcontrol" value="<?php echo $ls_control ?>">
                </label></td>
              </tr>
              <tr class="formato-blanco">
                <td height="22" style="text-align:right">Banco</td>
                <td height="22" colspan="3" style="text-align:left"><input name="txtcodban" type="text" id="txtcodban"  style="text-align:center" value="<?php print $ls_codban;?>" size="10" maxlength="3" readonly><a href="javascript:cat_bancos();"><img src="../../shared/imagebank/tools15/buscar.png" width="15" height="15" border="0" alt="Catálogo de Bancos"></a>
                  <input name="txtdenban" type="text" class="sin-borde" id="txtdenban" value="<?php print $ls_denban?>" size="70" maxlength="50" readonly>              </td>
              </tr>
              ...
              <tr class="formato-blanco">
                ...
                <td height="22" colspan="3" align="left"><div align="left">
                  ...
                  <a href="javascript:catalogo_cuentabanco();"><img src="../../shared/imagebank/tools15/buscar.png" width="15" height="15" border="0" alt="Catálogo de Cuentas Bancarias"></a>
                  <input name="txtdenominacion" type="text" class="sin-borde" id="txtdenominacion" style="text-align:left" value="<?php echo $ls_denctaban; ?>" size="49" maxlength="254" readonly>
                  <input name="txtcuenta_scg" type="hidden" id="txtcuenta_scg">
                  <input name="txtdisponible" type="hidden" id="txtdisponible">
                </div></td>
              </tr>
              <tr class="formato-blanco">
                <td height="22" style="text-align:right">Tipo Cuenta</td>
                <td height="22" colspan="3" align="left"><div align="left">
                  <input name="txttipocuenta" type="text" id="txttipocuenta" style="text-align:center" value="<?php print $ls_tipcta;?>" size="10" readonly>
                  <input name="txtdentipocuenta" type="text" id="txtdentipocuenta" value="<?php print $ls_dentipcta;?>" class="sin-borde" size="72" readonly>
            ...
            <tr class="formato-blanco">
              ...
              <td width="224"><div align="left">
                <input name="txtdesde" type="text" id="txtdesde" onBlur="javascript:rellenar_cad(this.value,15,this)" style="text-align:center" maxlength="15" onKeyPress="return keyRestrict(event,'1234567890'); ">
              </div></td>
              <td width="58" style="text-align:right">Hasta</td>
              <td><div align="left">
                <input name="txthasta" type="text" id="txthasta" onBlur="javascript:rellenar_cad(this.value,15,this)" style="text-align:center" maxlength="15" onKeyPress="return keyRestrict(event,'1234567890'); ">
              </div></td>
            </tr>
            <tr class="formato-blanco">
              <td height="20">&nbsp;</td>
              <td height="20" colspan="3" style="text-align:left"><a href="javascript: uf_generar_cheques();"><img src="../../shared/imagebank/tools20/ejecutar.png" width="20" height="20" border="0">Generar Cheques</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:uf_agregar_cheque();"><img src="../../shared/imagebank/tools20/nuevo.png" alt="Agregar Cheque..." width="20" height="20" border="0">Agregar Cheque</a></td>
            </tr>
            <tr class="formato-blanco">
            ...
            <tr class="formato-blanco">
              <td height="20">&nbsp;</td>
              <td colspan="3"><a href="javascript: catalogo_usuarios();"><img src="../../shared/imagebank/tools20/nuevo.png" alt="Agregar Usuarios" width="20" height="20" border="0">Agregar Usuarios</a>
...
function ue_nuevo()
{
  ...
  if (li_incluir==1)
	 {	
       ...
       f.action="tepuy_scb_d_chequera.php";
	   f.submit();
	 }
  else
	 {
		  ...
		  if ((ls_chequera!="")&&(ls_codban!="")&&(ls_ctaban!=""))
	         {
			   ...
			   if (lb_valido)
			      ...
			      {
				    alert("La Chequera No puede ser eliminada, posee Cheques Emitidos !!!");
				  }
			 }
	      else
	         {
...
function ue_buscar()
{
	...
	if (li_leer==1)
       {
	     window.open("tepuy_scb_cat_cheques.php","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=650,height=400,left=50,top=50,location=no,resizable=yes");
       }
	else
	   {
		 alert("No tiene permiso para realizar esta operacion");
...
function catalogo_cuentabanco()
{
	if(!f.txtchequera.readOnly)
	{
	   ...
	   if (ls_chequera!='')
			...
			if (ls_codban!='')
			   {
				 ...
				 window.open(pagina,"_blank","menubar=no,toolbar=no,scrollbars=yes,width=680,height=400,resizable=yes,location=no,dependent=yes");
			   } 
		    else
			   {
	      ...
	      {
		    alert("Por Favor Indique el Número de Chequera !!!");
		  }
	}
	else
	{
	  ...
	  if (ls_chequera!='')
	     {
		   ...
		   window.open(pagina,"_blank","menubar=no,toolbar=no,scrollbars=yes,width=516,height=400,resizable=yes,location=no,dependent=yes");
		 }
	  else
	     {
		   alert("Por Favor Indique el Número de Chequera !!!");
...
function catalogo_usuarios()
{
	...
	if (li_totrowche>=2)
	   {
	     window.open("tepuy_cat_usuarios.php","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=520,height=400,resizable=yes,location=no");
	   }
	else
	   {
...
function uf_delete_dt(li_i)  
{
	...
	if (eval("f.chk"+li_i+".checked==true"))
	   {
	     alert("El Cheque ya ha sido Emitido, No puede ser Eliminado !!!");
	   }
	else
	   {
	     if (eval("f.txtnumrefche"+li_i+".value!=''"))
		    {
			  li_lastrowche = f.total.value;
			  if (li_lastrowche==li_i)
				 {
				 ...
				 {
				   alert("Los Cheques deben ser Eliminados en forma Ascendente y a partir del Último !!!");
				 }
			}
	   }
}
		         ...
		         if (li_totrowusu>2)
			        {
					  ...
					  for (li_z=1;li_z<=li_totrowche;li_z++)
					      {
							...
							if (eval("f.chk"+li_z+".cheched==true"))
							   {
							     lb_checked = true;
							   }
							else
					  ...
					  window.open("tepuy_cat_asignacion_cheques.php?hidtotrowche="+li_totrowche+"&hidnumche="+la_cheques+"&hidcodusu="+la_usuarios+"&hidcheemi="+la_checked+"&hidusuasiche="+la_usuasiche+"&hidtotrowusu="+li_totrowusu,"_blank","menubar=no,toolbar=no,scrollbars=yes,width=540,height=460,resizable=yes,location=no,dependent=yes");
			        }
		       }	      
		  }
	   else
...
function uf_agregar_cheque()
{
  li_totrowche = f.total.value;
  if (li_totrowche==1 && eval("f.txtnumrefche"+li_totrowche+".value==''"))
     {
	   eval("f.txtnumrefche"+li_totrowche+".focus()");
	 }
  else
     {
	   f.status.value = 'N';
	   f.operacion.value = "AGREGAR_CHEQUE";
	   f.action="tepuy_scb_d_chequera.php";
	   f.submit();
	 }
}
