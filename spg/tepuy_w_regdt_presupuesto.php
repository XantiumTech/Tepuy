...
else
{
	...
	$ls_estpro3="";
	$ls_cuentaplan="";
	$ls_denominacion=$_GET["descripcion"];
	$ls_procedencia="";
	$ls_descripcion="";
...
if($ls_operacion=="GUARDARPRE")
{
	...
	if($li_estmodest==2)
	{
		...
		$ls_est1=$io_function->uf_cerosizquierda($ls_est1,20);
		$ls_est2=$io_function->uf_cerosizquierda($ls_est2,6);
		$ls_est3=$io_function->uf_cerosizquierda($ls_est3,3);
		$ls_est4=$io_function->uf_cerosizquierda($ls_est4,2);
		$ls_est5=$io_function->uf_cerosizquierda($ls_est5,2);
	}
	else
	{
		$ls_est4        = "00";
	...
	$ld_disponible=0;
	$lb_valido=$in_classcmp->uf_spg_select_disponibilidad($ls_cuenta,$ls_est1,$ls_est2,$ls_est3,$ls_est4,
	                                                      $ls_est5,$ld_disponible,$ls_operacionpre,$ldec_monto);
	...
	else
	{
		...
		$lb_valido=$in_classcmp->uf_guardar_automatico($ls_comprobante,$ld_fecha,$ls_proccomp,$ls_desccomp,
		                                               &$ls_prov,&$ls_bene,$ls_tipo,1,$ls_codban,$ls_ctaban);
		...
		if($lb_valido)
		{
			...
			$ldec_monto=str_replace(",",".",$ldec_monto);
			//$in_classcmp->io_int_scg->io_sql->begin_transaction();
			$lb_valido=$in_classcmp->uf_guardar_movimientos($arr_cmp,$ls_est1,$ls_est2,$ls_est3,$ls_est4,$ls_est5,$ls_cuenta,
			                                                $ls_procede,$ls_descripcion,$ls_documento,$ls_operacionpre,0,$ldec_monto,"C",$ls_codban,$ls_ctaban);
			if($lb_valido)
			{
/*				$ls_codemp=$_SESSION["la_empresa"]["codemp"];
				$ls_codestpro=$ls_est1.$ls_est2.$ls_est3.$ls_est4.$ls_est5;
				$lb_valido=$in_classcmp->uf_update_bsf_tepuycmp(0,$ls_codemp,$ls_procede,$ls_comprobante,
...
<table width="583" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
  ...
  <tr>
    ...
    <td><input name="txtprocedencia" type="text" id="txtprocedencia" size="22" maxlength="6" style="text-align:center" value="<?php print $ls_procedencia;?>" readonly>
      <a href="javascript:catalogo_procedencia();"><img src="../shared/imagebank/tools15/buscar.png" width="15" height="15" border="0" alt="Cat&aacute;logo de Procedencia"></a>
      <input name="txtdenpro" type="text" class="sin-borde" id="txtdenpro" size="53" readonly>
     </td>
  </tr>
   <tr>
    <?php 
...
function catalogo_procedencia()
{
	   pagina="tepuy_spg_cat_procedencia.php";
