<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsolcot,$as_fecsolcot,$as_dentipsolcot,$as_obssolcot,&$io_pdf)
	{
		...
		$io_pdf->saveState();
		$io_pdf->rectangle(40,925,550,40);
		$io_pdf->line(450,965,450,925);
		$io_pdf->line(450,945,590,945);
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],46,926.5,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
		$li_tm=$io_pdf->getTextWidth(10,$as_titulo);		
		$io_pdf->addText(240,940,10,"<b>".$as_titulo."</b>"); // Agregar el título
		$io_pdf->addText(460,950,10,"<b>   No.:</b>");      // Agregar texto
		$io_pdf->addText(495,950,10,$as_numsolcot); // Agregar Numero de la solicitud
		$io_pdf->addText(450,930,10,"<b>  Fecha:</b>"); // Agregar texto
		$io_pdf->addText(495,930,10,$as_fecsolcot); // Agregar la Fecha
	...
	function uf_print_encabezadosolicitud($as_dentipsolcot,$as_concepto,$io_pdf)
	{
       ...
       /*  $io_pdf->addText(60,795,10,"<i>Por medio de la presente permítame saludarles y al mismo tiempo, aprovecho la oportunidad para invitar su empresa a</i>"); // Agregar titulo
         ...
         $io_pdf->addText(60,810,10,"<i>Me es grato dirigirme a usted (es) con la finalidad de hacerle la invitacion para participar en la cotizacion de precios, la</i>"); // Agregar titulo
         $io_pdf->addText(42,800,10,"<i>cual se hara mediante el procedimiento de consulta de precios bajo la modalidad de <b>Orden de ".$as_dentipsolcot."</b>, de conformidad </i> "); // Agregar titulo
         $io_pdf->addText(42,790,10,"<i>con lo dispuesto en art. 73 de la Ley de Contrataciones Publicas, referente a: </i> "); // Agregar titulo
	$as_concepto1=substr($as_concepto,0,80);
	$as_concepto2=substr($as_concepto,81,80);
         $io_pdf->addText(42,780,10,"<i><b>".$as_concepto."</b></i> "); // Agregar titulo
	 $io_pdf->addText(42,770,10,"<i><b>".$as_concepto2."</b></i> "); // Agregar titulo
         $io_pdf->addText(42,760,10,"<i>Se exige como condicion de entrega lo siguiente: </i> "); // Agregar titulo
	$io_pdf->addText(55,750,10,"   <i><b>1.</b> El bien o servicio objeto de la contratacion sean transportados por la empresa adjudicada.</i>"); // Agregar titulo
	$io_pdf->addText(55,740,10,"   <i><b>2.</b> Deben estar inscritos en nuestro registro de proveedores con la documentación actualizada.</i>"); // Agregar titulo
	$io_pdf->addText(55,730,10,"   <i><b>3.</b> Especificar los rubros claramente en cuanto a su calidad y dimensiones.</i>"); // Agregar titulo
	$io_pdf->addText(55,720,10,"   <i><b>4.</b> La cotización debe venir en sobre cerrado y marcado con sello húmedo.</i>"); // Agregar titulo
	$io_pdf->addText(55,710,10,"   <i><b>5.</b> Debe estar inscrito Registro Nacional de Contratista y demás requisitos exigidos por la empresa.</i>"); // Agregar titulo
...
/*OBRA: CULTIVO PROTEGIDO EN EL ESTADO BARINAS Y EJECUTADA POR BARVIALSA. 

hasta las instalaciones de la Empresa BARVIALSA. Sera considerada como valida la oferta, si esta se adjunta a
las especificaciones requeridas y cumplan con las demas exigencias aplicables.
*/
	...
	function uf_print_pie($io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);
		$io_pdf->addText(65,205,10,"     <i>Debera presentar su oferta dentro de los tres (3) dias habiles luego de recibir este oficio.   Cualquier aclaratoria a
 </i>"); // Agregar titulo
		$io_pdf->addText(55,195,10,"<i>la presente invitacion podra ser hecha al siguiente dia habil a la aceptacion de la misma y sera respondida al siguiente
 </i>"); // Agregar titulo
		$io_pdf->addText(55,185,10,"<i>dia habil de recibirla;  por ante  la  Unidad  de  Compras  y  Suministros.   El criterio utilizado para la adjudicacion de la </i>"); // Agregar titulo
		$io_pdf->addText(55,175,10,"<i>contratacion  requerida,  sera  la  que  presenta  mejor  oferta  de  precios,  calidad y tiempo de entrega en cuanto a la
 </i>"); // Agregar titulo
		$io_pdf->addText(55,165,10,"<i>totalidad del material o servicio requerido. </i>"); // Agregar titulo


		$io_pdf->addText(65,145,10,"   <i>Sin mas a que hacer referencia, se suscribe de usted (es) </i>"); // Agregar titulo
	  	$io_pdf->addText(260,130,10,"   <i>Atentamente,</i>");
		//$io_pdf->addText(240,95,10,"   <i>Lcdo. Jhonny Mercado</i>");
		$io_pdf->addText(240,95,10,"   <i>Lcda. Alejandra Sandoval</i>");
		$io_pdf->addText(252,85,10,"   <i>Jefa de Compras</i>");
		$io_pdf->addText(135,65,12,'<b><i></b></i>');
		$io_pdf->addText(215,52,12,'<b><i></b></i>');
	}// end function uf_print_detalle
	//--------------------------------------------------------------------------------------------------------------------------------
