<?php
class tepuy_rpc_c_municipio
{

var $ls_sql;
	
	function tepuy_rpc_c_municipio($conn)
	{
...
//////////////////////////////////////////////////////////////////////////////
  ...
  $ls_sql = " INSERT INTO tepuy_municipio (codpai,codest,codmun,denmun) ".
  ...
  if ($rs_data===false)
	 {
	   $lb_valido=false; 
	   $this->io_msg->message("CLASE->tepuy_RPC_C_MUNICIPIO; METODO->uf_insert_municipio; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	 }
  else
	 {
	   /////////////////////////////////         SEGURIDAD               /////////////////////////////		
...
//	   Returns:	 $lb_valido.	
// Description:  Función que se encarga de actualizar los datos de un municipio en la tabla tepuy_municipio . 
//////////////////////////////////////////////////////////////////////////////

  $ls_sql=" UPDATE tepuy_municipio SET  denmun='".$as_denmun."' ".
  ...
  if ($rs_data===false)
	 {
	   $lb_valido=false; 
	   $this->io_msg->message("CLASE->tepuy_RPC_C_MUNICIPIO; METODO->uf_update_municipio; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	 }
  else
	 {
	   /////////////////////////////////         SEGURIDAD               /////////////////////////////		
...
//	   Returns:	 $lb_valido.	
// Description:  Función que se encarga de eliminar un municipio en la tabla tepuy_municipio . 
//////////////////////////////////////////////////////////////////////////////

  $ls_sql=" DELETE FROM tepuy_municipio WHERE codpai='".$as_codpais."' AND codest='".$as_codest."' AND codmun='".$as_codmun."'";
  $this->io_sql->begin_transaction();
  $rs_data=$this->io_sql->execute($ls_sql);
  if ($rs_data===false)
	 {
	   $lb_valido=false; 
	   $this->io_msg->message("CLASE->tepuy_RPC_C_MUNICIPIO; METODO->uf_delete_municipio; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	 }
  else
	 {
	   /////////////////////////////////         SEGURIDAD               /////////////////////////////		
...
//////////////////////////////////////////////////////////////////////////////
	...
	$ls_sql=" SELECT * FROM tepuy_municipio ".
	        " WHERE codpai='".$as_codpais."' AND codest='".$as_codest."' AND codmun='".$as_codmun."'" ;
...
//////////////////////////////////////////////////////////////////////////////
	...
	$ls_sql=" SELECT * FROM tepuy_pais ORDER BY codpai ASC";
	$rs_data=$this->io_sql->select($ls_sql);
	if ($rs_data===false)
	   {
		 $lb_valido=false; 
		 $this->io_msg->message("CLASE->tepuy_RPC_C_MUNICIPIO; METODO->uf_llenarcombo_pais; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	   }
	else
	   {
		 $li_numrows=$this->io_sql->num_rows($rs_data);	   
...
//////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql  = " SELECT * FROM tepuy_estados WHERE codpai='".$as_codpais."' ORDER BY desest ASC ";
		$rs_data = $this->io_sql->select($ls_sql);
	    if ($rs_data===false)
		   {
		     $lb_valido=false; 
		     $this->io_msg->message("CLASE->tepuy_RPC_C_MUNICIPIO; METODO->uf_load_estados; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		   }
		else
		   {
		     $li_numrows=$this->io_sql->num_rows($rs_data);	   
