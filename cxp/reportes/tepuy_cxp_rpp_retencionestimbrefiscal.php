<?php
	...
	function uf_insert_seguridad($as_titulo)
	{
		...
		$lb_valido=$io_fun_cxp->uf_load_seguridad_reporte("CXP","tepuy_cxp_r_retencionestimbrefiscal.php",$ls_descripcion);
		return $lb_valido;
	}
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_print_encabezado_pagina($as_titulo,&$io_pdf)
	{
		...
		$tm=304-($li_tm/2);
		$io_pdf->addText($tm,710,12,$as_titulo); // Agregar el título
		//$li_tm=$io_pdf->getTextWidth(12," RETENCION 1 X 1000");
		$io_pdf->addText(246,690,14,"<b>RETENCION 1 X 1000</b>"); // columna, fila, tamaño de letra
		//$tm=396-($li_tm/2);
		//$io_pdf->addText($tm,530,9,"ARTICULO 111 ORDENANZA SOBRE ACTIVIDADES ECONOMICAS"); // Agregar el título

		$io_pdf->addText(95,100,12,'____________________________'); // columna, fila, tamaño de letra
		$io_pdf->addText(382,100,12,'____________________________');
		$io_pdf->addText(112,86,12,'   RECIBIDO CONFORME');
		$io_pdf->addText(430,86,12,'	FIRMA Y SELLO');
		$ls_nombremp=$_SESSION["la_empresa"]["nombre"];
		$io_pdf->addText(410,76,8,$ls_nombremp);
		//$io_pdf->addText(380,76,8,'BARINESA DE VIALIDAD Y CONSTRUCCIONES S.A.');
...
//	$as_pie='BASE LEGAL: RETENCION DE IMPUESTO MUNICIPAL. ';
//	$as_pie1='Ordenanza de Impuesto sobre Actividades Econominas. Art. 111: Los Agentes de Retencion deberan retener el Impuesto correspondiente';
//	$as_pie2='a la actividad de la cual se desprende el pago, en los porcentajes establecidos en esta Ordenanza y de acuerdo a lo dispuesto en el Clasificador de Actividades Economicas';
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_cabecera($as_numcon,$ad_fecrep,$as_agenteret,$as_rifagenteret,$as_perfiscal,$as_licagenteret,$as_diragenteret,$as_dirsujret,
	...
	/*	$io_pdf->ezSetY(525);
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>300,
						 'cols'=>array('name'=>array('justification'=>'left','width'=>160))); // Ancho Minimo de la tabla
        $io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		unset($la_data);
		unset($la_columna);
		unset($la_config);
	...
	/*	$io_pdf->ezSetY(490);
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'shaded'=>0, // Sombra entre lineas
						 'shadeCol2'=>array(0.9,0.9,0.9), // Color de la sombra
						 'xPos'=>116, // OrientaciOn de la tabla
						 'width'=>300, // Ancho de la tabla						 
						 'maxWidth'=>300,
						 'cols'=>array('name'=>array('justification'=>'left','width'=>160))); // Ancho Minimo de la tabla
        $io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'shaded'=>0, // Sombra entre lineas
						 'shadeCol2'=>array(0.9,0.9,0.9), // Color de la sombra
						 'xPos'=>316, // OrientaciOn de la tabla
						 'width'=>560, // Ancho de la tabla						 
						 'maxWidth'=>560,
						 'cols'=>array('name'=>array('justification'=>'left','width'=>560))); // Ancho Minimo de la tabla
        $io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		unset($la_data);
		unset($la_columna);
		unset($la_config);								 
		...
		$la_data[2]=array('name'=>$as_dirsujret);				
		$la_columna=array('name'=>'');		
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 8, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Lieas
	...
	function uf_print_detalle($la_data,$ai_totbasimp,$ai_totmonimp,$as_rifagenteret,&$io_pdf)
	{
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$io_pdf->ezSetY(465); // mientras mas alto sube


		$la_data1[1]=array('numope'=>'<b>Operacion Nro.</b>',
						  'fecfac'=>'<b>Fecha de la Factura</b>',
  						  'tipinst'=>'<b>Tipo de Instrumento</b>',		
						  'numfactura'=>'<b>Numero de Factura</b>',
						  'numfacturacontrol'=>'<b>Factura Control</b>',
						  'baseimp'=>'<b>Base Imponible</b>',
						  'impret'=>'<b>Imp. Retenido</b>');

		$la_columna=array('numope'=>'<b>Operacion Nro.</b>',
						  'fecfac'=>'<b>Fecha de la Factura</b>',
  						  'tipinst'=>'<b>Tipo de Instrumento</b>',		
						  'numfactura'=>'<b>Numero de Factura</b>',
						  'numfacturacontrol'=>'<b>Factura Control</b>',
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'xPos'=>318, // OrientaciOn de la tabla
						 'cols'=>array('numope'=>array('justification'=>'center','width'=>50), // Justificacion y ancho de la columna
						 			   ...
						 			   'tipinst'=>array('justification'=>'center','width'=>88),
						 			   'numfactura'=>array('justification'=>'center','width'=>80),
						 			   'numfacturacontrol'=>array('justification'=>'center','width'=>80),
						 			   'baseimp'=>array('justification'=>'center','width'=>100),
   						 			   'impret'=>array('justification'=>'center','width'=>90))); 
		...
		unset($la_columna);
		unset($la_config);
		$la_data1[1]=array('total'=>'<b>Totales</b>',		
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'maxWidth'=>250, // Ancho Mínimo de la tabla
						 'xPos'=>472, // OrientaciOn de la tabla
						 'cols'=>array('total'=>array('justification'=>'center','width'=>50), // Justificacion y ancho de la columna
	...
	$ls_tiporeten=$io_report->uf_obtengo_retencion("SELECT codcmp FROM cxp_contador WHERE codemp='$ls_codemp' AND tipo='T'",'C');
			...
			for ($li_z=0;($li_z<$li_totrow)&&($lb_valido);$li_z++)
			{
				...
				if($lb_valido)
				{
					...
					uf_print_cabecera($ls_numcon,$ls_fecrep,$ls_agenteret,$ls_rifagenteret,$ls_perfiscal,$ls_licagenteret,
									  $ls_diragenteret,$ls_dirsujret,$ls_nomsujret,$ls_rif,$ls_numlic,$li_estcmpret,&$io_pdf);
					$lb_valido=$io_report->uf_retencionestimbrefiscal_detalles($ls_numcom,$ls_tiporeten,2);
					if($lb_valido)
					{
						$li_totalbaseimp=0;
						$li_totalmontoimp=0;
						...
						for($li_i=1;$li_i<=$li_total;$li_i++)
						{
							...
							$ls_numfacontrol=$io_report->ds_detalle->data["numcon"][$li_i];
							$ls_numref=$io_report->ds_detalle->data["numcon"][$li_i];	              
							$li_baseimp=$io_report->ds_detalle->data["basimp"][$li_i];	
							$li_montototal=$io_report->ds_detalle->data["montototal"][$li_i];
							...
							$la_data[$li_i]=array('numope'=>$li_i,'fecfac'=>$ld_fecfac,'tipinst'=>'Factura','numfactura'=>$ls_numfac,'numfacturacontrol'=>$ls_numfacontrol,'baseimp'=>$li_baseimp,'impret'=>$li_totimp);														
