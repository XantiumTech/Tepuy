...
var $is_msg_error;
		...
		function tepuy_rpc_c_pais($conn)
		{
...
function uf_insert_pais($as_codpais,$as_denpais,$aa_seguridad) 
{
 ...
 //	Description: Funcion que se encarga de insertar pais dentro de la tabla tepuy_pais. 
 //////////////////////////////////////////////////////////////////////////////
	$ls_sql = " INSERT INTO tepuy_pais (codpai,despai) VALUES('".$as_codpais."','".$as_denpais."')";
	$this->io_sql->begin_transaction();
	$rs_data=$this->io_sql->execute($ls_sql);
	if ($rs_data===false)
	   {
	     $lb_valido=false; 
	     $this->io_msg->message("CLASE->tepuy_RPC_C_PAIS; METODO->uf_insert_pais; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	   }
	else
	   {
		 /////////////////////////////////         SEGURIDAD               /////////////////////////////		
...
//////////////////////////////////////////////////////////////////////////////
	$ls_sql=" UPDATE tepuy_pais SET  despai='".$as_denpais."' WHERE codpai = '" .$as_codpais. "'";
	$this->io_sql->begin_transaction();
	$rs_data=$this->io_sql->execute($ls_sql);
	if ($rs_data===false)
	   {
	     $lb_valido=false; 
	     $this->io_msg->message("CLASE->tepuy_RPC_C_PAIS; METODO->uf_update_pais; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	   }
	else
	   {
		 /////////////////////////////////         SEGURIDAD               /////////////////////////////		
...
//	    Access:  public
//	 Arguments:  $as_codpais,$as_denpais,$aa_seguridad           
// Description:  Funcion que se encarga de eliminar un pais de la tabla tepuy_pais. 
//////////////////////////////////////////////////////////////////////////////
	
	 $ls_sql=" DELETE FROM tepuy_pais WHERE codpai='".$as_codpais."'";
	 $this->io_sql->begin_transaction();
	 $rs_data=$this->io_sql->execute($ls_sql);
	 if ($rs_data===false)
		{
		  $lb_valido=false; 
		  $this->io_msg_error = "CLASE->tepuy_RPC_C_PAIS; METODO->uf_update_pais; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message);
		}
	 else
		{
		  /////////////////////////////////         SEGURIDAD               /////////////////////////////		
...
//////////////////////////////////////////////////////////////////////////////
	...
	$lb_valido=false;
	$ls_sql=" SELECT * FROM tepuy_pais WHERE codpai='".$as_codpais."'";
	$rs_data=$this->io_sql->select($ls_sql);
	if ($rs_data===false)
	   {
	     $lb_valido=false; 
	     $this->io_msg->message("CLASE->tepuy_RPC_C_PAIS; METODO->uf_load_pais; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	   }
	else
	   {
...
//////////////////////////////////////////////////////////////////////////////
	...
	if($rs_data===false)
	  {
		$lb_valido=false;
	    $this->io_msg->message("CLASE->tepuy_RPC_C_PAIS; METODO->uf_check_relaciones; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	  }
	else
	  {
