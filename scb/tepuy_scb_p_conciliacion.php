...
$io_fun_banco->uf_load_seguridad("SCB","tepuy_scb_p_conciliacion.php",$ls_permisos,$la_seguridad,$la_permisos);
$li_diasem = date('w');
switch ($li_diasem){
...
<!--  <tr>
  ...
  <tr>
    ...
    <td class="toolbar" width="24"><div align="center"><a href="javascript: ue_buscar();"><img src="../shared/imagebank/tools20/buscar.png" alt="Buscar" title="Buscar" width="20" height="20" border="0"></a></div></td>
    <td class="toolbar" width="27"><div align="center"><a href="javascript: ue_eliminar();"><img src="../shared/imagebank/tools20/eliminar.png" alt="Eliminar" title="Eliminar" width="20" height="20" border="0"></a></div></td>
    <td class="toolbar" width="24"><div align="center"><a href="javascript: ue_cerrar();"><img src="../shared/imagebank/tools20/salir.png" alt="Salir" title="Salir" width="20" height="20" border="0"></a></div></td>
    <td class="toolbar" width="27"><div align="center"><img src="../shared/imagebank/tools20/ayuda.png" alt="Ayuda" title="Ayuda" width="20" height="20"></div></td>
...
<?php
	...
	require_once("tepuy_scb_c_conciliacion.php");
	$in_classconciliacion=new tepuy_scb_c_conciliacion($la_seguridad);

	if( array_key_exists("operacion",$_POST))
	{
		$ls_operacion= $_POST["operacion"];
		$ls_codban=$_POST["txtcodban"];
		$cant_conciliado=$_POST["txtcantconciliado"];
		$cant_noconciliado=$_POST["txtcantnoconciliado"];
		...
		$ldec_salse	  = str_replace(",",".",$ldec_salsegban);
...
if (array_key_exists("hidorden",$_POST))
...
else
   {
	...
	function uf_nuevo()
	{
		...
		global $ld_desde;
		global $ld_desde;
		global $cant_conciliado,$cant_noconciliado;
		$cant_conciliado=0;$cant_noconciliado=0;
		$li_temp=1;	
		$li_rows=$li_temp;
		$object[$li_temp][1]  = "<input name=chk".$li_temp." type=checkbox id=chk".$li_temp." value=1 class=sin-borde onClick=javascript:uf_selected();>";
	...
	if ($ls_operacion=="CARGAR_DT")
	   {
		 ...
		 $ld_hasta=$in_classfecha->uf_last_day(substr($ld_fecha,0,2),substr($ld_fecha,3,4));
		$cant_conciliado=0;$cant_noconciliado=0;
		 $in_classconciliacion->uf_cargar_movimientos_a_conciliar($ls_codban,$ls_cuenta_banco,&$object,$ld_desde,$ld_hasta,&$li_rows,$ls_filtro,$ls_orden,&$cant_conciliado,&$cant_noconciliado);
	...
	if ($ls_operacion=="GUARDAR")
	   {	
		// print "aqui";die();	
		 ...
		 ?>
		 <script>
		   location.href="tepuy_scb_p_conciliacion.php";
		 </script>
		 <?php
	   }//Fin GUARDAR
  ...
  <table width="780" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    ...
    <tr>
      ...
      <td align="left">
          ...
          <input name="txtdenban" type="text" id="txtdenban" value="<?php print $ls_denban?>" size="45" class="sin-borde" readonly>      </td>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    </tr>
    <tr>
		  ...
		  <label>
            ...
            Notas de D&eacute;bito </label>
              ...
              <input type="radio" name="filtro" id="filtro" value="CH" class="sin-borde" onClick="javascript: uf_cambiar_dt();"   style="cursor:default" <?php print $ls_filtroch;?>>
                Cheques </label>
              <input type="radio" name="filtro" id="filtro" value="TR" class="sin-borde" onClick="javascript: uf_cambiar_dt();"   style="cursor:default" <?php print $ls_filtroch;?>>
                Pago x Transferencias </label>
            </td>
        </tr>
      </table>
      </td>
    ...
    <tr>
      <td height="13" colspan="4"><div align="center">
        <table width="770" border="0" cellpadding="0" cellspacing="0" class="formato-blancotabla">
          ...
          <tr>
            <td height="22"><div align="right">Total Movimientos No Conciliados </div></td>
            <td>
              <div align="left">
		<input name="txtcantnoconciliado" type="text" id="txtcantnoconciliado" style="text-align:right" value="<?php print $cant_noconciliado;?>" readonly>
          ...
          <tr>
            ...
            <td><div align="left">
		<input name="txtcantconciliado" type="text" id="txtcantconciliado" style="text-align:right" value="<?php print $cant_conciliado;?>" readonly>
...
function ue_guardar()
{
	...
	if((ls_codban!="")&&(ls_cuenta!="")&&(ls_mesano!=""))
	{
		...
		f.action="tepuy_scb_p_conciliacion.php";
		f.submit();
	}
	else
	{
...
function uf_verificar_provbene(lb_checked,obj)
{
 ...
 function  uf_reg_error(tip_mov)
 {
	...
	if((ls_codban!="")&&(ls_cuenta!="")&&(ls_mesano!=""))
	{
		if(tip_mov=='L')
		{
			ls_pagina="tepuy_scb_p_concilerror.php?tip_mov=L&nombre=Errores en Libro&txtcodban="+ls_codban+"&txtdenban="+ls_nomban+"&txtcuenta="+ls_cuenta+"&txtdenominacion="+ls_dencta+"&txtcuenta_scg="+ls_cuenta_scg+"&txtdisponible="+ldec_disponible+"&mesano="+ls_mesano;
		...
		if(tip_mov=='B')
		{
			ls_pagina="tepuy_scb_p_concilerror.php?tip_mov=B&nombre=Errores en Banco&txtcodban="+ls_codban+"&txtdenban="+ls_nomban+"&txtcuenta="+ls_cuenta+"&txtdenominacion="+ls_dencta+"&txtcuenta_scg="+ls_cuenta_scg+"&txtdisponible="+ldec_disponible+"&mesano="+ls_mesano;
			window.open(ls_pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=800,height=600,left=0,top=0,location=no,resizable=yes");
		}
	}
	else
