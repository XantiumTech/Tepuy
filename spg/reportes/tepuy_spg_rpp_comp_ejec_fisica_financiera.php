...
// datos de la institucion // extraidos de la tabla empresa
		...
		//$tm=296-($li_tm/2);
		$io_pdf->addText(12,594,6,"<b> REPUBLICA BOLIVARIANA DE VENEZUELA</b>");
		$io_pdf->addText(49,587,6,"<b> ESTADO BARINAS</b>");
		$io_pdf->addText(16,520,5,"<b>".$ls_nomemp."</b>");
		$io_pdf->rectangle(10,470,985,130);
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],32,524,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
...
//		$io_pdf->addText(65,518,11,"<b>Estado Barinas</b>"); // Agregar la Fecha
		...
		$io_pdf->addText(940,580,9,date("d/m/Y")); // Agregar la Fecha
		$io_pdf->addText(940,570,9,date("h:i a")); // Agregar la hora

//DATOS DE LA INSTITUCION //
		//$li_tm=$io_pdf->getTextWidth(5,"R.I.F.: ".$ls_telemp);
		//$tm=550-($li_tm/2);
		//$io_pdf->addText($tm,23,5,"<b> R.I.F.: ".$ls_rifemp."</b>");
		$li_tm=$io_pdf->getTextWidth(5,$ls_diremp." Telef.: ".$ls_telemp);
		$tm=550-($li_tm/2);
		$io_pdf->addText($tm,23,5,$ls_diremp." ".$ls_telemp);
		$li_tm=$io_pdf->getTextWidth(5,$ls_ciuemp.", ESTADO ".$ls_estemp);
		$tm=550-($li_tm/2);
		$io_pdf->addText($tm,18,5,$ls_ciuemp.", ESTADO ".$ls_estemp);
		$io_pdf->restoreState();
		$io_pdf->closeObject();
