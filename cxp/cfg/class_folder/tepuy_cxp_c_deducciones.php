<?php 
class tepuy_cxp_c_deducciones
{
	var $ls_sql;
	var $is_msg_error;
	
	function tepuy_cxp_c_deducciones($conn)
	{
...
function uf_insert_deduccion($as_codemp,$ar_datos,$aa_seguridad) 
{
	...
	{	
		$ls_codseniat="'".$ls_codseniat."'";
	}
	//print "aqui: ".$ls_codseniat;
	...
	$ls_sql="INSERT INTO tepuy_deducciones (codemp,codded,dended,sc_cuenta,porded,monded,islr,iva,estretmun,formula,".
			"                                otras,codseniat,tipopers,retaposol,cubretotal)".
			" VALUES ('".$as_codemp."','".$ls_codigo."','".$ls_denominacion."','".$ls_contable."',".$ld_porcentaje.", ".
			"          ".$ld_deducible.",".$li_islr.",".$li_iva.",".$li_estretmun.",'".$ls_formula."',".$li_timbrefiscal.",".$ls_codseniat.",".
			"         '".$ls_tipoperdeduccion."','".$li_retaposol."','".$cubretotal."')";
	//print $ls_sql;
	...
	if ($li_numrows===false)
	   {
	     $lb_valido=false;
		 $this->io_msg->message("CLASE->tepuy_CXP_C_DEDUCCIONES; METODO->uf_insert_deduccion; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	   }
	else
	   {
		 /////////////////////////////////         SEGURIDAD               /////////////////////////////		
		...
		$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("tepuy_deducciones",$this->li_candeccon,$this->li_tipconmon,$this->li_redconmon,$aa_seguridad);*/
	   }
	   return $lb_valido;
}
...
//////////////////////////////////////////////////////////////////////////////
	...
	$ls_tipoperdeduccion=$ar_datos["tipoperdeduccion"];
	$cubretotal=$ar_datos["cubretotal"];
	//print "Tipo deduccion: ".$ls_tipodeduccion;
	if ($ls_tipodeduccion=="S")
	   {
		 $li_islr=1;   
	   }
	...
	$ls_sql=" UPDATE tepuy_deducciones ".
			...
			"        retaposol=".$li_retaposol.",".
			"        cubretotal='".$cubretotal."'".
	...
	if ($li_numrows===false)
	   {
		 $this->io_msg->message("CLASE->tepuy_CXP_C_DEDUCCIONES; METODO->uf_update_deduccion; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		 $lb_valido=false;
	   }
	else
	   {
...
function uf_delete_deduccion($as_codemp,$as_codigo,$as_dendeduc,$aa_seguridad)
{
	...
	//////////////////////////////////////////////////////////////////////////////
	  $lb_valido = false;
	  $ls_sql    = " DELETE FROM tepuy_deducciones WHERE codemp='".$as_codemp."' AND codded='".$as_codigo."'";	    
	  $this->io_sql->begin_transaction();
	  $rs_data   = $this->io_sql->execute($ls_sql);
	  if ($rs_data===false)
		 {
		   $this->io_msg->message("CLASE->tepuy_CXP_C_DEDUCCIONES; METODO->uf_delete_deduccion; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		   $lb_valido=false;
		 }
	  else
		 {
...
//               tepuy_deducciones.
//////////////////////////////////////////////////////////////////////////////
	$ls_sql=" SELECT * FROM tepuy_deducciones WHERE codemp='".$as_codemp."' AND codded='".$as_codigo."'";
