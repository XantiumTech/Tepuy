<?php
	...
	// para crear el libro excel
		...
		$lo_archivo = tempnam("/tmp", "libro_diario_legal.xls");
	...
	//print "Hitbot: ".$ls_hidbot." fecha: ".$fecdes. "Hasta: ".$fechas; die();
	//---------------------------------------------------------------------------------------------------------------------------
	//Parámetros del encabezado
		$ldt_fecha=" Desde  ".substr($fecdes,8,2)."/".substr($fecdes,5,2)."/".substr($fecdes,0,4)."  al ".$fechas;//"".$io_fecha->uf_load_nombre_mes(substr($ls_fecha,3,2))." - ".substr($ls_fecha,6,4)."";
		$ls_titulo=" LIBRO DIARIO LEGAL ";      
	//---------------------------------------------------------------------------------------------------------------------------
	
	//Busqueda de la data
	$lb_valido=uf_insert_seguridad("<b>Libro Diario Legal en Excel</b>"); // Seguridad de Reporte
	if($lb_valido)
	{
		$lb_valido=$io_report->uf_scg_reporte_libro_diario_legal($ls_cuentadesde,$ls_cuentahasta,$ld_fecdesde,$ld_fechasta,$li_nivel);
					//uf_scg_reporte_movimientos_mes($ls_cuentadesde,$ls_cuentahasta,$ls_fecha,$ls_fechasta);
	}
	//---------------------------------------------------------------------------------------------------------------------------
	...
	else // Imprimimos el reporte
	{
		...
		$lo_libro->close();
		header("Content-Type: application/x-msexcel; name=\"libro_diario_legal.xls\"");
		header("Content-Disposition: inline; filename=\"libro_diario_legal.xls\"");
