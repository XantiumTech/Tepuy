...
<form name="form1" method="post" action="">
  ...
  <div align="center">
    <table width="565" border="0" cellpadding="0" cellspacing="0" class="formato-blanco">
      ...
      <tr>
        <td height="22">&nbsp;</td>
        <td height="22" colspan="5" style="text-align:right"><a href="javascript: ue_search();"><img src="../shared/imagebank/tools20/buscar.png" alt="Buscar..." width="20" height="20" border="0">Buscar</a></td>
      </tr>
      <tr>
...
print "<td width=70 style=text-align:center>Proveedor</td>";
print "<td width=265 style=text-align:center>Nombre Proveedor</td>";
print "<td width=100 style=text-align:center>Banco</td>";
print "<td width=130 style=text-align:center>Cuenta</td>";
print "</tr>";
if ($ls_operacion=="BUSCAR")
   {
	 ...
	 $ls_sql    = "SELECT DISTINCT cxp_solicitudes.cod_pro, rpc_proveedor.nompro, scb_prog_pago.codban,
				          scb_prog_pago.ctaban, scb_banco.nomban, scb_ctabanco.dencta, scb_ctabanco.sc_cuenta
			     ...
			     ORDER BY cxp_solicitudes.cod_pro, rpc_proveedor.nompro, scb_prog_pago.codban,
				          scb_prog_pago.ctaban, scb_banco.nomban, scb_ctabanco.dencta, scb_ctabanco.sc_cuenta ASC";
	$rs_data = $io_sql->select($ls_sql);
	if ($rs_data===false)
	   {
	     $io_msg->message("Error al cargar programaciones");		
	   }
	else
	   {
		 ...
		 if ($li_numrows>0)
		    {
			  while($row=$io_sql->fetch_row($rs_data))
			       {
				     $ls_codpro = str_pad(trim($row["cod_pro"]),10,0,0);
					 $ls_nompro = $row["nompro"];
				     $ls_codban	= trim($row["codban"]);
					 $ls_nomban = $row["nomban"];
					 $ls_ctaban	= trim($row["ctaban"]);
					 $ls_nomcta = $row["dencta"];
					 $ls_scgcta = trim($row["sc_cuenta"]);
					 $lb_valido = $io_ctaban->uf_verificar_saldo($ls_codban,$ls_ctaban,&$adec_saldo);
					 print "<tr class=celdas-blancas>";
					 print "<td width=70 style=text-align:center><a href=\"javascript: uf_aceptar('$ls_codpro','$ls_nompro','$ls_codban','$ls_nomban','$ls_ctaban','$ls_nomcta','$ls_scgcta','$adec_saldo');\">".$ls_codpro."</a></td>";
					 print "<td width=265 style=text-align:left>".$ls_nompro."</td>";
					 print "<td width=100 style=text-align:center>".$ls_codban." - ".$ls_nomban."</td>";
					 print "<td width=130 style=text-align:center>".$ls_ctaban." - ".$ls_nomcta."</td>";
				     print "</tr>";			
				   }
...
<script language="JavaScript">
  ...
  function uf_aceptar_transf(ls_prov,ls_provbene,ls_codban,ls_nomban,ls_ctaban,ls_dencta,ls_sc_cuenta,ldec_saldo)
  {
   	f=opener.document.form1;
	f.txtcodban.value=ls_codban;
	f.txtdenban.value=ls_nomban;
  ...
  function ue_search()
  {
	  ...
	  f.action="tepuy_cat_prog_proveedores.php";
	  f.submit();
  }
