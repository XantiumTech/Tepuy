...
class tepuy_scb_class_report
{
	...
	var $ds_data;
	var $io_validacion;
	function tepuy_scb_class_report($conn)
	{
...
///////////////////////////////////////////////////////////////////////////////////////////////
	...
	if ((!empty($as_proben_d))&&(!empty($as_proben_h)))
	   {
		if ($as_tipproben=="B")
		{
		$ls_sqlaux = $ls_sqlaux." AND BRH.ced_bene BETWEEN '".$as_proben_d."' AND '".$as_proben_h."'";
		}
		else
		{
		$ls_sqlaux = $ls_sqlaux." AND BRH.cod_pro BETWEEN '".$as_proben_d."' AND '".$as_proben_h."'";
		}
	   }
	...
	if ($as_orden=='CH')//Documento
	   {
		// $ls_aux=" ORDER BY BRH.numdoc";
		 $ls_aux=" ORDER BY substr(BRH.numdoc,-4) "; // Ordenado solo para Barvialsa segun los ultimos 4 numeros del cheque
	   }
	...
	if ($as_operacion=='T')
	   {
	     $ls_operacion="((BRH.codope='CH' OR BRH.codope='TR') OR (BRH.codope='ND' AND BRH.procede='SNOCNO') OR (BRH.codope='ND' AND BRH.estbpd='T'))";
	   }
	elseif($as_operacion=='CH')
	   {	
	     $ls_operacion="BRH.codope='CH'";
	   }
	elseif($as_operacion=='TR')
	   {	
	     $ls_operacion="BRH.codope='TR'";
	   }
	...
	if ($as_tiprep=='G')
	   {
		//$filtro="' a.numrecdoc=e.numrecdoc AND XSC.numdoc='".$as_numdoc."' AND e.numsol=XSC.numsol ";
		//			  LEFT OUTER JOIN cxp_rd a
		//			    ON (a.numrecdoc=e.numrecdoc AND XSC.numsol=e.numsol AND (a.cod_pro=e.cod_pro OR a.ced_bene=e.ced_bene ) )
	     $ls_sql = "SELECT DISTINCT BRH.ctaban as ctaban,BRH.numdoc as numdoc,BRH.nomproben as nomproben,BRH.codope,
					  ...
					  LEFT OUTER JOIN cxp_dt_solicitudes e
					    ON (XSC.numsol=e.numsol)

					
					 WHERE BRH.codemp='".$ls_codemp."' AND $ls_operacion $ls_sqlaux $ls_aux";
	   }
	else
	   {
		 if ($as_tipproben=='P')
	...
	$filtro="' a.numrecdoc=e.numrecdoc AND XSC.numdoc='".$as_numdoc."' AND e.numsol=XSC.numsol ";
         $ls_sql = "SELECT DISTINCT BRH.ctaban as ctaban, BRH.numdoc as numdoc, BRH.nomproben as nomproben, BRH.codope, 
						   ...
						   (BRH.monto-BRH.monret) as monto, BRH.estmov, COALESCE(XSC.monto,0) as monsol, BRH.conmov as conmov
					  FROM scb_movbco BRH
					  LEFT OUTER JOIN cxp_sol_banco XSC, cxp_rd a, cxp_dt_solicitudes e
					    ON (BRH.numdoc=XSC.numdoc AND BRH.codban=XSC.codban AND BRH.ctaban=XSC.ctaban AND BRH.codope=XSC.codope AND BRH.estmov=XSC.estmov)
					  LEFT OUTER JOIN cxp_dt_solicitudes e
					    ON (XSC.numsol=e.numsol)

					 WHERE BRH.codemp='".$ls_codemp."'
					   AND $ls_operacion $ls_sqlaux $ls_aux";
	   }

	/*		$filtro="' AND a.numrecdoc=e.numrecdoc AND d.numdoc='".$as_numdoc."' AND e.numsol=d.numsol ";
		$ls_sql1="SELECT  a.sc_cuenta as scg_cuenta,a.debhab as debhab,a.monto as monto,b.denominacion as denominacion
				 FROM     cxp_rd a,cxp_sol_banco d, cxp_dt_solicitudes e
				 WHERE    a.codemp='".$ls_codemp."' AND c.numdoc ='".$as_numdoc.$filtro." AND c.codban='".$as_codban."' AND c.ctaban='".$as_ctaban."' AND a.sc_cuenta=b.sc_cuenta "."	
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    {///////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 // Fecha Creación :    26/09/2006          Fecha última Modificacion :      Hora :
	  ...
	  $ls_sql=" SELECT a.numdoc as numdoc,a.codban as codban,a.ctaban as ctaban,a.codope as codope,a.fecha as fecha,a.conmov as conmov,a.cod_pro as cod_pro,a.ced_bene as ced_bene,
				a.nomproben as nomproben,a.tipo_destino as tipo_destino,sum(b.monto) as monto,b.codestpro,b.spg_cuenta,
	  ...
	  if($rs_data===false)
	  {   // error interno sql
		   $this->io_msg->message("CLASE->tepuy_scb_class_report  
	 ...
	 function uf_cargar_cheques_custodia_entregados($ls_fechades,$ls_fechahas,$ls_codban,$ls_ctaban,$ls_probendesde,$ls_probenhasta,$ls_tipo_destino,$ls_tiprep)
	{
		...
		$ls_sql="SELECT codban,ctaban,codope,(monto - monret) as monto_total,fecmov,nomproben,numdoc,estmov,conmov,emicheproc,emichefec,emicheced,emichenom
				   FROM scb_movbco 
				  WHERE codemp='".$ls_codemp."'
				    AND codope='CH' ".$ls_aux;//print $ls_sql;
		$rs_documentos=$this->SQL->select($ls_sql);
...
function uf_cargar_cheques($as_codban,$as_ctaban,&$lb_valido)
{		
	$ls_codemp = $this->dat_emp["codemp"];
	$lb_valido = false;	
	
	$ls_sql = "select numchequera, numche                 ".
			  "  FROM  scb_cheques                        ".
			  " WHERE  codemp='".$ls_codemp."'            ".
