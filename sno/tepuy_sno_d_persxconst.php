<?php
   session_start();
	...
	$io_fun_nomina->uf_load_seguridad_nomina("SNO","tepuy_sno_d_persxconst.php",$ls_codnom,$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_agregarlineablanca(&$aa_object,$ai_totrows)
   {
		...
		$aa_object[$ai_totrows][3]="<input type=text name=txtmon".$ai_totrows." class=sin-borde size=20 style=text-align:right readonly>";
   }
   //--------------------------------------------------------------
?>
	...
	switch ($ls_operacion) 
	{
		...
		case "GUARDAR":
			...
			for($li_i=1;$li_i<=$li_totrows&&$lb_valido;$li_i++)
			{
				...
				$lb_valido=$io_constante->uf_update_constantes_x_personal($ls_codcons,$ls_codper,$li_moncon,$la_seguridad);
			}
			if($lb_valido)
			{
		...
		if(li_cambiar==1)
		{
			...
			for(li_i=1;((li_i<=li_total)&&(lb_valido));li_i++)
			{
				...
				if(tope!=0)
				{
					...
					{
					   lb_valido=false;
					   alert("El valor no es valido, es mayor que el tope "+eval("f.topcon"+li_i+".value"));
			...
			if(lb_valido)
			{
				...
				f.action="tepuy_sno_d_persxconst.php";
				f.submit();
			}
		}
		else
...
function ue_buscar()
{
	...
	if (li_leer==1)
   	{
		window.open("tepuy_sno_cat_persxconst.php","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=530,height=400,left=50,top=50,location=no,resizable=no");
   	}
	else
   	{
 		alert("No tiene permiso para realizar esta operacion");
...
{
	f=document.form1;
	tope=eval("f.topcon"+li_row+".value");
	while(tope.indexOf('.')>0)
	{
		tope=tope.replace(".","");
	}
	...
	if(tope!=0)
	{
		if(parseFloat(valor)>parseFloat(tope))
		{
		   alert("El valor no es valido, es mayor que el tope "+eval("f.topcon"+li_row+".value"));
		   eval("f.txtmon"+li_row+".value=0"); 
		}
	}
}
