<?php
   ...
   // para crear el libro excel
		...
		$lo_archivo = tempnam("/tmp", "listado_proveedor.xls");
		...
		if ($li_total>0)
		   {
			...
			$lo_hoja->write($li_row, 5, "Contable",$lo_titulo);
			 while($row=$io_sql->fetch_row($rs_proveedor))
			      {
					...
					$lo_hoja->write($li_row, 5, $ls_scgcta,$lo_dataleft);
				  }
		   }
	    $lo_libro->close();
		header("Content-Type: application/x-msexcel; name=\"listado__proveedor.xls\"");
		header("Content-Disposition: inline; filename=\"listado_proveedor.xls\"");
