<?php
class tepuy_rpc_c_estado
{
var $ls_sql;
var $is_msg_error;

	function tepuy_rpc_c_estado($conn)
	{
...
//	         Returns:  $lb_valido.
//	     Description:  Función que se encarga de insertar en la tabla tepuy_estado
...
//////////////////////////////////////////////////////////////////////////////  
	  ...
	  $ls_sql = " INSERT INTO tepuy_estados (codpai,codest,desest) VALUES ('".$as_codpais."','".$as_codest."','".$as_desest."')";
	  $this->io_sql->begin_transaction();
	  $rs_data=$this->io_sql->execute($ls_sql);
	  if ($rs_data===false)
		 {
		   $lb_valido=false; 
		   $this->io_msg->message("CLASE->tepuy_RPC_C_ESTADO; METODO->uf_insert_estado; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		 }
	  else
		 {
           $lb_valido=true;
...
//                     nombre de la ventana,nombre del usuario etc.
//	         Returns:  $lb_valido.
//	     Description:  Función que se encarga de actualizar en la tabla tepuy_estado la denominación para un estado.  
//     Elaborado Por:  Ing. Néstor Falcón.
// Fecha de Creación:  15/08/2006       Fecha Última Actualización:23/04/2006.	 
//////////////////////////////////////////////////////////////////////////////  

	  $ls_sql=" UPDATE tepuy_estados SET desest='".$as_desest."' WHERE codpai='".$as_codpais."' AND codest='".$as_codest."'";
	  $this->io_sql->begin_transaction();
	  $rs_data=$this->io_sql->execute($ls_sql);
	  if ($rs_data===false)
		 {
		   $lb_valido=false; 
		   $this->io_msg->message("CLASE->tepuy_RPC_C_ESTADO; METODO->uf_update_estado; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		 }
	  else
		 {
           $lb_valido=true;
...
//                     nombre de la ventana,nombre del usuario etc.
//	         Returns:  $lb_valido.
//	     Description:  Función que se encarga de eliminar en la tabla tepuy_estado un estado.  
//     Elaborado Por:  Ing. Néstor Falcón.
// Fecha de Creación:  15/08/2006       Fecha Última Actualización:24/04/2006.	 
//////////////////////////////////////////////////////////////////////////////  

  $lb_valido = false;
  $ls_sql    = "DELETE FROM tepuy_estados WHERE codpai='".$as_codpais."' AND codest='".$as_codest."'";
  $this->io_sql->begin_transaction();
  $rs_data=$this->io_sql->execute($ls_sql);
  if ($rs_data===false)
	 {
	   $lb_valido=false; 
	   $this->io_msg->message("CLASE->tepuy_RPC_C_ESTADO; METODO->uf_delete_estado; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	 }
  else
	 {
	   $lb_valido=true;
...
//////////////////////////////////////////////////////////////////////////////  
  ...
  $lb_valido = false;
  $ls_sql    = " SELECT * FROM tepuy_estados WHERE codpai='".$as_codpais."' AND codest='".$as_codest."'";
  $rs_data   = $this->io_sql->select($ls_sql);
  if ($rs_data===false)
	 {
 	   $lb_valido=false;
	   $this->io_msg->message("CLASE->tepuy_RPC_C_ESTADO; METODO->uf_load_estado; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	 }
  else
	 {
...
//////////////////////////////////////////////////////////////////////////////
	...
	if ($rs_data===false)
	   {
	     $lb_valido=false;
	     $this->io_msg->message("CLASE->tepuy_RPC_C_ESTADO; METODO->uf_llenarcombo_pais; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	   }
	else
	   {
	     $li_numrows=$this->io_sql->num_rows($rs_data);
