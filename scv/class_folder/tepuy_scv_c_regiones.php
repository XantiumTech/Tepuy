<?php
class tepuy_scv_c_regiones
{
	var $ls_sql;
	var $ds_dtregion;
	
	function tepuy_scv_c_regiones($conn)
	{
	...
	} // fin de la function tepuy_scv_c_regiones
	...
	function uf_insert_region($as_codemp,$as_codreg,$as_codpai,$as_denreg,$ar_grid,$ai_total,$aa_seguridad) 
	{
		...
		if ($rs_data===false)
		{
			...
			$this->io_msg->message("CLASE->tepuy_SCV_C_REGIONES; METODO->uf_insert_region; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
		...
		for ($li_i=1;$li_i<=$ai_total;$li_i++)
		{
			if ($lb_valido)
			{
				...
				if (!empty($ls_codest))			            
				{
					...
					{				 
						$this->io_sql->rollback();
						$this->io_msg->message("CLASE->tepuy_SCV_C_REGIONES; METODO->uf_insert_dt_region; ERROR->".$this->io_funcion->                     uf_convertirmsg($this->io_sql->message));
					}
					else
					{				 
						$lb_valido=true;  		                    
	...
	function uf_update_region($as_codemp,$as_codreg,$as_codpai,$as_denreg,$ar_grid,$ai_total,$aa_seguridad) 
	{
		...
		{
			$this->io_sql->rollback();
			$this->io_msg->message("CLASE->tepuy_SCV_C_REGIONES; METODO->uf_update_region; ERROR->".$this->io_funcion->uf_convertirmsg(       $this->io_sql->message));
		}
		else
		{
	...
	{          		 
		...
		if (!$lb_relacion)
		{
			if ($this->uf_delete_estados_region($as_codemp,$as_codpai,$as_codreg,$aa_seguridad))  
			{
				...
				{
					$this->io_sql->rollback();
					$this->io_msg->message("CLASE->tepuy_SCV_C_REGIONES; METODO->uf_delete_region; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
				}
				else
				{
					$lb_valido=true;
	...
	{          		 
		...
		{
			$this->io_sql->rollback();
			$this->io_msg->message("CLASE->tepuy_SCV_C_REGIONES; METODO->uf_delete_estados_region; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$lb_valido=true;
	...
	function uf_load_region($as_codemp,$as_codpai,$as_codreg) 
	{
		...
		if ($rs_data===false)
		{
			$this->io_msg->message("CLASE->tepuy_SCV_C_REGIONES; METODO->uf_load_region; ERROR->".$this->io_funcion->uf_convertirmsg(       $this->io_sql->message));
		}
		else
		{
	...
	function uf_load_dt_region($as_codemp,$as_codreg,$as_codpai)
	{
		...
		//////////////////////////////////////////////////////////////////////////////
		$lb_valido= false;
		$ls_sql=" SELECT scv_dt_regiones.codest,tepuy_estados.desest".
				"   FROM scv_dt_regiones,tepuy_estados ".
				"  WHERE scv_dt_regiones.codemp='".$as_codemp."'".
				"    AND scv_dt_regiones.codreg='".$as_codreg."'".
				"    AND scv_dt_regiones.codpai='".$as_codpai."'".
				"    AND scv_dt_regiones.codpai=tepuy_estados.codpai".
				"    AND scv_dt_regiones.codest=tepuy_estados.codest";
		$rs_data = $this->io_sql->select($ls_sql);
		if ($rs_data===false)
		{
			$this->io_msg->message("CLASE->tepuy_SCV_C_REGIONES; METODO->uf_load_dt_region; ERROR->".$this->io_funcion->uf_convertirmsg(       $this->io_sql->message));
		}
		else
		{
			$li_numrows=$this->io_sql->num_rows($rs_data);
	...
	function uf_load_paises()
	{
		...
		//////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		$ls_sql= "SELECT codpai,despai FROM tepuy_pais".
				 " ORDER BY codpai ASC";
		$rs_data = $this->io_sql->select($ls_sql);
		if ($rs_data===false)
		{
			$this->io_msg->message("CLASE->tepuy_SCV_C_REGIONES; METODO->uf_load_paises; ERROR->".$this->io_funcion->uf_convertirmsg(       $this->io_sql->message));
		}
		else
		{
			$li_numrows = $this->io_sql->num_rows($rs_data);	    
	...
	function uf_check_relaciones($as_codemp,$as_codrut)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->tepuy_scv_c_rutas METODO->uf_check_relaciones; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
...
}   // fin de la class tepuy_scv_c_regiones
?> 