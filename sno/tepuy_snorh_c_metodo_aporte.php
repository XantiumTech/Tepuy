<?PHP
class tepuy_snorh_c_metodo_aporte
{
	var $io_mensajes;
	var $io_sno;
	var $io_fecha;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function tepuy_snorh_c_metodo_aporte()
	{	
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: tepuy_snorh_c_metodo_aporte
		//		   Access: public 
	...
	function uf_listado_gendisk($aa_codconc,$as_codnomdes,$as_codnomhas,$as_ano,$as_perdes,$as_perhas)
	{
		...
		//		   Access: public (desde la clase tepuy_sno_r_aportepatronal)  
		...
		$ls_sql="SELECT sno_hpersonalnomina.codper,sno_personal.cedper, sno_personal.apeper, sno_personal.nomper, sno_hpersonalnomina.sueper,  sno_hpersonalnomina.sueintper, sno_hpersonalnomina.sueproper,".
			...
			//	"		sno_personal.cuecajahoper, sno_personal.edocivper, ".
				"		sno_personal.cuecajahoper, sno_personal.edocivper, sno_personal.fecingper as fecha_ingreso_real,".
				"       (SELECT tipnom ".
				"		   FROM sno_hnomina ".
				...
				"		   sno_personal.cuecajahoper, sno_hpersonalnomina.fecegrper,sno_personal.edocivper ".
				"   ".$ls_orden;
	//print $ls_sql;
...
//print $ls_sql;
	...
	{ 
		...
		if ($li_count>0)
		{
			...
			for($li_i=1;(($li_i<=$li_count)&&($lb_valido));$li_i++)
			{
				...
				$li_sueper=number_format($li_sueper,2,",","0");
				$li_personal=abs($aa_ds_banco->data["personal"][$li_i]);				
				$li_personal=number_format($li_personal,2,",","0");
				$li_patron=abs($aa_ds_banco->data["patron"][$li_i]);				
				$li_patron=number_format($li_patron,2,",","0");
	...
	{	
		...
		// Modificado Por: Ing. Miguel Palencia						Fecha Última Modificación : 30/08/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
//print $as_metodo;
	  	switch ($as_metodo)
		{
			case "FAOV":
				$lb_valido=$this->uf_metodo_lph_FAOV($as_ruta,$ad_fecproc,$aa_ds_lph);
				break;
	...
	function uf_metodo_lph_venezuela($as_ruta,$ad_fecproc,$aa_ds_lph)
	{
		...
		{	
			...
			for($li_i=1;$li_i<=$li_count;$li_i++)
			{
				...
				$ls_sueintper=$aa_ds_lph->data["sueintper"][$li_i]; //sueldo integral
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_metodo_lph_FAOV($as_ruta,$ad_fecproc,$aa_ds_lph)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//	   Creado Por: Ing. Miguel Palencia
		// Fecha Creación: 01/01/2006 								
		// Modificado Por: Ing. Miguel Palencia						Fecha Última Modificación : 31/08/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//			$ls_nombrearchivo=$as_ruta."/lph-".substr($ls_periodo,0,2).substr($ls_periodo,4,2).".txt";
		//CODIGO DEL ARCHIVO PARA NOMINA DE EMPLEADOS
			$ls_nombrearchivo=$as_ruta."/N03212000002010265311".substr($ls_periodo,0,2).substr($ls_periodo,2,4).".txt";
			//Chequea si existe el archivo.
			if (file_exists("$ls_nombrearchivo"))
			{
				if(@unlink("$ls_nombrearchivo")===false)//Borrar el archivo de texto existente para crearlo nuevo.
			...
			$ls_codagelph="";
			$ls_apaposlph="";
			$lb_valido=$this->io_metbanco->uf_load_metodobanco_lph("FAOV","1",$ls_debcuelph,$ls_numplalph,$ls_numconlph,$ls_suclph,
			...
			for($li_i=1;$li_i<=$li_count;$li_i++)
			{
				$ld_fecingper=$aa_ds_lph->data["fecingper"][$li_i]; 
				$ls_sueintper=$aa_ds_lph->data["sueintper"][$li_i]; //sueldo integral
				$ls_sueproper=$aa_ds_lph->data["sueproper"][$li_i]; //sueldo promedio mensual
				$ls_sueper=$aa_ds_lph->data["sueper"][$li_i]; //sueldo mensual
				$ls_fecingper=$aa_ds_lph->data["fecha_ingreso_real"][$li_i]; //fECHA DE INGRESO REAL
			//ELIMINA EL . Y POSTERIOR LA COMA PARA EL FORMATO SOLICITADO POR FAOV
				/*$ls_sueintper=str_replace('.','',$ls_sueintper);
				$ls_sueintper=str_replace(',','',$ls_sueintper);*/
				$ls_sueproper=str_replace('.','',$ls_sueproper);
				$ls_sueproper=str_replace(',','',$ls_sueproper);
				$largo_sueldo=strlen($ls_sueproper)-2;
				$ls_sueproper=substr($ls_sueproper,0,$largo_sueldo);

				$ld_fecingper=substr($ld_fecingper,8,2).substr($ld_fecingper,5,2).substr($ld_fecingper,0,4); //DDMMAAAA
				$ls_fecingper=substr($ls_fecingper,8,2).substr($ls_fecingper,5,2).substr($ls_fecingper,0,4); //DDMMAAAA
				$ls_nacper=$this->io_funciones->uf_trim($aa_ds_lph->data["nacper"][$li_i]); //nacionalidad				
				$ls_cedper=$this->io_funciones->uf_trim($aa_ds_lph->data["cedper"][$li_i]); //cedula
				...
				//PERMITE SEPARAR O CORTAR LOS APELLIDOS
				$largo=strlen($ls_apeper);
				$hasta=strpos($ls_apeper, " ");
				$ape1=substr($ls_apeper,0,$hasta);
				$ape2=rtrim(substr($ls_apeper,$hasta,$largo));
				$ape2=ltrim($ape2);
				$ls_nomper=$this->io_funciones->uf_rellenar_der(substr($aa_ds_lph->data["nomper"][$li_i],0,20), " ",20); //nombres
				//PERMITE SEPARAR O CORTAR LOS NOMBRES
				$largo=strlen($ls_nomper);
				$hasta=strpos($ls_nomper, " ");
				$nom1=substr($ls_nomper,0,$hasta);
				$nom2=rtrim(substr($ls_nomper,$hasta,$largo));
				$nom2=ltrim($nom2);
				...
				//$ls_cadena = $ls_nacper.",".$ls_cedper.",".$ls_nomper.",".$nom1.";".$nom2.";".$ls_apeper.",".$ape1.";".$ape2.";".$ls_sueintper.",".$ls_fecingper."\r\n";
				$ls_cadena = $ls_nacper.",".$ls_cedper.",".$nom1.",".$nom2.",".$ape1.",".$ape2.",".$ls_sueproper.",".$ls_fecingper.",\r\n";
				if ($ls_creararchivo)  //Chequea que el archivo este abierto				
				{
					if (@fwrite($ls_creararchivo,$ls_cadena)===false)//Escritura
					{
		...
		return $lb_valido;
	}// end function uf_metodo_lph_FAOV
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_metodo_lph_canarias($as_ruta,$ad_fecproc,$aa_ds_lph)
	{
		...
		{	
			...
			$lb_valido=$this->io_metbanco->uf_load_metodobanco_lph("CANARIAS","1",$ls_debcuelph,$ls_numplalph,$ls_numconlph,$ls_suclph,
																   $ls_cuelph,$ls_grulph,$ls_subgrulph,$ls_conlph,$ls_numactlph,$ls_codagelph,$ls_apaposlph);
			$ls_nombrearchivo=$as_ruta."/lph_canarias.txt";
			//Chequea si existe el archivo.
			if (file_exists("$ls_nombrearchivo"))
			{
				if(@unlink("$ls_nombrearchivo")===false)//Borrar el archivo de texto existente para crearlo nuevo.
			...
			$ls_rifdig="";
			$this->uf_disgrega_rif($ls_riflet,$ls_rifnum,$ls_rifdig);	
			$ls_rif=$this->ls_rifemp; ///print $ls_rif;
			$ls_rif=str_replace("-","",$ls_rif);			
	...
	function uf_metodo_lph_central($as_ruta,$ad_fecproc,$aa_ds_lph)
	{
		...
		//	   Creado Por: Ing. Miguel Palencia
		// Fecha Creación: 01/01/2006 								
		// Modificado Por: Ing. Miguel Palencia						Fecha Última Modificación : 01/09/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ld_desde=substr($ad_fecproc,0,10);
		$li_count=$aa_ds_lph->getRowCount("cedper");
	...
	function uf_metodo_lph_miranda($as_ruta,$ad_fecproc,$aa_ds_lph)
	{
		...
		//	   Creado Por: Ing. Miguel Palencia
		// Fecha Creación: 01/01/2006 								
		// Modificado Por: Ing. Miguel Palencia						Fecha Última Modificación : 04/09/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ld_desde=$ad_fecproc;
		$li_count=$aa_ds_lph->getRowCount("cedper");
	...
	function uf_metodo_lph_fondocomun_eap($as_ruta,$ad_fecproc,$aa_ds_lph)
	{
		...
		{	
			...
			{   				
				...
				$ldec_monto=$this->io_funciones->uf_cerosizquierda($ldec_monper+$ldec_monper,8);
				$ld_fecret=substr($ld_desde,3,2).substr($ld_desde,8,2);
	...
	function uf_metodo_fpa_mercantil($as_ruta,$aa_ds_fpa)
	{
		...
		//	   Creado Por: Ing. Miguel Palencia
		// Fecha Creación: 01/01/2006 								
		// Modificado Por: Ing. Miguel Palencia						Fecha Última Modificación : 20/03/2007
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$li_count=$aa_ds_fpa->getRowCount("cedper");
		if($li_count>0)
...
//------------------------------------------------------------------------------------------------------------------------------------
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_listado_prestacionantiguedad($as_codnom,$as_anocurper,$as_mescurper)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_listado_prestacionantiguedad
		//         Access: public (desde la clase tepuy_snorh_r_prestacionantiguedad)  
		...
		$ls_sql="SELECT sno_personal.cedper,sno_personal.nomper,sno_personal.apeper, sno_fideiperiodo.sueintper, ".
				...
				"  FROM sno_personal, sno_fideiperiodo, sno_fideicomiso ".
	...
	function uf_metodo_fps_mercantil($as_ruta,$ad_fecha,$aa_ds_fps)
	{
		...
		{	
			...
			for($li_i=1;$li_i<=$li_count;$li_i++)
			{
				...
				$ld_apoper=(abs($aa_ds_fps->data["apoper"][$li_i])*100);   //Monto aporte 				
				$ld_apoper=$this->io_funciones->uf_rellenar_izq($ld_apoper,"0",13);
				$ls_cadena="01".$ld_fecha."1".$ls_cuefidper.$ls_nacper.$ls_cedper."0".$ld_apoper."\r\n";
				if ($ls_creararchivo)  //Chequea que el archivo este abierto				
				{
					if (@fwrite($ls_creararchivo,$ls_cadena)===false)//Escritura
					{
	...
	function uf_metodo_fps_banesco($as_ruta,$ad_fecha,$aa_ds_fps)
	{
		...
		$ls_nroplafps=$this->io_funciones->uf_rellenar_izq(substr($ls_nroplafps,0,8)," ",8);
		$li_count=$aa_ds_fps->getRowCount("cedper");
		if($li_count>0)
		{	
			...
			for($li_i=1;$li_i<=$li_count;$li_i++)
			{
				$ld_fecingper=$aa_ds_fps->data["fecingper"][$li_i]; 
				$ld_mesingper=substr($ld_fecingper,5,2);
				$ld_anoingper=substr($ld_fecingper,0,4);
				...
				if(($ld_mesingper==$ld_mesfecdes)&&($ld_anoingper==$ld_anofecdes))
				{
					...
					$ls_nombre=$this->io_funciones->uf_rellenar_der(substr($ls_apeper.", ".$ls_nomper,0,40), " ", 40);
					$ls_codigo="          "; // Código 1
					$ls_unidad="                                        "; // Unidad Administrativa
			...
			for($li_i=1;$li_i<=$li_count;$li_i++)
			{
				...
				$ls_cadena=$ls_nroplafps.$ls_nacper.$ls_cedper.$ls_tipafi.$ls_codtra.$ld_apoper."\r\n";
				if ($ls_creararchivo)  //Chequea que el archivo este abierto				
				{
					if (@fwrite($ls_creararchivo,$ls_cadena)===false)//Escritura
					{
	...
	function uf_metodo_fps_central($as_ruta,$ad_fecha,$aa_ds_fps)
	{
		...
		{	
			...
			for($li_i=1;$li_i<=$li_count;$li_i++)
			{
				$ls_cedper=$this->io_funciones->uf_trim($aa_ds_fps->data["cedper"][$li_i]); //print $ls_cedper."cedeul <br>";
				$ls_cedper=str_replace("-","",$ls_cedper);
				$ls_cedper=str_replace(".","",$ls_cedper);
	...
	function uf_metodo_fps_delsur($as_ruta,$ad_fecha,$aa_ds_fps)
	{
		...
		//	   Creado Por: Ing. Miguel Palencia de Lang
