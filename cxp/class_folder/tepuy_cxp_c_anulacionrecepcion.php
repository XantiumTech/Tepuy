<?php
class tepuy_cxp_c_anulacionrecepcion
 {
	var $io_sql;
	var $io_mensajes;
	var $io_funciones;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function tepuy_cxp_c_anulacionrecepcion($as_path)
	{	
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: tepuy_cxp_c_aprobacionrecepcion
		//		   Access: public 
	...
	function uf_select_recepcion($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Anulacion MÉTODO->uf_validar_recepciones ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
	...
	function uf_insert_recepcion_anulada($as_numrecdoc,&$as_numrecdocnew,$as_codpro,$as_cedben,$as_codtipdoc,$aa_seguridad)
	{
		...
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ls_fecemidoc= $row["fecemidoc"];    
				$ls_fecregdoc= $row["fecregdoc"];    
				$ls_fecvendoc= $row["fecvendoc"];    
				...
				if($li_row===false)
				{
					...
					{
						$li_i=substr($as_numrecdocnew,1,1);
						settype($li_i,'int');
						$li_i=$li_i+1;
						$as_numrecdocnew=substr($as_numrecdocnew,2,13);
						$as_numrecdocnew="@".$li_i.$as_numrecdocnew;
						$lb_valido=$this->uf_insert_recepcion_anulada($as_numrecdoc,$as_numrecdocnew,$as_codpro,$as_cedben,$as_codtipdoc,$aa_seguridad);
					}
					else
					{
						$this->io_mensajes->message("CLASE->Anulacion MÉTODO->uf_insert_recepcion_anulada ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
				...
				/*	$this->io_rcbsf->io_ds_datos->insertRow("campo","montotdocaux");
					...
					$this->io_rcbsf->io_ds_filtro->insertRow("valor",$as_numrecdocnew);
					$this->io_rcbsf->io_ds_filtro->insertRow("tipo","C");
	...
	function uf_select_dt_cargos($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc)
	{
		...
		$ls_sql="SELECT codemp, numrecdoc, codtipdoc, cod_pro, ced_bene, codcar, procede_doc,".
				...
				"		codestpro5, spg_cuenta, porcar, formula".
				"  FROM cxp_rd_cargos".
				" WHERE codemp = '".$this->ls_codemp."'".
				"   AND numrecdoc = '".$as_numrecdoc."'".
				"   AND cod_pro = '".$as_codpro."'".
			...
			while(($row=$this->io_sql->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$ls_sql=" INSERT INTO cxp_rd_cargos (codemp, numrecdoc, codtipdoc, cod_pro, ced_bene, codcar, procede_doc,".
						...
						"					 		 codestpro5, spg_cuenta, porcar, formula) ".
					  	...
					  	" 		  '".$ls_spgcuenta."',".$li_porcar.",'".$ls_formula."')";	
				$li_row=$this->io_sql->execute($ls_sql);
				if($li_row===false)
				{
					$lb_valido=false;
		...
		else
		{
			while(($row=$this->io_sql->fetch_row($rs_data))&&($lb_valido))
			{
				...
				$ls_sql=" INSERT INTO cxp_rd_spg (codemp, numrecdoc, codtipdoc, ced_bene, cod_pro, procede_doc, numdoccom,".
						"					 	  codestpro, spg_cuenta, monto) ".
					  	" VALUES ('".$this->ls_codemp."','".$as_numrecdocnew."','".$ls_codtipdoc."','".$ls_cedbene."','".$ls_codpro."', ".
					  	" 		  '".$ls_procededoc."','".$ls_numdoccom."','".$ls_codestpro."','".$ls_spgcuenta."',".$li_monto.")";	
				$li_row=$this->io_sql->execute($ls_sql);
				if($li_row===false)
				{
					$lb_valido=false;
	...
	function uf_select_dt_scg($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc)
	{
		...
		// Fecha Creación: 07/05/2007								Fecha Última Modificación : 
		//////////////////////////////////////////////////////////////////////////////
		$ls_sql="SELECT codemp, numrecdoc, codtipdoc, ced_bene, cod_pro, procede_doc, numdoccom, estasicon,".
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_select_notas($as_numrecdoc,$as_codpro,$as_cedben,$as_codtipdoc)
	{
		//////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_select_notas
		//		   Access: public
		//		 Argument: as_numrecdoc  // Numero de la recepcion de documentos
		//                 as_codpro     //  Codigo de Proveedor
		//                 as_cedben     //  Codigo de Beneficiario
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Anulacion MÉTODO->uf_select_notas ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		else
		{
	...
	{		
		...
		if($lb_encontrado)
		{
			...
			if($lb_valido)
			{ 
				$lb_valido=$this->uf_select_notas($as_numrecdoc,$as_codpro,$as_cedbene,$as_codtipdoc);
				if(!$lb_valido)
				{
					$this->io_mensajes->message("La Recepción de Documentos no se puede eliminar, Tiene Notas de Credito/Debito Asociadas.");           
				}
			}
			if($lb_valido)
			{	
