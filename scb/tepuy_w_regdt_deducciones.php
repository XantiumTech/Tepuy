...
if ($ls_operacion=="")
   {
   	 $ls_sql = "SELECT codded,dended,formula,islr,iva,estretmun,monded,sc_cuenta 
     ...
     else
	    {
		  ...
		  if ($li_numrows>0)
			   ...
			   while($row=$io_sql->fetch_row($rs_data))
			        {
					  ...
					  $li_status_islr	= $row["islr"];
					  $li_status_iva	= $row["iva"];
					  $li_status_retmun = $row["estretmun"];
					  $ldec_monded		= $row["monded"];
					  ...
					  $object[$z][3]="<input name=txtcodded".$z." type=hidden id=txtcodded".$z." value='".$ls_codded."'><input type=text name=txtdended".$z." value='".$ls_dended."' title='".$ls_dended."' id=txtdended".$z." class=sin-borde readonly style=text-align:left   size=40 maxlength=254>";
          ...
          else
		     {
			   ...
			   $object[$z][3]="<input name=txtcodded".$z." type=hidden id=txtcodded".$z." value=''><input type=text name=txtdended".$z." value='' id=txtdended".$z." class=sin-borde readonly style=text-align:left   size=40 maxlength=254>";
...
elseif($ls_operacion=="EVALUAR")
{
	...
	for($z=1;$z<=$totrow;$z++)
	{
		...
		if(array_key_exists("chk".$z,$_POST))
		{
				...
				$lb_sel="checked";  
				$ldec_montoretaux=round($ldec_montoret,2); 
				$ldec_totret=$ldec_totret+$ldec_antret+$ldec_montoretaux;  
		}
		else
		{
			$ldec_montoret=0;
			$lb_sel="";
					...
					$object[$z][2]="<input type=text name=txtdoc".$z." value='".$ls_documento."'     id=txtdoc".$z." class=sin-borde readonly style=text-align:right  size=17 maxlength=15>";
					$object[$z][3]="<input name=txtcodded".$z." type=hidden id=txtcodded".$z." value='".$ls_codded."'><input type=text name=txtdended".$z." value='".$ls_dended."' title='".$ls_dended."' id=txtdencar".$z." class=sin-borde readonly style=text-align:left   size=40 maxlength=254>";
					$object[$z][4]="<input type=text name=txtmonobjret".$z." value='".number_format($ldec_objret,2,',','.')."' id=txtmonobjret".$z." class=sin-borde style=text-align:right  size=10 maxlength=20 onBlur=javascript:uf_calcular_monret($z);>";
...
<form name="form1" method="post" action="">
  <table width="550" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-celda">
      <td height="22" colspan="2" align="center">Cat&aacute;logo Deducciones            </tr>
    <tr>
    ...
    <tr>
      ...
      <td height="22" align="center">    <div align="left">
        <input name="txttotal" type="text" id="txttotal" value="<?php print number_format($ldec_totret,2,",",".")?>" style="text-align:right" >
    </div>    </tr>
    <tr>
      <td height="22" align="center"><p align="right">&nbsp;</p>            
    <td height="22" align="center"><div align="left">          <a href="javascript: uf_aceptar();"><img src="../shared/imagebank/tools20/aprobado.png" alt="Aceptar" width="20" height="20" border="0"></a> <a href="javascript: uf_aceptar();">Procesar deducciones </a>      </div>    </tr>
  </table>
  <div align="center">
    <p>
      <?php
$grid->makegrid($totrow,$title,$object,550,'Catalogo de Deducciones',$grid1);
?>
  ...
  function uf_aceptar()
  {
	...
	if(li_total>0)
	{
		...
		if(parseFloat(ldec_total)<=parseFloat(ldec_monobjret))
		{
			...
			f.action="tepuy_w_regdt_deducciones.php";
			f.submit();
		}
		else
		{
