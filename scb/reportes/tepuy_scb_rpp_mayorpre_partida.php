...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina($as_fecdes,$as_fechas,$as_codestpro1,$as_codestpro2,$as_denestpro1,$as_denestpro2,$ad_montotpro,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_print_encabezado_pagina
		//		   Access: private 
		//	    Arguments: $as_fecdes     : Fecha a partir del cual se hará la búsqueda de los datos.
		//                 $as_fechas     : Fecha hasta del cual se hará la búsqueda de los datos.
		//                 $as_codestpro1 : Código de la Estructura Presupuestaria de Nivel 1.
		//                 $as_codestpro2 : Código de la Estructura Presupuestaria de Nivel 2.
		//                 $as_denestpro1 : Denominación de la Estructura Presupuestaria de Nivel 1.
		//                 $as_denestpro2 : Denominación de la Estructura Presupuestaria de Nivel 2.
		//                 $ad_montotpro  : Monto Total del Proyecto y su Acción Centralizada.
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],40,705.7,55,40); // Agregar Logo
		...
		$la_data[2] = array('row1'=>'<b>GESTIÓN ADMINISTRATIVA</b>');
		$la_data[3] = array('row1'=>'');
		...
		$la_data[1] = array('col1'=>'<b>'.$as_codestpro1.' - '.$as_denestpro1.'</b>');
		$la_data[2] = array('col1'=>'<b>'.$as_codestpro2.' - '.$as_denestpro2.'</b>');

		$la_columna = array('col1'=>'');
		$la_config  = array('showHeadings'=>0,
		                    'fontSize'=>9,// Tamaño de Letras
						    'showLines'=>1,// Mostrar Líneas
						    'colGap'=>1,// Mostrar Líneas						 
						    ...
						    'xPos'=>305,// Orientación de la tabla
						    'shaded'=>0,// Sombra entre líneas
						    'cols'=>array('col1'=>array('justification'=>'left','width'=>580)));
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		unset($la_data,$la_columna,$la_config);
    ...
    function uf_print_detalles($aa_data,&$io_pdf)
    {
		...
		$la_data[1] = array('spgcta'=>'<b>Imputación Presupuestaria</b>',
		                    'denspgcta'=>'<b>Descripción</b>',
							'monspgcta'=>'<b>Monto</b>');

		$la_columna = array('spgcta'=>'','denspgcta'=>'','monspgcta'=>'');
		...
		$la_config=array('showHeadings'=>0,// Mostrar encabezados
						 ...
						 'cols'=>array('spgcta'=>array('justification'=>'center','width'=>130),
						               'denspgcta'=>array('justification'=>'center','width'=>350), // Justificación y ancho de la columna						 			   
						 			   'monspgcta'=>array('justification'=>'center','width'=>100))); // Justificación y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
		unset($la_data,$la_columna,$la_config);
		...
		$la_config=array('showHeadings'=>0,// Mostrar encabezados
						 ...
						 'shadeCol2'=>array(0.9,0.9,0.9), // Color de la sombra
						 'cols'=>array('spgcta'=>array('justification'=>'center','width'=>130),
						               'denspgcta'=>array('justification'=>'left','width'=>350), // Justificación y ancho de la columna						 			   
						 			   'monspgcta'=>array('justification'=>'right','width'=>100))); // Justificación y ancho de la columna
    ...
    function uf_print_total($ad_montotpro,&$io_pdf)
    {
		...
		$la_data[1] = array('monto1'=>'<b>TOTAL...</b>','monto2'=>'<b>'.$ld_montotpro.'</b>');
		...
		$la_config  = array('showHeadings'=>0,// Mostrar encabezados
						    ...
						    'shadeCol'=>array(0.9,0.9,0.9), // Color de la sombra
						    'shadeCol2'=>array(0.9,0.9,0.9), // Color de la sombra
						    'cols'=>array('monto1'=>array('justification'=>'center','width'=>480),
		  ...
		  while($row=$io_sql->fetch_row($rs_data))
			   {
				 ...
				 $ls_denspgcta  = $row["denominacion"];
 				 $ld_mondetspg  = $row["monto"];
				 $io_dsdata->insertRow("codestpro1",$ls_codestpro1);
				 $io_dsdata->insertRow("codestpro2",$ls_codestpro2);
				 $io_dsdata->insertRow("spg_cuenta",$ls_spgcta);
				 $io_dsdata->insertRow("denspgcta",$ls_denspgcta);
				 $io_dsdata->insertRow("mondetspg",$ld_mondetspg);
			   }		
		  $io_dsdata->group_by(array('0'=>'codestpro1','1'=>'codestpro2','2'=>'spg_cuenta'),array('0'=>'mondetspg'),'mondetspg');
          $io_dsdata->sortData('codestpro2');
		  $li_totrow = $io_dsdata->getRowCount('spg_cuenta'); 
		  $ls_codestpro1ant = "";$ls_codestpro2ant = "";$li_fila = 0;$ld_montotpro = 0;
		  for ($li_i=1;$li_i<=$li_totrow;$li_i++)
		      {
			    $ls_codestpro1 = $io_dsdata->getValue("codestpro1",$li_i);
				$ls_codestpro2 = $io_dsdata->getValue("codestpro2",$li_i);
				$ls_spgcta     = $io_dsdata->getValue("spg_cuenta",$li_i);
				$ls_denspgcta  = $io_dsdata->getValue("denspgcta",$li_i);
				$ld_mondetspg  = $io_dsdata->getValue("mondetspg",$li_i);
				if ($li_i==1)
				   {
				     $li_fila++;
					 $la_datrep[$li_fila] = array('spgcta'=>$ls_spgcta,
					 						      'denspgcta'=>$ls_denspgcta,
											      'monspgcta'=>number_format($ld_mondetspg,2,',','.'));
				   ...
				   {
				     if ($ls_codestpro1!=$ls_codestpro1ant || $ls_codestpro2!=$ls_codestpro2ant)
					    {
						  uf_print_encabezado_pagina($ls_fecdes,$ls_fechas,$ls_codestpro1,$ls_codestpro2,$ls_denestpro1,$ls_denestpro2,$ld_montotpro,$io_pdf);
