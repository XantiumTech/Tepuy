...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,$as_nompro,$as_dirpro,$as_estatus,$as_denfuefin,$as_consol,$as_obssol,$as_nomusu,$as_codigo,$as_tipo_proben,$as_monto,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],25,725,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
		...
		$io_pdf->addText(160,753,9,"REPUBLICA BOLIVARIANA DE VENEZUELA"); // Agregar el título 1
		$ls_nombre=$_SESSION["la_empresa"]["nombre"];
		$io_pdf->addText(225,743,9,$ls_nombre); // Agregar el título 2
		$ls_rif=$_SESSION["la_empresa"]["rifemp"];
		$io_pdf->addText(215,733,9,"RIF-".$ls_rif); // Agregar el título 3

		$li_tm=$io_pdf->getTextWidth(13,$as_titulo);
		$tm=296-($li_tm/2);
		$io_pdf->addText(438,753,12,$as_titulo); // Agregar el título
		$io_pdf->Rectangle(430,730,120,20); /// ubi. y, ubi. x, ancho, alto
		$io_pdf->addText(450,737,11,"Nro.:     ".substr($as_numsol,9,6)); // Número de Orden de compra
		...
		$io_pdf->addText(225,175,6,"<b>FIRMAS Y SELLOS PARA LA APROBACIÓN DE LA ORDEN DE PAGO</b>"); // Agregar el título
		...
		$io_pdf->addText($tm,118,6,$ls_jefe_contabilidad); // Agregar el Nombre del Jefe de Contabilidad
		$ls_cargo_contabilidad=$_SESSION["la_empresa"]["cargo_contabilidad"];
		$li_tm=$io_pdf->getTextWidth(13,$ls_cargo_contabilidad);
		$tm=241-($li_tm/2);

		$io_pdf->addText($tm,165,6,$ls_cargo_contabilidad); // Agregar el título
		// ADMINISTRADOR (A) ///
		$ls_jefe_administracion=$_SESSION["la_empresa"]["jefe_administracion"];
		$li_tm=$io_pdf->getTextWidth(13,$ls_jefe_administracion);
		$tm=371-($li_tm/2);

		$io_pdf->addText($tm,118,6,$ls_jefe_administracion); // Agregar el Nombre del Jefe de Contabilidad
		$ls_cargo_administracion=$_SESSION["la_empresa"]["cargo_administracion"];
		$li_tm=$io_pdf->getTextWidth(13,$ls_cargo_administracion);
		$tm=380-($li_tm/2);

		$io_pdf->addText($tm,165,6,$ls_cargo_administracion); // Agregar el título

		// TESORERO (A) ///
		$ls_jefe_tesoreria=$_SESSION["la_empresa"]["jefe_tesoreria"];
		$li_tm=$io_pdf->getTextWidth(13,$ls_jefe_tesoreria);
		$tm=475-($li_tm/2);
		$io_pdf->addText($tm,118,6,$ls_jefe_tesoreria); // Agregar el Nombre del Jefe de Contabilidad

		$ls_cargo_tesoreria=$_SESSION["la_empresa"]["cargo_tesoreria"];
		$li_tm=$io_pdf->getTextWidth(13,$ls_cargo_tesoreria);
		$tm=478-($li_tm/2);

		$io_pdf->addText($tm,165,6,$ls_cargo_tesoreria); // Agregar el título

		$io_pdf->addText(505,118,6,""); // Agregar el título
///////////////
		$io_pdf->addText(500,165,6,"CONTRALORIA MUNICIPAL"); // Agregar el título
		...
		{
			$io_pdf->addText(236,105,7,"<b>RECEPCION CONFORME DE LA ORDEN POR EL BENEFICIARIO</b>");
			$bene="BENEFICIARIO: ";
		}

		$io_pdf->line(20,92,590,92); // LINEA QUE SE MUESTRA EN LA PARTE INFERIOR DE LOS DATOS DE RECEPCION
		$io_pdf->addText(45,94,7,"FECHA");
		$io_pdf->line(42,47,42,91); // DIVISION DEL MES	
		$io_pdf->line(65,47,65,91); // DIVISION DEL AÑO
		$io_pdf->line(20,74,100,74); // LINEA QUE DIVIDE FECHA DE DIA MES Y AÑO
		$io_pdf->addText(25,80,7," DIA    MES      AÑO");
		$io_pdf->line(100,47,100,101);	// DIVIDE AÑO DE APELLIDOS Y NOMBRES
		$io_pdf->addText(144,94,7,"APELLIDO (S) Y NOMBRE (S)");
		$io_pdf->line(280,47,280,101); // LINEA QUE DIVIDE APELLIDOS Y NOMBRES DE CEDULA
		$io_pdf->addText(285,94,7,"CEDULA DE IDENTIDAD N°");
		$io_pdf->line(380,47,380,101);	
		$io_pdf->addText(445,94,7,"FIRMA, SELLO Y HUELLA");
		...
		$io_pdf->ezSetY(475);
		$dia=" ".substr($ad_fecregsol,0,2);
		$mes=" ".substr($ad_fecregsol,3,2);
		$ano="  ".substr($ad_fecregsol,6,4);
		$la_data=array(array('name'=>'<b>'.$bene.'</b>'." ".$as_nompro,'dia'=>'<b>'.' DIA '.'</b>'.$dia,'mes'=>'<b>'.' MES '.'</b>'.$mes,'ano'=>'<b>'.'AÑO '.'</b>'.$ano));			
					
		$la_columna=array('name'=>'','dia'=>'','mes'=>'','ano'=>'');
		$la_config=array('showHeadings'=>0,
						 ...
						 'width'=>570,
						 'maxWidth'=>570,
						 'cols'=>array('beneficiario'=>array('justification'=>'center','width'=>330), // Justificación y ancho de la columna
						 			   'dia'=>array('justification'=>'center','width'=>35), // Justificación y ancho de la columna
						 			   'mes'=>array('justification'=>'center','width'=>35), // 
						 			   'ano'=>array('justification'=>'center','width'=>48))); //
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_detalle_contable($ai_totpre,&$io_pdf)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_print_detalle_cuentas
		//		   Access: private 
		//	    Arguments: ai_totpre // monto total de presupuesto
		//	    		   io_pdf // Instancia de objeto pdf
		...
		$io_pdf->ezSetDy(-5);
		//$io_pdf->ezSetDy(12);
		$la_datatit[1]=array('titulo'=>'<b>CANTIDAD (Cifras)</b>','monto'=>'<b> Bs.'.$ai_totpre.'</b>');
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'xPos'=>485, // sepaacion del margen
						 'xOrientation'=>'center', // Orientación de la tabla
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,

						 'cols'=>array('titulo'=>array('justification'=>'center','width'=>100),
								'monto'=>array('justification'=>'center','width'=>120))); // Justificación y ancho de la columna
	...
	function uf_print_detalle_spg($aa_data,$ai_totpre,&$io_pdf)
	{
		...
		// cantidad (cifras) //
		//$io_pdf->Rectangle(470,$iniciax,120,45); /// ubi. y, ubi. x, ancho, alto antes 390
		//$io_pdf->addText(474,$iniciax,8,"CANTIDAD (Cifras)"); // antes 425
		//$li_tm=$io_pdf->getTextWidth(12,$ai_totpre); 
		//$tm=454+(104-($li_tm/2));
		//$io_pdf->addText($tm,$iniciax,12,$ai_totpre); // Agregar el título // antes 405
		///
		$la_datatit[1]=array('titulo'=>'<b>CÓDIGO PRESUPUESTARIO</b>');
		...
		//			     	     'monto'=>'<b>MONTO</b>');
		$la_data[1]=array('codestpro1'=>'Sector','codestpro2'=>'Prog.','codestpro3'=>'Act.','partida'=>'Partida','generica'=>'Genérica','especifica'=>'Específica','subespecifica'=>'Sub-Específica','ordinal'=>'Ordinal','monto'=>'Monto Bs.');
		$la_columnas=array('codestpro1'=>'Sec','codestpro2'=>'Prog.','codestpro3'=>'Act.','partida'=>'P','generica'=>'G','especifica'=>'E','subespecifica'=>'Sub','ordinal'=>'Ord','monto'=>'Monto Bs.');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('codestpro1'=>array('justification'=>'center','width'=>30), // Justificación y ancho de la columna
								'codestpro2'=>array('justification'=>'center','width'=>25),
								'codestpro3'=>array('justification'=>'center','width'=>25),
								'partida'=>array('justification'=>'center','width'=>30), // Justificación y ancho de la columna
								'generica'=>array('justification'=>'center','width'=>35),
								'especifica'=>array('justification'=>'center','width'=>35),
								'subespecifica'=>array('justification'=>'center','width'=>40),
								'ordinal'=>array('justification'=>'center','width'=>30),
	...
	function uf_print_carded($la_datacar,$deducciones,&$io_pdf)
	{
       ...
       /* $la_data1=array(array('name'=>''));				
		...
		$la_configcar=array('showHeadings'=>0, // Mostrar encabezados
						    'fontSize' =>6, // Tamaño de Letras
						    'titleFontSize' =>7,  // Tamaño de Letras de los títulos
						    'showLines'=>1, // Mostrar Líeas
						    'shaded'=>0, // Sombra entre líneas
						    'width'=>205, // Ancho de la tabla
						    'maxWidth'=>205, // Ancho Mínimo de la tabla
						    'xPos'=>495, // Orientación de la tabla
						    'cols'=>array('denominacion'=>array('justification'=>'center','width'=>90),
						 			      'monobjret'=>array('justification'=>'center','width'=>65),
						 			      'objret'=>array('justification'=>'right','width'=>50)));
			...
			for ($li_i=1;$li_i<=$li_totrow;$li_i++)
			    {
				  ...
				  $la_nomusu    = $io_report->DS->data["usuaprosol"][$li_i];
				  $tipo_proben  = $io_report->DS->data["tipproben"][$li_i];
				  if (strlen($la_nomusu)>0)
				  {
					$ld_nomusu=$io_report->uf_load_usuario($la_nomusu);
				  }
				  ...
				  else
				  {
					if($ls_rifpro!="")
					{
				  	...
				  	else
				  	{
						...
						$ls_codigo="<b>N° de Cédula </b>".$ls_nacben.'-'.$ls_cedbene;
					}
				  }
				  if($ls_estprosol=="E"){
				...
				uf_print_encabezado_pagina($ls_titulo,$ls_numsol,$ld_fecemisol,$ls_nompro,$ls_dirpro,$ls_estprosol,$ls_denfuefin,$ls_consol,$ls_obssol,$ld_nomusu,$ls_codigo,$tipo_proben,$ls_monto,&$io_pdf);
...
// ESTA MODIFICACION ME PERMITE VISUALIZAR MEJOR EL CODIGO PRESUPUESTARIO //
		...
		$la_data[$li_s]=array('codestpro1'=>$ls_codestpro1,'codestpro2'=>$ls_codestpro2,'codestpro3'=>$ls_codestpro3,'partida'=>$ls_partida,'generica'=>$ls_generica,'especifica'=>$ls_especifica,'subespecifica'=>$ls_subespecifica,'ordinal'=>$ls_ordinal,'monto'=>$li_monto);
				...
				////  ANTES $li_totpre ahora $li_monsol
						uf_print_detalle_spg($la_data,$li_monsol,&$io_pdf);
						unset($la_data);
					}
					if($li_totrowspg==0)uf_print_detalle_contable($li_monsol,&$io_pdf);
