...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$io_report    = new tepuy_soc_class_report();
		 	  ...
		 	  while ($row=$io_sql->fetch_row($rs_data))
			        {
				      ...
				      $ls_pais           = $io_report->uf_select_denominacion('tepuy_pais','despai',"WHERE codpai='".$ls_codpais."'");
					  if ($ls_pais=="---seleccione---")
				         {
				           $ls_pais = "";
				         }
					  $ls_codestado = $row["codest"];
					  $ls_estado = $io_report->uf_select_denominacion('tepuy_estados','desest',"WHERE codpai='".$ls_codpais."' AND codest='".$ls_codestado."'");   
					  if ($ls_estado=="---seleccione---")
					     {
						   $ls_estado = "";   
					     }
				      $ls_codmunicipio = $row["codmun"];
				      $ls_municipio = $io_report->uf_select_denominacion('tepuy_municipio','denmun',"WHERE codpai='".$ls_codpais."' AND codest='".$ls_codestado."' AND codmun='".$ls_codmunicipio."'");				   
				      if ($ls_municipio)
				         {
				           $ls_municipio = "";
				         }
				      $ls_codparroquia = $row["codpar"];
				      $ls_parroquia = $io_report->uf_select_denominacion('tepuy_parroquia','denpar',"WHERE codpai='".$ls_codpais."' AND codest='".$ls_codestado."' AND codmun='".$ls_codmunicipio."' AND codpar='".$ls_codparroquia."'");				   
				      if ($ls_parroquia)
				         {
				           $ls_parroquia ="";
				         }
				      ...
				      if ($ls_codfuefin!="--")
				         {
				           $ls_denfuefin = $io_report->uf_select_denominacion('tepuy_fuentefinanciamiento','denfuefin',"WHERE codfuefin='".$ls_codfuefin."'");				   
				         }  
				      else
				         {
				           $ls_denfuefin="";
					  ...
					  if ($ls_codmoneda!="---")
				         {
				           $ls_moneda = $io_report->uf_select_denominacion('tepuy_moneda','denmon',"WHERE codmon='".$ls_codmoneda."'");				   
				         }  
				      else
				         {
				           $ls_moneda = "";
