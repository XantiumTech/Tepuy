...
<html>
<head>
<title>Cat&aacute;logo de Solicitudes de Ayudas</title>
...
<table width="630" border="0" align="center" cellpadding="1" cellspacing="1">
    <tr>
      <td width="600" height="20" colspan="2" class="titulo-ventana">Cat&aacute;logo de Solicitudes de Ayudas</td>
    </tr>
  </table>
  <br>
...
function ue_aceptar(ls_numsol,ls_codtipsol,ls_coduniadm,ls_codfuefin,ls_estsol,ls_tipo_destino,ls_codigo,ls_denuniadm,
					...
					ls_modsep,ls_codestpro1,ls_codestpro2,ls_codestpro3,ls_codestpro4,ls_codestpro5)
{
	opener.document.formulario.txtnumsol.value=ls_numsol;
	opener.document.formulario.txtnumsol.readOnly=true;
	opener.document.formulario.cmbcodtipsol.value=ls_codtipsol+"-"+ls_modsep+"-"+ls_estope;
	//opener.document.formulario.cmbcodtipsol.value=ls_codtipsol;
	opener.document.formulario.cmbcodtipsol.disabled=true;
	opener.document.formulario.txttipsol.value=ls_codtipsol+"-"+ls_modsep+"-"+ls_estope;
	//alert(ls_codtipsol);
	//opener.document.formulario.txttipsol.value=ls_codtipsol;
...
{
   window.open('tepuy_sep_cat_unidad_ejecutora.php?tipo=BUSCAR_CAT_SEP',"_blank","menubar=no,toolbar=no,scrollbars=yes,width=550,height=400,left=50,top=50,location=no,resizable=yes");
}
...
function ue_search()
{
	...
	if((fecregdes!="")&&(fecreghas!=""))
	{
		...
		ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
		// Enviar todos los campos a la pagina para que haga el procesamiento
		ajax.send("catalogo=SOLICITUDAYUDA&numsol="+numsol+"&coduniadm="+coduniadm+"&codtipsol="+codtipsol+"&fecregdes="+fecregdes+
