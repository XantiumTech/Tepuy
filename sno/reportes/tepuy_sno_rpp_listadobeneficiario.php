<?php
	...
	function uf_insert_seguridad($as_titulo,$as_desnom,$as_periodo,$ai_tipo)
	{
		...
		if($ai_tipo==1)
		{
			$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte_nomina("SNO","tepuy_sno_r_listadobeneficiario.php",$ls_descripcion,$ls_codnom);
		}
		else
		{
			$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte_nomina("SNO","tepuy_sno_r_listadobeneficiario.php",$ls_descripcion,$ls_codnom);
	...
	$io_monedabsf=new tepuy_c_reconvertir_monedabsf();				
	...
	if($lb_valido)
	{
		$lb_valido=$io_report->uf_listadobeneficiario_personal($ls_codperdes,$ls_codperhas,$ls_quincena,$ls_codban,$ls_subnomdes,$ls_subnomhas,$ls_orden); // Cargar el DS con los datos de la cabecera del reporte
	}
	...
	else  // Imprimimos el reporte
	{
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			$li_neto=$io_report->DS->data["monnetres"][$li_i];
			$li_monnet=$io_fun_nomina->uf_formatonumerico($io_report->DS->data["monnetres"][$li_i]);
			uf_print_cabecera($ls_codper,$ls_nomper,$li_monnet,$io_pdf); // Imprimimos la cabecera del registro
			$lb_valido=$io_report->uf_listadobeneficiario_beneficiario($ls_codper,$ls_codban); // Obtenemos el detalle del reporte
			if($lb_valido)
			{
				...
				for($li_s=1;$li_s<=$li_totrow_det;$li_s++)
				{
					...
					$ls_apenomben=$io_report->DS_detalle->data["apeben"][$li_s].", ". $io_report->DS_detalle->data["nomben"][$li_s];
					$ls_tipben=$io_report->DS_detalle->data["tipben"][$li_s];
					switch($ls_tipben)
					{
						case "0":
					...
					$ls_forpagben=$io_report->DS_detalle->data["forpagben"][$li_s];
					$ls_pago="";
					switch($ls_forpagben)
					{
						case "0":
							$ls_forpagben="Cheque";
							$ls_pago=$io_report->DS_detalle->data["nomcheben"][$li_s];
							break;
						case "1":
							$ls_forpagben="Deposito en Cuenta";
							$ls_pago=$io_report->DS_detalle->data["nomban"][$li_s]." - ".$io_report->DS_detalle->data["ctaban"][$li_s];
							break;
					}
					$li_porpagben=$io_report->DS_detalle->data["porpagben"][$li_s];
					$li_monpagben=$io_report->DS_detalle->data["monpagben"][$li_s];
					$li_monto=0;
					if($li_porpagben>0)
					{
				...
				$io_report->DS_detalle->resetds("codben");
				uf_print_detalle($la_data,$io_pdf); // Imprimimos el detalle 
				$li_montototalbene=$io_fun_nomina->uf_formatonumerico($li_montototalbene);
				uf_print_piecabecera($li_totrow_det,$li_montototalbene,$io_pdf); // Imprimimos el pie de la cabecera
