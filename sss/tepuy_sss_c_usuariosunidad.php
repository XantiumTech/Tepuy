<?php 
class tepuy_sss_c_usuariosunidad
{
	var $obj="";
	var $io_sql;
	var $siginc;
	...
	function tepuy_sss_c_usuariosunidad()
	{
	...
	function  uf_sss_load_unidadesdisponibles($as_codemp,$as_codusu,$as_codsis,&$aa_disponibles)
	{
		...
		else
		{
			...
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ls_unidad= str_pad($ls_codunidad,33,"0",0);
	...
	function  uf_sss_load_unidadesasignadas($as_codemp,$as_codusu,$as_codsis,&$aa_asignados)
	{
		...
		else
		{
			...
			{
				$lb_valido=true;
				$ls_coduniadm = substr($row["codintper"],23,10);
...
}//  end  class tepuy_sss_c_usuariosunidad
