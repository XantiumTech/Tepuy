<?php 
class tepuy_spg_c_fuentfinan
{

var $ls_sql;
	
		function tepuy_spg_c_fuentfinan($conn)
		{
...
//////////////////////////////////////////////////////////////////////////////  
	  ...
	  $ls_sql = " INSERT INTO tepuy_fuentefinanciamiento ".
	  ...
	  if ($rs_clausula===false)		     
		 {
		   ...
		   $this->io_msg->message("CLASE->tepuy_SPG_C_FUENTFINAN; METODO->uf_insert_fuente_financiamiento; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
...
////////////////////////////////////////////////////////////////////////////// 
	  ...
	  if($ls_status=='C')
	  {
			$ls_sql=" UPDATE tepuy_fuentefinanciamiento ".
		  ...
		  if ($rs_clausula===false)
			 {
			   $this->io_sql->rollback();
			   $this->io_msg->message("CLASE->tepuy_SPG_C_FUENTFINAN; METODO->uf_update_fuente_financiamiento; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			   $this->io_msg->message($this->io_funcion->uf_convertirmsg($this->io_sql->message));
			 }
		  else
			 {
...
////////////////////////////////////////////////////////////////////////////// 
  ...
  if (($lb_valido) && (!$lb_tiene))
	 {
       $ls_sql = " DELETE FROM tepuy_fuentefinanciamiento WHERE codemp='".$as_codemp."' AND codfuefin='".$as_codfuefin."'";	    
	   $this->io_sql->begin_transaction();
...
//                     tepuy_fuentefinanciamiento.
//	     Description:  ///.  
//     Elaborado Por:  Ing. Miguel Palencia.
// Fecha de Creación:  20/02/2006       Fecha Última Actualización:20/02/2006.	 
////////////////////////////////////////////////////////////////////////////////// 

  $ls_sql=" SELECT * FROM tepuy_fuentefinanciamiento WHERE codemp='".$as_codemp."'AND codfuefin='".$as_codfuefin."'";
...
if ($rs_data===false)
   {
	 $lb_valido=false;
	 $this->io_msg->message("CLASE->tepuy_SPG_C_FUENTFINAN; METODO->uf_check_relaciones; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
   }
else
   {
