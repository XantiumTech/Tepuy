...
class tepuy_srh_c_evaluacion_adiestramiento
{
	var $obj="";
	var $io_sql;
	var $siginc;
...
function tepuy_srh_c_evaluacion_adiestramiento($path)
	{   
...
function uf_srh_guardarevaluacion_adiestramiento ($ao_evaluacion,$as_operacion="insertar", $ao_seguridad)
  { 
		...
		//         access: public (tepuy_srh_evaluacion_adiestramiento)															
...
function uf_srh_buscar_evaluacion_adiestramiento($as_nroreg,$as_fecha1,$as_fecha2)
	{
		...
		$ls_sql= "SELECT *  FROM srh_evaluacion_adiestramiento  INNER JOIN srh_solicitud_adiestramiento ON ".
		         " (srh_solicitud_adiestramiento.nroreg = srh_evaluacion_adiestramiento.nroreg) ".
				 " INNER JOIN sno_personal ON (srh_solicitud_adiestramiento.codper = sno_personal.codper) ".
		...
		{	
			...
			while ($row=$this->io_sql->fetch_row($rs_data)) 
			{
					...
					$ls_denprov=htmlentities ($row["denprov"]);
					...
					$cell->appendChild($dom->createTextNode($row['nroreg']." ^javascript:aceptar( \"$ls_nroreg\", \"$ls_fecsol\", \"$ls_des\", \"$ls_codsol\", \"$ls_nomsol\", \"$ls_apesol\",  \"$ls_fecha\", \"$ls_codprov\",\"$ls_denprov\", \"$ls_fecini\", \"$ls_fecfin\", \"$ls_durhras\" ,  \"$ls_costo\" ,  \"$ls_obs\", \"$ls_obseval\", \"$ls_fecha\" , \"$ls_nrodestino\" ,  \"$ls_fecsoldestino\" ,  \"$ls_desdestino\" ,  \"$ls_codsoldestino\" ,  \"$ls_nomdestino\" ,  \"$ls_fechadestino\" ,\"$ls_codprovdestino\" ,  \"$ls_denprovdestino\" ,  \"$ls_fecinidestino\" ,  \"$ls_fecfindestino\" ,  \"$ls_durhrasdestino\" ,  \"$ls_costodestino\" ,  \"$ls_obsdestino\", \"$ls_obsevaldestino\", \"$ls_fechadestino\");^_self"));
...
function uf_srh_guardar_dt_evaluacion_adiestramiento($ao_evaluacion, $ao_seguridad)
  { 
		...
		//         access: public (tepuy_dt_srh_evaluacion_adiestramiento)														
...
function uf_srh_eliminar_dt_evaluacion_adiestramiento($as_nroreg,$as_fecha, $ao_seguridad)
  {
		...
		//        access:  public (tepuy_srh_dt_evaluacion_adiestramiento)														
...
function uf_srh_load_evaluacion_adiestramiento_campos($as_nroreg, $as_fecha,&$ai_totrows,&$ao_object)
	{
		...
		//		   Access: public (tepuy_srh_d_escala)
...
}// end   class tepuy_srh_c_evaluacion_adiestramiento
?>