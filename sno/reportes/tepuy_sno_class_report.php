<?php
class tepuy_sno_class_report
{
	//-----------------------------------------------------------------------------------------------------------------------------------
	function tepuy_sno_class_report()
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: tepuy_sno_class_report
		//		   Access: public 
	...
	}// end function tepuy_sno_class_report
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	/*	$ls_sql=$ls_sql." UNION ".
			// Buscamos todas aquellas cuentas presupuestarias de los conceptos D , que se integran directamente con presupuesto
				"SELECT sno_concepto.codpro as programatica, spg_cuentas.spg_cuenta as cueprecon, spg_cuentas.denominacion as denominacion, sum(".$tabla.".".$suma.") as total ".
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report MÉTODO->uf_cargar_presupuesto_asignaciones ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			// ESTA MODIFICACION ME PERMITE VISUALIZAR MEJOR EL CODIGO PRESUPUESTARIO //
				...
				$ls_spg_cuenta=substr($ls_spg_cuenta,0,3).".".substr($ls_spg_cuenta,3,2).".".substr($ls_spg_cuenta,5,2).".".substr($ls_spg_cuenta,7,2).substr($ls_spg_cuenta,9,4);
				}
	...
	function uf_cargar_presupuesto_aporte_deducciones(&$tabla_aporte_deducciones,$tabla,$variable,$suma)
	{
		...
		$ls_sql="SELECT sno_concepto.codpro1 as programatica, spg_cuentas.spg_cuenta as cueprepatcon, spg_cuentas.denominacion as denominacion,sum(".$tabla.".".$suma.") as total, ".
				...
				/// CUANDO INTEGRA PROGRAMATICA O PROYECTO AL CONCEPTO //
				/////"   AND sno_concepto.intprocon = '1'".
				/// CUANDO NO LA INTEGRA LA PROGRAMATICA AL PROYECTO ///
				"   AND sno_concepto.intprocon = '0'".
				"   AND spg_cuentas.status = 'C'".
				"   AND sno_personalnomina.codemp = ".$tabla.".codemp ".
				"   AND sno_personalnomina.codnom = ".$tabla.".codnom ".
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_cargar_conceptos(&$columnas,&$columnas1,&$titulos,&$configuracion,&$posicion,$as_conceptocero,$as_conceptop2,$tabla,$tabla1)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report MÉTODO->uf_cargar_conceptos ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			$columnas1[0] = array();
			$titulos[0] = array();
			$columnas[0]['concepto'] = "<b>Cédula</b>";
			$columnas1[0]['codigo'] = "cedula";
			$columnas[1]['concepto'] = "<b>Apellidos y Nombres</b>";
			$columnas1[1]['codigo'] = "nomper";
			$titulos[0]['cedula'] = "<b>Cédula</b>";
			$titulos[1]['nomper'] = "<b>Apellidos y Nombres</b>";
			$configuracion = array();
			$configuracion['cedula'] =array('justification'=>'center','width'=>10);
			$configuracion['nomper'] =array('justification'=>'center','width'=>20);
			$configuracion['sueldo'] =array('justification'=>'center','width'=>10);
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$titulos[$as_nconceptos][$row['conprep']] = $row['conprep'];
				if($as_nconceptos>3)
				{
					$configuracion[$row['conprep']] =array('justification'=>'center','width'=>31);
				}
				$as_nconceptos++;
			}
			$columnas[$as_nconceptos]['concepto'] = "Neto a Cobrar";
			$configuracion["Neto a Cobrar"] =array('justification'=>'center','width'=>40);
	...
	function uf_prenomina_conceptopersonal($as_codper,$as_conceptocero,$as_conceptop2)
	{
		...
		$ls_sql="SELECT sno_prenomina.codconc, sno_concepto.nomcon,sno_concepto.conprep, sno_prenomina.tipprenom, sno_prenomina.valprenom, sno_prenomina.valhis ".
									...
									$as_codpai,$as_codest,$as_codmun,$as_codpar,$as_subnomdes,$as_subnomhas,$as_orden)
	{
		...
		$ls_sql="SELECT sno_personalnomina.codper, sno_personal.cedper, sno_personal.nomper, sno_personal.apeper, ".
				"		sno_personal.fecingper,sno_personalnomina.fecculcontr, sno_personalnomina.fecingper as fecingnom,".
				"       sno_personalnomina.codcueban, sno_unidadadmin.desuniadm, ".
				...
				"   ".$ls_union.
				"   ".$ls_orden;  
		//print " Orden: ".$as_orden." ".$ls_sql;die();
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		{
	...
	function uf_pagonomina_conceptopersonal($as_codper,$as_conceptocero,$as_tituloconcepto,$as_conceptoreporte,$as_conceptop2)
	{
		...
		{
			$ls_union="UNION ".
					  "SELECT sno_concepto.codconc, ".$ls_campo." as nomcon, sno_salida.valsal, sno_salida.tipsal, sno_concepto.frevarcon, sno_concepto.conprep  ".
		...
		$ls_sql="SELECT sno_concepto.codconc, sno_concepto.conprep,".$ls_campo." as nomcon, sno_salida.valsal, sno_salida.tipsal, sno_concepto.frevarcon ".
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	// ELIMINE LAS DOS ULTIMAS VARIABLES
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	// SE ELIMINA LAS DOS ULTIMAS VARIABLES	
									...
									$as_codubifis,$as_codpai,$as_codest,$as_codmun,$as_codpar,$as_subnomdes,$as_subnomhas,$as_orden)
	{
		...
		$ls_sql="SELECT sno_personal.codper, sno_personal.cedper, sno_personal.nomper, sno_personal.apeper, sno_personal.nacper,".
				"		sno_personalnomina.codcueban, sno_personal.fecingper, sum(sno_salida.valsal) as total, sno_unidadadmin.desuniadm, ".
				"		sno_unidadadmin.minorguniadm,sno_unidadadmin.ofiuniadm,sno_unidadadmin.uniuniadm,sno_unidadadmin.depuniadm,".
				"		sno_unidadadmin.prouniadm, MAX(sno_personalnomina.sueper) AS sueper, MAX(sno_personalnomina.pagbanper) AS pagbanper, ".
	...
	function uf_recibopago_conceptopersonal($as_codper,$as_conceptocero,$as_conceptop2,$as_conceptoreporte,$as_tituloconcepto,$as_quincena)
	{
		...
		$ls_sql="SELECT sno_concepto.codconc, ".$ls_campo." as nomcon, ".$ls_campomonto.", sno_salida.tipsal, abs(sno_conceptopersonal.acuemp) AS acuemp, ".
				"		abs(sno_conceptopersonal.acupat) AS acupat, sno_concepto.repacucon,  ".
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_listadobeneficiario_personal($as_codperdes,$as_codperhas,$as_quincena,$as_codban,$as_subnomdes,$as_subnomhas,$as_orden)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_listadobeneficiario_beneficiario($as_codper,$as_codban)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_listadobeneficiario_beneficiario
		//         Access: public (desde la clase tepuy_sno_rpp_listadobeneficiario)  
		//	    Arguments: as_codper // Código del personal 
