...
<?php
	...
	$ls_sistema               = "SPG";
	$ls_ventanas              = "tepuy_spg_d_estprog2.php";
...
if ($ls_operacion == "GUARDAR")
   {
	 ...
	 $lb_encontrado = $io_estpro1->uf_spg_select_estprog1($ls_codemp,$ls_codestpro1);
	 if ($lb_encontrado)
	    {
	      $lb_existe     = $io_estpro2->uf_spg_select_estprog2($ls_codemp,$ls_codestpro1,$ls_codestpro2); 
	      if (!$lb_existe)
	         {
               $lb_valido = $io_estpro2->uf_spg_insert_estprog2($ls_codemp,$ls_codestpro1,$ls_codestpro2,$ls_denestpro2,$li_estmodest,$la_seguridad);		
		       if ($lb_valido)
		          {
			        $io_sql->commit();
	         ...
	         {
		       $lb_valido = $io_estpro2->uf_spg_update_estprog2($ls_codemp,$ls_codestpro1,$ls_codestpro2,$ls_denestpro2,$la_seguridad);		
		       if ($lb_valido)
		          {
			        $io_sql->commit();
...
if ($ls_operacion == "ELIMINAR")
   {
     $ls_codestpro1 = $io_funcion->uf_cerosizquierda($ls_codestpro1,20);
	 $ls_codestpro2 = $io_funcion->uf_cerosizquierda($ls_codestpro2,6);
	 $lb_valido     = $io_estpro2->uf_spg_delete_estpro2($ls_codemp,$ls_codestpro1,$ls_codestpro2,$ls_denestpro2,$la_seguridad);
	 if ($lb_valido)
	    {
		  $io_sql->commit();
          ...
          <table width="649" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
              ...
              <tr class="formato-blanco">
                ...
                <td height="22" colspan="2" align="left">
                  <input name="txtcodestpro1" type="text" id="txtcodestpro1" size="<?php print $li_size_1 ?>" maxlength="<?php print $li_maxlength_1 ?>" value="<?php print  $ls_codestpro1?>" readonly="" style="text-align:center">
              ...
              <tr class="formato-blanco">
                ...
                <td height="22" colspan="2"><div align="left" >
                    <input name="txtcodestpro2" type="text" id="txtcodestpro2" style="text-align:center " value="<?php print $ls_codestpro2?>" size="<?php print $li_size_2 ?>" maxlength="<?php print $li_maxlength_2 ?>" onBlur="javascript:rellenar_cad(this.value,<?php print $li_maxlength_2 ?>,'cod')" <?php print $readonly?> onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnopqrstuvwxyz '+'.,-');">
                </div></td>
              </tr>
              <tr class="formato-blanco">
...
function ue_buscar()
{
	...
	window.open("tepuy_spg_cat_estpro2.php?txtcodestpro1="+ls_codestpro1+"&txtdenestpro1="+ls_denestpro1,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=800,height=600,left=50,top=50,location=no,resizable=yes");
}
...
function uf_estprog3()
{
	...
	{
		alert("Debe seleccionar algun Programa del Sector para continuar");
	}
	...
	if (cadena!="")
	   {
		 ...
		 if (campo=="cod")
			{
				document.form1.txtcodestpro2.value=cadena;
			}
	   } 
}
</script>
