...
class tepuy_saf_class_report
{
	...
	function tepuy_saf_class_report()
	{
	...
	//   Fecha de Cracion:   09/06/2006						Fecha de Ultima Modificación: 12/12/2006
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		$ls_sqlint="";
		$ls_estcat=$this->uf_select_valor_config($as_codemp);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	function uf_saf_load_defactivos($as_codemp,$ai_ordenact,$ad_desde,$ad_hasta,$as_coddesde,$as_codhasta,$as_codresuso)
	{
		...
		if(!empty($as_codresuso))
		{
			$ls_cadena="saf_dta.codres='".$as_codresuso."'  AND ";
		}
		$ls_sql=" SELECT saf_activo.codact,saf_activo.denact,saf_activo.maract,saf_activo.modact, ".
				...
				" FROM   saf_activo, saf_dta ".
				" WHERE  saf_activo.codemp='".$as_codemp."'  AND ".$ls_cadena.
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql=" SELECT saf_dt_movimiento.codemp,saf_dt_movimiento.cmpmov,saf_dt_movimiento.codact,".
				...
				"          saf_dt_movimiento.codcau,saf_dt_movimiento.feccmp ";	 //print $ls_sql;
		$rs_data=$this->io_sql->select($ls_sql);
	    $li_numrows=$this->io_sql->num_rows($rs_data);	
		if($rs_data===false)
		{
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  $this->io_sql=new class_sql($this->con);
	...
	if((!empty($ad_mes))&&(!empty($ad_anno)))
		{
		 $ls_sqlint = $ls_sqlint. " AND Extract(month from a.feccmp) = '".$ad_mes."'"." 
                                    AND Extract(year from a.feccmp) = '".$ad_anno."'";
		}
		...
		$ls_sql= "Select a.codact,c.estact as estatus,
                  ...
                  Join saf_grupo  e on e.codgru = d.codgru
                  Join saf_subgrupo f on f.codgru = d.codgru and
				                         f.codsubgru = d.codsubgru
                  Join saf_seccion g on g.codgru = d.codgru and
				                        g.codsubgru = d.codsubgru and
										g.codsec = d.codsec
				  Join saf_item h on h.codgru = d.codgru and
				                     h.codsubgru = d.codsubgru and
								     h.codsec = d.codsec and
									 h.codite = d.codite                      
                  where a.codemp = '".$as_codemp."'".$ls_sqlint."
                  group by a.codact,c.ideact,a.cmpmov,a.codemp,a.feccmp, d.denact,
                           c.estact,d.codgru,d.codsubgru,d.codsec,d.codite, a.codcau, d.costo, b.descmp ";
		if($ai_orden!="")
		{
			if($ai_orden==0)
			{
   ...
   $rs_data=$this->io_sql->select($ls_sql);
    $li_numrows=$this->io_sql->num_rows($rs_data);	
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Report MÉTODO->uf_saf_load_existencia ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if ($li_numrows>0)
  ...
  $this->io_sql=new class_sql($this->con);
   ...
   $rs_data=$this->io_sql->select($ls_sql);
    $li_numrows=$this->io_sql->num_rows($rs_data);	
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Report MÉTODO->uf_saf_load_dt_resctabiemue_desinc ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if ($li_numrows>0)
  ...
  $this->io_sql=new class_sql($this->con);
   ...
   $rs_data=$this->io_sql->select($ls_sql);
    $li_numrows=$this->io_sql->num_rows($rs_data);	
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Report MÉTODO->uf_saf_load_dt_resctabiemue_inc ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if ($li_numrows>0)
	...
	if((!empty($ad_fecini))&&(!empty($ad_fecfin)))
		...
		$ls_sql= "Select a.codact,
                  ...
                  Join saf_seccion g on g.codgru = d.codgru and
				                        g.codsubgru = d.codsubgru and
										g.codsec = d.codsec
				  Join saf_ite h on h.codgru = d.codgru and
				                        h.codsubgru = d.codsubgru and
										h.codsec = d.codsec and
										h.codite = d.codite						
                  where a.codemp = '".$as_codemp."'".$ls_sqlint."
...
function uf_saf_load_resbiegru($as_codemp,$ai_ordenact,$ad_desde,$ad_hasta,$as_coddesde,$as_codhasta,$as_grupo,$as_subgrupo,$as_seccion)
	{
		...
		$ls_sql=" Select distinct(activo.codgru), grupo.dengru
                  ...
                  Join saf_subgrupo subgrupo on subgrupo.codgru = activo.codgru and 
				                                subgrupo.codsubgru = activo.codsubgru
                  Join saf_seccion seccion on seccion.codgru = activo.codgru and 
                                              seccion.codsubgru = activo.codsubgru and 
                                              seccion.codsec = activo.codsec
				  Join saf_item item on item.codgru = activo.codgru and 
                                        item.codsubgru = activo.codsubgru and 
                                        item.codsec = activo.codsec and
										item.codite = activo.codite							  
...
function uf_saf_load_dt_resbiegru($as_codemp,$ai_ordenact,$ad_desde,$ad_hasta,$as_coddesde,$as_codhasta,$as_grupo,$as_subgrupo,$as_seccion)
	{
		...
		$ls_sql=" Select a.codact, 
                  ...
                  Join saf_grupo  c on c.codgru = b.codgru
                  Join saf_subgrupo d on d.codgru = b.codgru and 
				                         d.codsubgru = b.codsubgru
                  Join saf_seccion e on e.codgru = b.codgru and 
				                        e.codsubgru = b.codsubgru and 
									    e.codsec = b.codsec
				  Join saf_item f on f.codgru = b.codgru and 
				                     f.codsubgru = b.codsubgru and 
									 f.codsec = b.codsec and
									 f.codite = b.codite ".
				" WHERE  a.codemp='".$as_codemp."'".$ls_sqlint.
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  $this->io_sql=new class_sql($this->con);
		...
		{
		  $ls_sqlint = $ls_sqlint." AND saf_dta.coduniadm.coduniadm >= '".$as_coduniadm_desde."'
		                            AND saf_dta.coduniadm.coduniadm <= '".$as_coduniadm_hasta."'";
		}   
		...
		$ls_sql= "SELECT saf_dta.coduniadm,spg_unidadadministrativa.denuniadm,((SELECT (SUM(saf_dt_movimiento.monact))
                  ...
                  WHERE saf_dt_movimiento.codemp='".$as_codemp."'
                  AND Extract(month from saf_dt_movimiento.feccmp) = '".$ad_mes."'
                  AND Extract(Year from saf_dt_movimiento.feccmp) = '".$ad_anno."'
                  AND saf_dta.codemp=saf_dt_movimiento.codemp
                  AND saf_dta.codact=saf_dt_movimiento.codact
                  AND saf_dta.ideact=saf_dt_movimiento.ideact
                  AND saf_dt_movimiento.estcat = 2
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_saf_load_ubicacion($as_codpai,$as_codest,$as_codmun,&$as_denpai,&$as_denest,&$as_denmun)
	{
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		$ls_sql="SELECT tepuy_municipio.denmun,".
				" (SELECT desest FROM tepuy_estados".
				"   WHERE tepuy_municipio.codpai=tepuy_estados.codpai".
				"   AND   tepuy_estados.codest='". $as_codest ."') AS denest,".
				" (SELECT despai FROM tepuy_pais".
				"   WHERE tepuy_municipio.codpai=tepuy_pais.codpai) AS denpai".
				" FROM  tepuy_municipio".
				" WHERE tepuy_municipio.codpai='".$as_codpai."'".
				" AND   tepuy_municipio.codest='".$as_codest."'".
				" AND   tepuy_municipio.codmun='".$as_codmun."'"; 
		$rs_data=$this->io_sql->select($ls_sql);	
	    $li_numrows=$this->io_sql->num_rows($rs_data);	
		if($rs_data===false)
		{
   ...
   //----------------------------------------------------------------------------------------------------------------------------------
 /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ///funcion para mostrar los bienes que se encuentran en una unidad de trabajo dada una fecha
 /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////  
    function uf_select_inventario_unidad($coduniadmi,$fecha1,$fecha2,$estatus, $orden, $cod1, $cod2,$grupo,$subgrupo,$seccion)
	{
		...
		if($coduniadmi!="")
		{
		 $unidad=" and b.coduniadm='".$coduniadmi."'";
		}
		if (($fecha1!="") && ($fecha2!=""))
		{
		 $ld_auxdesde=$this->io_funcion->uf_convertirdatetobd($fecha1);
		 $ld_auxhasta=$this->io_funcion->uf_convertirdatetobd($fecha2);
		 $fecha=" and (b.fecincact>=".$ld_auxdesde." or b.fecincact <=".$ld_auxhasta.")".
		       " or (a.fecregact>=".$ld_auxdesde." or a.fecregact <=".$ld_auxhasta.")";	
		}
		
		if($estatus==1)
		{
		  $ls_estac = $ls_estac." and b.estact ='I'";
        }
		
		if($estatus==2)
		{
		 $ls_estac = $ls_estac. " and b.estact ='R'";
		}	
		
		if($orden==0)
		{
		$ls_orden=" order by b.ideact,a.codact";
		}
	    else
		{
		$ls_orden=" order by a.denact";
		}
		
		if($cod1!="" && $cod2!="")
		{
		 $ls_codigo=" and (a.codact>='".$cod1."' and a.codact<='".$cod2."')";
		}
		
		if($grupo!="" && $subgrupo!="" && $seccion!="")
		{
		  $clasifica=" and a.codgru='".$grupo."' and a.codsubgru='".$subgrupo."' and a.codsec='".$seccion."'";
		}
		...
		{		
		 $ls_sql="select distinct (a.codact),c.codgru, d.codsubgru, e.codsec, a.codemp, a.denact,a.maract, a.modact, a.costo,a.costoaux, b.seract,b.estact, 
                 (SELECT coduniadm||'-'||denuniadm FROM spg_unidadadministrativa WHERE b.coduniadm=spg_unidadadministrativa.coduniadm) AS denuniadm, b.ideact, '1' as cantidad, g.denuniadm as servicio
                     from saf_activo a
                  join saf_dta b on (a.codact=b.codact)
                  join saf_grupo c on a.codgru=c.codgru
                  join saf_subgrupo d on d.codgru = a.codgru and
				                         d.codsubgru = a.codsubgru              
                  join saf_seccion e on  e.codgru = a.codgru and
				                         e.codsubgru = a.codsubgru and
										 e.codsec = a.codsec
				  join saf_item f on f.codgru = a.codgru and
				                     f.codsubgru = a.codsubgru and
									 f.codsec = a.codsec and
									 f.codite = a.codite
				  join saf_unidadadministrativa g on (b.coduniadm=g.coduniadm) 	
                  where b.estact <> 'D' ".$ls_estac.$unidad.$fecha.$ls_codigo.$clasifica.$ls_orden; 
		}
		else
		{
		 $ls_sql="select distinct (a.codact),c.codgru, d.codsubgru, e.codsec, a.codemp, a.denact,a.maract, a.modact, a.costo,a.costoaux, b.seract,b.estact, 
                 (SELECT CONCAT(coduniadm,' - ',denuniadm) FROM spg_unidadadministrativa WHERE b.coduniadm=spg_unidadadministrativa.coduniadm) AS denuniadm, b.ideact, '1' as cantidad, g.denuniadm as servicio
                     from saf_activo a
                  join saf_dta b on (a.codact=b.codact)
                  join saf_grupo c on a.codgru=c.codgru
                  join saf_subgrupo d on d.codgru = a.codgru and
 ...
 function uf_select_bienes_general($codpro1,$codpro2,$codart1,$codart2,$fecha1,$fecha2,$orden)
	{
		...
		{
		  $ls_orden=" order by e.nompro,a.denact";
		}
		...
		if (strtoupper($ls_gestor) <> "MYSQL")
		{		
		 $ls_sql="select a.ideact as ideact, a.codact as codact, b.denact as denact,  b.codgru||' -'||b.codsubgru||'-'||b.codsec as grupo,
                  (select count(ideact) from saf_dta where codact=a.codact group by codact) as cantidad, b.costo as costo,
                  b.cod_pro as cod_pro, e.nompro as nompro, b.numordcom as numordcom, f.fecordcom as fecordcom, 
                  a.fecincact as fecincact 
                  from saf_dta a
                  ...
                  left join soc_ordencompra f on (b.numordcom=f.numordcom)
		   		  where a.codemp='".$ls_codemp."' ".$ls_fecha.$ls_codigo.$ls_codpro." group by b.cod_pro, a.codact ".$ls_orden;        
		}
		else
		{
		 $ls_sql="select a.ideact as ideact, a.codact as codact, b.denact as denact,  CONCAT(b.codgru,'-',b.codsubgru,'-',b.codsec) as grupo,
                  (select count(ideact) from saf_dta where codact=a.codact group by codact) as cantidad, b.costo as costo,
                  b.cod_pro as cod_pro, e.nompro as nompro, b.numordcom as numordcom, f.fecordcom as fecordcom, 
                  a.fecincact as fecincact 
                  from saf_dta a
 ...
 function uf_select_tipo_bienes($codpro1,$codpro2,$codart1,$codart2,$fecha1,$fecha2,$coduni,$orden)
	{
		...
		{	
		 $ls_sql="select  a.ideact as ideact, a.codact as codact, b.denact as denact,  b.codgru||'-'||b.codsubgru||'-'||b.codsec as grupo,
                  b.costo as costo,
                  b.cod_pro as cod_pro, b.numordcom as numordcom, f.fecordcom as fecordcom, 
                  b.maract as maract, b.modact as modact, a.seract as seract, b.spg_cuenta_act as spg_cuenta_act,
		  		  b.sc_cuenta as sc_cuenta
                  from saf_dta a
                  join saf_activo b on (a.codact=b.codact)
                  left join rpc_proveedor e on (b.cod_pro =e.cod_pro)
                  left join soc_ordencompra f on (b.numordcom=f.numordcom)
				  left join saf_unidadadministrativa g on (a.coduniadm=g.coduniadm)
				  where a.codemp='".$ls_codemp."' ".$ls_fecha.$ls_codigo.$ls_codpro.$ls_codunidad." group by b.cod_pro, a.codact ".$ls_orden;        
		}
		else
		{
		 $ls_sql="select  a.ideact as ideact, a.codact as codact, b.denact as denact, CONCAT(b.codgru,'-',b.codsubgru,'-',b.codsec) as grupo,
...
function uf_select_proveedor($codpro1,$codpro2,$codart1,$codart2,$fecha1,$fecha2,$orden)
	{
		...
		$ls_sql="select b.cod_pro as cod_pro, e.nompro as nompro
                 from saf_dta a
                 join saf_activo b on (a.codact=b.codact)
                 left join rpc_proveedor e on (b.cod_pro =e.cod_pro)
                 left join soc_ordencompra f on (b.numordcom=f.numordcom)
...
function uf_select_proveedor_tipo_bien($codpro1,$codpro2,$codart1,$codart2,$fecha1,$fecha2,$coduni,$orden)
	{
		...
		$ls_sql="select (select count(p.ideact) 
		                    from saf_dta p join saf_activo m on (p.codact=m.codact) 
		                    join rpc_proveedor n on (m.cod_pro=n.cod_pro) 
				            where  p.coduniadm=a.coduniadm  and m.cod_pro=b.cod_pro ) as cantidad, 
                 b.cod_pro as cod_pro, e.nompro as nompro,g.coduniadm as coduniadm, g.denuniadm as denuniadm
                 from saf_dta a
                 join saf_activo b on (a.codact=b.codact)
                 left join rpc_proveedor e on (b.cod_pro =e.cod_pro)
                 left join saf_unidadadministrativa g on (a.coduniadm=g.coduniadm)
				 where a.codemp='".$ls_codemp."' ".$ls_fecha.$ls_codigo.$ls_codpro.$ls_codunidad." group by b.cod_pro ".$ls_orden;        
				//print $ls_sql;
	    $rs_data=$this->io_sql->select($ls_sql);
	    $li_numrows=$this->io_sql->num_rows($rs_data);		
		if($rs_data===false)
 ...
 function uf_select_movimientos_bien($fecha1,$fecha2,$cmpmov,$causa,$orden)
	{
		...
		{	
		 ...
		 {
		  $ls_sql="select a.codemp as codemp, a.cmpmov as cmpmov,a.feccmp as feccmp, b.codact as codact, e.denact as denact, e.maract as maract, 
                 CONCAT(e.codgru,'-',e.codsubgru,'-',e.codsec) as grupo, f.seract as seract, f.coduniadm as coduniadm, g.denuniadm as denuniadm,
	  	 		 h.dencau as dencau, a.fecentact  as fecentact,
				 a.codrespri as codrespri, a.codresuso as codresuso,
				 (select cedper from sno_personal where codper=a.codrespri ) as cedrespri,
				 (select cedper from sno_personal where codper=a.codresuso ) as cedresuso,
...
} //fin  class tepuy_siv_class_report
?>
