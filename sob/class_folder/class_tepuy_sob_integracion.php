<?php
  ////////////////////////////////////////////////////////////////////////////////////////////////////////
  //       Class : class_tepuy_sob_integracion_php                                                     //    
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////
class class_tepuy_sob_integracion
{
	var $sqlca;   
    var $is_msg_error;
	var $dts_empresa; 
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function class_tepuy_sob_integracion()
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: class_tepuy_sob_integracion
		//		   Access: public 
		...
		//	   Creado Por: Ing. 
		// Modificado Por: Ing. Miguel Palencia								Fecha Última Modificación : 25/04/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$this->as_codbanaux="";
		$this->as_ctabanaux="";
	}// end function class_tepuy_sob_integracion
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_procesar_contabilizacion_asignacion($as_codasi,$adt_fecha,$aa_seguridad)
	{
		...
		//		   Access: public (tepuy_mis_p_contabiliza_asignacion_sob.php)
		...
		//	   Creado Por: Ing. 
		// Modificado Por: Ing. Miguel Palencia								Fecha Última Modificación : 26/04/2007
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    $ls_codemp=$this->dts_empresa["codemp"];
        $ls_comprobante=$this->io_tepuy_int->uf_fill_comprobante($as_codasi);		
        ...
        $ls_mensaje=$this->io_tepuy_int_spg->uf_operacion_codigo_mensaje("PC");	
        $ls_tipo_destino="P" ;		
        $ls_procede="SOBASI";
		...
		$fecha_cont=$adt_fecha;
		$this->ad_fecha=$this->io_function->uf_convertirdatetobd($adt_fecha);
		$this->as_codban=$ls_codban;
		$this->as_ctaban=$ls_ctaban;
		$lb_valido=$this->io_tepuy_int->uf_int_init($ls_codemp,$ls_procede,$ls_comprobante,$adt_fecha,$ls_descripcion,
		...
		if($lb_valido) 
		{
			if($lb_valido)
			{
				$lb_valido=$this->io_tepuy_int->uf_init_end_transaccion_integracion($aa_seguridad,$fecha_cont); 
				if(!$lb_valido)
				{
					$this->io_msg->message($this->io_tepuy_int->is_msg_error);
				}
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
    function uf_procesar_detalles_gastos_asignacion($as_codasi,$as_mensaje,$as_procede_doc,$as_descripcion,$as_process,$as_scgcta_proben)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    $lb_valido=true;
		//print "largo: ".strlen(trim($as_scgcta_proben));
		if(strlen(trim($as_scgcta_proben))>0)
		{
			$ls_sql="SELECT a.codestpro1,a.codestpro2,a.codestpro3,a.codestpro4,a.codestpro5,a.spg_cuenta,a.monto,b.sc_cuenta ".
				"  FROM sob_cuentasasignacion a, spg_cuentas b".
	           		" WHERE a.codemp='".$this->io_codemp."' ".
				"   AND a.codasi='".$as_codasi."'".
		...
		{                 
			while($row=$this->io_sql->fetch_row($rs_data) and ($lb_valido))
		    {
				...
				$ls_documento=$this->io_tepuy_int->uf_fill_comprobante($as_codasi);		
				$ldec_monto=$row["monto"];
                if($as_process=="PC")
				...
				$lb_valido = $this->io_tepuy_int->uf_spg_insert_datastore($this->io_codemp,$ls_codestpro1,$ls_codestpro2,$ls_codestpro3,
				...
				else
				{
					if(strlen(trim($as_scgcta_proben))>0)
					{
						// CONTABILIDAD FISCAL, CONTABILIZA EL COMPROMISO //
						if($tipocontabilidad==2)
						{
							// CREA EL DATASTORE DE SCG //
							// INSERTA EN EL DEBE //
							$ls_debhab="D";
							$lb_valido=$this->io_tepuy_int->uf_scg_insert_datastore($this->io_codemp,$ls_scg_cuenta,$ls_debhab,$ldec_monto,
																		 $ls_documento,$ls_procede_doc,$ls_descripcion);
							// INSERTA EN EL HABER //
							$ls_debhab="H";
							$lb_valido=$this->io_tepuy_int->uf_scg_insert_datastore($this->io_codemp,$as_scgcta_proben,$ls_debhab,$ldec_monto,
																		 $ls_documento,$ls_procede_doc,$ls_descripcion);
							if($lb_valido===false)
							{
								$this->io_msg->message("".$this->io_tepuy_int->is_msg_error);
	...
	function uf_procesar_reverso_asignacion($as_codasi,$ad_fechaconta,$aa_seguridad)
	{
		...
		$ls_codban="---";
		$ls_ctaban="-------------------------";
	    $lb_valido=$this->io_tepuy_int->uf_obtener_comprobante($this->io_codemp,$ls_procede,$ls_comprobante,$ldt_fecha,
        ...
        $this->io_tepuy_int->uf_int_init_transaction_begin();
		$lb_valido = $this->io_tepuy_int->uf_init_delete($this->io_codemp,$ls_procede,$ls_comprobante,$ldt_fecha,
	...
	function uf_procesar_anulacion_asignacion($as_codasi,$adt_fecha,$ad_fechaconta,$aa_seguridad)
	{
		...
		//	   Creado Por: Ing. 
		// Modificado Por: Ing. Miguel Palencia								Fecha Última Modificación : 30/04/2007
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    $ls_codemp=$this->io_codemp;
        $ls_comprobante=$this->io_tepuy_int->uf_fill_comprobante($as_codasi);		
        $this->dts_data->resetds("codasi"); // inicializa el datastore en 0 registro.
		...
		$this->as_codban=$ls_codban;
		$this->as_ctaban=$ls_ctaban;
		$lb_valido=$this->io_tepuy_int->uf_int_anular($ls_codemp,$ls_procede,$ls_comprobante,$ad_fechaconta,$ls_procede_anula,
	...
	function uf_procesar_reverso_anulacion_asignacion($as_codasi,$ad_fechaanula,$aa_seguridad)
	{
		...
		$lb_valido=$this->io_tepuy_int->uf_init_delete($this->io_codemp,$ls_procede,$ls_comprobante,$ldt_fecha,$ls_tipo_destino,
    ...
    function uf_procesar_contabilizacion_contrato($as_codcon,$as_codasi,$adt_fecha,$ad_fechacontaasig,$aa_seguridad)
	{
		...
		$ls_comprobante=$this->io_tepuy_int->uf_fill_comprobante($as_codcon);
		$fecha_cont=substr($adt_fecha,6,4)."-".substr($adt_fecha,3,2)."-".substr($adt_fecha,0,2)." 00:00:00";
		//print "Fecha:".$fecha_cont;
		$ldt_fecha=$this->io_function->uf_convertirfecmostrar($adt_fecha); 		
		...
		$ls_cod_pro=$ls_codigo_destino;  
		$ls_scgcta_proben=$this->uf_obtener_datos_proveedor($ls_codemp,$ls_cod_pro);
		...
		//}
		//print "aqui";
		$lb_valido=$this->uf_procesar_detalles_gastos_asignacion($as_codasi,$ls_mensaje,$ls_procede,$ls_descripcion,"PC",$ls_scgcta_proben);
		if($lb_valido) 
		{
			if($lb_valido)
			{
    ...
    function uf_procesar_anular_contabilizacion_contrato($as_codcon,$as_codasi,$adt_fecha_anula,$ad_fechaconta,$aa_seguridad)
	{
		...
		$ls_ctaban="-------------------------";
		$li_tipo_comp=1; // comprobante Normal
	    $lb_valido=$this->io_tepuy_int->uf_obtener_comprobante($ls_codemp,$ls_procede,$ls_comprobante,$ldt_fecha_cmp,$ls_codban,
		...
		$this->as_codban=$ls_codban;
		$this->as_ctaban=$ls_ctaban;
		$lb_valido = $this->io_tepuy_int->uf_int_anular($ls_codemp,$ls_procede,$ls_comprobante,$ldt_fecha_cmp,$ls_procede_anula,
														 $ldt_fecha_anula,$ls_descripcion,$ls_codban,$ls_ctaban,$li_tipo_comp);
		if(!$lb_valido)	
		{ 
			$this->io_msg->message("ERROR->".$this->io_tepuy_int->is_msg_error);
    ...
    function uf_procesar_reverso_anulacion_contrato($as_codcon,$as_codasi,$ad_fechaconta,$ad_fechaanula,$aa_seguridad)
	{
		...
		//	  Description: Este metodo tiene como fin anular la contabilizacion del contrato
		//	   Creado Por: Ing. 
		// Modificado Por: Ing. Miguel Palencia								Fecha Última Modificación : 02/05/2007
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    $ls_codemp=$this->io_codemp;
		$ls_comprobante=$this->io_tepuy_int->uf_fill_comprobante($as_codcon);
		$ls_procede="SOBACO";
...
/*
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_anulacion_asignacion_contrato($as_codasi,$adt_fecha,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_anulacion_asignacion_contrato
		//		   Access: public
		//	    Arguments: as_codasi  // Código de Asignacióna
		//	    		   adt_fecha  // Fecha de Anulación
		//	    		   aa_seguridad  // Arreglo de seguridad
		...
		$ls_ctaban="-------------------------";
		$li_tipo_comp=1; // comprobante Normal
		$lb_valido = $this->io_tepuy_int->uf_int_anular($ls_codemp,$ls_procede,$ls_comprobante,$ldt_fecasi,$ls_procede_anula,
		                                                 $ldt_fecha_anula,$ls_descripcion,$ls_codban,$ls_ctaban,$li_tipo_comp);
		if(!$lb_valido)	
		{ 
			$this->io_msg->message("Error->".$this->io_tepuy_int->is_msg_error);
		...
		return $lb_valido;
    }// end function uf_anulacion_asignacion_contrato
	//-----------------------------------------------------------------------------------------------------------------------------------

*/
	//-----------------------------------------------------------------------------------------------------------------------------------

	function uf_obtener_datos_proveedor($as_codemp,$as_cod_proveedor)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql="SELECT sc_cuenta_comp ".
		        "  FROM rpc_proveedor ".
        	        " WHERE codemp='".$as_codemp."' ".
				"   AND cod_pro='".$as_cod_proveedor."'";
		$rs_data=$this->io_sql->select($ls_sql);		
		if($rs_data===false)
		{
            $this->io_msg->message("CLASE->Integración CXP MÉTODO->uf_obtener_datos_proveedor_beneficiario ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));			
			return false;
		}
		else
		{                 
		...
		return $lb_existe;
	}// end function uf_obtener_datos_prov_bene
	//-----------------------------------------------------------------------------------------------------------------------------------
}
?>
