...
{	
  ...
  {	
	...
	if ($ps_operacion=="modificar")
	{
		...
		$lb_valido = $this->eliminarData( $po_object->id_art, $po_object->numart, $ls_fecvig);
		if ($lb_valido)
		{
  ...
  {	
	...
	$li_punto  = strpos($po_articulo->canmes,",");
	$li_canmes = substr($po_articulo->canmes,0,$li_punto);
