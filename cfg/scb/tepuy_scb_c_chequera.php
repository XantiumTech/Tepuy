...
class tepuy_scb_c_chequera
{
	...
	function tepuy_scb_c_chequera($aa_security)
	{
	...
	function uf_select_chequera($as_codemp,$as_codban,$as_ctaban,$as_chequera)
	{
		...
		if ($rs_data===false)
		   {
			 $lb_valido = false;
			 $this->is_msg_error = "Error:CLASS->tepuy_scb_c_chequera;Método:uf_select_chequera();.".$this->fun->uf_convertirmsg($this->io_sql->message);
		...
		else
		   {
			 if ($row=$this->io_sql->fetch_row($rs_data))
			 ...
			 else
			    {
				  $lb_valido=false;
				  $as_status=0;
				  $this->is_msg_error="No se encontraron registro !!!";
			    }
			 $this->io_sql->free_result($rs_data);
		}
		return $lb_valido;
	}//Function uf_select_cheques.
	...
	function uf_validar_cheque($as_codemp,$as_codban,$as_ctaban,$as_numche)
	{
		...
		if ($rs_data===false)
		   {
			 $lb_valido = false;
			 $this->is_msg_error = "Error:CLASS->tepuy_scb_c_chequera;Método:uf_validar_cheque();.".$this->fun->uf_convertirmsg($this->io_sql->message);
			 echo $this->io_sql->message;
		   }
		else
		   {
	...
	function uf_delete_cheques($as_chequera,$as_codban,$as_ctaban,$as_cheque)
	{
		...
		if ($lb_existe)
		   {
			   $ls_cadena = " DELETE FROM scb_cheques 
							   WHERE codemp='".$ls_codemp."' 
								 AND ctaban='".$as_ctaban."' 
								 AND codban='".$as_codban."' 
								 AND numchequera='".$as_chequera."'
			   ...
			   if ($rs_data===false)
			   ...
			   else
				  {
					...
					$this->io_seguridad->uf_sss_insert_eventos_ventana($this->is_empresa,$this->is_sistema,'DELETE',$this->is_logusr,$this->is_ventana,$ls_descripcion);
				  }
		   }
		return $lb_valido;
	}	
