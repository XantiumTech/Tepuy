<?php
	...
	function uf_insert_seguridad($as_titulo,$as_desnom,$as_periodo,$ai_tipo)
	{
		...
		if($ai_tipo==1)
		{
			$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte_nomina("SNO","tepuy_sno_r_recibopago.php",$ls_descripcion,$ls_codnom);
		}
		else
		{
			$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte_nomina("SNO","tepuy_sno_r_hrecibopago.php",$ls_descripcion,$ls_codnom);
		}
		return $lb_valido;
	}
	//--------------------------------------------------------------------------------------------------------------------------------
	...
	else // Imprimimos el reporte
	{
		...
		for($li_i=1;(($li_i<=$li_totrow)&&($lb_valido));$li_i++)
		{
			...
			$ls_unidad=$io_report->DS->data["minorguniadm"][$li_i].$io_report->DS->data["ofiuniadm"][$li_i].
					   $io_report->DS->data["uniuniadm"][$li_i].$io_report->DS->data["depuniadm"][$li_i].
					   $io_report->DS->data["prouniadm"][$li_i];
			$ls_desunidad=$io_report->DS->data["desuniadm"][$li_i];	   
