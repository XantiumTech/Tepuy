...
<form name="form1" method="post" action="">
  <p align="center">
	...
	{		
		...
		{
		 $ls_sql_seguridad = " AND CONCAT('".$ls_codemp."','SPG','".$ls_logusr."',codestpro1) IN (SELECT CONCAT(codemp,codsis,codusu,substr(codintper,1,26)) 
		                       FROM sss_permisos_internos WHERE codusu = '".$ls_logusr."' AND codsis = 'SPG')";
		}
		else
		{
		 $ls_sql_seguridad = " AND '".$ls_codemp."'||'SPG'||'".$ls_logusr."'||codestpro1 IN (SELECT codemp||codsis||codusu||substr(codintper,1,26)
		                       FROM sss_permisos_internos WHERE codusu = '".$ls_logusr."' AND codsis = 'SPG')";
		}
		
		$ls_sql=" SELECT codestpro2,denestpro2 ".
		        " FROM spg_ep2 ".
...
<script language="JavaScript">
  ...
  f.action="tepuy_cat_estpro2.php?tipo=<?php print $ls_tipo; ?>";
  f.submit();
  }
</script>
</html>
