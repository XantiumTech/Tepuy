...
//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_insert_seguridad($as_titulo)
	{
		...
		$lb_valido=$io_fun_srh->uf_load_seguridad_reporte("SRH","tepuy_srh_r_listado_concursante.php",$ls_descripcion);
		return $lb_valido;
	}
...
//---------------------------------------------------------------------------------------------------------------------------------
	...
	require_once("class_folder/tepuy_srh_class_report_2.php");
	$io_report=new tepuy_srh_class_report_2();
...
//---------------------------------------------------------------------------------------------------------------------------------
	...
	if($lb_valido)
	{
		...
		{       
			...
			$li_conact=0;
			$li_conexc=0;
			while (($rowcon=$io_report->io_sql->fetch_row($rs_datcon))&&($lb_valido))
		    {
		   		$ls_codigo=$rowcon["codcon"];	
				...
				$ls_codper=$rowcon["codper"];
				$ls_nombre= strtoupper(trim($rowcon["apeper"]." ".$rowcon["nomper"]));
				$ls_tipper=$rowcon["tipper"];
				if ($ls_tipper=='I')
				{
					$ls_tipper="INTERNO";
				}
