<?php
class tepuy_scv_c_rutas
{

	var $ls_sql;
	
	function tepuy_scv_c_rutas($conn)
	{
	...
	function uf_scv_load_rutas($as_codemp,$as_codrut,$as_codpaiori,$as_codestori,$as_codciuori,&$ai_totrows,&$ao_object) 
	{
		...
		$ls_sql=" SELECT scv_rutas.codpaides,scv_rutas.codestdes,scv_rutas.codciudes,".
				...
				" 	       WHERE scv_rutas.codpaides=tepuy_pais.codpai) AS despaides,".
				"        (SELECT  desest FROM tepuy_estados".
				"	       WHERE scv_rutas.codpaides=tepuy_estados.codpai".
				"	         AND   scv_rutas.codestdes=tepuy_estados.codest) AS desestdes,".
		...
		else
		{
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ao_object[$ai_totrows][4]="<a href=javascript:uf_delete_dt(".$ai_totrows.");><img src=../shared/imagebank/tools15/eliminar.png alt=Eliminar width=15 height=15 border=0></a>";
			}
	...
	function uf_scv_select_rutas($as_codemp,$as_codrut,$as_codpaiori,$as_codestori,$as_codciuori,$as_codpaides,$as_codestdes,$as_codciudes) 
	{
		...
		if ($rs_data===false)
		{
			...
			$this->io_msg->message("CLASE->tepuy_scv_c_rutas METODO->uf_scv_select_rutas ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
	...
	function uf_scv_insert_rutas($as_codemp,$as_codrut,$as_codpaiori,$as_codestori,$as_codciuori,$as_codpaides,$as_codestdes,
								 $as_codciudes,$as_desrut,$aa_seguridad) 
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_scv_update_rutas($as_codemp,$as_codrut,$as_desrut,$aa_seguridad) 
	{
		...
		if ($li_row===false)
		{
			$this->io_msg->message("CLASE->tepuy_scv_c_rutas METODO->uf_scv_update_rutas ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_scv_delete_destinos($as_codemp,$as_codrut,$as_codpaiori,$as_codestori,$as_codciuori,$as_codpaides,$as_codestdes,$as_codciudes,$aa_seguridad) 
	{
		...
		if(!$lb_existe)
		{
			...
			{
				$this->io_sql->rollback();
				$this->io_msg->message("CLASE->tepuy_scv_c_rutas METODO->uf_scv_delete_destinos ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			}
			else
			{
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_scv_delete_ruta($as_codemp,$as_codrut,$aa_seguridad) 
	{
		...
		if(!$lb_existe)
		{
			...
			{
				$this->io_sql->rollback();
				$this->io_msg->message("CLASE->tepuy_scv_c_rutas METODO->uf_scv_delete_ruta ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			}
			else
			{
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_check_existe($as_codemp,$as_codrut)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->tepuy_scv_c_rutas METODO->uf_check_existe; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
