...
class tepuy_saf_c_movimiento
{
	...
	function tepuy_saf_c_movimiento()
	{
		...
		$this->io_activo = new tepuy_saf_c_activo();
	...
	function uf_saf_select_activo($as_codemp,$as_codact,&$ai_monact)
	{
		...
		//         Access: public (tepuy_siv_p_traslado)
		//      Argumento: $as_codemp //codigo de empresa 
	...
	function uf_siv_load_dt_movimiento($as_codemp,$as_cmpmov,$ad_feccmp,&$ai_totrows,&$ao_object,&$ai_montot)
	{
		...
		else
		{
			...
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ao_object[$ai_totrows][5]="<a href=javascript:uf_delete_dt(".$ai_totrows.");><img src=../shared/imagebank/tools15/eliminar.png alt=Aceptar width=15 height=15 border=0></a>";
	...
	function uf_siv_load_dt_movreasignacion($as_codemp,$as_cmpmov,$ad_feccmp,&$ai_totrows,&$ao_object,&$ai_montot)
	{
		...
		$ls_sql="SELECT saf_dt_traslado.*, saf_dt_movimiento.desmov,saf_dt_movimiento.monact".
				...
				" AND saf_dt_movimiento.feccmp='". $ad_feccmp ." 00:00:00'".
				" ORDER BY codact";
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		{
		...
		else
		{
			...
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ao_object[$ai_totrows][9]="<a href=javascript:uf_delete_dt(".$ai_totrows.");><img src=../shared/imagebank/tools15/eliminar.png alt=Aceptar width=15 height=15 border=0></a>";
	...
	function uf_saf_load_activos($as_codemp,&$ai_totrows,&$ao_object)
	{
		...
		//       Creado por:  Ing. Luis Anibal Lang           
		// Fecha de Cracion:   11/04/2006							Fecha de Ultima Modificación: 11/04/2006	
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////		
		$lb_valido=true;
		$ls_sql=" SELECT saf_activo.*,saf_dta.seract,saf_dta.ideact".
				...
				" ORDER BY saf_dta.codact ASC"; //print $ls_sql;   
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		{
			$lb_valido=false;
		...
		else
		{
			...
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ao_object[$ai_totrows][1]="<div align=center><img src=../shared/imagebank/tools15/aprobado.png width=15 height=15  onClick='javascript: ue_agregar(".$ai_totrows.");'></div>";
...
/*		$ls_sql="SELECT saf_dta.*,saf_activo.spg_cuenta_act,saf_activo.costo,".
				"       spg_unidadadministrativa.codestpro1 as codestpro1,".
				"       spg_unidadadministrativa.codestpro2 as codestpro2,".
				"       spg_unidadadministrativa.codestpro3 as codestpro3,".
				"       spg_unidadadministrativa.codestpro4 as codestpro4,".
				"       spg_unidadadministrativa.codestpro5 as codestpro5,".
				...
				"           AND   spg_cuentas.codemp=saf_dta.codemp".
				"           AND   spg_cuentas.spg_cuenta=saf_activo.spg_cuenta_act) as sc_cuenta".
				"  FROM  saf_dta,saf_activo,spg_unidadadministrativa".
				...
				" GROUP BY codact";
*/
		$ls_sql="SELECT saf_dta.codemp,saf_dta.codact,saf_dta.ideact,saf_dta.coduniadm, ".
				"       saf_activo.spg_cuenta_act,saf_activo.costo,".
				"       spg_unidadadministrativa.codestpro1 as codestpro1,".
				"       spg_unidadadministrativa.codestpro2 as codestpro2,".
				"       spg_unidadadministrativa.codestpro3 as codestpro3,".
				"       spg_unidadadministrativa.codestpro4 as codestpro4,".
				...
				"   AND saf_dta.codact='".$as_codact."'" .
				"   AND saf_dta.ideact='".$as_ideact."'" .
				" GROUP BY saf_dta.codemp,saf_dta.codact,saf_dta.ideact,saf_dta.coduniadm, ".
				"          saf_activo.spg_cuenta_act,saf_activo.costo,".
				"          spg_unidadadministrativa.codestpro1,".
				"          spg_unidadadministrativa.codestpro2,".
				"          spg_unidadadministrativa.codestpro3,".
				"          spg_unidadadministrativa.codestpro4,".
	...
	//	print $ls_sql;
		...
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				if($as_sccuenta=="")
				{
					$this->io_msg->message("No existe cuenta contable asociada a la estructura presupuestaria a la que esta asociado el activo");
				}
			}
			$this->io_sql->free_result($rs_data);
		}
	...
	function uf_siv_load_dt_movimientocontable($as_codemp,$as_cmpmov,$ad_feccmp,$as_codcau,&$ai_totrowsscg,&$ao_objectscg,&$ai_totdeb,&$ai_tothab)
	{
		...
		else
		{
			...
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ao_objectscg[$ai_totrowsscg][5] = "<a href=javascript:uf_delete_scg('".$ai_totrowsscg."');><img src=../shared/imagebank/tools15/eliminar.png alt='Eliminar detalle contable' width=15 height=15 border=0></a>";
	...
	function uf_saf_select_activoreasignacion($as_codemp,$as_codact,$as_seract,$as_ideact,&$as_codres,&$as_nomres,&$as_coduniadm,&$as_denuniadm,&$ai_monact,&$as_sccuenta)
	{
		...
		$ls_sql="SELECT saf_dta.*,saf_activo.spg_cuenta_act,saf_activo.costo,".
				...
				"       spg_unidadadministrativa.codestpro5 as codestpro5,".
				"       (SELECT sc_cuenta FROM spg_cuentas".
				"         WHERE spg_cuentas.codemp=spg_unidadadministrativa.codemp".
				"           AND spg_cuentas.codestpro1=spg_unidadadministrativa.codestpro1".
				"           AND spg_cuentas.codestpro2=spg_unidadadministrativa.codestpro2".
				"           AND spg_cuentas.codestpro3=spg_unidadadministrativa.codestpro3".
				"           AND spg_cuentas.codestpro4=spg_unidadadministrativa.codestpro4".
				"           AND spg_cuentas.codestpro5=spg_unidadadministrativa.codestpro5".
	...
	function uf_saf_load_dt_modificacion($as_codemp,$as_cmpmov,$as_codact,$as_ideact,&$ai_totrows,&$ao_object,&$ai_montot)
	{
		...
		else
		{
			...
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ao_object[$ai_totrows][6]="<a href=javascript:uf_delete_dt(".$ai_totrows.");><img src=../shared/imagebank/tools15/eliminar.png alt=Aceptar width=15 height=15 border=0></a>";
...
//---------------------------------------------------------------------------------------------------------------------------------
	function uf_saf_select_activos_contabilizado($as_codemp,$adt_fecdep,$as_codact)
	{
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		$ls_sql=" SELECT count(codact) as codact ".
                " FROM   saf_depreciacion ".
                " WHERE  codemp='".$as_codemp."' AND  fecdep>='".$adt_fecdep."' AND codact='".$as_codact."' AND estcon='1'";		
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->tepuy_saf_c_movimiento 
...
//---------------------------------------------------------------------------------------------------------------------------------
	function uf_saf_guardar_en_lote($as_codemp,$as_cmpmov,$as_codcau,$ad_feccmpbd,$as_descmp,$as_codpro,$as_cedbene,
	                                $as_codtipdoc,$as_codusureg,$as_estpromov,$aa_seguridad)
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$lb_valido=$this->uf_saf_insert_movimento($as_codemp,$as_cmpmov,$as_codcau,$ad_feccmpbd,$as_descmp,$as_codpro,$as_cedbene,$as_codtipdoc,$as_codusureg,$as_estpromov,$aa_seguridad);
		if($lb_valido)
		{
			...
			while($row=$this->io_sql->fetch_row($rs_data))	  
			{
				...
				$lb_valido=$this->uf_saf_insert_dt_movimiento($as_codemp,$as_cmpmov,$as_codcau,$ad_feccmpbd,$ls_codact,
				                                              $ls_ideact,$as_descmp,$ld_monact,$ls_estsoc,$ls_estmov,
															  $aa_seguridad,$ls_codrespri,$ls_codresuso,$ls_coduniadm,
															  $ls_ubigeo,$ls_tiprespri,$ls_tipresuso,$ls_fecent);
				if($lb_valido)
				{
					$ls_estact="I";
  ...
  function uf_saf_load_bienes_unidadcedente($as_codemp,&$ai_totrows,&$ao_object,$as_coduniadmcede)
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	       Function:  uf_saf_load_bienes_unidadcedente
		//           Access:  public 
		//	     Argumentos:  $as_codemp   // codigo de empresa
		//  		          $ai_totrows  // total de lineas del grid
		//  		          $ao_object   // arreglo de objetos
		//                    $as_coduniadmcede  // codigo de la unidadd cedente
		//	        Returns:  Retorna un Booleano
		//	    Description:  Funcion que busca los activos existentes en la unidad cedente en la tabla saf_activos y se trae el 
		//                    resultado de la busqueda
		//       Creado por:  Ing. Gloriely Fréitez          
		// Fecha de Cracion:  21/04/2008							Fecha de Ultima Modificación: 
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////		
		$lb_valido=true;
		$ls_sql=" SELECT saf_activo.*,saf_dta.seract,saf_dta.ideact".
				"   FROM saf_activo,saf_dta ".
				" WHERE saf_activo.codemp='". $as_codemp ."'".
				"   AND saf_dta.coduniadm='".$as_coduniadmcede."'".
				"   AND saf_activo.codact=saf_dta.codact".
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->movimiento MÉTODO->uf_saf_load_bienes_unidadcedente ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$ai_totrows=0;
		...
		$this->io_sql->free_result($rs_data);
		return $lb_valido;
	} // end function uf_saf_load_bienes_unidadcedente
 //-----------------------------------------------------------------------------------------------------------------------------------
	                                  ...
	                                  $as_codreserec,$as_codper,$as_estpres,$aa_seguridad)
	{
		...
		else
		{
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insertó el préstamo".cmppre."de la unidad cedente".$as_codunicede." a la unidad receptora".$as_codunirece." asociado a la Empresa ".$as_codemp;
				$lb_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
 ...
 //-----------------------------------------------------------------------------------------------------------------------------------
	function uf_saf_insert_dt_prestamo($as_codemp,$as_cmpres,$ad_fecenacta,$as_coduniadmcede,
					                   $as_coduniadmrece,$as_codact,$as_idact,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		else
		{
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insertó detalle del prestamo".$as_cmppre." ".
								 " de la unidad cedente".$as_codunicede." a la unidad receptora ".$as_codunirece." asociado a la Empresa ".$as_codemp;
				$lb_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
		...
		return $lb_valido;
	}// end function uf_saf_buscareponsable
//---------------------------------------------------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------------------------------------------------------
	                                        ...
	                                        $as_coduniadmrece,$ad_fecenacta,$as_estpres,$aa_seguridad) 
	{
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql = "UPDATE saf_prestamo SET  estpropre='". $as_estpres ."'".
					" WHERE codemp='" . $as_codemp ."'".
					" AND cmppre='" . $as_cmpres ."'".
...
function uf_saf_update_saf_dta($as_codemp,$as_codact,$as_coduniadmcede,$as_idact,$as_estactpre,
                               $ad_fecenacta,$as_coduniadmrece,$aa_seguridad)
	{
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql = "UPDATE saf_dta SET   estactpre='". $as_estactpre ."' , codunipre='".$as_coduniadmrece."'".
					" WHERE codemp='" .$as_codemp."'".
					" AND codact='" .$as_codact."'".
					" AND ideact='" .$as_idact."'".
					" AND coduniadm='" .$as_coduniadmcede."'";
		//print "actualiza saf_dta".$ls_sql."<br>";
		$li_row = $this->io_sql->select($ls_sql);
		if($li_row===false)
		{
	                                      ...
	                                      $ad_fecent,$ad_fecdevo,$as_estauto,$as_concepto,$as_obser,$aa_seguridad)
	{
       ...
       // print "cabecera".$ls_sql."<br><br>";
		...
		else
		{
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insertó la autorización de salida".$as_cmpsal."de la unidad cedente".$as_codunicede." a la empresa".$as_codprov." asociado a la Empresa ".$as_codemp;
				$lb_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
...
//---------------------------------------------------------------------------------------------------------------------------------
	                                      ...
	                                      $as_codact,$as_idact,$aa_seguridad)
	{
		...
		else
		{
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="INSERT";
				$ls_descripcion ="Insertó el detalle de la autorización de salida".$as_cmpsal."de la unidad cedente".$as_codunicede." a la empresa".$as_codprov." asociado a la Empresa ".$as_codemp;
				$lb_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
...
function uf_saf_select_prestamo($as_codemp,$as_cmpres,$ad_fecenacta,$as_coduniadmcede,$as_coduniadmrece)
	{
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		$ls_sql="SELECT * FROM saf_prestamo".
