<?php
class tepuy_spi_c_mod_presupuestarias
{
	var $is_msg_error;
	var $io_sql;
	var $io_include;
	...
	var $is_descripcion;
	var $is_tipo;
function tepuy_spi_c_mod_presupuestarias()
{
	...
	$this->io_int_spi=new class_tepuy_int_spi();	
...
function uf_generar_num_cmp($as_codemp,$as_procede)
{
   ...
   ///////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////
	function uf_tepuy_insert_comprobante($as_codemp,$as_procede,$as_comprobante,$as_fecha,$ai_tipo_comp,$as_descripcion,$as_tipo,$as_cod_prov,$as_ced_ben)
	{
		$lb_valido=true;
		$ls_fec=$this->io_function->uf_convertirdatetobd($as_fecha);
		$ls_sql = " INSERT INTO tepuy_cmp_md (codemp,procede,comprobante,fecha,descripcion,tipo_comp,tipo_destino,cod_pro, ".
		...
		else
		{
				...
				$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("tepuy_cmp_md",$this->li_candeccon,$this->li_tipconmon,$this->li_redconmon,"");
	...
	function uf_tepuy_update_comprobante($as_codemp,$as_procede,$as_comprobante,$as_fecha,$ai_tipo_comp,$as_descripcion,$as_tipo,$as_cod_prov,$as_ced_ben,$li_estapro)
    {		
		$lb_valido=true;
		$ls_fecha=$this->io_function->uf_convertirdatetobd($as_fecha);
		
		$ls_sql = " UPDATE tepuy_cmp_md SET descripcion='".$as_descripcion."' ,estapro=".$li_estapro.
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////
	function uf_tepuy_comprobante($as_codemp,$as_procedencia,$as_comprobante,$as_fecha,$ai_tipo_comp,$as_descripcion,$as_tipo,$as_cod_pro,$as_ced_bene,$adec_monto,$li_estapro)
	{
		...
		{	
           ...
           $lb_valido=$this->uf_tepuy_update_comprobante($as_codemp,$as_procedencia,$as_comprobante,$as_fecha,$ai_tipo_comp,$as_descripcion,$as_tipo,$this->is_cod_prov,$this->is_ced_ben,$li_estapro);
		}
		else
		{
		   $this->ib_new_comprobante=true;		
		   $lb_valido=$this->uf_tepuy_insert_comprobante($as_codemp,$as_procedencia,$as_comprobante,$as_fecha,$ai_tipo_comp,$as_descripcion,$as_tipo,$this->is_cod_prov,$this->is_ced_ben);
		}
		return $lb_valido;
...
function uf_guardar_automatico($as_comprobante,$ad_fecha,$as_proccomp,$as_desccomp,$as_prov,$as_bene,$as_tipo,$ai_tipo_comp,$li_estapro)
{
	...
	{	
	   $lb_valido=$this->uf_tepuy_comprobante($dat["codemp"],$as_proccomp,$as_comprobante,$ad_fecha,$ai_tipo_comp,$as_desccomp,$as_tipo,$as_prov,$as_bene,0,$li_estapro);
	   if (!$lb_valido)
	   {
...
function uf_cargar_dt_comprobante($as_codemp,$as_procede,$as_comprobante,$adt_fecha)
{
	...
	$ld_fecha=$this->io_function->uf_convertirdatetobd($adt_fecha);
 	$ls_sql=" SELECT DISTINCT DT.spi_cuenta as spi_cuenta,C.denominacion as denominacion,DT.procede_doc as procede_doc, ".
			...
			"				  DT.descripcion as descripcion,DT.monto as monto,DT.orden as orden, OP.denominacion as denominacion ".
  		    " FROM spi_dtmp_cmp DT,spi_cuentas C, tepuy_procedencias P,spi_operaciones OP ".
		    " WHERE DT.procede=P.procede AND DT.codemp=C.codemp AND DT.spi_cuenta=C.spi_cuenta AND OP.operacion = DT.operacion  ".
			"       AND  DT.codemp='".$as_codemp."' AND DT.procede='".$as_procede."' AND DT.comprobante='".$as_comprobante."' AND ".
			"       DT.fecha='".$ld_fecha."' ORDER BY DT.orden "; 
                                ...
                                $ls_documento,$ls_operacionpre,$ldec_monto_ant,$ldec_monto_act,$ls_tipocomp)
{
	...
	if($ls_mensaje!="")
	{
		...
		if($lb_valido)
		{
		    ...
		    if($this->uf_spi_select_movimiento( $ls_cuenta, $ls_procede_doc, $ls_documento, $ls_operacion, $lo_monto_movimiento,$lo_orden))  
		    {
			...
			if($lb_valido)
			{
				$lb_valido = $this->uf_insert_movimiento_spi($ls_cuenta,$ls_procede_doc,$ls_documento,$ls_operacion,$ls_descripcion,$ldec_monto_act);
				if($lb_valido)
				{
