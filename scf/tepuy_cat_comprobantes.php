      ...
      <tr>
        ...
        <td colspan="2"><div align="left">
          <input name="txtdocumento" type="text" id="txtdocumento" onBlur="javascript: rellenar_cad(document.form1.txtdocumento.value,15,'doc');">        
            ...
            <td align="left"><input name="txtfechadesde" type="text" id="txtfechadesde" style="text-align:center" onKeyPress="javascript: ue_formatofecha(this,'/',patron,true);" datepicker="true" size="15" maxlength="10" ></td>
      </tr>
      <tr>
        ...
        <td width="126" align="left">
          ...
          <a href="javascript:catprovbene(document.form1.tipo.value)"><img src="../shared/imagebank/tools15/buscar.png" alt="Catalogo Proveedores/Beneficiarios" width="15" height="15" border="0"></a>		</td>
        ...
        <td width="82" align="left"><input name="txtfechahasta" type="text" id="txtfechahasta" size="15" maxlength="10" style="text-align:center" onKeyPress="javascript: ue_formatofecha(this,'/',patron,true);" datepicker="true"> </td>
      </tr>
      <tr>
...
function uf_cargar_procedencias($sql)
{
	...
	$ls_sql="SELECT procede, desproc FROM tepuy_procedencias";
...
if($ls_operacion=="BUSCAR")
{
		...
		if($_SESSION["ls_gestor"]=='MYSQL')
		{
			$ls_sql="SELECT   ( CASE   cmp.tipo_destino
								...
								ELSE 'Ninguno'
								END )  as  nombre,cmp.codemp,cmp.procede,cmp.comprobante,cmp.descripcion,
								cmp.fecha,cmp.cod_pro,cmp.ced_bene,cmp.tipo_destino,cmp.codban, cmp.ctaban
					FROM tepuy_cmp cmp,rpc_beneficiario xbf,rpc_proveedor prv
					WHERE cmp.codemp=prv.codemp 
					  AND cmp.cod_pro=prv.cod_pro 
					  AND cmp.codemp=xbf.codemp
					  AND cmp.ced_bene=xbf.ced_bene 
					  AND cmp.codemp='".$as_codemp."' 
					  AND tipo_comp=1 
					  AND comprobante like '%".$ls_comprobante."%' 
					  AND (cmp.codemp  IN  (SELECT cmp.codemp 
											 FROM scg_dt_cmp 
											WHERE scg_dt_cmp.codemp=cmp.codemp
											  ...
											  AND scg_dt_cmp.ctaban=cmp.ctaban )
					OR cmp.procede='SCGCMP')";	
...
//				$ls_fechasta=substr($ls_fechasta,6,4)."/".substr($ls_fechasta,3,2)."/".substr($ls_fechasta,0,2);
		...
		{
			$ls_sql="SELECT ( CASE WHEN cmp.tipo_destino='P' THEN prv.nompro 
							  WHEN cmp.tipo_destino='B' THEN RTRIM(xbf.apebene)||','||xbf.nombene
							  ELSE 'Ninguno' END ) as nombre,cmp.codemp,cmp.procede,cmp.comprobante,
							  cmp.descripcion,cmp.fecha,cmp.cod_pro,cmp.codban, cmp.ctaban,
		...
		$li_total=$SQL_cmp->num_rows($rs_cta);
		if($li_total>=0)
		{
			while($row=$SQL_cmp->fetch_row($rs_cta))
			{
					...
					$ls_ctaban=$row["ctaban"];
						...
						if($ls_tipocat=="")
						{
							...
							print "<td><a href=\"javascript: uf_aceptar('$ls_comprobante','$ls_descripcion','$ls_procedencia','$ls_fecha','$ls_tip','$ls_prov','$ls_bene','$ls_nomproben','$ls_codban','$ls_ctaban');\">".$ls_comprobante."</a></td>";
...
var patron2 = new Array(1,3,3,3,3);
  ...
  function uf_aceptar(comprobante,descripcion,procede,fecha,tipo,prov,bene,ls_nomproben,codban,ctaban)
  {
   ...
   function uf_aceptar_repcompdes(comprobante,descripcion,procede,fecha,tipo,prov,bene,ls_provbene)
   {
    ...
    function ue_search()
    {
	  ...
	  f.action="tepuy_cat_comprobantes.php?tipocat=<? print $ls_tipocat; ?>";
	  f.submit();
    }
	function catprovbene(provbene)
	{
		...
		if(provbene=="P")
		{
			...
			window.open("tepuy_catdinamic_prov.php","catalogo","_blank","menubar=no,toolbar=no,scrollbars=yes,width=530,height=400,resizable=yes,location=no");
		}
		else if(provbene=="B")
		{
			f.txtprovbene.disabled=false;	
			window.open("tepuy_catdinamic_bene.php","catalogo","_blank","menubar=no,toolbar=no,scrollbars=yes,width=530,height=400,resizable=yes,location=no");
		}
	}
