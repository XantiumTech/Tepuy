...
class scb_report_conciliacion
{
	...
	function uf_obtener_mov_conciliacion($ls_mesano,$ls_codban,$ls_ctaban,$ldec_salseglib,$ldec_salsegbco)
	{
		...
		if (abs($ldec_saldo_ant-$ldec_salseglib)>0.01)
		   {
			 $this->io_msg->message("Vuelva a modulo conciliación ya que hay movimientos no registrados Anterior = $ldec_saldo_ant ; Según Libro = $ldec_salseglib");
			 return false;
		   } 		
	...
	//	Return	    :  ldec_saldo
	//
	//	Descripcion :  Funcion que se encarga de obtener el saldo de los movimientos registrados en libro
	///////////////////////////////////////////////////////////////////////////// 
...
function uf_select_denctaban($as_codemp,$as_codban,$as_ctaban)
{
  $ls_denctaban="";
  $ls_sql = "SELECT dencta 
               FROM scb_ctabanco 
  ...
  if ($rs_data===false)
     {
	   print "ClASS->scb_report_conciliacion.php;Método->uf_select_denctaban;ERROR->".$this->SQL->message;
	 }
  else
     {
