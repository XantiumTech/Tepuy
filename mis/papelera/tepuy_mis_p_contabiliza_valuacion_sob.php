<?Php
   ...
   $in_class_contabiliza->uf_load_seguridad("MIS","tepuy_mis_p_contabiliza_valuacion_sob.php",$ls_permisos,$la_seguridad,$la_permisos);
?>
  ...
  {  
		...
		for($li_i=1;$li_i<=$li_total_record;$li_i++)
		{
			if(array_key_exists("chksel".$li_i,$_POST))
			{
				...
				$ls_obsval   = $_POST["txtobsval".$li_i];
				$lb_valido = $in_class_int->uf_procesar_contabilizar_valuacion( $ls_codcon, $ls_codval , $ls_fecha);
				if ($lb_valido) { $in_class_int->io_msg->message("La Valuación Nº ".$ls_codval." fue contabilizado"); }
				else { $in_class_int->io_msg->message("Error en la contabilización de la Valuación Nº ".$ls_codval ); }		
			}
		}
  }
 ...
 ?>	
  ...
  <table width="502" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Contabilizaci&oacute;n de Valuaci&oacute;n de Obras </td>
    </tr>
    <tr>
    ...
    <tr>
	  ...
	  <?php
	     $title[1]="";$title[2]="Nº Contrato";$title[3]="Nº Valuación";$title[4]="Fecha ";$title[5]="Observación"; 
	     $in_class_contabiliza->uf_select_sob_valuacion_contabilizar( $arr_object ,$li_total_record,0);
		 $io_grid->makegrid($li_total_record,$title,$arr_object,500,"Valuaciones Emitidas","grdsep" );
	  ?>
	  </div></td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		f.action="tepuy_mis_p_contabiliza_valuacion_sob.php";
		f.submit();
	}
	else
	{
