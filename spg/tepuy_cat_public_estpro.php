...
if($ls_operacion=="BUSCAR")
{
	...
	{
     $ls_sql_seguridad = " AND CONCAT('".$ls_codemp."','SPG','".$ls_logusr."',a.codestpro1,b.codestpro2,c.codestpro3) IN (SELECT CONCAT(codemp,codsis,codusu,substr(codintper,1,29)) 
		                       FROM sss_permisos_internos WHERE codusu = '".$ls_logusr."' AND codsis = 'SPG')";
	}
	else
	{
	 $ls_sql_seguridad = " AND '".$ls_codemp."'||'SPG'||'".$ls_logusr."'||a.codestpro1||b.codestpro2||c.codestpro3 IN (SELECT codemp||codsis||codusu||substr(codintper,1,29)
		                       FROM sss_permisos_internos WHERE codusu = '".$ls_logusr."' AND codsis = 'SPG')";
	}
	...
	$ls_sql="SELECT a.codestpro1 as codestpro1,a.denestpro1 as denestpro1,b.codestpro2 as codestpro2,".
			...
			" AND c.codestpro3 like '".$ls_codigo."' AND c.denestpro3 like '".$ls_denominacion."'".$ls_sql_seguridad."".
			" ORDER BY a.codestpro1, b.codestpro2, c.codestpro3 ";
...
//	opener.document.form1.submit();
  ...
  f.action="tepuy_cat_public_estpro.php?tipo=<?php print $ls_tipo; ?>";
  f.submit();
  }
</script>
</html>
