<?php
							  ...
							  $as_telefono,$ai_sueper,$as_denestpro1,$as_denestpro2,$as_desrut,$ai_solviaext,$as_obssolvia,&$io_pdf)
	{
		...
		$la_columna=array('nombre'=>'<b>Nombre del Funcionario</b>','cedula'=>'<b>Cédula de Identidad N°</b>',
						  'cargo'=>'<b>Denominación del Cargo</b>','unidad'=>'<b>Sueldo</b>','categoria'=>'<b>Telefono Oficina / Celular</b>');
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('estpro1'=>'<b>Proyecto / Acción Centralizada:</b> '.$as_denestpro1,'estpro2'=>'<b>Acción Específica:</b> '.$as_denestpro2));
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('desde'=>'<b>Tipo de Viático:            Nacional    '.$ls_extno.'               Exterior    '.$ls_extsi.'</b> ','dias'=>'<b>Pernocta:                 Si '.$ls_pernoctasi.'            No '.$ls_pernoctano.'</b> '));
		...
		unset($la_columna);
		unset($la_config);
		$la_data[1]=array('desde'=>'<b>Vehiculo Oficial:</b>           Si      No','hasta'=>'<b>Pasaje Terrestre</b>            Si      No','dias'=>'<b>Vehiculo Propio</b>          Si      No');
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('titulo'=>'Pasaje Aéreo                       Si         No                                  Ruta:'));
		$la_columna=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('titulo'=>'Código:                              Nacional  4.03.09.01.00     <b>'.$ls_extno.'</b>                       Extranjero   4.03.09.02.00     <b>'.$ls_extsi.'</b>'));
		$la_columna=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
	...
	function uf_print_firmas(&$io_pdf)
	{
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('elaborado'=>'<b>Solicitado Por:</b>','revisado'=>'<b>Autorizado por:</b>','autorizado'=>'<b>Oficina de Administración y Servicios:</b>'),
	...
	$lb_tipben="P";
	//--------------------------------------------------------------------------------------------------------------------------------
	$lb_valido=$io_report->uf_select_solicitudpago_personal($ls_codemp,$ls_codsoldes,$ls_codsolhas,$ld_desde,$ld_hasta,$li_orden,$ls_codsolvia,"0"); // Cargar el DS con los datos de la cabecera del reporte
	if(!$lb_valido)
	{
