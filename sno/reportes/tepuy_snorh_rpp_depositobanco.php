<?PHP
	...
	function uf_insert_seguridad($as_titulo,$as_desnom,$as_periodo)
	{
		...
		$lb_valido=$io_fun_nomina->uf_load_seguridad_reporte("SNR","tepuy_snorh_r_depositobanco.php",$ls_descripcion);
		return $lb_valido;
	}		
	//--------------------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------------------
  //-----------------------------------------------------  Instancia de las clases  ------------------------------------------------
	...
	else // Imprimimos el reporte
	{
		...
		$li_totrow=$io_report->DS->getRowCount("codnom");
		$ls_nomina_aux= $ls_nomina=$io_report->DS->data["codnom"][1];	;
		$ls_total_mes=0;
		for ($li_i=1;$li_i<=$li_totrow;$li_i++)
		{
		   $ls_nomina=$io_report->DS->data["codnom"][$li_i];	
		   $ls_desnom=$io_report->DS->data["desnom"][$li_i];
		   $ls_banco=$io_report->DS->data["codban"][$li_i];
		   $ls_cuenta=$io_report->DS->data["codcueban"][$li_i];
		   $ls_desban=$io_report->DS->data["nomban"][$li_i];		   
		   $lb_valido=$io_report->uf_depositos_bancarios($ls_nomina,$ls_banco, $ls_des_periodo, $ls_dhas_periodo,$ls_cuenta);
		   $li_depositos=$io_report->DS_detalle->getRowCount("codnom");
		   $ls_monto_mes=0;	
		   if ($ls_nomina_aux!=$ls_nomina)
		     {
				$ls_nomina_aux=$ls_nomina;
				uf_total_mes_nomina($io_report->DS->data["desnom"][$li_i-1],
				                    $io_fun_nomina->uf_formatonumerico($ls_total_mes), &$io_pdf);		   
				$ls_total_mes=0;
			 } 		    
		        for ($li_j=1;$li_j<=$li_depositos;$li_j++)
				{				    
				    $ls_momto_neto_ahorro=$io_report->DS_detalle->data["monnetresahorro"][$li_j];	
				    $ls_priquin_ahorro=$io_report->DS_detalle->data["priquiresahorro"][$li_j];	
				    $ls_segquin_ahorro=$io_report->DS_detalle->data["segquiresahorro"][$li_j];
				    $ls_momto_neto_corriente=$io_report->DS_detalle->data["monnetrescorriente"][$li_j];	
				    $ls_priquin_corriente=$io_report->DS_detalle->data["priquirescorriente"][$li_j];	
				    $ls_segquin_corriente=$io_report->DS_detalle->data["segquirescorriente"][$li_j];
		...
		$lb_valido=$io_report->uf_total_depositos_bancarios($ls_codnomdes,$ls_codnomhas,$ls_bancdes,
			...
			if($lb_valido)
			{
			  $li_bancos=$io_report->DS_depositos->getRowCount("codban");
			  $io_pdf->ezSetDy(-5);
			  $total_nomina=0;
			  for ($li=1;$li<=$li_bancos;$li++)
				{   
					...
					$priquin=0;
					$segquin=0;
				    $ls_priquinA=$io_report->DS_depositos->data["priquiresahorro"][$li];	
				    $ls_segquinA=$io_report->DS_depositos->data["segquiresahorro"][$li];				   	
				    $ls_priquinC=$io_report->DS_depositos->data["priquirescorriente"][$li];	
				    $ls_segquinC=$io_report->DS_depositos->data["segquirescorriente"][$li];
					$ls_codban=$io_report->DS_depositos->data["codban"][$li];
					$ls_momto_netoA=$io_report->DS_depositos->data["monnetresahorro"][$li];
					$ls_momto_netoC=$io_report->DS_depositos->data["monnetrescorriente"][$li];	
