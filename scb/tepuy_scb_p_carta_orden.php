							 ...
							 if ($ls_procede=='SCVSOV')
								{
								  ...
								  $aa_seguridad["ventanas"]="tepuy_sno_r_listadobanco.php";
										...
										if(array_key_exists("codestpro1",$in_class_carta->ds_sol->data))
										{
											...
											for($li_y=1;$li_y<=$li_total_rows;$li_y++)
											{
												...
												if($lb_valido)
												{
													...
													$lb_valido=$in_classmovbanco->uf_procesar_dt_gasto($ls_codban,$ls_cuenta_banco,$ls_numcarord,'ND',$ls_estmov,$ls_programa,$ls_cuentaspg,$ls_numsol,$ls_desmov,'CXPSOP',$ldec_MontoSpgDet,'PG');
												}//End if								
											}//End for							
										}							
								}//End cuarto if
	  ...
	  $lb_valido = $io_metodobanco->uf_metodo_banco($ls_ruta,$ls_nommetban,'','','',$ld_fecha,$ldec_montotot,$ls_ctaban,$ds_banco_nomina,$ls_metban,$ls_consol,'',$la_seguridad);
		...
		if($lb_valido)
		{
			...
			if($ls_codigo!="000")//distinto de chequevoucher
				$ls_pagina="reportes/tepuy_scb_rpp_cartaorden_pdf.php?codigo=$ls_codigo&codban=$ls_codban&ctaban=$ls_cuenta_banco&numdoc=$ls_numcarord&chevau=&codope=ND&tipproben=$ls_tipo";
			else
				$ls_pagina="reportes/tepuy_scb_rpp_voucher_pdf.php?codban=$ls_codban&ctaban=$ls_cuenta_banco&numdoc=$ls_numcarord&chevau=&codope=ND";			
			?>
			<script language="javascript">						
...
<script language="javascript">
	...
	f.action="tepuy_scb_p_carta_orden.php";
	f.submit();
	}
...
{
  window.open("tepuy_scb_cat_directorio.php?ruta="+ruta+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=530,height=400,left=50,top=50,location=no,resizable=no");
}	
</script>
