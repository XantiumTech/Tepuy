<?PHP
	...
	//------------------------------------------------------------------------------------------------------
	//---------------------------------------------------------------------------------------------------------------
	function uf_print_encabezado_pagina($as_numanacot,$as_tipsolcot,$observacion,$ad_fecha,$as_concepto,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->add_texto(90,20,11,"<b>Análisis de Cotizaciones</b>");
		$io_pdf->add_texto(190,18,10,"$ad_fecha");
		$ls_institucion=ucfirst(trim($_SESSION["la_empresa"]["nombre"]));
		if($as_tipsolcot=="S")
		{
			$t="Servicio.";
		}
		...
		$io_pdf->ezSetDy(-70);
		/*$linea="Para dar cumplimiento al  artículo Nro. 75 del Decreto con Rango, Valor y Fuerza de Ley de Contrataciones Públicas y ";
		$linea=$linea."habiéndose cumplido con el Artículo Nro. 74  del mismo Decreto se procede a realizar el análisis de  precios  del  ";
		$linea=$linea."procedimiento de  consulta de  precios  -  ".$ls_institucion.",  para  la  elaboración de la Orden de ".$t;*/
		$linea ="Para dar cumplimiento al artículo Nro.  96 y 97 del Decreto Nro. 1.399 de fecha  13/11/2014  con Rango, Valor y  Fuerza ";
		$linea.="de  Ley  de  Contrataciones  Públicas  publicado  en  la  Gaceta  Extraordinaria  Nro.  6.154  en  fecha  19/11/2014 y su ";
		$linea.="reglamento donde  establece la invitación de al menos ( 3 ) tres empresas mediante oficio y habiéndose cumplido con ";
		$linea.="el Artículo 97 ejusdem, se realiza el procedimiento de consulta de precios - ".$ls_institucion.", para la elaboración de la Orden de ".$t;
		$la_data[0]["1"]="<b>Nro.:</b>";
		...
		$io_pdf->add_tabla(10,$la_data,$la_opciones);
				//////////// NUEVO //////////
		$la_data[1]  = array('titulo'=>$linea);
		$la_columnas = array('titulo'=>'');
		$la_config	 = array('showHeadings'=>0, // Mostrar encabezados
						     //'fontSize'=> 11, //8 Tamaño de Letras
						     'fontSize'=> 10, //8 Tamaño de Letras
						     'titleFontSize'=>12,  //9 Tamaño de Letras de los títulos
						     'showLines'=>0, // Mostrar Líneas
						 	 ...
						 	 'xpos'=>10,
						 	 'cols'=>array('titulo'=>array('justification'=>'justify','width'=>546))); // Justificación y ancho de la columna
	...
	function uf_print_proveedores($la_cotizaciones,&$io_pdf)
	{
		...
		$la_anchos_col = array(30,30,84,18,20,18);
	...
	function uf_print_items($as_tipsolcot,$la_items,&$io_pdf)
	{
		...
		$la_anchos_col = array(13,33,37,17,24,18,24,36);
		...
		$la_anchos_col = array(20,24,18,24);
		...
		$la_opciones = array("color_texto"     => array(0,0,0),
							   ...
							   "alineacion_col"=> $la_justificaciones,
								   "color_fondo"=>array(232,232,232));
		$io_pdf->add_tabla(90,$la_data,$la_opciones);
	...
	function uf_print_ganadores($as_numanacot,$as_tipsolcot,$aa_ganadores,&$io_pdf)
	{
		...
		$la_anchos_col = array(25,80,33,32,33);
		...
		$la_anchos_col = array(30,33,32,33);
		$la_justificaciones = array("center","right","right","right");
		$la_opciones = array("color_texto"     => array(0,0,0),
							   "anchos_col"    => $la_anchos_col,
							   "tamano_texto"  => 10,
							   ...
							   "alineacion_col"=> $la_justificaciones,
								   "color_fondo"=>array(232,232,232));
		$io_pdf->add_tabla(85,$la_data,$la_opciones);
	...
	function uf_print_pie_pagina(&$io_pdf)
	{
		...
		$la_data[1]["1"]="";
		$la_data[2]["1"]="";
		$la_data[0]["2"]="<b>Revisado: ".$ls_cargo_compras;//Jefe de Compras</b>";
		$la_data[1]["2"]="".$ls_jefe_compras;
		$la_data[2]["2"]="\n";
		$la_anchos_col = array(90,110);
	...
	$io_pdf->set_margenes(0,11,0,5);	//inferior, derecho,arriba
	...
	if($lb_valido)
	{
		//$concepto=$io_class_report->uf_cargar_concepto($ls_numanacot, $la_cotizaciones);
		$concepto="";
		$lb_valido=$io_class_report->uf_cargar_cotizaciones($ls_numanacot, $la_cotizaciones);
		$concepto=$la_cotizaciones[1]["consolcot"];
		uf_print_encabezado_pagina($ls_numanacot,$ls_tipsolcot,$ls_observacion,$ld_fecha,$concepto,$io_pdf);
		//$lb_valido=$io_class_report->uf_cargar_cotizaciones($ls_numanacot, $la_cotizaciones);
		if($lb_valido)
		{
