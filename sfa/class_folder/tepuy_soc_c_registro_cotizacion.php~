<?php
class tepuy_soc_c_registro_cotizacion
{
  function tepuy_soc_c_registro_cotizacion($as_path)
  {
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//	     Function: tepuy_soc_c_registro_cotizacion
	//		   Access: public 
	...
	{		
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_guardar
		//		   Access: public (tepuy_soc_p_registro_cotizacion.php)
		//	    Arguments: $as_existe
	                              ...
	                              $as_codpro,$ai_diaent,$ad_porcentaje,$as_tabla,$as_campo,$ad_subtotal,$ad_creditos,$ad_total,$ai_estinciva,$aa_seguridad)
	{
		...
		if($lb_valido)
		{
			...
			if($li_row===false)
			{
				...
				{
				  $lb_valido=false;
				  $this->io_mensajes->message("CLASE->tepuy_soc_c_registro_cotizacion.php->MÉTODO->uf_insert_cotizacion ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
				}
	                              ...
	                              $as_codpro,$ai_diaent,$ad_porcentaje,$ad_subtotal,$ad_creditos,$ad_total,$ai_estinciva,$aa_seguridad)
	{
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->tepuy_soc_c_registro_cotizacion.php->MÉTODO->uf_update_cotizacion ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$this->io_sql->rollback();
		}
		else
		{
	...
	function uf_validar_fecha_cotizacion($ad_fecregcot)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion->MÉTODO->uf_validar_fecha_cotizacion ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_valido=false;
		}
		else
		{
	...
	function uf_select_cotizacion($as_numcot)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->tepuy_soc_c_registro_cotizacion.php->MÉTODO->uf_select_cotizacion->ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_existe=false;
		}
		else
		{
	...
	function uf_load_bienes_solicitud($as_numsolcot,$as_codpro)
	{
		...
		$ls_sql ="SELECT soc_dtsc_bienes.codart, soc_dtsc_bienes.codcar,		".
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->tepuy_soc_c_registro_cotizacion.php->MÉTODO->uf_load_bienes_solicitud.ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		return $rs_data;
	...
	function uf_load_servicios_solicitud($as_numsolcot,$as_codpro)
	{
		...
		$ls_sql ="SELECT soc_dtsc_servicios.codser,soc_dtsc_servicios.codcar, 							   ".
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_load_servicios_solicitud ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		return $rs_data;
		...
		for ($i=1;($i<$ai_totrowbienes)&&($lb_valido);$i++)
			{
			  ...
			  if ($rs_data===false)
				 {
					$lb_valido=false;
					$this->io_mensajes->message("CLASE->tepuy_soc_c_registro_cotizacion.php->MÉTODO->uf_insert_bienes. ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
				 }
			  else
				 {
			       /////////////////////////////////         SEGURIDAD               /////////////////////////////		
	   ...
	   for ($i=1;($i<$ai_totrowservicios)&&($lb_valido);$i++)
		   {
			 ...
			 if ($rs_data===false)
				{
					$lb_valido=false;
					$this->io_mensajes->message("CLASE->tepuy_soc_c_registro_cotizacion.php->MÉTODO->uf_insert_servicios ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
				}
				else
				{
					   /////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_delete_cotizacion($as_numcot,$as_tipcot,$as_codpro,$ai_totrowbienes,$ai_totrowservicios,$aa_seguridad)
	{
		...
		if($lb_valido)
		{
			...
			{
				$lb_valido=false;
				$this->io_mensajes->message("CLASE->tepuy_soc_c_registro_cotizacion.php->MÉTODO->uf_delete_cotizacion.php->ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
				$this->io_sql->rollback();
			}
			else
			{
				  ...
				  if ($rs_data===false)
				     {
					   $this->io_sql->rollback();
					   $lb_valido = false;
			           $this->io_mensajes->message("CLASE->tepuy_soc_c_registro_cotizacion.php->MÉTODO->uf_delete_detalles(Bienes)->ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
					 }
				}
			}
		elseif($as_tipcot=='S')
				    ...
				    if ($rs_data===false)
				       {
					     $this->io_sql->rollback();
					     $lb_valido = false;
			             $this->io_mensajes->message("CLASE->tepuy_soc_c_registro_cotizacion.php->MÉTODO->uf_delete_detalles(Servicios)->ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
					   }
				  }
			}
		if($lb_valido)
	...
	function uf_load_bienes($as_numcot,$as_codpro)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->tepuy_soc_c_registro_cotizacion.php->MÉTODO->uf_load_bienes.ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		return $rs_data;
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$ls_sql = "SELECT tepuy_cargos.porcar 
			     FROM tepuy_cargos, $ls_tabla1, $ls_tabla2
			    WHERE $ls_tabla2.codemp='".$this->ls_codemp."'
			      AND $ls_tabla2.$ls_campo='".$as_coditem."'
			      AND tepuy_cargos.codemp=$ls_tabla2.codemp
				  AND tepuy_cargos.codcar=$ls_tabla2.codcar
	 ...
	 if ($rs_dato===false)
		{
		  $this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_load_porcentaje_credito ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		  return false;
		}
	 else
	    {
    ...
    function uf_load_porcentaje_iva_2016($as_codcar,&$lb_valido)
    {
  ///////////////////////////////////////////////////////////////////////////////////////////////
  //	     Function: uf_load_porcentaje_iva_2016
  //		   Access: public
  //		 Argument: as_coditem // Número de la Cotización.
  //                   as_tipitem //Código del Proveedor.
  //	  Description: Función que busca los cargos asociados al Item (Bien o Servicio).
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$ls_sql = "SELECT tepuy_cargos.porcar FROM tepuy_cargos WHERE ".
	" tepuy_cargos.codemp='".$this->ls_codemp."' AND tepuy_cargos.codcar='".$as_codcar."'";
	//print $ls_sql;
