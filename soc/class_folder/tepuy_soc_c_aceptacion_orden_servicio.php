<?php
class tepuy_soc_c_aceptacion_orden_servicio
{
  function tepuy_soc_c_aceptacion_orden_servicio($as_path)
  {
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//	     Function: tepuy_soc_c_aceptacion_orden_servicio
	//		   Access: public 
	...
	function uf_load_ordenes_servicios($as_tipope)
	{
		...
		elseif($as_tipope=='R')
		   {
		     $ls_sql="SELECT soc_ordencompra.numordcom,                   										   ".
					 ...
					 "   AND soc_ordencompra.codemp=rpc_proveedor.codemp  										   ".
					 "   AND soc_ordencompra.cod_pro=rpc_proveedor.cod_pro										   ".
					 "   AND soc_ordencompra.numordcom NOT IN(SELECT numdoccom									   ".
					 "                                          FROM cxp_rd_spg									   ".
					 "										   WHERE cxp_rd_spg.procede_doc='SOCCOS'               ".
					 "                                          AND substr(cxp_rd_spg.numrecdoc,1,1)<>'@'          ".
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->tepuy_soc_c_aceptacion_orden_servicio.MÉTODO->uf_soc_load_ordenes ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		return $rs_data;
			   ...
			   if (array_key_exists("chk".$i,$_POST))
			      {
		            ...
		            if ($rs_data===false)
		               {
			             $this->io_msg->message("CLASE->tepuy_soc_c_aceptacion_orden_servicio.MÉTODO->uf_aceptar_orden_servicio->ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			             $lb_valido=false;
		                 break; 
					   }
					else
