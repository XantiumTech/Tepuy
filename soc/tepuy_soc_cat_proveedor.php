...
function ue_aceptar_proveedor_solicitud_cotizacion(as_codpro,as_nompro,as_dirpro,as_telpro,as_tipconpro)
{
	//---------------------------------------------------------------------------------
	...
	if (as_tipconpro=='O' || as_tipconpro=='E' || as_tipconpro=='F')
	   {
	   ...
	   {
	     totalproveedores=eval(rowproveedores);
		 alert("El Proveedor "+as_nompro+", No tiene definido Tipo de Contribuyente. Por Favor asigne Tipo !!!");
	   }
	...
	if (rowbienes>1)
	   {
		 for (j=1;(j<=rowbienes)&&(valido);j++)
			 { 
		       ...
		       ls_codiva  = eval("fop.txtcodiva"+j+".value");
...
function ue_aceptar(ls_codpro,ls_nompro,ls_tipconpro)
{
