...
class class_tepuy_int_int extends class_tepuy_int
{
	...
	}// end function class_tepuy_int_int
	//-----------------------------------------------------------------------------------------------------------------------------------
...
//print "CREACION DE COMPROBANTE " .$this->is_salto. "  Comprobante: ".$this->is_procedencia.$this->is_comprobante." / ".$this->id_fecha." ".$this->is_salto." Descripción: ".$this->is_descripcion." " .$this->is_salto;
									 ...
									 $as_descripcion)
	{
		...
		//print "empresa: ".$as_codemp;
		//print "cuenta:  ".$as_sc_cuenta;
		//print "status:  ".$ls_status;
		//print "denominacion: ".$ls_denominacion;
	...
	function uf_init_load_datastore_integracion()
	{
		...
		if($rs_data===false)
		{
			$this->is_msg_error="CLASE->tepuy_int_int MÉTODO->uf_init_load_datastore_integracion ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			return false;
		}
		else
		{
    ...
    function uf_valida_comprobante_cuadre_scg()
	{
		...
		if($this->ds_scg->data!="")
		{
			...
			//print "contable: ".$ll_tot_row;
			//print "presupuestario: ".$ll_row_spg;
			//print "ingresos: ".$ll_row_spi;
			if($ll_tot_row > 0)
			{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_init_end_transaccion_integracion($aa_seguridad,$as_fecha) // antes no aplicaba fecha
	{ 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//print "comprobante: ".$this->is_comprobante;
		//print "tipo comp:   ".$this->ii_tipo_comp;
		//print "descripcion: ".$this->is_descripcion;
		//print "tipo i:      ".$this->is_tipo;
		//print "cod. prov.   ".$this->is_cod_prov;
		//print "ced. ben.    ".$this->is_ced_ben;
		//print "cod. banco:  ".$this->as_codban;
		//print "nro. de cta. ".$this->as_ctaban;
		//print "Fecha:".$as_fecha;
		// ANTES UBICABA LA FECHA QUE SE ESTABA CONTABILIZANDO AHORA LA D ELA SOLICITUD //
		$lb_valido = $this->uf_tepuy_comprobante($this->is_codemp,$this->is_procedencia,$this->is_comprobante,
				//								  $this->id_fecha,$this->ii_tipo_comp,$this->is_descripcion,
								  $as_fecha,$this->ii_tipo_comp,$this->is_descripcion,
												  $this->is_tipo,$this->is_cod_prov,$this->is_ced_ben,0,
												  $this->as_codban,$this->as_ctaban);
		if($lb_valido)
		{
			//print "sistema: ".$aa_seguridad["sistema"];
		...
		if($lb_valido) 
		{
			// PROCEDE A CREAR EL ASIENTO CONTABLE // 
			//print "genero:".$this->is_procedencia." hace el asiento contable";
		...
		if ($lb_valido) // Borrar la cabecera del comprobante si es el caso.
		{
			...
			{
				$lb_valido = $this->uf_tepuy_delete_comprobante(); 
				if($lb_valido)
				{
					if($aa_seguridad["sistema"]!="")
					{
		...
		if($lb_valido)
		{
			...
			{
				$this->is_msg_error = $invo_tepuy_int_scg->is_msg_error;
			}
			if ($lb_Flag_Error_SPG) 
			{   
				$this->is_msg_error = $invo_tepuy_int_spg->is_msg_error;
			}
			if ($lb_Flag_Error_SPI)
			{
				$this->is_msg_error = $invo_tepuy_int_spi->is_msg_error;
			}	
		}
		$this->ib_procesando_cmp=false;
	    return $lb_valido;
	...
	function uf_int_valida_informacion_datastore()
	{
		...
		// Modificado Por: Ing. Miguel Palencia								Fecha Última Modificación : 31/05/2007
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//print "Presupuesto:".$this->ds_spg->getRowCount("spg_cuenta");
		if($this->ds_spg->getRowCount("spg_cuenta")<=0)
		{
			//print "Contable:".$this->ds_scg->getRowCount("sc_cuenta");
		...
		//print "Debe-->".doubleval(trim($this->idec_monto_debe));
		//print "Haber-->".doubleval(trim($this->idec_monto_haber));
	...
	function uf_int_make_scg($aa_seguridad)
	{
		...
		// 	Fecha Última Modificación : 31/05/2007
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		if($this->ds_scg->data!="")
		{
			...
			//print "cuentas: ".$ll_tot_row;
			//print "presupuesto: ".$li_total_spg;
			for($ll_row=1;($ll_row<=$ll_tot_row)&&($lb_valido);$ll_row++)
			{
				...
				//print "cuenta: ".$ls_sc_cuenta." documento: ".$ls_documento." debe o haber: ".$ls_debhab;
	...
	function uf_init_load_anular_datastore_integracion($as_codemp,$as_procedencia,$as_comprobante,$adt_fecha,$as_codban,$as_ctaban)
	{
		...
		if($rs_data===false)
		{
			$this->is_msg_error="CLASE->tepuy_int_int MÉTODO->uf_init_load_anular_datastore_integracion ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			return false;
		}
		else
		{
