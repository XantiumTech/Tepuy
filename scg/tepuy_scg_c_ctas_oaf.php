<?php
class tepuy_scg_c_ctas_oaf
{
	var $io_sql;
	var $io_msg;
	var $io_fun;
	
	function tepuy_scg_c_ctas_oaf()
	{
	...
	function uf_cargar_cuentas($object,$li_rows)
	{
		$i=0;
		$object=array();
		$ls_sql="SELECT * FROM scg_pc_reporte WHERE cod_report='0719'";
		...
		else
		{
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$object[$i][5]="<input type=text name=txtdeb_hab".$i."      value='".$ls_deb_hab."' id=txdeb_hab".$i." readonly  class=sin-borde style=text-align:left size=15 >";
