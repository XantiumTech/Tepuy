...
if($ls_operacion=="")
{
    ...
    $ls_sql=" SELECT * FROM tepuy_deducciones WHERE codemp='".$ls_codemp."'";  
...
if($ls_operacion=="GUARDARRET")
{
	...
	$in_classmovbanco=new tepuy_scb_c_ordenpago($la_security);
	$totrow=$_POST["total"];
  ...
  function uf_aceptar()
  {
	...
	if(li_total>0)
	{
		...
		if(parseFloat(ldec_total)<=parseFloat(ldec_monobjret))
		{
			...
			f.action="tepuy_w_regdt_deducciones_op.php";
			f.submit();
		}
		else
		{
