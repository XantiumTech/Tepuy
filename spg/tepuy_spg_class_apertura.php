...
//---------------------------------------------------------------------------------------------------------------------------------
class tepuy_spg_class_apertura 
{
   var $int_spg;
   function tepuy_spg_class_apertura()
   { 
...
//---------------------------------------------------------------------------------------------------------------------------------
  function uf_spg_cuenta_cuentas_apertura($as_codemp,$as_ep1,$as_ep2,$as_ep3,$as_ep4,$as_ep5,$mostrar_cuentas)
  { ////////////////////////////////////////////////////////////////////////////////////////////////////////
	//	      Function:  uf_spg_cuenta_cuentas_apertura()                                   
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
	  ...
	  $ls_sql= " SELECT COUNT(*) ".
			   ...
			   "        status='C'                   ".
			   " ORDER BY spg_cuenta  ";  
			  //" ORDER BY spg_cuenta ".$mostrar_cuentas;
	  $rs_load=$this->io_sql->select($ls_sql);
	  return $rs_load;
}//uf_spg_cuenta_cuentas_apertura  

	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_paginasspg($as_codemp,$as_ep1,$as_ep2,$as_ep3,$as_ep4,$as_ep5)
	{
		//////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_existesalida
		//		   Access: public (tepuy_spg_p_apertura.php)
		...
		$lb_valido=false;
		$pagina=0;
		$ls_sql="SELECT count(spg_cuenta) as total".
				"  FROM spg_cuentas ".
			   " WHERE  codemp='".$as_codemp."'  AND ".
			   "        codestpro1='".$as_ep1."' AND ".
			   "        codestpro2='".$as_ep2."' AND ". 
		...
		{
			$lb_valido=true;
			$this->io_mensajes->message("CLASE->Formulación del Presupuesto MÉTODO->uf_paginas ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
...
//---------------------------------------------------------------------------------------------------------------------------------
  function uf_spg_load_cuentas_apertura($as_codemp,$as_ep1,$as_ep2,$as_ep3,$as_ep4,$as_ep5,$mostrar_cuentas)
  { ////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//---------------------------------------------------------------------------------------------------------------------------------
  ...
  {	 //////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 //////////////////////////////////////////////////////////////////////////////////////////////
	 $lb_valido=true;
     $ls_sql=" SELECT estmodape FROM tepuy_empresa  WHERE codemp='".$as_codemp."' ";
...
{   ////////////////////////////////////////////////////////////////////////////////////////////////
	...
	{
	   $lb_valido = $this->sig_int->uf_tepuy_insert_comprobante($this->is_codemp,$this->is_procedencia,$this->is_comprobante,
	   ...
	   { 
		 ...
		 /////////////////////////////////         SEGURIDAD               //////////////////////////////////////////////////////	
			...
			$lb_valido=$this->io_rcbsf->uf_reconvertir_datos("tepuy_cmp",$this->li_candeccon,$this->li_tipconmon,$this->li_redconmon,$aa_seguridad);*/
			if($lb_valido)	 
			{
				 $this->io_sql->commit();
				 $lb_valido=true;
       ...
       //$this->io_sql->begin_transaction();
		...
		{								
		  if ($ad_asignado <> 0) 
		  {
			...
			{
				$ld_comprometido=0;
				$lb_valido=$this->uf_select_compromiso_cuenta($this->is_codemp,$as_cuenta,$estprog,$ld_comprometido);
				$ad_asignado=trim($ad_asignado);
				$ld_comprometido=trim($ld_comprometido);
				if($lb_valido)
				{
				$ad_asignado1=$ad_asignado+10000000; //trampilla
			          if($ad_asignado1>=$ld_comprometido)
					  {	
					   ...
					   {
						  $this->io_msg->message(" Esta ".$as_cuenta."  tiene comprometido ".$ld_comprometido." esta asignando menos de lo comprometido por favor revise su monto asignado... ");
						  $lb_valido=false;						  
					   }	
				 }		
			 }
				   ...
				   if($lb_valido)
				   {
					    ...
					    if($lb_valido)
					    {
						 ...
						 /* $lb_valido=$this->uf_update_bsf_spgcuentas($ad_asignado,$ad_precomprometido,$ad_comprometido,
																	 ...
																	 $ad_m1,$ad_m2,$ad_m3,$ad_m4,$ad_m5,$ad_m6,$ad_m7,
																	 $ad_m8,$ad_m9,$ad_m10,$ad_m11, $ad_m12,$this->is_codemp,
																	 $estprog,$as_spg_cuenta,$aa_seguridad);*/
					   }  
				  }
			  }  
		   }				
...
//---------------------------------------------------------------------------------------------------------------------------------
function uf_select_compromiso_cuenta($as_codemp,$as_spg_cuenta,$aa_estprog,&$ad_comprometido)
{
     ////////////////////////////////////////////////////////////////////////////////////////
...
function uf_verificar_administrador($as_codemp,&$as_codusu)
{
	...
	$ls_sql = " SELECT codusu ".
			  ...
			  "        nomven='tepuy_spg_p_apertura.php' AND ".
