...
class tepuy_sob_c_revanticipo_rd
{
	var $obj="";
	var $io_sql;
	var $siginc;
	...
	//----------------------------------------------------------------------------------------------------------------------------------------------------------------
	function tepuy_sob_c_revanticipo_rd()
	{
	...
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_sob_select_estatus_recepcion
		//         Access: public  
		...
		//	   Creado Por: Ing. Miguel Palencia
		// Fecha Creación: 11/04/2014							Fecha Última Modificación :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->revanticipo MÉTODO->uf_sob_select_estatus_recepcion ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
		...
		return $lb_valido;
	}  // end  function uf_sob_select_estatus_recepcion
	//----------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_delete_dt_rd_scg($as_numrecdoc,$as_codpro,$aa_seguridad)
	{
		...
		//	   Creado Por: Ing. Miguel Palencia
		// Fecha Creación: 18/04/2014							Fecha Última Modificación :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
	...
	//----------------------------------------------------------------------------------------------------------------------------------------------------------------
	function uf_sob_obtener_anticipos($as_codcon,$ad_fecregdes,$ad_fecreghas,&$ai_totrows,&$ao_object,&$ao_title,&$as_titletable)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_sob_obtener_anticipos
		//         Access: public  
		...
		//	   Creado Por: Ing. Miguel Palencia
		// Fecha Creación: 10/04/2014							Fecha Última Modificación :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		$ls_sql= "SELECT a.codcon,a.codant,a.fecant,a.conant,a.numrecdoc".
				 "  FROM sob_anticipo a, sob_contrato b".
				 " WHERE a.codemp='".$this->ls_codemp."'".
				 "   AND a.estgenrd='1'".
				 "   AND a.codcon LIKE '%".$as_codcon."%'".
				 "   AND a.fecant >='".$ad_fecregdes."'".
				 "   AND a.fecant <='".$ad_fecreghas."'".
				 "   AND a.codcon=b.codcon ".
				 " ORDER BY a.codcon,a.codant ";
	//print $ls_sql;
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->revanticipo MÉTODO->uf_sob_obtener_anticipos ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$li_i=0;
			$ao_title[1]="Nro. de Ficha";
			$ao_title[2]="Contrato";
			$ao_title[3]="Anticipo";
			$ao_title[4]="Concepto";
			$ao_title[5]="Fecha";
			...
			{
				$li_i=$li_i + 1;
				$ls_numrecdoc=substr($row["numrecdoc"],0,15);
				...
				$ao_object[$ai_totrows][1]="<input name=txtnumrecdoc".$ai_totrows." type=text id=numrecdoc".$ai_totrows." class=sin-borde size=18 value='".$ls_numrecdoc."' readonly>";
				$ao_object[$ai_totrows][2]="<input name=txtcodcon".$ai_totrows." type=text id=txtcodcon".$ai_totrows." class=sin-borde size=18 value='".$ls_codcon."' readonly>";
				$ao_object[$ai_totrows][3]="<input name=txtcodant".$ai_totrows." type=text id=txtcodant".$ai_totrows." class=sin-borde size=10 value='".$ls_codant."' readonly>";
				$ao_object[$ai_totrows][4]="<input name=txtconant".$ai_totrows." type=text id=txtconant".$ai_totrows." class=sin-borde size=50 value='".$ls_conant."' readonly>";
				$ao_object[$ai_totrows][5]="<input name=txtfecant".$ai_totrows." type=text id=txtfecant".$ai_totrows." class=sin-borde size=10 value='".$ld_fecant."' readonly>";
			...
			{
				$ai_totrows=$ai_totrows+1;
				$ao_object[$ai_totrows][1]="<input name=txtnumrecdoc".$ai_totrows." type=text id=txtnumrecdoc".$ai_totrows." class=sin-borde size=18 readonly>";
				$ao_object[$ai_totrows][2]="<input name=txtcodcon".$ai_totrows." type=text id=txtcodcon".$ai_totrows." class=sin-borde size=18 readonly>";
				$ao_object[$ai_totrows][3]="<input name=txtcodant".$ai_totrows." type=text id=txtcodant".$ai_totrows." class=sin-borde size=10 readonly>";
				$ao_object[$ai_totrows][4]="<input name=txtconant".$ai_totrows." type=text id=txtconant".$ai_totrows." class=sin-borde size=50 readonly>";
				$ao_object[$ai_totrows][5]="<input name=txtfecant".$ai_totrows." type=text id=txtfecant".$ai_totrows." class=sin-borde size=10 readonly>";
				$ao_object[$ai_totrows][6]="<input type='checkbox' name=chkreversar".$ai_totrows." class= sin-borde value=1>";
			}
			$this->io_sql->free_result($rs_data);
		}
		...
		{$lb_valido=false;}
		return $lb_valido;
	}// fin de la function uf_sob_obtener_anticipos
	//----------------------------------------------------------------------------------------------------------------------------------------------------------------

	//----------------------------------------------------------------------------------------------------------------------------------------------------------------
	function uf_sob_obtener_valuaciones($as_codcon,$ad_fecregdes,$ad_fecreghas,&$ai_totrows,&$ao_object,&$ao_title,&$as_titletable)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_sob_obtener_valuaciones
		//         Access: public  
		//      Argumento: $as_codcon      // Codigo de Contrato
		//  			   $ad_fecregdes   // Fecha desde
		//  			   $ad_fecreghas   // Fecha hasta
		...
		$ad_fecregdes=$this->io_funcion->uf_convertirdatetobd($ad_fecregdes);
		$ad_fecreghas=$this->io_funcion->uf_convertirdatetobd($ad_fecreghas);
		$ls_sql= "SELECT codcon,codval,obsval,fecha,numrecdoc".
				 "  FROM sob_valuacion".
				 " WHERE codemp='".$this->ls_codemp."'".
				 "   AND estgenrd='1'".
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->revanticipo MÉTODO->uf_sob_obtener_valuaciones ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$li_i=0;
			...
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ai_totrows=$ai_totrows+1;
				$ao_object[$ai_totrows][1]="<input name=txtnumrecdoc".$ai_totrows." type=text id=txtnumrecdoc".$ai_totrows." class=sin-borde size=15 value='".$ls_numrecdoc."' readonly>";
				$ao_object[$ai_totrows][2]="<input name=txtcodcon".$ai_totrows." type=text id=txtcodcon".$ai_totrows." class=sin-borde size=18 value='".$ls_codcon."' readonly>";
				$ao_object[$ai_totrows][3]="<input name=txtcodval".$ai_totrows." type=text id=txtcodval".$ai_totrows." class=sin-borde size=10 value='".$ls_codval."' readonly>";
				$ao_object[$ai_totrows][4]="<input name=txtobsval".$ai_totrows." type=text id=txtobsval".$ai_totrows." class=sin-borde size=50 value='".$ls_obsval."' readonly>";
				$ao_object[$ai_totrows][5]="<input name=txtfecval".$ai_totrows." type=text id=txtfecval".$ai_totrows." class=sin-borde size=10 value='".$ld_fecha."' readonly>";
				$ao_object[$ai_totrows][6]="<input type='checkbox' name=chkreversar".$ai_totrows." class= sin-borde value=1>";
			...
			{
				$ai_totrows=$ai_totrows+1;
				$ao_object[$ai_totrows][2]="<input name=txtnumrecdoc".$ai_totrows." type=text id=txtnumrecdoc".$ai_totrows." class=sin-borde size=15 readonly>";
				$ao_object[$ai_totrows][2]="<input name=txtcodcon".$ai_totrows." type=text id=txtcodcon".$ai_totrows." class=sin-borde size=18 readonly>";
				$ao_object[$ai_totrows][3]="<input name=txtcodval".$ai_totrows." type=text id=txtcodval".$ai_totrows." class=sin-borde size=10 readonly>";
				$ao_object[$ai_totrows][4]="<input name=txtobsval".$ai_totrows." type=text id=txtobsval".$ai_totrows." class=sin-borde size=50 readonly>";
				$ao_object[$ai_totrows][5]="<input name=txtfecval".$ai_totrows." type=text id=txtfecval".$ai_totrows." class=sin-borde size=10 readonly>";
				$ao_object[$ai_totrows][6]="<input type='checkbox' name=chkreversar".$ai_totrows." class= sin-borde value=1>";
			}
			$this->io_sql->free_result($rs_data);
		}
		...
		{$lb_valido=false;}
		return $lb_valido;
	}// fin de la function uf_sob_obtener_valuaciones
	//----------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_update_estatus_valuacion($as_codcon,$as_codval,$aa_seguridad) 
	{
		...
		if ($li_row===false)
		{
			$this->io_msg->message("CLASE->revvaluacion METODO->uf_update_estatus_valuacion ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="UPDATE";
			$ls_descripcion= "Modifico el estatus de R.D. de la valuacion ".$as_codval." Asociada al contrato ".$as_codcon;
			$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               ///////////////////////////
