...
//--------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_print_cabecera($as_cedper,$as_nomper,$as_desuniadm,$as_codcueban,$as_tipcuebanper,$as_desded,$as_destipper,
							  $as_codclavia,$ad_fecsalvia,$ad_fecregvia,$ai_numdiavia,$as_denmis,$ai_acompanante,$as_cargo,$as_sueldomensual,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('titulo'=>'<b>SUELDO MENSUAL Bs. </b>'.$as_sueldomensual));
		$la_columna=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('titulo'=>'<b></b>'));
		$la_columna=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
		...
		unset($la_columna);
		unset($la_config);
		$la_data=array(array('titulo'=>'<b>DETALLES DEL VIATICO</b>'));
		$la_columna=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
		...
		$la_data=array(array('codigo'=>'<b>Código</b>','descripcion'=>'<b>Descripción</b>','tarifa'=>'<b>Tarifa</b>',
							 'dias'=>'<b>Cantidad</b>','subtotal'=>'<b>Subtotal</b>'));
	...
	function uf_print_detalle_asignaciones($la_data,$ai_total,$as_totalletras,&$io_pdf)
	{
		...
		if($ls_tiporeporte==1)
		{
			$ls_titulo="Total Viaticos Bs.F.:";
		}
		else
		{
			$ls_titulo="Total Viaticos Bs.:";
		}
		$la_columna=array('codigo'=>'','descripcion'=>'','tarifa'=>'','dias'=>'','subtotal'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 8, // Tamaño de Letras
	...
	function uf_print_detalle_presupuestario($la_data,&$io_pdf)
	{
		...
		// Fecha Creación: 21/04/2006 
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$la_datos=array(array('titulo'=>'<b>DETALLE PRESUPUESTARIO</b>'));
		$la_columna=array('titulo'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 9, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
		...
		for($li_z=1;$li_z<=$li_totrowsol;$li_z++)
		{
			...
			else // Imprimimos el reporte
			{
				...
				for($li_i=1;$li_i<=$li_totrow;$li_i++)
				{
					...
					$li_sueldomensual= $io_report->ds_solicitud->data["sueper"][$li_i];
					$porcentaje=$io_report->ds_solicitud->data["porcentaje"][$li_i];
					$dentro_fuera=$io_report->ds_solicitud->data["dentro_fuera"][$li_i];
					$ls_sueldomensual=number_format($li_sueldomensual,2,",",".");
					uf_print_cabecera($ls_cedper,$ls_nomper,$ls_denuniadm,$ls_codcueban,$ls_tipcuebanper,$ls_desded,$ls_destipper,
									  $ls_codclavia,$ld_fecsalvia,$ld_fecregvia,$li_numdiavia,$ls_denmis,$li_acompanante,$ls_cargo,$ls_sueldomensual,&$io_pdf); // Imprimimos la cabecera del registro
					...
					if($lb_valido)
					{
						...
						for($li_s=1;$li_s<=$li_totrow_det;$li_s++)
						{
							...
							if($ls_proasi=="TVS")
							{
								$li_monto=$io_report->uf_select_monto_viatico($ls_codemp,$ls_codasi,$porcentaje
,$dentro_fuera,$ld_fecregvia);
								$ls_denasi=$ls_denasi.".<b> Cubriendo el ".$porcentaje." %</b>";
							}
