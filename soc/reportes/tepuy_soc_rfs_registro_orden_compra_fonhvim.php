...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
									   ...
									   $ls_forpagcom,&$io_pdf)
	{
		...
		$io_pdf->addText(40,63,7,"COMPRAS"); // Agregar el título
		$io_pdf->line(115,60,115,130);	//VERTICAL
			
		$io_pdf->addText(135,122,7,"APROBADO POR"); // Agregar el título
		$io_pdf->addText(135,63,7,"PRESUPUESTO"); // Agregar el título
		$io_pdf->line(210,60,210,130);	//VERTICAL
				
		$io_pdf->addText(260,122,7,"AUTORIZADO POR"); // Agregar el título
		$io_pdf->addText(213,63,7,"GERENCIA DE ADMINISTRACION Y FINANZAS"); // Agregar el título
		$io_pdf->line(370,60,370,130); //VERTICAL
		
		$io_pdf->addText(380,122,7,"GERENCIA SOLICITANTE"); // Agregar el título
	...
	//require_once('../../xml/CrearReporte.php');
	
	//$oReporte = new Reporte(); //Crear un objeto de tipo reporte
	//$row=$io_sql->fetch_row($rs_data);	//$rs_data datos de cabecera
	//$oReporte->CrearXmlArr('OrdCompraEnc',$row); //Llamar al método que crea el XML
	//$oReporte->NomRep = "new_report_1";  //nombre del reporte
	//$oReporte->MostrarReporte();         //llamar al mostrar de la clase
	//var_dump($row);
	...
	else  // Imprimimos el reporte
	{
		...
		if($lb_valido)	
		{
			...
			{  
			   ...
			   //$rs_datos //detalle
			/*  $oReporte2 = new Reporte();
			  $row=$io_sql->fetch_row($rs_datos);
			  var_dump($row);
			  $row2=$io_sql->fetch_row($rs_datos);	//$rs_data datos de cabecera
	          $oReporte2->CrearXml('OrdCompraDet',$rs_datos); //Llamar al método que crea el XML
	          $oReporte2->NomRep = "new_report_1";  //nombre del reporte
	          $oReporte2->MostrarReporte();         //llamar al mostrar de la clase
	          var_dump($row2);
	          die();*/
			   ...
			   if ($lb_valido)
			   {
				...
				/*	for ($i=$li_i+1;$i<=25;$i++)
					...
					//$rs_datos_cuenta det cuenta
				  /*  $oReporte3 = new Reporte();
			        $row3=$io_sql->fetch_row($rs_datos_cuenta);	//$rs_data datos de cabecera
	                $oReporte3->CrearXmlArr('OrdCompraCue',$row3); //Llamar al método que crea el XML
	                $oReporte3->NomRep = "new_report_1";  //nombre del reporte
	                $oReporte3->MostrarReporte();        //llamar al mostrar de la clase
	             //   var_dump($row3);
	                die();*/
