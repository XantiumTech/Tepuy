...
class tepuy_srh_c_solicitud_adiestramiento
{
	var $obj="";
	var $io_sql;
	var $siginc;
...
function tepuy_srh_c_solicitud_adiestramiento($path)
	{   
	 ...
	 function uf_srh_getProximoCodigo()
  {
        /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_srh_getProximoCodigo
		//         Access: public (tepuy_srh_p_solicitud_adiestramiento)
		//      Argumento: 
...
function uf_srh_guardarsolicitud_adiestramiento ($ao_solicitud,$as_operacion="insertar", $aa_seguridad)
  { 
		...
		//         access: public (tepuy_srh_solicitud_adiestramiento)														    			
	...
	{ $this->io_sql->begin_transaction();
	 ...
	 $ao_solicitud->costo=str_replace(",",".",$ao_solicitud->costo);
	  ...
	  $ls_sql = "INSERT INTO srh_solicitud_adiestramiento (nroreg, codper,  fecha, codprov, descripcion, observacion, fecini, fecfin, costo, durhras, objetivo, area, estrategia, codemp) ".	  
			"VALUES ('$ao_solicitud->nroreg', '$ao_solicitud->codsol','$ao_solicitud->fecsol', '$ao_solicitud->prov','$ao_solicitud->descrip','$ao_solicitud->obs','$ao_solicitud->fecini','$ao_solicitud->fecfin','$ao_solicitud->costo','$ao_solicitud->durhras', '$ao_solicitud->obj','$ao_solicitud->are','$ao_solicitud->est','".$this->ls_codemp."')";
...
function uf_srh_buscar_solicitud_adiestramiento($as_nroreg,$as_fecsol1, $as_fecsol2,$as_codprov,$as_des)
	{
		...
		$ls_sql= "SELECT *  FROM srh_solicitud_adiestramiento INNER JOIN sno_personal ON (sno_personal.codper = srh_solicitud_adiestramiento.codper) INNER JOIN rpc_proveedor ON (srh_solicitud_adiestramiento.codprov = rpc_proveedor.cod_pro)
		           ".
				"   AND fecha between  '".$as_fecsol1."' AND '".$as_fecsol2."' ".
				"   AND nroreg like '$as_nroreg' ".
				"   AND codprov like '$as_codprov' ".
				"   AND descripcion like '$as_des' ".
				" ORDER BY nroreg";
	    $rs_data=$this->io_sql->select($ls_sql);
		...
		{	
			...
			while ($row=$this->io_sql->fetch_row($rs_data)) 
			{
					...
					$cell->appendChild($dom->createTextNode($row['nroreg']." ^javascript:aceptar( \"$ls_nroreg\", \"$ls_fecsol\", \"$ls_des\", \"$ls_codsol\", \"$ls_nomsol\", \"$ls_apesol\",  \"$ls_codprov\", \"$ls_denprov\", \"$ls_fecini\", \"$ls_fecfin\", \"$ls_durhras\" ,  \"$ls_costo\" ,  \"$ls_obs\" , \"$ls_obj\", \"$ls_est\", \"$ls_are\" , \"$ls_nrodestino\" , \"$ls_fecsoldestino\" ,  \"$ls_desdestino\" ,  \"$ls_codsoldestino\" ,  \"$ls_nomdestino\" , \"$ls_codprovdestino\" ,  \"$ls_denprovdestino\" ,  \"$ls_fecinidestino\" ,  \"$ls_fecfindestino\" ,  \"$ls_durhrasdestino\" , \"$ls_costodestino\" , \"$ls_obsdestino\",\"$ls_objdestino\",\"$ls_estdestino\",\"$ls_aredestino\");^_self"));
...
function uf_srh_guardar_dt_solicitud_adiestramiento($ao_solicitud, $aa_seguridad)
  { 
		...
		//         access: public (tepuy_dt_srh_solicitud_adiestramiento)														
...
function uf_srh_eliminar_dt_solicitud_adiestramiento($as_nroreg, $aa_seguridad)
  {
		...
		//        access:  public (tepuy_srh_dt_solicitud_adiestramiento)														
...
}// end   class tepuy_srh_c_solicitud_adiestramiento
?>