<?PHP
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	require_once("tepuy_scb_c_report.php");
	...
	$io_report    = new tepuy_scb_c_report($con);
	...
	if($ls_tiporeporte==1)
	{
		require_once("tepuy_scb_c_reportbsf.php");
		$io_report=new tepuy_scb_c_reportbsf($con);
		$ls_tipbol = 'Bs.F.';
	}
	...
	for($i=1;$i<=$li_totrow;$i++)
	{
		...
		$ls_ctabannext			= $io_report->ds_disponibilidad->getValue("ctabannext",$i);
		...
		$la_data[$i]        = array('nomban'=>$ls_nomban,'ctaban'=>$ls_ctabannext,'tipcta'=>$ls_nomtipcta,'debitos'=>$io_funciones->iif_string("$ldec_debitos>=0",number_format($ldec_debitos,2,",","."),"(".number_format($ldec_debitos,2,",",".").")"),'creditos'=>$io_funciones->iif_string("$ldec_creditos>=0",number_format($ldec_creditos,2,",","."),"(".number_format($ldec_creditos,2,",",".").")"),'saldo'=>$io_funciones->iif_string("$ldec_saldo>=0",number_format($ldec_saldo,2,",","."),"(".number_format($ldec_saldo,2,",",".").")"));						
	}
