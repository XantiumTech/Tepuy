...
class clsTinyButStrongOOo extends clsTinyButStrong
{
  ...
  function LoadXmlFromDoc($xml_file,$ooo_template_filename)
  {
	...
	//exec('C:/Archivos de programa/ZIP-UNZIP/UNZIP/unzip '.$this->_ooo_basename.'.'.$this->_ooo_file_ext.' -d '.$this->_ooo_basename.' '.$this->_xml_filename);
  ...
  function SaveXmlToDoc($ooo_template_filename)
  {
	...
	if(filemtime($ooo_template_filename)>=mktime(date("H")-12,date("i"),date("s"),date("m"),date("d"),date("y")))
	{
		if (substr(PHP_OS, 0, 3) == 'WIN')//Funciones de Semaforo para windows
		{ 
		...
		$key=ftok("/sitioweb/tepuy_php/SOB/class_folder","L");
		$sem=sem_get($key);
		sem_acquire($sem); 
		$dia=date("d");
		$mes=date("m");
		$año=date("y");    
		$futura = mktime(date("H")+15,date("i"),date("s"),$mes,$dia,$año);
		$nuevafecha=date("d-m-y",$futura);
		$actual = date("d-m-y");
		exec("date ".$nuevafecha);
