<?php
class tepuy_cfg_c_unidad_tributaria
 {
  	var $ls_sql="";
	var $io_msg_error;
	
	
function tepuy_cfg_c_unidad_tributaria()//Constructor de la Clase.
	{
...
{	
	...
	if ($rs_data===false)
	   {
		$this->io_msg->message("CLASE->tepuy_cfg_c_unidad_tributaria; METODO->uf_select_unidad_tributaria;ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));   
  ...
  if ($lb_val_fecha)
   {
	...
	$as_valunitri=str_replace(",",".",$as_valunitri);
   	if(!$lb_existe)
		...
		{
			$lb_valido=false;
		    $this->io_msg->message("CLASE->tepuy_cfg_c_unidad_tributaria; METODO->uf_guardar_unidad_tributaria;ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));   
...
function uf_delete_unidad_tributaria($as_codigo,$aa_seguridad)
{
	...
	if ($rs_data===false)
	   {
		 $lb_valido=false;
	     $this->io_msg_error->message("CLASE->tepuy_CFG_C_UNIDAD_TRIBUTARIA; METODO->uf_select_unidad_tributaria;ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));   
	   }
	else
	   {
	   	 $ls_evento="DELETE";
...
function AgregarUno($cod="")
 {
  ...
  function retonar_ultimo()
	{
	 $ls_sql= "select max(codunitri) from tepuy_unidad_tributaria "; 
	/* if ($ls_sql==="")
	  {
	   $codigo='0001'
	  }
