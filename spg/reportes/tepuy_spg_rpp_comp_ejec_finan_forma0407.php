<?php
	...
	function uf_print_titulo_reporte($io_encabezado,$as_programatica,$ai_ano,$as_mes,$as_denestpro,&$io_pdf)
	{
		...
		$la_data=array(array('name'=>'<b>CODIGO DEL ENTE:     </b>'.'<b>'.$ls_codemp.'</b>'),
		               array('name'=>'<b>CODIGO ASIGNADO POR LA ONAPRE:     </b>'.'<b>'.$ls_codasiona.'</b>'),
		               array('name'=>'<b>DENOMINACION:    </b>'.'<b>'.$ls_nombre.'</b>'),
					   array('name'=>'<b>ORGANO DE ADSCRIPCION:    </b>'.'<b>'.$ls_nombre.'</b>'),
		               array('name'=>'<b>PROYECTO O ACCION CENTRALIAZADA:    </b>'.'<b>'.$as_programatica.'</b>'),
...
//-----------------------------------------------------------------------------------------------------------------------------
		...
		global $la_data_tot;
		 if($ls_tipoformato==1)
		 {
			require_once("tepuy_spg_reporte_comparados_forma04_bsf.php");
			$io_report = new tepuy_spg_reporte_comparados_forma04_bsf();
		 }
		 else
		 {
			require_once("tepuy_spg_reporte_comparados_forma04.php");
			$io_report = new tepuy_spg_reporte_comparados_forma04();
		 }	
	...
	//--------------------------------------------------  Parámetros para Filtar el Reporte  -----------------------------------------
		...
		$ls_codestpro4h  = $io_funciones->uf_cerosizquierda($ls_codestpro5h_max,2);
    ...
    // Cargar el dts_cab con los datos de la cabecera del reporte( Selecciono todos comprobantes )	
	 ...
	 else // Imprimimos el reporte
	 {
		...
		{		
			...
			{
		       $io_pdf->ezSetCmMargins(8.05,3,3,3); // Configuración de los margenes en centímetros
			}
