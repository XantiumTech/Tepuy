...
if($ls_operacion=="BUSCAR")
{
		...
		$ls_sql=" SELECT distinct a.procede,a.codemp,a.comprobante,a.fecha,a.ced_bene,a.cod_pro,a.descripcion, ".
		        "        a.tipo_destino,a.total ".
		 		" FROM   tepuy_cmp a,spi_dt_cmp b ".
		  		" WHERE  a.procede=b.procede AND (a.tipo_comp=1 OR a.tipo_comp=2) AND a.comprobante=b.comprobante AND ".
				"        a.fecha=b.fecha AND a.codemp=b.codemp AND a.codemp='".$as_codemp."' AND  ".
				"        a.comprobante like '%".$ls_comprobante."%' AND a.procede like '%".$ls_procedencia."%' " ;		
			...
			//	$ldec_monto=$data["monto"][$z];
				...
				else
				{
					...
					if($ls_tipocat=="")
					{
						...
						'$ls_prov','$ls_bene','$ls_provbene','$ldec_monto');\">".$ls_comprobante."</a></td>";
...
<script language="JavaScript">
  ...
  function uf_aceptar(comprobante,descripcion,procede,fecha,tipo,prov,bene,ls_provbene,ld_monto)
  {
  ...
  function ue_search()
  {
	  ...
	  f.action="tepuy_cat_comprobantesspi.php?tipocat=<?php print $ls_tipocat; ?>";
	  f.submit();
  }
