...
/****************************************************************************************************************************************/	
class tepuy_spg_reporte_comparados_forma04
{
    //conexion	
	var $sqlca;   
	...
	var $dts_prog;
/****************************************************************************************************************************************/	
    function  tepuy_spg_reporte_comparados_forma04()
    {
				  ...
				  // $ls_codrep="00003";
				   ...
				   if($lb_valido)
				   {
					  ...
					  $lb_valido=$this->uf_spg_reporte_calcular_ejecutado_trimestre_act($ls_spg_cuenta,$ls_estructura_desde,$ls_estructura_hasta,
																						...
																						$ld_ejec_acum_t_ant,$ld_aumdis_t_ant,$as_codfuefindes,
																						$as_codfuefinhas);
				   }	   
				   if($lb_valido)
				   {
						 ...
						 $ld_dispon_t_ant=$ld_asignado+$ld_aumdis_t_ant-$ld_ejec_acum_t_ant;
						...
						// $this->dts_reporte->insertRow("ejec_t_ant",$ld_ejec_t_ant);
						 $this->dts_reporte->insertRow("ejec_t_ant",$ld_dispon_t_ant);
...
/****************************************************************************************************************************************/	
    ...
    {//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 else
	 {
		...
		//$row=$this->SQL->fetch_row($rs_prog);
		...
		if($li_row>0)
		{ // print '<br>   li_mesdes= '. $li_mesdes."<br>"." li_meshas= ".$li_meshas.'<br>';
...
/****************************************************************************************************************************************/	
    ...
    // $li_meshas=($dt_meshas-1);
	 $li_mesdes=($dt_mesdes-3);
     $li_meshas=($dt_meshas-3);
	 $ld_monto_programado=0;
	 $ld_monto_acumulado=0;
     ...
     $lb_valido=$this->uf_spg_reporte_calcular_programado($li_mesdes,$li_meshas,$ld_monto_programado,$ld_monto_acumulado,$as_estructura_desde,
	                                                               $as_estructura_hasta,"",$as_spg_cuenta);														 
	 $ld_monto_ejecutado=0;
...
/****************************************************************************************************************************************/	
    function uf_spg_reporte_calcular_ejecutado_trimestre_act($as_spg_cuenta,$as_estructura_desde,$as_estructura_hasta,$ai_mesdes,
															 ...
															 &$ad_pagado_acumulado,$as_tipo,&$ld_ejec_t_ant,&$ad_aumdis_t_ant)
    {//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 // Fecha Creación :    26/06/2006          Fecha última Modificacion :      Hora :
	  ...
	  $dt_mesdes=intval($dt_mesdes);
      $dt_meshas=intval($dt_meshas);
	  $li_mesdes_t_ant =($dt_mesdes-3);
	  $li_meshas_t_ant =($dt_meshas-3);
	  if (strtoupper($ls_gestor)=="MYSQL")
	  {
		   $ls_cadena="CONCAT(codestpro1,codestpro2,codestpro3,codestpro4,codestpro5)";
	  }
	  ...
	  else
	  {
		while($row=$this->SQL->fetch_row($rs_ejec))
		{
		  ...
		  $ldt_mescmp=substr($ldt_fecha_db,5,2);
		  $ldt_mescmp=intval($ldt_mescmp);
		  if($as_tipo=="O")
		  {
		    if(($li_comprometer)&&($ldt_fecha>=$li_mesdes)&&($ldt_fecha<=$li_meshas))
			{ 
		  ...
		  if(($li_disminucion)&&($ldt_mescmp>=$li_mesdes_t_ant)&&($ldt_mescmp<=$li_meshas_t_ant))
		  {  
			  $ld_dis_t_ant =$ld_dis_t_ant + $ld_monto;
		  }//if
		  if(($li_aumento)&&($ldt_mescmp>=$li_mesdes_t_ant)&&($ldt_mescmp<=$li_meshas_t_ant))
		  {  
			  $ld_aum_t_ant = $ld_aum_t_ant + $ld_monto;
		  }//if
		}//while
		$ad_aumdis_t_ant = $ld_aum_t_ant - $ld_dis_t_ant; 
		$ad_aumdismes=$ld_aumento-$ld_disminucion;
		$ad_aumdisacum=$ld_aumento_acum-$ld_disminucion_acum;
	   $this->SQL->free_result($rs_ejec);
	  }//else	
	...
	if($lb_valido)
	{
	 ...
	 for($z=1;$z<=$li_total;$z++)
	 {
		...
		{	 
					  ...
					  {//calculo el reprogramción del proximo mes 
						  ...
						  {		
							  ...
							  $lb_valido=$this->uf_spg_reporte_calcular_ejecutado_trimestre_act($ls_spg_cuenta,$ls_estructura_desde,$ls_estructura_hasta,
																								...
																								$ld_causado_acumulado,
																								$ld_pagado_acumulado,$ls_tipo,
																								$ld_ejec_acum_t_ant,$ld_aumdis_t_ant);
			...
			$this->dts_reporte->insertRow("spg_cuenta",$ls_estructura_desde);//$ls_spg_cuenta
