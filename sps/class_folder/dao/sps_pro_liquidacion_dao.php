...
class sps_pro_liquidacion_dao extends class_dao
{
  ...
  {
  	$lb_valido = false;
 	$ls_sql    = "SELECT conart,numlitart,operador,canmes,tiempo,diasal,condicion,estacu,diaacu,numcon FROM sps_articulos  WHERE id_art='".$ps_id_art."' ORDER BY numlitart,numcon DESC" ;			  
  ...
  {	 
	...
	$this->io_sql->begin_transaction();
		...
		$ls_sql = " INSERT INTO ".$this->as_tabla." (codemp,codper,codnom,numliq,codcauret,fecliq,fecing,fecegr,salint,descargo,anoser,messer,diaser,totasiliq,totdedliq,totpagliq,estliq,obsliq,dedicacion,tipopersonal)
	                VALUES ('".$this->ls_codemp."','".$po_liq->codper."','".$po_liq->codnom."','".$po_liq->numliq."','".$po_liq->codcauret."','".$ls_fecliq."','".$ls_fecing."','".$ls_fecegr."','".$ld_salint."','".$po_liq->descargo."','".$po_liq->anoser."','".$po_liq->messer."','".$po_liq->diaser."','".$ld_totasiliq."','".$ld_totdedliq."','".$ld_totpagliq."','".$po_liq->estliq."','".$ls_obsliq."','".$po_liq->dedicacion."','".$po_liq->tipopersonal."' ) ";						
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////   
   public function get_liquidacion($ps_numliq,$ps_nomper,$ps_apeper,$ps_order,&$pa_datos="")
   {
		 ...
		 $ls_nomper = $ps_nomper."%"; 
		 $ls_apeper = $ps_apeper."%";
		 $ls_sql    = "SELECT l.numliq, p.nomper, p.apeper, l.codper,l.codnom,n.desnom,l.codcauret,l.fecliq,l.fecing,l.fecegr,l.salint,l.descargo,l.anoser,l.messer,l.diaser,l.totasiliq,l.totdedliq,l.totpagliq,l.estliq,l.dedicacion,l.tipopersonal
 ...
 //-----------------------------------------Registros Contables y Presupuestarios----------------------------------//
	...
	function uf_contabilizar_liquidacion_spg($as_codnom,$as_codper,$as_numliq,$adt_fecliq)
	{
		...
		$ls_sql = " SELECT sps_liquidacion.totpagliq, sno_unidadadmin.codprouniadm, sno_fideiconfigurable.cueprefid ".
		 ...
		 elseif($row=$this->io_sql->fetch_row($rs_data))	
		 {
			...
			$lb_valido = $this->uf_insert_contabilizacion_spg($as_codnom,$ls_comprobante,$li_tipo,$ls_programatica,$ls_cueprefid,$ls_operacion,
  ...
  //      Retorna : 
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////   
  public function uf_insert_contabilizacion_spg( $ps_codnom,$ps_comprobante,$pi_tipo,$ps_programatica,$ps_cueprefid,$ps_operacion,
  ...
  {	
		...
		$ls_codestpro1=substr($ps_programatica,0,20);
		$ls_codestpro2=substr($ps_programatica,20,6);
		$ls_codestpro3=substr($ps_programatica,26,3);
		$ls_codestpro4=substr($ps_programatica,29,2);
		$ls_codestpro5=substr($ps_programatica,31,2);

		$ls_sql    = "INSERT INTO sps_dt_spg(codemp, codnom, codcom, tipo, codestpro1, codestpro2, codestpro3, codestpro4, codestpro5, spg_cuenta, operacion, ced_bene, descripcion, monto, estatus, estrd, codtipdoc, fechaconta, fechaanula) VALUES 
		              ( '".$this->ls_codemp."','" .$ps_codnom."','".$ps_comprobante."','".$pi_tipo."','".$ls_codestpro1."','".$ls_codestpro2."','".$ls_codestpro3."','".$ls_codestpro4."','".$ls_codestpro5."',
					    '".$ps_cueprefid."','".$ps_operacion."','".$ps_codper."','".$ps_descripcion."','".$pd_monliq."','".$pi_estatus."','".$pi_genrecdoc."','".$ps_tipdoc."','".$ldt_fecha."','".$ldt_fecha."' ) ";
