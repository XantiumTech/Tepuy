<?php
   ...
   function uf_limpiarvariables()
   {
		...
		global $ls_totrowrecepciones,$io_fun_sfa,$ls_numcom,$ls_parametros,$ls_rif,$ls_ano,$ls_codigo;
		...
		$ls_existe=$io_fun_sfa->uf_obtenerexiste();
   ...
   function uf_load_variables()
   {
		...
		global $io_fun_sfa,$li_totrowrecepciones,$ls_indice;
   ...
   function uf_load_data(&$as_parametros)
   {
		...
		// Fecha Creación: 20/09/2017
		//////////////////////////////////////////////////////////////////////////////
		global $li_totrowrecepciones,$io_fun_sfa;	
			
		for($li_i=1;$li_i<$li_totrowrecepciones;$li_i++)
		{
			$ls_codret=trim($io_fun_sfa->uf_obtenervalor("txtcodret".$li_i,""));
			$ls_numope=trim($io_fun_sfa->uf_obtenervalor("txtnumope".$li_i,""));
			$ls_fecfac=trim($io_fun_sfa->uf_obtenervalor("txtfecfac".$li_i,""));
			$ls_numfac=trim($io_fun_sfa->uf_obtenervalor("txtnumfac".$li_i,""));
			$ls_numcon=trim($io_fun_sfa->uf_obtenervalor("txtnumcon".$li_i,""));
			$ls_numnd=trim($io_fun_sfa->uf_obtenervalor("txtnumnd".$li_i,""));
			$ls_numnc=trim($io_fun_sfa->uf_obtenervalor("txtnumnc".$li_i,""));
			$ls_tiptrans=trim($io_fun_sfa->uf_obtenervalor("txttiptrans".$li_i,""));
			$ls_tot_cmp_sin_iva=trim($io_fun_sfa->uf_obtenervalor("txttotsiniva".$li_i,""));
			$ls_tot_cmp_con_iva=trim($io_fun_sfa->uf_obtenervalor("txttotconiva".$li_i,""));
			$ls_basimp=trim($io_fun_sfa->uf_obtenervalor("txtbasimp".$li_i,""));
			$ls_porimp=trim($io_fun_sfa->uf_obtenervalor("txtporimp".$li_i,""));
			$ls_totimp=trim($io_fun_sfa->uf_obtenervalor("txttotimp".$li_i,""));
			$ls_ivaret=trim($io_fun_sfa->uf_obtenervalor("txtivaret".$li_i,""));
			$ls_numsop=trim($io_fun_sfa->uf_obtenervalor("txtnumsop".$li_i,""));
			$ls_numdoc=trim($io_fun_sfa->uf_obtenervalor("txtnumdoc".$li_i,""));
...
<?php
	...
	switch($ls_operacion)
	{
		...
		case "ELIMINAR":
			...
			if($lb_ulitmo===false)
			{
				$io_msg->message("Usted decidio ELIMINAR un comprobante con Numeracion Intermedia.....!!!");
			}
			if($lb_ulitmo1)
			{
				$ls_tiporeten=$io_fun_sfa->uf_obtenervalor("cmbtipo","");
				$largo=2-strlen($ls_tiporeten);
				$ls_codret=str_pad($ls_tiporeten,$largo,"0",STR_PAD_LEFT);
				//print "aqui:".$ls_codret;
				//$estadeduccion=$io_modcmpret->uf_select_ded($ls_numcom,$ls_codret);
				//print "estaded: ".$estadeduccion."      ";
				// cambio el orden, primero libero los cxp_rd_deducciones y luego elimino el comprobante //
				$lb_flag=$io_modcmpret->uf_liberar_rd($ls_codret,$ls_probene,$ls_codigo,$li_totrowrecepciones,$ls_numcom);
			   	//$lb_flag=$io_modcmpret->uf_delete_cmpret($ls_numcom,$ls_codret,$la_seguridad);
			   	if($lb_flag)
			    	{
					// cambio el orden
				  //$lb_flag=$io_modcmpret->uf_liberar_rd($ls_codret,$ls_probene,$ls_codigo,$li_totrowrecepciones,$estadeduccion);
					$lb_flag=$io_modcmpret->uf_delete_cmpret($ls_numcom,$ls_codret,$la_seguridad);
				  	if($lb_flag)
					{
						$io_msg->message("El comprobante fue eliminado correctamente.....!!!");
						uf_limpiarvariables();
						$io_sql->commit();
				  	}
				  	else
              ...
              <tr>
                ...
                <td height="22" align="right"><div align="left">
				  ...
				  <?php if($ls_modageret=$_SESSION["la_empresa"]["estretiva"]!="B")
					{?>
                    <option value="01" <?php if($ls_codret=="01"){ print "selected";} ?>>IVA</option>
					{?>
                    <option value="03" <?php if($ls_codret=="03"){ print "selected";} ?>>I.S.A.E.</option>
					 }?>
				<?php }if($ls_modageret=$_SESSION["la_empresa"]["modageret"]!="B")
					{?>
                    <option value="02" <?php if($ls_codret=="02"){ print "selected";} ?>>I.S.L.R.</option>
				<?php }?>
                    <option value="04" <?php if($ls_codret=="04"){ print "selected";} ?>>Timbre Fiscal</option>
                    <option value="05" <?php if($ls_codret=="05"){ print "selected";} ?>>Otros</option>
                  </select>
                  </label>
                </div></td>
              </tr>
                  ...
                  <tr>
                    ...
                    <td width="24%"><div align="left">
					  ...
					  <?php 
					    if($ls_operacion=="NEW")
                         {
						  print"<a href=javascript:uf_catalogo_proben('D');><img src=../shared/imagebank/tools15/buscar.png name=buscar1 width=15 height=15 border=0 id=buscar1></a>";
...
function ue_imprimir()
{
	...
	if(li_imprimir==1)
	{
		...
		ls_tipo=f.cmbtipo.value;
		total=f.txtnumcom.value;
		totrow=ue_calcular_total_fila_local("txtnumope");
		ls_comprobantes1=f.txtnumcom.value;
		//f.total.value=totrow;
		//for(li_i=1;li_i<=totat;li_i++)
		//{
		//	ls_documento=eval("f.txtnumcom"+li_i+".value");
		//	ls_documento=eval("f.txtnumope"+li_i+".value");
		//	alert(ls_documento);
		if(totrow>0)
		{
			var ls_comprobantes = new Array (ls_comprobantes1);
			tiporeporte=0;
			//formato=f.formato.value;
			switch(ls_tipo)
			{
			case '01': formato="tepuy_sfa_rpp_retencionesiva_barinas.php"+"?comprobantes="+ls_comprobantes+"&tiporeporte="+tiporeporte; break;
			case '02': formato="tepuy_sfa_rpp_retencionislr.php"+"?comprobantes="+ls_comprobantes+"&mes="+ls_mes+"&anio="+ls_anio+"&tiporeporte="+tiporeporte; break;
			case '03': formato="tepuy_sfa_rpp_retencionesmunicipales.php"+"?comprobantes="+ls_comprobantes+"&mes="+ls_mes+"&anio="+ls_anio+"&tiporeporte="+tiporeporte; break;
			case '04': formato="tepuy_sfa_rpp_retencionestimbrefiscal.php"+"?comprobantes="+ls_comprobantes+"&mes="+ls_mes+"&anio="+ls_anio+"&tiporeporte="+tiporeporte; break;
			case '05': formato="tepuy_sfa_rpp_retencionesaporte.php"+"?comprobantes="+ls_comprobantes+"&mes="+ls_mes+"&anio="+ls_anio+"&tiporeporte="+tiporeporte; break;
			}
			//alert(ls_comprobantes[0]);
			pagina="reportes/"+formato;
		...
		else
		{
			...
			alert("El Comprobante debe tener al menos un Número de Documento.");	   
		}
		//}
		//alert(ls_tipo);
		//if(ls_comprobantes!="")
...
function ue_nuevo()
{
	...
	if((li_modageret!="B")||(li_conivaret!="B"))
	{
		...
		{	
			...
			f.action="tepuy_sfa_d_editcmpreten.php";
			f.submit();
		}
		else
		{
...
function ue_buscar()
{
	...
	if (li_leer==1)
	{
		window.open("tepuy_sfa_cat_cmpreten.php","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=630,height=400,left=50,top=50,location=no,resizable=yes");
	}
	else
	{
		alert("No tiene permiso para realizar esta operacion");
...
function ue_cargardatos(ls_numcom,ls_anofiscal,ls_mesfiscal,ls_codsujret,ls_nomsujret,ls_dirsujret,ls_rifsujret,ls_codret,ls_probene,ls_estcmpret)
{
	f=document.formulario;
    ...
    if (fop.estprov[0].checked==true)
	   {
		 ...
		 pagina="tepuy_sfa_cat_proveedor.php?tipo="+ls_tipo;
	...
	else
	   {
			if (fop.estprov[1].checked==true)
			{
				...
				pagina="tepuy_sfa_cat_beneficiario.php?tipo="+ls_tipo;
				window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=560,height=400,resizable=yes,location=no");
		    }
	   }
}
...
function ue_cat_solicitud(li_fila)
{
  ...
  ls_catalogo="tepuy_sfa_cat_solicitudpago.php?tipo=MODCMPRET";
...
function uf_iva(li_row)
{
  ...
  if ((ld_basimp!="0,00") && (ld_basimp!=""))	  
	 {	 
	   ...
	   pagina="tepuy_sfa_pdt_otroscreditos.php?tipo=CMPRET";
  ...
  switch (ls_tipret)
  {
  	case "0000000001":
	   ...
	   if ((ld_porcar!="0,00") && (ld_porcar!=""))
		  {
		    pagina="tepuy_sfa_pdt_deducciones.php?tipo=CMPRETIVA";
	...
	break;
  	case "0000000003":
	   ...
	   if ((ld_basimp!="0,00") && (ld_basimp!=""))
		  {
		    pagina="tepuy_sfa_pdt_deducciones.php?tipo=CMPRETMUN";
		    window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=700,height=600,resizable=yes,location=no");	  
		  }  
	   else
		  {
	...
	break;
  	case "0000000004":
	   ...
	   if ((ld_basimp!="0,00") && (ld_basimp!=""))
		  {
		    pagina="tepuy_sfa_pdt_deducciones.php?tipo=CMPRETAPO";
		    window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=700,height=600,resizable=yes,location=no");	  
		  }  
	   else
		  {
