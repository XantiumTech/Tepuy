<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("SCB","tepuy_mis_p_contabiliza_scb.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_banco_contabilizar($ls_numdoc,$ls_fecdoc,$ls_tipo_operacion,$lo_object,$li_totrows,"N",0,$ls_mes);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="tepuy_mis_p_contabiliza_scb.php";
			f.submit();
		}
		else
		{
