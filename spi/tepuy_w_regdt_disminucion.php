...
$io_seguridad= new tepuy_c_seguridad();
	...
	$ls_sistema="SPI";
	$ls_ventana="tepuy_spi_p_disminucion.php";
...
if($ls_operacion=="GUARDARPRE")
{
	...
	if($ldec_monto<=$ld_previsto_disponible){ $lb_pasar=true;}
	else { $lb_pasar=false;	}
	if(($lb_pasar)&&($lb_valido))										 
	{
		...
		if($lb_valido)
		{
			...
			$lb_valido=$in_classcmp->uf_guardar_movimientos($arr_cmp,$ls_cuenta,$ls_procede,$ls_descripcion,$ls_documento,$ls_operacionpre,0,$ldec_monto,"C");
			if($lb_valido)
			{
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="INSERT";
...
<script language="JavaScript">
  function aceptar_presupuestario()
  {
	...
	if((ls_cuenta!="")&&(ls_descripcion!="")&&(ls_procedencia!="")&&(ls_documento!="")&&(ls_operacion!="")&&(ldec_monto!=""))
	{
		...
		f.action="tepuy_w_regdt_disminucion.php";
		f.submit();
	}
	else
	{
