<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","tepuy_mis_p_reverso_soc.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_limpiarvariables()
   {
		...
		global $io_ddlb,$ls_numordcom,$ls_codprov,$ls_fecapr;
		...
		$lo_title[6]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Orden de Compras Contabilizadas";
		$ls_nametable="grid";
...
<html>
<head>
<title>Reverso de Contabilizaci&oacute;n de Compras</title>
...
<?php
	...
	$in_class = new class_tepuy_soc_integracion();  
	uf_limpiarvariables();
	switch ($ls_operacion) 
	{
		case "NUEVO":
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_reversar_ordencompra($ls_numordcom,$ls_estcondat,$ls_fechaconta,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("La Orden de Compra ".$ls_numordcom." fue reversada.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo reversar La Orden de Compra ".$ls_numordcom);
					}
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
		    $in_class_contabiliza->uf_select_ordencompra_contabilizar($ls_operacion_soc,$ls_numordcom,$ls_codprov,$ls_fecapr,2,$lo_object,$li_totrows,"REVERSAR");
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
?>		  
  ...
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Reverso de Contabilizaci&oacute;n de Ordenes de Compra </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td ><div align="left">
          ...
          <a href="javascript: ue_buscarproveedor();"><img  src="../shared/imagebank/tools20/buscar.png" alt="Buscar" width="15" height="15" border="0"></a>
...
function ue_procesar()
{
	...
	{	
		...
		if(lb_valido)
		{
			...
			f.action="tepuy_mis_p_reverso_soc.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(numordcom,operacion)
{
	...
	window.open("tepuy_mis_pdt_soc.php?numordcom="+numordcom+"&operacion="+operacion+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
</script>
