<?php
   ...
   function uf_print($as_codper, $as_cedben, $as_nomben, $as_apeben, $as_tipo, $as_codperdes, $as_codperhas)
   {
		...
		$ls_sql="SELECT codemp, codper, codben, cedben, nomben, apeben, dirben, telben, tipben, nomcheben, porpagben, monpagben, ".
				"		codban, ctaban, sc_cuenta, forpagben, nacben, tipcueben, nexben, cedaut, ".
			...
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				switch ($as_tipo)
				{
					case "":
						...
						print "'$ls_tipben','$ls_nomcheben','$li_porpagben','$li_monpagben','$ls_codban','$ls_nomban','$ls_ctaban','$ls_forpagben','$ls_nacben','$ls_tipcueben','$ls_nexben','$ls_cedaut');\">".$ls_cedben."</a></td>";
...
function aceptar(codben,cedben,nomben,apeben,dirben,telben,tipben,nomcheben,porpagben,monpagben,codban,nomban,ctaban,forpagben,nacben,tipcueben, nexben, cedaut)
{
...
function ue_search()
{
	f=document.form1;
  	...
  	f.action="tepuy_snorh_cat_beneficiario.php?tipo=<?php print $ls_tipo;?>&codperdes=<?php print $ls_codperdes;?>&codperhas=<?php print $ls_codperhas;?>";
  	f.submit();
}
</script>
</html>
