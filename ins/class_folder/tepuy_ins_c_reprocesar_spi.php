<?php
class tepuy_ins_c_reprocesar_spi
{
	var $io_sql;
	var $io_message;
	var $io_function;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function tepuy_ins_c_reprocesar_spi()
	{
	...
	function uf_reprocesar_saldos($ls_codemp,$aa_seguridad)
	{
		...
		for($li_i=1;($li_i<=$li_total_codigos)&&($lb_valido);$li_i++)		
		{
			...
			$ls_sql="SELECT spi_cuenta, sum(monto) AS monto ". 
					"  FROM spi_dt_cmp  ".
					" WHERE codemp = '".$ls_codemp."' ".
		...
		while(($li_nivel >= 1)&&($lb_valido)&&($ls_nextcuenta!=''))
		{
			if($this->io_int_spi->uf_spi_saldo_select($as_codemp,$ls_nextcuenta,&$ls_status,&$ldec_previsto,&$ldec_aumento,&$ldec_disminucion,&$ldec_devengado,&$ldec_cobrado,&$ldec_cobrado_anticipado))
			{
				...
				//{
					...
					if($li_rows===false)
					{
						$this->is_msg_error="CLASE->tepuy_int_spi MÉTODO->uf_spi_saldos_update ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
						$lb_valido=false;
					}
				//}
				//else
