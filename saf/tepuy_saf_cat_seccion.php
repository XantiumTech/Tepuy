...
if($ls_operacion=="BUSCAR")
{
	$ls_sql=" SELECT * FROM saf_seccion".
			" WHERE codgru='".$ls_codgru."'  AND ".
			"       codsubgru='".$ls_codsubgru."' AND ".
			"       codsec like '".$ls_codsec."'  AND ".
			"       densec like '".$ls_densec."' ".
			" ORDER BY codgru, codsubgru, codsec ";		
	...
	if($li_rows>0)
	{
		while($row=$io_sql->fetch_row($rs_cta))
		{
			...
			print "<td><a href=\"javascript: aceptar('$ls_codgru','$ls_codsubgru','$ls_codsec','$ls_densec');\">".$ls_codsubgru."</td>";
			print "<td><a href=\"javascript: aceptar('$ls_codgru','$ls_codsubgru','$ls_codsec','$ls_densec');\">".$ls_codsec."</td>";
...
<script language="JavaScript">
	...
	function ue_search()
	{
		...
		f.action="tepuy_saf_cat_seccion.php";
		f.submit();
	}
</script>
</html>
