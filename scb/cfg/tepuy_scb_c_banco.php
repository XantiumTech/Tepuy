...
class tepuy_scb_c_banco
{
 ...
 var $is_logusr;
 var $is_ventanas;
function tepuy_scb_c_banco($aa_security)
{
	...
	function uf_guardar_banco($ls_codigo,$ls_nombre,$ls_direccion,$ls_gerente,$ls_telefono,$ls_celular,$ls_email,$li_esttesnac,$ls_status,$ls_codsude)
	{
		...
		{
			$lb_valido=false;
			$this->is_msg_error="Error en metodo uf_guardar_banco".$this->fun->uf_convertirmsg($this->io_sql->message);
...
function uf_delete_banco($ls_codigo,$ls_nombre,$ls_direccion,$ls_gerente,$ls_telefono,$ls_celular,$ls_email,$ls_codsude)
{
	$lb_valido = false;
	$ls_codemp = $this->datemp["codemp"];
	...
	if ($rs_data===false)
	   {
		 $lb_valido=false;
		 $this->is_msg_error="CLASE->tepuy_SCB_C_BANCO->Metodo->uf_delete_banco ".$this->fun->uf_convertirmsg($this->io_sql->message);
	   }
	else
	   {
	     $lb_valido      = true;
