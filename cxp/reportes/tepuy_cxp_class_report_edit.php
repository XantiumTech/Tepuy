...
////////////////////////// FIN DE LA BUSQUEDA DEL NRO DE CODIGO DE LA RETENCION QUE VIENE///////////////////////
	...
	function uf_load_usuario($as_nomusu)
	{
		...
		//	  Description: Función que busca Nombre y apellido del usuario que aprobo la orden
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Buscar Usuario MÉTODO->uf_load_usuario ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message));
			return false;
		}
		else
		{
	...
	function uf_select_licencia($as_sql)
	{
		...
		//    Description: función que busca el numero de Licencia del SAMAT del proveedor
		//	   Creado Por: Ing. Miguel Palencia / Ing. Juniors Fraga
		// Fecha Creación: 16/05/2016									Fecha Última Modificación :  
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$rs_data=$this->io_sql->select($as_sql);
		if ($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report MÉTODO->uf_select_licencia ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
...
//////////////////////////////////////////////////////
	...
	function uf_select_contribuyentes_libro_timbrefiscal($as_mes,$as_anio,$as_codret)
	{
		...
		$ls_sql="SELECT  scb_cmp_ret.numcom, scb_cmp_ret.nomsujret, scb_cmp_ret.rif, ".
			...
			" scb_dt_cmp_ret.totcmp_con_iva, scb_dt_cmp_ret.totcmp_sin_iva, cxp_solicitudes.fecaprosol, tepuy_deducciones.porded, tepuy_deducciones.codseniat ".
			"  FROM scb_cmp_ret,scb_dt_cmp_ret,cxp_solicitudes, tepuy_deducciones ".
...
////////////////////////// FIN DE LA BUSQUEDA DE LA DENOMINACION DE LA CUENTA///////////////////////
	...
	function uf_select_retenciones_buscar($as_codded,$as_coddedhas)
	{
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;				
		$ls_sql="SELECT codded AS codigo, dended AS denominacion, sc_cuenta, porded, monded, formula, islr, iva, estretmun, otras, retaposol ".
				"  FROM tepuy_deducciones".
				" WHERE codemp='".$this->ls_codemp."'".
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_select_retenciones($as_codded,$as_as_coddedhasta,$as_tipproben,$as_codprobenhas,$as_codprobendes,$ad_fecdes,$ad_fechas,$as_codret)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_retencionesespecifico
		//         Access: public  
		...
		$ls_sql="SELECT cxp_solicitudes.numsol, MAX(rpc_proveedor.nitpro) AS nitpro, MAX(cxp_solicitudes.tipproben) AS tipproben, MAX(cxp_solicitudes.fecemisol) AS fecemisol, ".
			  ...
			  " SUM(cxp_rd_deducciones.monret) AS monret, ".
			   ...
			   "     						  	       ELSE 'RIF. ó CI. N/D'END) AS cedula_rif ".
			   "   FROM cxp_solicitudes, cxp_dt_solicitudes, cxp_rd, cxp_rd_deducciones, tepuy_deducciones, rpc_beneficiario, rpc_proveedor,scb_dt_cmp_ret, cxp_rd_cargos  ".
			   "  WHERE cxp_solicitudes.codemp = '".$this->ls_codemp."' ".
			   "    AND (cxp_solicitudes.estprosol = 'E' OR cxp_solicitudes.estprosol='C' OR cxp_solicitudes.estprosol='S' OR cxp_solicitudes.estprosol='P') ".
			   "    AND cxp_solicitudes.fecemisol >= '".$ad_fecdes."' ".
			   "    AND cxp_solicitudes.fecemisol <= '".$ad_fechas."' ".
	...
	function uf_retencionesiva_detalle($as_numcom,$as_tiporeten)
	{
		...
		//    Description: Función que busca la información de los comprobantes de iva
		//	   Creado Por: Ing. Miguel Palencia / Ing. Juniors Fraga
		// Fecha Creación: 14/01/2016									Fecha Última Modificación :  
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql="SELECT max(scb_dt_cmp_ret.codret) as codret, max(scb_dt_cmp_ret.numcom) as numcom, max(scb_dt_cmp_ret.numope) as numope, max(scb_dt_cmp_ret.fecfac) as fecfac, ".
				...
				"		 SUM(scb_dt_cmp_ret.basimp) as basimp, scb_dt_cmp_ret.porimp, SUM(scb_dt_cmp_ret.totimp) as totimp, SUM(scb_dt_cmp_ret.iva_ret) as iva_ret, max(scb_dt_cmp_ret.desope) as desope, ".
				"		 max(scb_dt_cmp_ret.numsop) as numsop, max(scb_dt_cmp_ret.codban) as codban, max(scb_dt_cmp_ret.ctaban) as ctaban, max(scb_dt_cmp_ret.numdoc) as numdoc, max(scb_dt_cmp_ret.codope) as codope, cxp_solicitudes.monsol, cxp_rd.numfac as numfac ".
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_declaracioninformativa($as_quincena,$as_tiporeten,$as_mes,$as_anio,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_declaracioninformativaislr($as_mes,$as_anio,$aa_seguridad,&$nombredelarchivoxml)
	{
		...
		//    Description: Función que genera los txt de la declaración informativa
		//	   Creado Por: Ing. Miguel Palencia / Ing. Juniors Fraga
		// Fecha Creación: 05/05/2015		Fecha Última Modificación :  05/05/2015
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ld_fechadesde=$as_anio."-".$as_mes."-01";
		$ld_fechahasta=$as_anio."-".$as_mes."-".substr($this->io_fecha->uf_last_day($as_mes,$as_anio),0,2);
	...
	function uf_retenciones_del_proveedor($as_numcom,$as_tiporeten,$as_mes,$as_anio)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report MÉTODO->uf_retenciones_del_proveedor ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_retenciones_del_proveedor_unificadas($as_rif,$as_mes,$as_anio,$as_numsol)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_retenciones_del_proveedor
		//         Access: public  
		...
		$ld_fechadesde=$as_anio."-".$as_mes."-01";
		$ld_fechahasta=$as_anio."-".$as_mes."-".substr($this->io_fecha->uf_last_day($as_mes,$as_anio),0,2);
		$ls_sql="SELECT scb_cmp_ret.numcom, scb_cmp_ret.codret, scb_cmp_ret.fecrep, scb_cmp_ret.perfiscal, scb_cmp_ret.codsujret, scb_cmp_ret.nomsujret, scb_cmp_ret.rif, scb_cmp_ret.dirsujret, scb_cmp_ret.estcmpret, scb_cmp_ret.numlic, scb_dt_cmp_ret.numfac as numficha,cxp_rd.numfac as numfac, scb_dt_cmp_ret.fecfac, scb_dt_cmp_ret.numcon, scb_dt_cmp_ret.totcmp_con_iva, cxp_dt_solicitudes.numsol ".
				"  FROM scb_cmp_ret, scb_dt_cmp_ret, cxp_rd, cxp_dt_solicitudes ".
				" WHERE scb_cmp_ret.codemp='".$this->ls_codemp."' ".
				//"   AND codret ='".$as_tiporeten."'".
				"   AND scb_cmp_ret.numcom=scb_dt_cmp_ret.numcom ".
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report MÉTODO->uf_retenciones_del_proveedor_unificadas ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_retencionesmunicipales_detalles($as_numcom,$as_tiporeten)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_retencionestimbrefiscal_detalles($as_numcom,$as_tiporeten)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_retencionesislr_detalles($as_numcom,$as_tiporeten)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_retencionesmunicipales_detalles
		//         Access: public  
		...
		$ls_sql="SELECT max(scb_dt_cmp_ret.codret) as codret, max(scb_dt_cmp_ret.numsop) as numsop, max(scb_dt_cmp_ret.fecfac) as fecfac,  max(scb_dt_cmp_ret.numfac) as numfactura, max(scb_dt_cmp_ret.numcon) as numcontrol, max(scb_dt_cmp_ret.numdoc) as numficha, ".
				"		SUM(scb_dt_cmp_ret.basimp) as basimp,  scb_dt_cmp_ret.porimp, SUM(scb_dt_cmp_ret.iva_ret) as totimp, cxp_solicitudes.monsol, cxp_rd.numfac as numfac ".
				"  FROM scb_dt_cmp_ret, cxp_solicitudes, cxp_rd ".
				" WHERE scb_dt_cmp_ret.codemp='".$this->ls_codemp."' ".
				"   AND scb_dt_cmp_ret.numcom='".$as_numcom."' ".
				"   AND scb_dt_cmp_ret.codret ='".$as_tiporeten."'".
...
//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_buscar_cod_seniat($as_numcom,$as_numfac,$as_islr,$as_proveedor)
	{
		...
		//	    Arguments: as_numcom     // Numero de comprobante de iva
		//	      Returns: lb_valido True si se creo el Data stored correctamente ó False si no se creo
		//    Description: Función que busca la información del codigo del seniat de la retencion
		//	   Creado Por: Ing. Miguel Palencia / Ing. Juniors Fraga
		// Fecha Creación: 17/07/2007									Fecha Última Modificación :  
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$cod_seniat="";
	...
	function uf_select_report_libcompra($ld_fecdesde,$ld_fechasta,$rs_data)
	{
		...
		$ls_sql= "SELECT RD.numrecdoc,RD.tipproben,RD.cod_pro,RD.ced_bene,MAX(RD.montotdoc) AS montotdoc,MAX(RD.mondeddoc) AS mondeddoc,RD.codtipdoc,MAX(RD.fecemidoc) AS fecemidoc,MAX(RD.numref) AS numref,SUM(COALESCE(RDDED.monret,0)) as monret,MAX(DED.iva) AS iva
				 FROM tepuy_deducciones DED,cxp_rd RD
				 LEFT OUTER JOIN cxp_rd_deducciones RDDED
