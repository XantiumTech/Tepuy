   ...
   else
   {
		...
		for($li_i=0;$li_i<$li_total;$li_i++)
		{
			if($li_i==0)
			{
				...
				//$ls_sql=$ls_sql."   AND (spg_cuenta like '404%'";
			}
			else
			{
				 $ls_sql=$ls_sql."    OR spg_cuenta like '".$la_spg_cuenta[$li_i]."%'";
				//$ls_sql=$ls_sql."    OR spg_cuenta like '404%'";
			}
		
		}						
		$ls_sql= $ls_sql." ) GROUP BY spg_cuenta, denominacion, codestpro1,codestpro2, codestpro3,sc_cuenta,status ORDER BY spg_cuenta"; 	
...
<script language="JavaScript">
  ...
  function ue_search()
  {
	  ...
	  f.action="tepuy_saf_cat_ctasspg.php";
	  f.submit();
  }
</script>
</html>