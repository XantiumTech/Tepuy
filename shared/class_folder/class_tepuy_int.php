...
class class_tepuy_int
{
	...
	function uf_total_niveles($as_formato)
	{
		...
		//		   Access: public 
		//       Argument: as_formato //   formato de l cuenta definida en tepuy_empresa
	...
	function uf_valida_procedencia($as_procedencia,&$as_desproc)
	{
		...
		if($rs_data===false)
		{
			$this->is_msg_error="CLASE->tepuy_int MÉTODO->uf_valida_procedencia ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			$lb_valido=false;
		}
		else
		{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_tepuy_comprobante($as_codemp,$as_procedencia,$as_comprobante,$as_fecha,$ai_tipo_comp,$as_descripcion,$as_tipo,
								   $as_cod_pro,$as_ced_bene,$adec_monto,$as_codban,$as_ctaban)
	{
        ...
        if($this->uf_select_comprobante($as_codemp,$as_procedencia,$as_comprobante,$as_fecha,$as_codban,$as_ctaban))
		{
           ...
           $lb_valido=$this->uf_tepuy_update_comprobante($as_codemp,$as_procedencia,$as_comprobante,$as_fecha,$ai_tipo_comp,
		...
		else
		{
		   $this->ib_new_comprobante=true;		
 		   $lb_valido=$this->uf_tepuy_insert_comprobante($as_codemp,$as_procedencia,$as_comprobante,$as_fecha,$ai_tipo_comp,
		   												  $as_descripcion,$as_tipo,$this->is_cod_prov,$this->is_ced_ben,
														  $as_codban,$as_ctaban);
		}
		return $lb_valido;
	} // end function uf_tepuy_comprobante
	//-----------------------------------------------------------------------------------------------------------------------------------

	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_tepuy_insert_comprobante($as_codemp,$as_procede,$as_comprobante,$as_fecha,$ai_tipo_comp,$as_descripcion,$as_tipo,
										  $as_cod_prov,$as_ced_ben,$as_codban,$as_ctaban)
	{
		...
		{
			$lb_valido=false;
			$this->is_msg_error="CLASE->tepuy_int MÉTODO->uf_tepuy_insert_comprobante ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
		}
		return $lb_valido;
	} // end function uf_tepuy_insert_comprobante
	//-----------------------------------------------------------------------------------------------------------------------------------

	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_tepuy_update_comprobante($as_codemp,$as_procede,$as_comprobante,$as_fecha,$ai_tipo_comp,$as_descripcion,$as_tipo,
										  $as_cod_prov,$as_ced_ben,$as_codban,$as_ctaban)
    {
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_tepuy_update_comprobante
		//		   Access: public 
		//       Argument: as_codemp // Código de empresa
		...
		if($li_numrows===false)
		{
			$this->is_msg_error = "CLASE->tepuy_int MÉTODO->uf_tepuy_update_comprobante ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			$lb_valido=false;
		}
		return $lb_valido;
	} // end function uf_tepuy_update_comprobante
	//-----------------------------------------------------------------------------------------------------------------------------------
    ...
    function uf_tepuy_delete_comprobante()	
    {
		...
		if($li_numrows===false)
		{
			$this->is_msg_error="CLASE->tepuy_int MÉTODO->uf_tepuy_delete_comprobante ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			return false;
		}
		return $lb_valido;
	...
	function uf_select_comprobante($as_codemp,$as_procedencia,$as_comprobante,$as_fecha,$as_codban,$as_ctaban)
	{
		...
		if($li_numrows===false)
		{
			$this->is_msg_error="CLASE->tepuy_int MÉTODO->uf_select_comprobante ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			return false;
		}
		else  
		{ 
								    ...
								    &$as_tipo_destino,&$as_ced_bene,&$as_cod_pro)
	{
		...
		if($lr_result===false)
		{
			$this->is_msg_error="CLASE->tepuy_int MÉTODO->uf_obtener_comprobante ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			return false;
		}
		else  
		{ 
	...
	function uf_select_proveedor($as_codemp,$as_codpro)
	{
		...
		if($li_numrows===false)
		{
			$this->is_msg_error="CLASE->tepuy_int MÉTODO->uf_select_proveedor ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			return false;
		}
		else  
		{ 
	...
	function uf_select_beneficiario($as_codemp,$as_cedbene)
	{
		...
		if($li_numrows===false)
		{
			$this->is_msg_error="CLASE->tepuy_int MÉTODO->uf_select_beneficiario ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			return false;
		}
		else  
		{ 
