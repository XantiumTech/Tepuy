...
if($ls_operacion=="BUSCAR")
{
	$ls_sql=" SELECT * ".
	        " FROM scb_banco  ".
			" WHERE codemp='".$ls_codemp."' AND codban like '".$ls_codigo."'";
			...
			else
			{
				...
				if($row=$SQL->fetch_row($rs_banco))
				{
					...
					for($z=1;$z<=$totrow;$z++)
					{
						...
						$email=$data["conban"][$z];
						$movil=$data["movcon"][$z];
						print "<td><a href=\"javascript: aceptar('$codigo','$nombre','$direccion','$gerente','$telefono','$movil','$email');\">".$codigo."</a></td>";
						print "<td>".$nombre."</td>";
						print "</tr>";			
					}
				}
...
<script language="JavaScript">
  function aceptar(codigo,deno,direccion,gerente,telefono,celular,email)
  {
  ...
  f.action="tepuy_catdinamic_bancos.php";
  f.submit();
  }
</script>
</html>
