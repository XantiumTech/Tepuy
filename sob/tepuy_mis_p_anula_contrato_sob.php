<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("SOB","tepuy_mis_p_anula_contrato_sob.php",$ls_permisos,$la_seguridad,$la_permisos);
   ...
   function uf_limpiarvariables()
   {
		...
		$lo_title[6]="Detalle";
		$li_widthtable=700;
		$ls_titletable="Contratos Contabilizados";
		$ls_nametable="grid";
...
<html>
<head>
<title>Anulaci&oacute;n de contabilizaci&oacute;n de Contratos</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_anular_contabilizacion_contrato($ls_codcon,$ls_codasi,$ls_fecha,$ld_fechaconta,
																					  $la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("Contrato  ".$ls_codcon." fue anulada la contabilización.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo anular la contabilización el Contrato ".$ls_codcon);
					}		
				}
			}
			$li_totrows=1;
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_sob_contratos_contabilizar($ls_codasi,$ls_codcon,$ls_feccon,$ls_fecinicon,1,
...
<table width="762" border="0" align="center" cellpadding="0" cellspacing="0" class="contorno">
  ...
  <tr>
    <td width="432" height="20" colspan="11" bgcolor="#E7E7E7"><table width="762" border="0" align="center" cellpadding="0" cellspacing="0">
      <td width="432" height="20" bgcolor="#E7E7E7" class="descripcion_sistema">Sistema Control de Obrasl -><i>M&oacute;dulo Integrador</i></td>
...
<p>&nbsp;</p>
  <table width="750" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Anulaci&oacute;n de Contabilizaci&oacute;n de Contratos </td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="tepuy_mis_p_anula_contrato_sob.php";
			f.submit();
		}
		else
		{
...
function uf_verdetalle(codcon)
{
	...
	window.open("tepuy_mis_pdt_sobcon.php?codcon="+codcon+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
</script>
