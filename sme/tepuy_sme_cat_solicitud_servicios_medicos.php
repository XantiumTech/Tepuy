<?php
	...
	$ls_coduniadm = $io_fun_sme->uf_obtenervalor("txtcoduniadm","");
	$ls_denuniadm = $io_fun_sme->uf_obtenervalor("txtdenuniadm","");
	$ls_tipodestino = $io_fun_sme->uf_obtenervalor("cmbtipdes","-");
	$ls_codprovben = $io_fun_sme->uf_obtenervalor("txtcodigo","");
	$ls_nomprovben = $io_fun_sme->uf_obtenervalor("txtnombre","");
	unset($io_fun_sme);
...
<html>
<head>
<title>Cat&aacute;logo de Solicitudes de Ejecuci&oacute;n Presupuestaria-> Servicios M&eacute;dicos</title>
...
<table width="630" border="0" align="center" cellpadding="1" cellspacing="1">
    <tr>
      <td width="600" height="20" colspan="2" class="titulo-ventana">Cat&aacute;logo de Solicitudes de Ejecuci&oacute;n Presupuestaria-> Servicios M&eacute;dicos</td>
    </tr>
  </table>
  <br>
    <table width="630" border="0" align="center" cellpadding="1" cellspacing="0" class="formato-blanco">
      ...
      <tr>
        ...
        <td height="29"><div align="left">
            <?php $io_sme->uf_load_tiposolicitudservicio(""); ?>
        </div></td>
      </tr>
      <tr>
        ...
        <div align="left">
          ...
          <a href="javascript: ue_catalogo_unidad_ejecutora();"><img src="../shared/imagebank/tools15/buscar.png" alt="Buscar" width="15" height="15" border="0" /></a>
          <label>
...
function ue_aceptar(ls_numsol,ls_codtipsol,ls_coduniadm,ls_codfuefin,ls_estsol,ls_tipo_destino,ls_codigo,ls_denuniadm,
					...
					ls_modsep,ls_codestpro1,ls_codestpro2,ls_codestpro3,ls_codestpro4,ls_codestpro5,ls_consol,ls_codtar,ls_codnom,ls_combo,ls_quienes,ls_cedquien)
{
	opener.document.formulario.txtnumsol.value=ls_numsol;
	opener.document.formulario.txtnumsol.readOnly=true;
	opener.document.formulario.cmbcodtipsol.value=ls_codtipsol+"-"+ls_modsep+"-"+ls_estope+"-"+ls_codtar+"-"+ls_codnom;
	opener.document.formulario.cmbcodtipsol.disabled=true;
	opener.document.formulario.txttipsol.value=ls_codtipsol+"-"+ls_modsep+"-"+ls_estope+"-"+ls_codtar+"-"+ls_codnom;
	...
	//opener.document.formulario.txtconsol.value=eval("f.txtconsol"+ls_numsol+".value");
	opener.document.formulario.txtconsol.value=ls_consol;
	//opener.document.formulario.cmbtipdes.value=ls_tipo_destino;
	opener.document.formulario.cmbtipdes.value=ls_combo;
	//opener.document.formulario.txtcodigo.value=ls_codigo;

	opener.document.formulario.txtcodigo.value=ls_cedquien;
	//opener.document.formulario.txtnombre.value=ls_nombre;
	//alert(ls_quienes);
	opener.document.formulario.txtnombre.value=ls_quienes;

	opener.document.formulario.txtnompro.value=ls_nombre;
	opener.document.formulario.txtcodpro.value=ls_codigo;
...
{
   window.open('tepuy_sme_cat_unidad_ejecutora.php?tipo=BUSCAR_CAT_SEP',"_blank","menubar=no,toolbar=no,scrollbars=yes,width=550,height=400,left=50,top=50,location=no,resizable=yes");
}
...
function ue_cambiardestino()
{
	...
	if(tipdes!="-")
	{
		if(tipdes=="P")
		{
          window.open("tepuy_sep_cat_proveedor.php","_blank","menubar=no,toolbar=no,scrollbars=no,width=600,height=400,resizable=yes,location=no,left=50,top=50");
		}
		else
		{
            window.open("tepuy_sep_cat_beneficiario.php","_blank","menubar=no,toolbar=no,scrollbars=no,width=600,height=400,resizable=yes,location=no,left=50,top=50");
		}	
	}
}
...
function ue_search()
{
	...
	if((fecregdes!="")&&(fecreghas!=""))
	{
		...
		ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
		// Enviar todos los campos a la pagina para que haga el procesamiento
		ajax.send("catalogo=SOLICITUDSME&numsol="+numsol+"&coduniadm="+coduniadm+"&codtipsol="+codtipsol+"&fecregdes="+fecregdes+
