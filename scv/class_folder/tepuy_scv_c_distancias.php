<?php
class tepuy_scv_c_distancias
{

	var $ls_sql;
	
	function tepuy_scv_c_distancias($conn)
	{
	...
	function uf_scv_load_destinos($as_codpaiori,$as_codestori,$as_codciuori,&$ai_totrows,&$ao_object) 
	{
		...
		else
		{
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ao_object[$ai_totrows][5]="<a href=javascript:uf_delete_dt(".$ai_totrows.");><img src=../shared/imagebank/tools15/eliminar.png alt=Eliminar width=15 height=15 border=0></a>";
			}
	...
	function uf_scv_select_distancias($as_codpaiori,$as_codestori,$as_codciuori,$as_codpaides,$as_codestdes,$as_codciudes) 
	{
		...
		if ($rs_data===false)
		{
			$this->io_msg->message("CLASE->tepuy_scv_c_distancias METODO->uf_scv_select_distancias ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
									  ...
									  $ai_cankms,$aa_seguridad) 
	{
		...
		if ($li_row===false)
		{
			$this->io_msg->message("CLASE->tepuy_scv_c_distancias METODO->uf_scv_update_distancias ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_scv_delete_distancias($as_codpaiori,$as_codestori,$as_codciuori,$as_codpaides,$as_codestdes,$as_codciudes,$aa_seguridad) 
	{
		...
		if ($rs_data===false)
		{
			...
			$this->io_msg->message("CLASE->tepuy_scv_c_ciudad METODO->uf_scv_delete_distancias ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
