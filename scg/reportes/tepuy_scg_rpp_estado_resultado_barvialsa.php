<?php
	...
	function uf_insert_seguridad($as_titulo)
	{
		...
		$lb_valido=$io_fun_scg->uf_load_seguridad_reporte("SCG","tepuy_scg_r_estado_resultado.php",$ls_descripcion);
		return $lb_valido;
	}
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_detalle_costos($la_data_cos,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_print_detalle
		//		    Acess: private
		...
		$la_columnas=array('cuenta'=>'<b>Cuenta</b>',
						   ...
						   'saldomen'=>'<b></b>',
						   'saldo'=>'<b></b>');
		$io_pdf->ezTable($la_data_cos,$la_columnas,'',$la_config);
	}// end function uf_print_detalle
	//--------------------------------------------------------------------------------------------------------------------------------
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_pie_cabecera_costos($ld_total_costos,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function : uf_print_pie_cabecera
		//		    Acess : private
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		global $ls_bolivares;
		$la_data=array(array('total'=>'<b>Total Costos '.$ls_bolivares.'</b>','saldomay'=>$ld_total_costos));
		$la_columna=array('total'=>'','saldomay'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 7, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
    ...
    // Cargar datastore con los datos del reporte
		...
		$io_pdf->ezSetCmMargins(4.5,4.5,3,3); // Configuración de los margenes en centímetros
		...
		if((($lb_valido_ing==false)&&($lb_valido_cos==false)&&($lb_valido_egr==false))||($lb_valido==false)) // Existe algún error ó no hay registros
	    {
			print("<script language=JavaScript>");
		 ...
		 if($lb_valido_ing)
		 {
			...
			for($li_i=1;$li_i<=$li_tot;$li_i++)
			{
					...
					for($li=$li_total;$li>1;$li--)
					{
						...
						$li_len=strlen(trim($ls_sc_cuenta));
		...
		}//if($lb_valido_ing) // FIN DE INGRESOS
		// COSTOS
		if($lb_valido_cos)
		 {
			...
			for($li_i=1;$li_i<=$li_tot;$li_i++)
			{
					...
					$ld_total_costos=$io_report->dts_costos->data["total_costos"][$li_i];
			...
			uf_print_detalle_costos($la_data_cos,$io_pdf); // Imprimimos el detalle 
			$ld_total_costos=abs($ld_total_costos);
			$ld_total_costos1=$ld_total_costos;
			$ld_total_costos=number_format($ld_total_costos,2,",",".");
			uf_print_pie_cabecera_costos($ld_total_costos,$io_pdf); // Imprimimos pie de la cabecera
		}//if($lb_valido_cos) 
//FIN DE COSTOS
// EGRESOS //
		if($lb_valido_egr)
	    {
				...
				for($li_i=1;$li_i<=$li_tot;$li_i++)
				{
					...
					for($li=$li_total;$li>1;$li--)
					{
						...
						$li_fila=$li_act-$li_ant; 
						$li_len=strlen(trim($ls_sc_cuenta));
						$li_len2=strlen($_SESSION["la_empresa"]["formcont"]);
						if ($li_len==25)
						{ 
						  if ($li_len2>=21)
						   {$li_len=$li_len-12;}
			...
			$ld_total_egresos=abs($ld_total_egresos);//-$ld_total_costos1;
		    $ld_total=trim($ld_total_ingresos)-($ld_total_egresos)-($ld_total_costos1);
