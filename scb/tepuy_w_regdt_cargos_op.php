...
<html>
<head>
<title>Cat&aacute;logo Cargos</title>
...
$title[1]="Check"; $title[2]="Documento"; /*$title[3]="Código";*/ $title[3]="Denominación"; /*$title[5]="Porcentaje";*/ $title[4]="Monto Obj.Ret.";  $title[5]="Monto Car.";  $title[6]="x";
$grid1="grid";	
$totrow=0;
$ldec_totcar=0;
if($ls_operacion=="")
{
    ...
    $ls_sql=" SELECT a.*,b.sc_cuenta FROM tepuy_cargos a,spg_cuentas b WHERE a.codemp='".$ls_codemp."' AND a.codemp=b.codemp AND a.spg_cuenta=b.spg_cuenta";  
		...
		{          
			...
			if($totrow>0)
			{
				for($z=1;$z<=$totrow;$z++)
				{
					...
					$ls_codestpro=$data["codestpro"][$z];
					$ls_spg_cuenta=$data["spg_cuenta"][$z];
					$ls_scg_cuenta=$data["sc_cuenta"][$z];
					...
					$object[$z][2]="<input type=text name=txtdoc".$z." value='".$ls_mov_document."'     id=txtdoc".$z." class=sin-borde readonly style=text-align:right  size=17 maxlength=15>";
					//$object[$z][3]="<input type=text name=txtcodded".$z." value='".$ls_codded."' id=txtcodded".$z." class=sin-borde readonly style=text-align:center size=15 maxlength=10 >";		
					...
					$object[$z][6]="<input name=txtcuenta".$z." type=hidden id=txtcuenta".$z." value='".$ls_spg_cuenta."'><input name=txtcodestpro".$z." type=hidden id=txtcodestpro".$z." value='".$ls_codestpro."'><input name=txtscgcuenta".$z." type=hidden id=txtscgcuenta".$z." value='".$ls_scg_cuenta."'>";
				}				
			}
			else
			{
					...
					//$object[$z][3]="<input type=text name=txtcodded".$z." value='' id=txtcodded".$z." class=sin-borde readonly style=text-align:center size=15 maxlength=10 >";		
					...
					//$object[$z][5]="<input type=text name=txtporcar".$z." value='' id=txtporcar".$z." class=sin-borde readonly style=text-align:right  size=10 maxlength=20>";
					$object[$z][4]="<input type=text name=txtmonobjret".$z." value='".number_format($ldec_objret,2,',','.')."' id=txtmonobjret".$z." class=sin-borde style=text-align:right  size=10 maxlength=20 onBlur=javascript:uf_calcular_monret($z);>";
...
{
	require_once("tepuy_scb_c_ordenpago.php");
	$in_classmovbanco=new tepuy_scb_c_ordenpago($la_seguridad);
	$totrow=$_POST["total"];
	...
	$lb_valido=$in_classmovorden->uf_guardar_automatico($ls_codban,$ls_ctaban,$ls_mov_document,$ls_codope,$ld_fecha,$ls_mov_descripcion,$ls_codconmov,$ls_codpro,$ls_cedbene,$ls_nomproben,($ldec_monto_mov+$ldec_retenido),($ldec_monto_mov+$ldec_retenido),$ldec_retenido,$ls_chevau,$ls_estmov,$li_estint,$li_cobrapaga,$ls_estbpd,$ls_mov_procede,$ls_estreglib,$ls_estdoc,$ls_tipo,$ls_tipdocres,$ls_numdocres,$ls_fecdocres,$ls_tipreg,$ls_fte_financiamiento,$ls_origen,$ls_tippag,$ls_mediopago,$ls_modalidad,$ls_coduniadm,$ls_codbansig,$ls_codestpro1,$ls_codbanbene,$ls_nombanbene,$ls_ctabanbene,$ls_codbanaut,$ls_nombanaut,$ls_ctabanaut,$ls_rifaut,$ls_nombreaut,$ls_nrocontrol);
		...
		for($z=1;$z<=$totrow && $lb_valido ;$z++)
		{
			if(array_key_exists("chk".$z,$_POST))
			{
				...
				if($lb_valido)
				{
					...
					$ls_operacion = 'CCP';
					$lb_valido=$in_classmovorden->uf_procesar_dt_gasto($ls_codban,$ls_ctaban,$ls_mov_document,$ls_codope,$ls_estmov,$ls_estpro,$ls_spgcuenta,$ls_mov_document,$ls_denominacion,$ls_mov_procede,$ldec_monto,$ls_operacion,$ls_uel,$ldec_baseimp,$ls_codcar);
					if($lb_valido)
					{
						...
						<script language="javascript">
							...
							f.txtmonto.value=<?php print ($ldec_monto_mov+$ldec_retenido);?>;
							f.txtmonobjret.value=<?php print ($ldec_monto_mov+$ldec_retenido);?>;
...
?>
  <table width="550" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    ...
    <tr>
      ...
      <td align="center"><div align="left">          <a href="javascript: uf_aceptar();"><img src="../shared/imagebank/tools20/aprobado.png" alt="Aceptar" width="20" height="20" border="0"></a> <a href="javascript: uf_aceptar();">Procesar Cargos </a>      </div>
    </tr>
  </table>
...
<script language="JavaScript">
  ...
  function uf_aceptar()
  {
	...
	if(li_total>0)
	{
		...
		f.action="tepuy_w_regdt_cargos_op.php";
		f.submit();
	}
  }
