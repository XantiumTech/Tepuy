<?PHP
	...
	function uf_print_encabezado_pagina($as_titulo,$as_periodo,$as_reporte,$as_mes1,&$io_pdf)
	{
		...
		if($as_reporte=="V") // Formato Vertical
		{
			...
			$tm1=306-($li_tm1/2);
			$io_pdf->addText($tm1,718,11,$as_periodo); // Agregar el título
		...
		else
		{
			...
			$tm1=406-($li_tm1/2);
			$io_pdf->addText($tm1,528,11,$as_periodo); // Agregar el título
		...
		else
		{
			...
			//		   array('title'=>'<b></b>','title2'=>$ldec_saldoant));
				...
				$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'shaded'=>0, // Sombra entre líneas
						 'shadeCol2'=>array(0.9,0.9,0.9), // Color de la sombra
						 'xPos'=>396, // Orientación de la tabla
						 'width'=>560, // Ancho de la tabla
						 'maxWidth'=>560,
						 'cols'=>array('title'=>array('justification'=>'right','width'=>85),'title2'=>array('justification'=>'left','width'=>195),'title3'=>array('justification'=>'right','width'=>80),'title4'=>array('justification'=>'left','width'=>165),'title5'=>array('justification'=>'right','width'=>105),'title6'=>array('justification'=>'right','width'=>135))); // Ancho Máximo de la tabla
		}
	...
	if($ls_reporte=="V")
	{
	$io_pdf->ezSetCmMargins(3.5,1,1.5,1.5); // Configuración de los margenes en centímetros //1=superior 2=derecho 3=izquierdo 4=abajo
	}
	else
	{
	$io_pdf->ezSetCmMargins(3.5,2,2.5,2.5); // Configuración de los margenes en centímetros //1=superior 2=derecho 3=izquierdo 4=abajo
	}
	uf_print_encabezado_pagina("<b>Estado de Cuenta $ls_tipbol</b>","<b>Del $ld_fecdesde al $ld_fechasta</b>",$ls_reporte,$ld_fechasta,$io_pdf); // Imprimimos el encabezado de la página
	if($ls_reporte=="V") // Formato Vertical
	{
		$io_pdf->ezStartPageNumbers(560,22,10,'','',1); // Insertar el número de página
	}
	else // formato Horizonal
	{
		$io_pdf->ezStartPageNumbers(760,22,10,'','',1); // Insertar el número de página
	}
	...
	if($li_totrow>0)
	{
		for($li_i=1;$li_i<=$li_totrow;$li_i++)
		{
			...
			$la_columna=array('fecha'=>'<b>Fecha</b>  ','operacion'=>'<b>Op. </b>  ','numdoc'=>'<b>Nº Documento</b>  ','beneficiario'=>'<b>Beneficiario </b>  ','concepto'=>'<b>Concepto de Pago </b>','debito'=>'<b>Debitos </b>  ','credito'=>'<b>Creditos </b>  ','saldo'=>'<b>Saldo </b>  ');
			if($ls_reporte=="V") // formato Vertical
			{
					$la_config=array('showHeadings'=>1, // Mostrar encabezados
							 ...
							 'cols'=>array('fecha'=>array('justification'=>'center','width'=>48),'operacion'=>array('justification'=>'center','width'=>22),
										...
										'saldo'=>array('justification'=>'right','width'=>82))); // Ancho Máximo de la tabla
			}
