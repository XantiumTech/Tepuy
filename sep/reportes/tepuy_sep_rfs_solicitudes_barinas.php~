<?php
	...
	function uf_insert_seguridad($as_titulo)
	{
		...
		$lb_valido=$io_fun_sep->uf_load_seguridad_reporte("SEP","tepuy_sep_p_solicitud.php",$ls_descripcion);
		return $lb_valido;
	}
	//-----------------------------------------------------------------------------------------------------------------------------------
	//-----------------------------------------------------------------------------------------------------------------------------------
...
//////////////////////////////////AGREGA TITULO  ///////////////////////////
		...
		$io_pdf->addText($tmy+115,$tmx-20,7,"REPUBLICA BOLIVARIANA DE VENEZUELA"); // Agregar el título
		$io_pdf->addText($tmy+126,$tmx-30,7,$ls_nomemp); // Agregar el título
		$io_pdf->addText($tmy+103,$tmx-40,7,"DIRECCION DE PLANIFICACION Y PRESUPUESTO"); // Agregar el título

		$as_numficha=$as_numsol;
		$ls_ano = substr($_SESSION["la_empresa"]["periodo"],0,4);
		$io_pdf->addText($tmy+30,$tmx-60,10,"HOJA DE IMPUTACIÓN PRESUPUESTARIA EJERCICIO FISCAL ".$ls_ano); // Agregar el título
		$io_pdf->addText(549,$tmx-40,7,"No.: ".substr($as_numsol,9,6)); // Agregar el título
		//$io_pdf->addText(518,$tmx-50,7,"No. de Ficha: ".substr($as_numficha,9,6)); // Agregar el título
		$io_pdf->addText(528,$tmx-50,7,"Fecha: ".$ad_fecregsol); // Agregar el título
...
/////////////////////////////////////  TAMAÑO CARTA
		...
		//$io_pdf->addText($tm,730,11,"SOLICITUD DE EJECUCION PRESUPUESTARIA"); // Agregar el título
		//$io_pdf->addText(485,748,9,"No. ".substr($as_numsol,9,6)); // Agregar el título
...
/*        $io_pdf->Rectangle(15,60,570,70);
		...
		$io_pdf->addText(257,63,7,"ADMINISTRACIÓN Y HACIENDA"); // Agregar el título
...
//FORMATO ALCALDIA DE BARINAS
		...
		//$io_pdf->addText(55,30,7,"Funcionario"); // Agregar el título
		$io_pdf->addText(55,30,7,$ad_funcionario); // Agregar el tï¿½tulo
		$ls_jefe_presupuesto  = $_SESSION["la_empresa"]["jefe_presupuesto"];
		$ls_cargo_presupuesto = $_SESSION["la_empresa"]["cargo_presupuesto"];

		//$io_pdf->addText(455,30,7,"Lcdo. Reinaldo Urbina"); // Agregar el título
		//$io_pdf->addText(350,22,6," Direccion de Planificación, Programación y Presupuesto"); // Agregar el título
                $io_pdf->addText(445,30,7, $ls_jefe_presupuesto); // Agregar el tï¿½tulo
                $io_pdf->addText(405,22,7,$ls_cargo_presupuesto); // Agregar el tï¿½tulo
...
//////////////////////////////////////////////
	...
	function uf_print_detalle_cuentas($la_data,&$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
				         ...
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,
						 'cols'=>array('codestpro1'=>array('justification'=>'center','width'=>40), // Justificación y ancho de la columna
						 			   'codestpro2'=>array('justification'=>'center','width'=>49), // Justificación y ancho de la columna
						 			   'codestpro3'=>array('justification'=>'center','width'=>49), // Justificación y ancho de la columna
						 			   'cuenta'=>array('justification'=>'center','width'=>102), // Justificación y ancho de la columna
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	 ...
	 if($ls_tipoformato==1)
	 {
		require_once("tepuy_sep_class_reportbsf.php");
		$io_report=new tepuy_sep_class_reportbsf();
	 }
	 else
	 {
		require_once("tepuy_sep_class_report.php");
		$io_report=new tepuy_sep_class_report();
  	 }	
	...
	if($lb_valido)
	{
		...
		else  // Imprimimos el reporte
		{
			...
			$io_pdf->selectFont('../../shared/ezpdf/fonts/Helvetica.afm'); // Seleccionamos el tipo de letra
			$io_pdf->ezSetCmMargins(2,1,3,3); // Configuración de los margenes en centímetros
			//$io_pdf->ezSetCmMargins(3.6,5,3,3); // Configuración de los margenes en centímetros
			//$io_pdf->ezStartPageNumbers(570,47,8,'','',1); // Insertar el número de página
			$io_pdf->ezStartPageNumbers(300,10,8,'','',1); // Insertar el número de página
			...
			for($li_i=1;$li_i<=$li_totrow;$li_i++)
			{
				...
				if($ayuda=="1")
				{
					$ls_dentipsol="Ayuda Economica de Tipo: ".$ls_dentipsol;
				}
				if($ld_funcionario1!="")
				{
