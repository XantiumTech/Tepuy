<?php
   ...
   function uf_print($as_codigo, $as_denominacion, $as_tipo)
   {
		...
		$ls_sql= "SELECT codemp, codnom, desnom, tippernom, despernom, anocurnom, fecininom, peractnom, numpernom, tipnom, ".
				 ...
				 "		 perresnom, recdocnom, tipdocnom, recdocapo, tipdocapo, conpernom, titrepnom, codorgcestic, ".
				 "		 (SELECT count(codperi) FROM sno_periodo ".
                 " 		   WHERE codemp='".$ls_codemp."' ".
				 "   		 AND codnom='".$as_codigo."' ".
				 "   		 AND cerper=1) AS total, ".
				 "		 (SELECT substr(fecdesper,6,2) FROM sno_periodo ".
		...
		if($as_tipo=="HISTORICO")
		{
			...
			$ls_sql= "SELECT codemp, codnom, desnom, tippernom, despernom, anocurnom, fecininom, peractnom, numpernom, tipnom, ".
					 "		 subnom, racnom, adenom, espnom, ctnom, ctmetnom, diabonvacnom, diareivacnom, diainivacnom, diatopvacnom, ".
					 "		 diaincvacnom, consulnom, descomnom, codpronom, codbennom, conaponom, cueconnom, notdebnom, numvounom, ".
					 "		 perresnom, recdocnom, tipdocnom, recdocapo, tipdocapo, 0 AS total, conpernom, titrepnom, codorgcestic ".
					 "  FROM sno_thnomina ".
					 "  WHERE codemp='".$ls_codemp."' ".
			...
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				switch ($as_tipo)
				{
					case "":
						...
						'$ls_descomnom','$ls_conpernom','$ls_titrepnom','$ls_codorgcestic');\">".$ls_codnom."</a></td>";
...
<script language="JavaScript">
  function aceptar(codnom,denominacion,cmbtipoperiodo,despernom,anocurnom,fecininom,peractnom,cmbtipnom,cmbmet,diabonvacnom,diainivacnom,
					...
					descomnom,conpernom,titrepnom,codorgcestic)
  {
	 ...
	 opener.document.form1.action="tepuy_snorh_d_nominas.php";
 ...
 function aceptarimportar(codnom,desnom)
 {
	 ...
	 opener.document.form1.operacion.value="BUSCAR";
	 opener.document.form1.action="tepuy_sno_p_importardefiniciones.php";
	 opener.document.form1.submit();
	 close();
 }
...
function aceptarhistorico(codnom)
{
	...
	opener.document.form1.txtcodnom.readOnly=true;
	opener.document.form1.operacion.value="BUSCAR";
	opener.document.form1.action="tepuy_sno_p_hmodificarnomina.php";
	opener.document.form1.submit();
	close();
}
...
function ue_search()
{
	...
	f.action="tepuy_snorh_cat_nomina.php?tipo=<?PHP print $ls_tipo;?>";
	f.submit();
}
</script>
</html>