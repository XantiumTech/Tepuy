<?Php
   ...
   $in_class_int = new class_tepuy_cxp_integracion();
   $in_class_contabiliza->uf_load_seguridad("MIS","tepuy_mis_p_contabiliza_actmov.php",$ls_permisos,$la_seguridad,$la_permisos);
?>
...
<?php
  ...
  {  
		...
		for($li_i=1;$li_i<=$li_total_record;$li_i++)
		{
			...
			{
				$ls_cmpmov = $_POST["txtcmpmov".$li_i];
				$ls_feccmp = $_POST["txtfeccmp".$li_i];		
				$lb_valido = $in_class_int->uf_procesar_contabilizacion_solicitud_sep( $ls_cmpmov , $ls_fecha);
				if($lb_valido) { $in_class_int->io_msg->message("El movimiento de activo fue contabilizado"); }
				else { $in_class_int->io_msg->message("Error en contabilizar movimiento de activo."); }		
			}
		}
  }
 ...
 ?>	  <p>&nbsp;</p>
  <table width="502" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Contabilizaci&oacute;n de Movimientos de Comprobantes Activos </td>
    </tr>
    <tr>
    ...
    <tr>
	  ...
	  <?php
	     $title[1]="";$title[2]="Nº Comprobante";$title[3]="Fecha"; 
	     $in_class_contabiliza->uf_select_activos_movimientos_contabilizar( $arr_object ,$li_total_record,0);
		 $io_grid->makegrid($li_total_record,$title,$arr_object,300,"Movimientos de Comprobantes","grdsep" );
	  ?>
	  </div></td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		f.action="tepuy_mis_p_contabiliza_actmov.php";
		f.submit();
	}
	else
	{
