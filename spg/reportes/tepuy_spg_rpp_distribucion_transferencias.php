...
// ESTAS LINEAS SON NECESARIAS PARA QUE SE PUEDA MOSTRAR EL PDF //
	...
	function uf_insert_seguridad()
	{
	    ...
	    $ls_evento      = "IMPRIMIR";
	    $ls_descripcion = "Imprimio Distribucion de Transferencias";
...
//print $ls_tipoformato;
	...
	//----------------------------------------------------  Parámetros del encabezado  -----------------------------------------------
		if($li_cmbnivel==0)
		{
			$cuenta_inversion="407";
			$cuenta_inversion1="407";
			$ls_titulo=" <b> RELACIÓN DE TRANSFERENCIAS </b>";
		}
		if($li_cmbnivel==1)
		{
			$cuenta_inversion="4070103";
			$cuenta_inversion1="4070102";
			$ls_titulo=" <b> RELACIÓN DE TRANSFERENCIAS A LOS ORGANISMOS DEL SECTOR PÚBLICO </b>";
		}
		if($li_cmbnivel==2)
		{
			$cuenta_inversion="4070101";
			$cuenta_inversion1="4070102";
			$ls_titulo=" <b> RELACIÓN DE TRANSFERENCIAS A LOS ORGANISMOS DEL SECTOR PRIVADO </b>";
			$ls_fuente="04";
		}
	...
	if ($lb_valido)
	   {*/
         ...
         $lb_valido=$io_report->uf_spg_reportes_distribucion_transferencias($li_cmbnivel,$cuenta_inversion,$cuenta_inversion1,$ls_fuente);
			  ...
			  for ($z=1;$z<=$li_total;$z++)
			      {
					...
					if($li_cmbnivel==0)
					{
						if($ls_partida_gen=="00"){$ld_total_pres_anual=$ld_total_pres_anual+$ld_asignado;}
					}
					{
						if($ls_partida_subesp=="00"){$ld_total_pres_anual=$ld_total_pres_anual+$ld_asignado;}
					}
