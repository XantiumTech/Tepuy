<?php 
class tepuy_sss_c_derechogrupo
{
	var $obj="";
	var $io_sql;
	var $siginc;
	...
	function tepuy_sss_c_derechogrupo()
	{
...
//---------------------------------------------------------------------------------------------------------------------------------	
	function  uf_sss_insert_permisos_internos($as_codemp,$as_nomgru,$as_codsis,$as_codintper,$aa_seguridad)
	{
		...
		$lb_existe=$this->uf_existe_permiso_interno($as_codemp,$as_codsis,$as_nomgru,$ls_auxcodintper);
		if(!$lb_existe)
		{
...
//---------------------------------------------------------------------------------------------------------------------------------	
	function  uf_sss_insert_permisos_internos_usuario($as_codemp,$as_codusu,$as_codsis,$as_codintper,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_sss_insert_permisos_internos
		//         Access: public (tepuy_sss_p_derecho_usuario)
		...
		//				   $as_codintper // Codigo Interno de Permisos
		//      		   $aa_seguridad // arreglo de registro de seguridad
		//    Description: Función que se encarga de insertar por defecto los datos en la tabla sss_permisos_internos_usuario
		//	   Creado Por: Ing. Arnaldo Suárez
		// Fecha Creación: 30/06/2008									Fecha Última Modificación: 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		$ls_auxcodintper=$as_codintper;
		$lb_existe=$this->uf_existe_permiso_interno_usuario($as_codemp,$as_codusu,$as_codsis,$ls_auxcodintper);
		if(!$lb_existe)
		{
			$ls_sql = " INSERT INTO sss_permisos_internos(codemp,codsis,codusu,codintper) ".
				      " VALUES('".$as_codemp."','".$as_codsis."','".$as_codusu."','".$as_codintper."')";	  		  
			$li_row=$this->io_sql->execute($ls_sql);
			if($li_row===false)
			{
				$this->io_msg->message("CLASE->derechogrupo MÉTODO->uf_sss_insert_permisos_internos_usuario ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			}
			else
			{
				$lb_valido=true;
...
//---------------------------------------------------------------------------------------------------------------------------------	
											 ...
											 $ai_ejecutar,$as_codintper,$aa_seguridad)
	{
		...
		if($lb_valido)
		{
			...
			{
				//print "Usuario: ".$ls_codusu;
				$ls_codusu=$aa_usuarios[$li_i];
				//print "Usuario: ".$ls_codusu;
				$lb_exicodint=$this->uf_select_permisos_internos($as_codemp,$as_codsis,$ls_codusu,&$aa_codintper);
				if($lb_exicodint)
				{
					 $li_codint   =count($aa_codintper);
	                 if($li_i <= $li_codint)
					 {
					  $ls_codintper=$aa_codintper[$li_i];
					 }
					 else
					 {
					  $ls_codintper=$aa_codintper[$li_codint];
					 } 
					 ...
					 {	
						if($li_j <= $li_codint)
					    {
					     $ls_codintper=$aa_codintper[$li_j]; 
					    }
					    else
					    {
					     $ls_codintper=$aa_codintper[$li_codint];
						...
						$lb_valido=$this->uf_sss_insert_permisos_internos_usuario($as_codemp,$ls_codusu,$as_codsis,$as_codintper,
																		  $aa_seguridad);
					 }
				}
				else
...
//---------------------------------------------------------------------------------------------------------------------------------	
	...
	function  uf_sss_procesar_usuariosgrupos_spg_sno($as_codemp,$as_nomgru,$as_codsis,$as_nomven,$ai_visible,$ai_enabled,$ai_leer,
											         $ai_incluir,$ai_cambiar,$ai_eliminar,$ai_imprimir,$ai_administrador,$ai_anular,
											         $ai_ejecutar,$as_codintper,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_sss_procesar_usuariosgrupos_spg_sno
		//         Access: public 
		//      Argumento: $as_codemp        // Codigo de Empresa
		//      		   $as_nomgru        // Nombre de Grupo
		//      		   $as_codsis        // Codigo de Sistema
		...
		if($lb_valido)
		{
			...
			for($li_i=1;$li_i<=$li_cont;$li_i++)
			{
				...
				else
				{
				 if($as_codsis=='SNO')
				 {
				  $this->io_msg->message("El usuario ".trim($ls_codusu)." no tiene ninguna Nomina asociada");
				  $lb_valido = false;
				 }
				 if($as_codsis=='SPG')
				 {
				  $this->io_msg->message("El usuario ".trim($ls_codusu)." no tiene ninguna Estructura Presupuestaria asociada");
				  $lb_valido = false;
				 }
...
//---------------------------------------------------------------------------------------------------------------------------------	
											...
											$ai_ejecutar,$as_codintper,$aa_seguridad)
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->derechogrupos MÉTODO->uf_sss_insert_derecho_usuario ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
...
function uf_select_permisos_internos_grupo($as_codemp,$as_codsis,$as_nomgru,&$aa_codintper)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_select_permisos_internos
		//         Access: public (tepuy_c_permisos_globales)
		...
		//      		   $as_nomgru    // nombre del grupo
		//      		   $aa_codintper // arreglo de los codigos internos de permisologia
		//    Description: Función que se encarga de verificar si el grupo tiene codigos internos de permisos para los sistemas 
		//				   sno y spg.
		//	   Creado Por: Ing. Arnaldo Suárez
		// Fecha Creación: 02/07/2008									Fecha Última Modificación : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
...
//---------------------------------------------------------------------------------------------------------------------------------	
	...
	function uf_existe_permiso_interno($as_codemp,$as_codsis,$as_nomgru,$as_codintper)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_existe_permiso_interno
		//         Access: public (tepuy_c_permisos_globales)
		//      Argumento: $as_codemp    // codigo de empresa
		//      		   $as_codsis    // codigo del sistema
		//      		   $as_nomgru    // nombre del grupo
		//      		   $as_codintper // codigo del permiso
		//    Description: Función que se encarga de verificar si el usuario ya tiene el codigo interno de permisos
		//	   Creado Por: Ing. Arnaldo Suárez
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->derechogrupo MÉTODO->uf_existe_permiso_interno ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$li_totrow=$this->io_sql->num_rows($rs_data);
		...
		return $lb_valido;
	}  //  end  function uf_existe_permiso_interno
//---------------------------------------------------------------------------------------------------------------------------------		

function uf_existe_permiso_interno_usuario($as_codemp,$as_codusu,$as_codsis,$as_codintper)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_existe_permiso_interno
		//         Access: public (tepuy_c_permisos_globales)
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->derechogrupo MÉTODO->uf_existe_permiso_interno_usuario ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$li_totrow=$this->io_sql->num_rows($rs_data);
...
}  // end  class tepuy_sss_c_derechogrupo
?>
