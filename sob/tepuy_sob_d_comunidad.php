...
require_once("class_folder/tepuy_sob_c_comunidad.php");
...
$io_comunidad = new tepuy_sob_c_comunidad($conn);//Instanciando la Clase tepuy Definiciones.
...
//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////////////////
	...
	$ls_sistema  = "SOB";
	$ls_ventanas = "tepuy_sob_d_comunidad.php";
...
if ($ls_operacion=="NUEVO") 
   {
//print "entre";
	 $ls_codigo = $io_comunidad->uf_generar_codigo($lb_empresa,$ls_codemp,'tepuy_comunidad','codcom',$ls_codpais,$ls_codest,$ls_codmun,$ls_codpar);
	 if (empty($ls_codigo))
	    {
...
if ($ls_operacion=="ue_guardar")
   { 
    $lb_existe=$io_comunidad->uf_select_comunidad($ls_codpais,$ls_codest,$ls_codmun,$ls_codpar,$ls_codigo);
	if ($lb_existe)
       { 
	     if ($ls_estatus=="NUEVO")
		    {
			  $io_msg->message("Este Código de Comunidad ya existe !!!");  
			  $lb_valido=false;
			}
		 elseif($ls_estatus=="GRABADO")
		    {
	          $lb_valido=$io_comunidad->uf_update_comunidad($ls_codpais,$ls_codest,$ls_codmun,$ls_codpar,$ls_codigo,$ls_denominacion,$la_seguridad);
	          if ($lb_valido)
		         {
	    	       $io_sql->commit();
	   ...
	   {   
		  $lb_valido=$io_comunidad->uf_insert_comunidad($ls_codpais,$ls_codest,$ls_codmun,$ls_codpar,$ls_codigo,$ls_denominacion,$la_seguridad);
	      if ($lb_valido)
			 {
			   $io_sql->commit();
...
if ($ls_operacion=="ue_eliminar")
   {
      ...
      if ($lb_existe)
	     {
     	   $ls_condicion = " AND (column_name='codcom')";//Nombre del o los campos que deseamos buscar.
	       $ls_mensaje   = "";  //Mensaje que será enviado al usuario si se encuentran relaciones a asociadas al campo.
	       $lb_tiene     = $io_chkrel->uf_check_relaciones($ls_codemp,$ls_condicion,'tepuy_comunidad',$ls_codigo,$ls_mensaje);//Verifica los movimientos asociados a la cuenta
           if (!$lb_tiene)
		      {
		        $lb_valido=$io_comunidad->uf_delete_comunidad($ls_codemp,$ls_codpais,$ls_codest,$ls_codmun,$ls_codpar,$ls_codigo,$ls_denominacion,$la_seguridad);
			    if ($lb_valido)
				   {
					 $io_sql->commit();
				   ...
				   {
					 $io_sql->rollback();
					 $io_msg->message($io_comunidad->is_msg_error);
		           }	
			  }	 
		   else
			  {
      ...
      <tr>
        <td width="562" height="231"><div align="center">
          <table  border="0" cellspacing="0" cellpadding="0" class="formato-blanco" align="center">
            <tr>
              <td height="22" colspan="2" class="titulo-ventana">Registro de Comunidad </td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td height="22"><?php
            //Llenar Combo Pais
            $rs_data = $io_comunidad->uf_llenarcombo_pais();
            ?>
            ...
            <tr>
              ...
              <td height="22"><?php
          //Llenar Combo Estado
		  $rs_data = $io_comunidad->uf_llenarcombo_estado($ls_codpais);
		   ?>
		  ...
		  $rs_data = $io_comunidad->uf_llenarcombo_municipio($ls_codpais,$ls_codest);
          ?>
            ...
            <tr>
			  ...
			  <?php
		        ...
		        $rs_data = $io_comunidad->uf_select_parroquia($ls_codpais,$ls_codest,$ls_codmun);
              ?>
                <select name="cmbparroquia" id="cmbparroquia"  style="width:150px" onChange="javascript:uf_cambio_parroquia();">
                <option value="---">---seleccione---</option>
              <?php
		      while ($row=$io_sql->fetch_row($rs_data))
		            {
		              ...
		              if ($ls_codpar==$ls_parroquia)
					     {
						   print "<option value='$ls_parroquia' selected>$ls_denpar</option>";
					     }
					  else
					     {
						   print "<option value='$ls_parroquia'>$ls_denpar</option>";
					     }
				    } 
	          ?>
                </select>
              <input name="hidparroquia" type="hidden" id="hidparroquia" value="<?php print $ls_codpar ?>"></td>
            </tr>
            <tr>
              <td height="22" align="right"><span class="style2">C&oacute;digo</span></td>
              <td height="22"><input name="txtcodigo" type="text" id="txtcodigo" value="<?php print  $ls_codigo ?>" size="6" maxlength="3" onKeyPress="return keyRestrict(event,'1234567890');"  onBlur="javascript:rellenar_cad(this.value,3)" style="text-align:center ">
            ...
            <tr>
              ...
              <td height="22"><input name="txtdenominacion" id="txtdenominacion" value="<?php print $ls_denominacion ?>" type="text" size="60" maxlength="60" onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnñopqrstuvwxyz '+',.-');"></td>
            </tr>
            <tr>
              <td height="22">&nbsp;</td>
              <td height="22">&nbsp;</td>
...
function ue_nuevo()
{
  ...
  if (li_incluir==1)
	 {	
	   ...
	   f.cmbpais.value         = "058";
	   f.cmbestado.value       = "006";
	   f.cmbmunicipio.value    = "---";
	   f.cmbparroquia.value    = "---";
	 }  
  else
     {
...
if (ls_codpais!='---' && ls_codest!='---' && ls_codmun!='---' && ls_codpar!='---')
   {
     if (((lb_status=="GRABADO")&&(li_cambiar==1))||(lb_status=="NUEVO")&&(li_incluir==1))
        {
          with (document.form1)
	           {
	             if (campo_requerido(txtcodigo,"El Código de la Comunidad debe estar lleno !!")==false)
			        {
			          txtcodigo.focus();
			        }
			     else
			        {   
			          ...
			          if (campo_requerido(txtdenominacion,"La Denominación de la Comunidad debe estar llena !!")==false)
				         {
				      ...
				      else
				         {
					       ...
					       f.action          = "tepuy_sob_d_comunidad.php";
					       f.submit();
					     }
			        }
		       }			
...
function ue_buscar()
{
	...
	if (li_leer==1)
	   {
         if (f.cmbparroquia.value=="---")
            {
	          alert("Debe seleccionar un Pais, Estado, Municipio y Parroquia para ver el Catálogo de Comunidades !!!");
            }
         else
            {   
	          f.operacion.value = "";
	          ...
	          ls_parroquia      = f.cmbparroquia.value;
			  pagina            = "tepuy_sob_cat_comunidad.php?hidpais="+ls_pais+"&hidestado="+ls_estado+"&hidmunicipio="+ls_municipio+"&hidparroquia="+ls_parroquia;
...
function uf_cambio_parroquia()
{
	f=document.form1;
	if (f.cmbmunicipio.value=="---")
	   {
		  f.operacion.value="";
		  f.txtcodigo.value="";
		  f.txtdenominacion.value=""; 
