<?php
	...
	$ls_nomfuefin=$class_report->uf_select_data($io_sql,"SELECT * FROM tepuy_fuentefinanciamiento WHERE codfuefin ='".$ls_fte_financiamiento."' AND codemp='".$ls_codemp."'","denfuefin");
 ...
 $io_pdf->add_tabla(26,$la_data,$la_opciones);
  ...
  //-----------------------------Datos de la tabla-------------------------------------------------//
	...
	$li_z=0;
	 ...
	 {
 		$ls_codded=$ds_dt_scg->getValue("codded",$li_x);
		$ls_dended=$class_report->uf_select_data($io_sql,"SELECT * FROM tepuy_deducciones WHERE codded='".$ls_codded."'","dended");
		$ls_debhab=$ds_dt_scg->getValue("debhab",$li_x);
