...
$io_fun_banco->uf_load_seguridad("SCB","tepuy_scb_p_emision_chq.php",$ls_permisos,$la_seguridad,$la_permisos);
		...
		{   
			...
			for($li_i=1;$li_i<=$li_totalRows;$li_i++)				
			{
				if(array_key_exists("chk".$li_i,$_POST))
				{
					...
					if(!$lb_fechavalida){
					$msg->message("La Fecha de emision del cheque no debe ser menor a la fecha de la solicitud... Esta opcion es solo de Muestra pero va a ejecutar el proceso y queda a responsabilidad absoluta del Usuario. El Sistema TEPUY no ejerce control con respecto a este proceso...");
										...
										if(array_key_exists("codestpro1",$in_class_emichq->ds_sol->data))
										{
											...
											for($li_y=1;$li_y<=$li_total_rows;$li_y++)
											{
												...
												{							
													...
													$lb_valido=$in_classmovbanco->uf_procesar_dt_gasto($ls_codban,$ls_cuenta_banco,$ls_documento,'CH',$ls_estmov,$ls_programa,$ls_cuentaspg,$ls_numsol,$ls_descripcion,'CXPSOP',$ldec_MontoSpgDet,'PG');
												}//End if								
											}//End for							
										}
									}//End quinto if
...
?>
  ...
  <table width="762" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td height="22" colspan="4">Emisi&oacute;n de Cheques </td>
    </tr>
    <tr>
   ...
   <!--     <input name="txtdocumento" type="text" id="txtdocumento" value="<?php print $ls_documento;?>" size="24" maxlength="15" onBlur="javascript:rellenar_cad(this.value,15,'doc');" style="text-align:center" onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnñopqrstuvwxyz '+'-()[]{}#/');" <?php print $ls_lectura?> >-->
		<?php
	     if ($_SESSION["la_empresa"]["confi_ch"]==0)
		    {
		?>
		      <a href="javascript:cat_cheque();"><img src="../shared/imagebank/tools15/buscar.png" width="15" height="15" border="0" ></a>
		<?php
		    }
		?>
...
f.action="tepuy_scb_p_emision_chq.php";
f.submit();
}
...
function catprovbene()
{
	...
	if(f.rb_provbene[0].checked==true)
	{
		...
		window.open("tepuy_cat_prog_proveedores.php","Catalogo","menubar=no,toolbar=no,scrollbars=yes,width=650,height=450,left=50,top=50,location=no,resizable=yes");
	}
	else if(f.rb_provbene[1].checked==true)
	{
		f.txtprovbene.disabled=false;	
		window.open("tepuy_cat_prog_beneficiario.php","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=650,height=450,left=50,top=50,location=no,resizable=yes");
	}	
}   
