...
class tepuy_saf_c_activo
{
	...
	function uf_saf_select_activo($as_codemp,$as_codact)
	{
		...
		//         Access: public (tepuy_siv_d_activos)
		...
		return $lb_valido;
	}//fin de la function uf_saf_select_activo()
								   ...
								   $as_codsubgru,$as_codsec,$as_codite, $as_clasif)
	{
		...
		//         Access: public (tepuy_siv_d_activo)
		...
		//                 $as_codsec    //  codsec
		//                 $as_clasif    //  clasificación del inmueble
		//	      Returns: Retorna un Booleano
		...
		$ls_sql = "INSERT INTO saf_activo (codemp,codact,denact,maract,modact,fecregact,feccmpact,codconbie,spg_cuenta_act,esttipinm,". 
				  ...
				  "        '".$as_codsitcon."','".$as_codconcom."','".$ls_codgru."','".$ls_codsubgru."','".$ls_codsec."','".$ls_codite."','".$as_clasif."')";		  		  
								   ...
								   $as_codsubgru,$as_codsec,$as_codite,  $as_clasif)
	{
		...
		//                 $as_codgru    // codigo del grupo                  $as_codsubgru // codigo del subgrupo
		//                 $as_codsec    //  codsec
		//                  $as_clasif   // clasificaciòn del inmueble
		//	      Returns: Retorna un Booleano
		...
		$ls_sql="UPDATE saf_activo".
				...
				" 		codgru='".$ls_codgru."',codsubgru='".$ls_codsubgru."',codsec='".$ls_codsec."',codite='".$ls_codite."', ".
				"       tipinm='".$as_clasif."' ".
				" WHERE codemp =  '".$as_codemp ."'". 
				"   AND codact =  '".$as_codact ."'";		
		$li_row = $this->io_sql->execute($ls_sql);
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->activo MÉTODO->uf_sss_update_activo ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
			$this->io_sql->rollback();
		}
		else
	...
	function  uf_saf_update_depreciacion($as_codemp,$as_codact,$as_metodo,$ai_vidautil,$as_valres,$as_ctadep,$as_ctacon,
										 $as_codestpro1,$as_codestpro2,$as_codestpro3,$as_codestpro4,$as_codestpro5,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_saf_load_seriales($as_codemp,$as_codact,&$ao_object,&$ai_totrows)
	{
		...
		else
		{
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ao_object[$ai_totrows][9]="<a href=javascript:uf_agregarpartes(".$ai_totrows.");><img src=../shared/imagebank/tools/nuevo.png alt='Agregar partes' width=15 height=15 border=0></a>";			
				$ao_object[$ai_totrows][10]="<a href=javascript:uf_delete_dt(".$ai_totrows.");><img src=../shared/imagebank/tools15/eliminar.png alt=Aceptar width=15 height=15 border=0></a>";			
									...
									$as_estact,$as_logusr,$as_codres,$aa_seguridad)
	{
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		/*$ls_encontradores= $this->uf_saf_select_responsable($as_codemp,$as_codrespri);
		if($ls_encontradores==1)
		{$ls_encontradores= $this->uf_saf_select_responsable($as_codemp,$as_codres);}
		$ls_encontradoadm= $this->uf_saf_select_unidad($as_codemp,$as_coduniadm);*/
		//if(($ls_encontradores==1)&&($ls_encontradoadm==1))
		//{
		...
		/*else
		{
			$this->io_msg->message("Responsable o Unidad Administrativa no coinciden");
			$lb_valido=false;
		}*/		
		return $lb_valido;
...
/*		$ls_encontradores= $this->uf_saf_select_responsable($as_codemp,$as_codrespri);
	...
	function uf_saf_delete_seriales($as_codemp,$as_seract,$as_codact,$as_idact,$aa_seguridad)
	{
		...
		if(!$lb_encontrado)
		{
			$lb_encontrado=$this-> uf_saf_verifica_partes($as_codemp,$as_codact,$as_idact);
			if(!$lb_encontrado)
			{ 
			...
			{
			$this->io_msg->message("El Serial tiene partes asociadas");
		    }	  
		}
		else
		{
	...
	function uf_saf_verifica_partes($as_codemp,$as_codact,$as_idact)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_saf_verifica_partes
		//         Access: public 
		//      Argumento: $as_codemp //codigo de empresa 
		//				   $as_codact // codigo de activo
		//				   $as_idact  // id de activo
		//	      Returns: Retorna un Booleano
		//    Description: Funcion que verifica la existencia de parte(s) asociada(s) a un activo en la tabla saf_partes
		//	   Creado Por: Ing. Arnaldo Suárez
		// Fecha Creación: 23/02/2008 								Fecha Última Modificación : 01/01/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		$ls_sql = "SELECT * FROM saf_partes  ".
				  "WHERE codemp='".$as_codemp."'".
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->activo MÉTODO->uf_saf_verifica_partes ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
	...
	function uf_saf_select_cuentaspg($as_codemp,&$as_cuenta,&$as_tipspg)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_saf_select_cuentaspg
		//         Access: public
		//      Argumento: $as_codemp //codigo de empresa 
		//				   $as_cuenta //numero de cuenta presupuestaria
		//                 $as_tipspg //tipo de spg_cuenta 1 si es 404 0 si es cualquier otra
		//	      Returns: Retorna un Booleano
		//    Description: Funcion que verifica si existe una determinada cuenta presupuestaria
		//	   Creado Por: Ing. Luis Anibal Lang
		// Fecha Creación: 28/03/2006 								Fecha Última Modificación : 23/01/2008 
		...
		$rs_data=$this->io_sql->select($ls_sql);
		 ...
		 else
		 {
			...
			{
			 $as_tipspg = 1;
			 $this->io_msg->message("La Cuenta Presupuestaria ".$as_cuenta." no se encuentra registrada");
			}
		 }
       }
	   else
	   {
		...
		return $lb_valido;
	}// end function uf_saf_select_cuentaspg
...
}//fin de la class tepuy_saf_c_activo
?>
