<?php
class tepuy_c_cuentas_banco
{
	var $dat;
	var $SQL;
	var $fun;
	
	function tepuy_c_cuentas_banco()
	{
		...
		$ls_sql="SELECT monhab,mondeb,(mondeb - monhab) As saldo
				   FROM (SELECT COALESCE(SUM(monto - monret),0) As monhab
					      ...
					      WHERE codban='".$as_codban."'
							...
							AND (codope='RE' OR codope='ND' OR codope='CH' OR codope='TR') 
							AND estmov<>'A' 
							AND estmov<>'O' 
							AND codemp='".$ls_codemp."') D,
