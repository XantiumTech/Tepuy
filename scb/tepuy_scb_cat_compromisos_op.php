...
require_once("tepuy_class_scb.php");
require_once("../shared/class_folder/class_funciones.php");
require_once("tepuy_cxp_c_recep_doc.php");
$io_conect        = new tepuy_include();
$conn             = $io_conect->uf_conectar();
$io_msg           = new class_mensajes();
$io_dscompromisos = new class_datastore();
$io_sql           = new class_sql($conn);
$io_classscb      = new tepuy_class_scb($conn);
$io_recepdoc      = new tepuy_cxp_c_recep_doc($conn);
...
else
{
 ...
 {	
	...
	// $lb_valido=false;
	 ...
	 {
		$ls_codestpro1 = $li_row["codestpro1"];
		$ls_codestpro2 = $li_row["codestpro2"];
		$ls_codestpro3 = $li_row["codestpro3"];
		$ls_codestpro4 = $li_row["codestpro4"];
		$ls_codestpro5 = $li_row["codestpro5"];
		$ls_programatica=$ls_codestpro1.$ls_codestpro2.$ls_codestpro3.$ls_codestpro4.$ls_codestpro5;	
		
		$ls_spgcuenta  = $li_row["spg_cuenta"];
		...
		$ld_ajuste     = $io_classscb->uf_rddc_ajustes($ls_procede,$ls_comprobante,$ls_tipodestino,$ls_codpro,$ls_cedbene,$ls_codestpro1,
		                                                   $ls_codestpro2,$ls_codestpro3,$ls_codestpro4,$ls_codestpro5,$ls_spgcuenta,$lb_valido);
		if (empty($ld_ajuste))
		{
			 $ld_ajuste=0;
		}
		...
		if ($lb_valido)
		{
			 $ld_montord=$io_classscb->uf_rddc_recdoc($ls_procede,$ls_comprobante,$ls_codestpro1,$ls_codestpro2,$ls_codestpro3,
													  $ls_codestpro4,$ls_codestpro5,$ls_spgcuenta,$lb_valido);   	    
		}   
		if (empty($ld_montord))
		{
			 $ld_montord=0;
		   ...
		   if($lb_valido)
		   {
			  $z=0;$y=0;
			   ...
			   $io_dscontable->insertRow('txtmontoorig',$ld_montopro); 
				...
				$lb_valido=$in_classmovorden->uf_guardar_automatico($ls_codban,$ls_ctaban,$ls_mov_document,$ls_codope,$ld_fecha,$ls_mov_descripcion,$ls_codconmov,$ls_codpro,$ls_cedbene,$ls_nomproben,$ld_montopro,$ldec_objret,$ldec_retenido,$ls_chevau,$ls_estmov,$li_estint,$li_cobrapaga,$ls_estbpd,$ls_mov_procede,$ls_estreglib,$ls_estdoc,$ls_tipodestino,$ls_tipdocres,$ls_numdocres,$ls_fecdocres,$ls_tipreg,$ls_fte_financiamiento,$ls_origen,$ls_tippag,$ls_mediopago,$ls_modalidad,$ls_coduniadm,$ls_codbansig,$ls_estpro1,$ls_codbanbene,$ls_nombanbene,$ls_ctabanbene,$ls_codbanaut,$ls_nombanaut,$ls_ctabanaut,$ls_rifaut,$ls_nombreaut,$ls_nrocontrol);
				if($lb_valido)
				{
					...
					{					
						...
						$arr_movbco["estmov"]=$ls_estmov;
						  ...
						  $lb_valido=$in_classmovorden->uf_procesar_dt_gasto($ls_codban,$ls_ctaban,$ls_mov_document,$ls_codope,$ls_estmov,$ls_programatica,$ls_spgcuenta,$ls_compromisopre,$ls_descripcion,$ls_procede,$ld_montopre,'CP',$ls_uel,$ldec_baseimp,$ls_codcar);
						  if(!$lb_valido) 
						  {
...
function aceptar(comprobante,procede,descripcion,fecha,total)
  {
	...
	f.action="tepuy_scb_cat_compromisos_op.php";
	f.submit();
