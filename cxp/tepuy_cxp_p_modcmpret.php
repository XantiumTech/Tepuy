<?php
	...
	$io_fun_cxp->uf_load_seguridad("CXP","tepuy_cxp_p_modcmpret.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
...
<?php
	...
	require_once("class_folder/tepuy_cxp_c_modcmpret.php");
	$io_modcmpret=new tepuy_cxp_c_modcmpret("../");
	require_once("class_folder/tepuy_cxp_c_cmp_retencion.php");
	$io_cmpret=new tepuy_cxp_c_cmp_retencion("../");
	...
	switch($ls_operacion)
	{
		...
		case "ELIMINAR":
			...
			if($lb_ulitmo1)
			{
				$ls_tiporeten=$io_fun_cxp->uf_obtenervalor("cmbtipo","");
...
function ue_imprimir()
{
	...
	if(li_imprimir==1)
	{
		...
		if(totrow>0)
		{
			...
			switch(ls_tipo)
			{
			case '01': formato="tepuy_cxp_rpp_retencionesiva_barinas.php"+"?comprobantes="+ls_comprobantes+"&tiporeporte="+tiporeporte; break;
			case '02': formato="tepuy_cxp_rpp_retencionislr.php"+"?comprobantes="+ls_comprobantes+"&mes="+ls_mes+"&anio="+ls_anio+"&tiporeporte="+tiporeporte; break;
			case '03': formato="tepuy_cxp_rpp_retencionesmunicipales.php"+"?comprobantes="+ls_comprobantes+"&mes="+ls_mes+"&anio="+ls_anio+"&tiporeporte="+tiporeporte; break;
			case '04': formato="tepuy_cxp_rpp_retencionestimbrefiscal.php"+"?comprobantes="+ls_comprobantes+"&mes="+ls_mes+"&anio="+ls_anio+"&tiporeporte="+tiporeporte; break;
			case '05': formato="tepuy_cxp_rpp_retencionesaporte.php"+"?comprobantes="+ls_comprobantes+"&mes="+ls_mes+"&anio="+ls_anio+"&tiporeporte="+tiporeporte; break;
			}
...
function ue_nuevo()
{
	...
	if((li_modageret!="B")||(li_conivaret!="B"))
	{
		...
		{	
			...
			f.action="tepuy_cxp_p_modcmpret.php";
			f.submit();
		}
		else
		{
...
function ue_buscar()
{
	...
	if((li_modageret!="B")||(li_conivaret!="B"))
	{
		if (li_leer==1)
		{
			window.open("tepuy_cxp_cat_cmpretiva.php","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=630,height=400,left=50,top=50,location=no,resizable=yes");
		}
		else
		{
			alert("No tiene permiso para realizar esta operacion");
	...
	if(confirm("¿Desea eliminar el Registro actual?"))
		{
			...
			if((parametros!="")&&(valido))
			{
				...
				ajax=objetoAjax();
				ajax.open("POST","class_folder/tepuy_cxp_c_modcmpret_ajax.php",true);
    ...
    if (fop.estprov[0].checked==true)
	   {
		 ...
		 pagina="tepuy_cxp_cat_proveedor.php?tipo="+ls_tipo;
	...
	else
	   {
			if (fop.estprov[1].checked==true)
			{
				...
				pagina="tepuy_cxp_cat_beneficiario.php?tipo="+ls_tipo;
				window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=560,height=400,resizable=yes,location=no");
		    }
	   }
}
...
function ue_cat_solicitud(li_fila)
{
  ...
  ls_catalogo="tepuy_cxp_cat_solicitudpago.php?tipo=MODCMPRET";
...
function uf_iva(li_row)
{
  ...
  if ((ld_basimp!="0,00") && (ld_basimp!=""))	  
	 {	 
	   ...
	   pagina="tepuy_cxp_pdt_otroscreditos.php?tipo=CMPRET";
  ...
  switch (ls_tipret)
  {
  	case "0000000001":
	   ...
	   if ((ld_porcar!="0,00") && (ld_porcar!=""))
		  {
		    pagina="tepuy_cxp_pdt_deducciones.php?tipo=CMPRETIVA";
	...
	break;
  	case "0000000003":
	   ...
	   if ((ld_basimp!="0,00") && (ld_basimp!=""))
		  {
		    pagina="tepuy_cxp_pdt_deducciones.php?tipo=CMPRETMUN";
		    window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=700,height=600,resizable=yes,location=no");	  
		  }  
	   else
		  {
	...
	break;
  	case "0000000004":
	   ...
	   if ((ld_basimp!="0,00") && (ld_basimp!=""))
		  {
		    pagina="tepuy_cxp_pdt_deducciones.php?tipo=CMPRETAPO";
		    window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=700,height=600,resizable=yes,location=no");	  
		  }  
	   else
		  {
