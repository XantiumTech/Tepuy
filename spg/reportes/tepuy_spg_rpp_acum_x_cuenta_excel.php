...
//-----------------------------------------------------------------------------------------------------------------------------
   ...
   //--------------------------------------------------  Parámetros para Filtar el Reporte  --------------------------------------
    ...
    // Cargar el dts_cab con los datos de la cabecera del reporte( Selecciono todos comprobantes )	
	  $ls_codestpro1  = str_pad($ls_codestpro1_min,20,0,0);
	  $ls_codestpro2  = str_pad($ls_codestpro2_min,6,0,0);
	  $ls_codestpro3  = str_pad($ls_codestpro3_min,3,0,0);
	  $ls_codestpro4  = str_pad($ls_codestpro4_min,2,0,0);
	  $ls_codestpro5  = str_pad($ls_codestpro5_min,2,0,0);
	  $ls_codestpro1h = str_pad($ls_codestpro1h_max,20,0,0);
	  $ls_codestpro2h = str_pad($ls_codestpro2h_max,6,0,0);
	  $ls_codestpro3h = str_pad($ls_codestpro3h_max,3,0,0);
	  $ls_codestpro4h = str_pad($ls_codestpro4h_max,2,0,0);
	  $ls_codestpro5h = str_pad($ls_codestpro5h_max,2,0,0);
      $lb_valido=$io_report->uf_spg_reporte_acumulado_cuentas2($ls_codestpro1,$ls_codestpro2,$ls_codestpro3,$ls_codestpro4,
															  ...
															  $ls_codfuefinhas,$rs_data,$ls_rango_estructura);
	...
	if ($lb_valido==false) // Existe algún error ó no hay registros
	...
	else // Imprimimos el reporte
	   {
		...
		while($row=$io_report->SQL->fetch_row($rs_data))
		{
			  ...
			  $ld_precompromiso=$row["precompromiso"];
			  $ld_compromiso=$row["compromiso"];
			  $ld_causado=$row["causado"];
			  $ld_pagado=$row["pagado"];
			  
			  $ld_monto_actualizado=$row["monto_actualizado"];
			  $ld_saldo_comprometer=$row["saldo_comprometer"];
			  $ld_por_paga=$row["por_pagar"];
