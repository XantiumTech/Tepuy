<?php
	...
	function uf_print_detalle($la_data,$as_tiprep,&$io_pdf)
	{
		...
		if ($as_tiprep=='E') 
		   {
		     $la_columna=array('beneficiario'=>'<b>Proveedor \ Benef.</b>','solicitud'=>'<b>Orden de Pago</b>', 'conmov'=>'<b>Concepto</b>',
		   ...
		   {
		     $la_columna=array('beneficiario'=>'<b>Proveedor \ Benef.</b>','solicitud'=>'<b>Orden de Pago</b>', 'cuenta'=>'<b>Cuenta</b>',
						       'documento'=>'<b>Documento</b>','operacion'=>'<b>Operación</b>', 'fecha'=>'<b>Fecha</b>','monto'=>'<b>Monto</b>','monret'=>'<b>Retenido</b>');
			 $la_config=array('showHeadings'=>1, // Mostrar encabezados
				 'fontSize' => 8, // Tamaño de Letras
				 'showLines'=>0, // Mostrar Líneas
	...
	require_once("tepuy_scb_class_report.php");
	...
	$io_report = new tepuy_scb_class_report($con);
	...
	if ($ls_tiporeporte==1)
	   {
		 require_once("tepuy_scb_class_reportbsf.php");
		 $io_report = new tepuy_scb_class_reportbsf($con);
		 $ls_tipbol = 'Bs.F.';
	   }
	if($ls_tiprep=="E")
	{
		...
		while ($row=$io_sql->fetch_row($rs_data))
		      {
				...
				$ldec_monto	   = $row["monto"];//Monto General del Cheque.
				$ldec_monsol   = $row["monsol"];//Monto detallado de cancelacion de las Ordenes de Pago.
				$ld_fecmov	   = $io_report->fun->uf_convertirfecmostrar($row["fecmov"]);
				$ls_nomproben  = $row["nomproben"].$ls_tipproben.$ls_probendesde.$ls_probenhasta;
				...
				if ($ls_estmov=="A")
		 	       {
			 	     $ld_monto_anulado = ($ld_monto_anulado+$ldec_monsol);
					 $ldec_monto=$ldec_monsol * (-1);
			       }
			    if ($ls_tipoope=="CH")
				     $ls_tipoope="CHEQUE";
				...
				$ldec_total=$ldec_total+$ldec_monsol;	
			    ...
			    $la_data[$i]=array('beneficiario'=>$ls_nomproben,'solicitud'=>$ls_numsol,'cuenta'=>$ls_ctaban,'documento'=>$ls_numdoc,'operacion'=>$ls_tipoope,
							       'fecha'=>$ld_fecmov,'monto'=>number_format($ldec_monto,2,",","."),'monret'=>number_format($ld_monret,2,",","."),'conmov'=>$ls_conmov); //$ldec_monto = monto en el cheque y $ldec_monsol=monto en la orden de pago
		      }
