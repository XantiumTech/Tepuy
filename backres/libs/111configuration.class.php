...
//		$this->table['tepuy_cmp'][0] = "REPLACE(REPLACE(comprobante,'0000',''), CONCAT('CIERRE','-',YEAR(fecha)), CONCAT('APERTURA','-',YEAR(fecha)+1)) AS comprobante, DATE_ADD(fecha, INTERVAL 1 YEAR) AS fecha, REPLACE(descripcion, 'CIERRE', 'APERTURA') AS descripcion, REPLACE(procede, 'SCGCIE', 'SCGAPR') AS procede";
		...
		$this->table['scg_dt_cmp'][0] = "DATE_ADD(A.fecha, INTERVAL 1 YEAR) AS fecha, IF(A.descripcion != '', 'APERTURA DE EJERCICIO', A.descripcion) AS descripcion, REPLACE(A.procede, 'SCGCMP', 'SCGAPR') AS procede, REPLACE(A.procede_doc, 'SCGCMP', 'SCGAPR') AS procede_doc, IF(B.debe_mes>0, 'D', IF(B.haber_mes>0, 'H', 'H')) AS debhab, IF(B.sc_cuenta='31401010102001', 0, IF(B.debe_mes>0, B.debe_mes, IF(B.haber_mes>0, B.haber_mes, 0))) AS monto";
		$this->table['scg_dt_cmp'][1] = "A JOIN scg_saldos B ON A.sc_cuenta = B.sc_cuenta WHERE A.procede = 'SCGCMP' AND A.comprobante LIKE '%APERTURA%' AND A.codemp = B.codemp AND B.fecsal = (SELECT MAX(fecsal) FROM scg_saldos WHERE sc_cuenta = A.sc_cuenta)";

		$this->table[2][2] = "scg_pc_reporte";

		$this->table[2][3] = "scg_saldos";
		$this->table['scg_saldos'][0] = "REPLACE(A.fecsal, A.fecsal, CONCAT(YEAR(A.fecsal)+1,'-01-01')) AS fecsal, IF(A.debe_mes>0, IF(A.sc_cuenta='31401010102001', 0, A.debe_mes), A.debe_mes) AS debe_mes, IF(A.haber_mes>0, IF(A.sc_cuenta='31401010102001', 0, A.haber_mes), A.haber_mes) AS haber_mes";
		$this->table['scg_saldos'][1] = "A WHERE A.fecsal = (SELECT MAX(fecsal) FROM scg_saldos WHERE sc_cuenta = A.sc_cuenta AND codemp = A.codemp)";
