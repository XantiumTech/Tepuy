<?php
class tepuy_scv_c_tarifasxdistancias
{
	var $ls_sql;
	var $is_msg_error;
		
	function tepuy_scv_c_tarifasxdistancias($con)
	{
	...
	function uf_scv_select_tarifa($as_codemp,$as_codtar) 
	{
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->tepuy_scv_c_tarifasxdistancias METODO->uf_scv_select_tarifa ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
	...
	function uf_scv_insert_tarifa($as_codemp,$as_codtar,$as_dentar,$ai_kmsdes,$ai_kmshas,$ai_montar,$aa_seguridad)
	{
		...
		if ($rs_data===false)		     
		{
			...
			$this->io_msg->message("CLASE->tepuy_scv_c_tarifasxdistancias METODO->uf_scv_insert_tarifa ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_scv_update_tarifa($as_codemp,$as_codtar,$as_dentar,$ai_kmsdes,$ai_kmshas,$ai_montar,$aa_seguridad) 
	{
		...
		{
			$this->io_sql->rollback();
			$this->io_msg->message("CLASE->tepuy_scv_c_tarifasxdistancias METODO->uf_scv_update_tarifa ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$lb_valido=true;
	...
	function uf_scv_delete_tarifa($as_codemp,$as_codtar,$aa_seguridad)
	{
		...
		if (!$lb_relacion)
		{
			...
			{
				$this->io_sql->rollback();
				$this->io_msg->message("CLASE->tepuy_scv_c_categoria METODO->uf_scv_delete_tarifa ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			}
			else
			{
				$lb_valido=true;
