<?php
    session_start();   //Contraloria Mérida.
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsolcot,$as_fecsolcot,$as_dentipsolcot,$as_obssolcot,&$io_pdf)
	{
		...
		$io_pdf->ezSetY(215);
		$ls_instrucciones = "LA COTIZACION DEBE PRESENTARSE A NOMBRE DE LA CONTRALORIA DEL ESTADO MÉRIDA, NO. DE R.I.F. ".$_SESSION["la_empresa"]["rifemp"]." Y CON LAS CARACTERISTICAS EXACTAS DE LO QUE SE SOLICITA.
PARA ESTE PROCEDIMIENTO SE APLICÓ EL DECRETO N° 4.998, DE FECHA 17 DE NOVIEMBRE DE 2006, SEGÚN GACETA OFICIAL N° 350.893, DE FECHA 20 DE NOVIEMBRE DE 2006 ( MEDIDAS TEMPORALES PARA LA PROMOCIÓN Y DESARROLLO DE LAS PEQUEÑAS Y MEDIANAS INDUSTRIAS (PYMIS),COOPERATIVAS Y OTRAS FORMAS ASOCIATIVAS, PRODUCTORAS DE SERVICIOS Y EJECUTORAS DE OBRAS, UBICADAS EN EL PAIS)";
		$la_datains=array(array('instrucciones'=>$ls_instrucciones));				
		$la_columna=array('instrucciones'=>'4. INSTRUCCIÓN PARA EL PROVEEDOR');		
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'justification'=>'center', // Ancho de la tabla						 
						 'maxWidth'=>548); // Ancho Máximo de la tabla
		$io_pdf->ezTable($la_datains,$la_columna,'',$la_config);
		unset($la_data);
		unset($la_columna);
		unset($la_config);
		
		$io_pdf->ezSetY(120);
		$la_datafir=array(array('firma'=>''),
		                  array('firma'=>'                                                                          _________________________________'),
		                  array('firma'=>'                                                                              AREA DE COMPRAS Y SERVICIOS'));				
		$la_columna=array('firma'=>'6. FIRMA AUTORIZADA POR LA DIRECCIÓN DE ADMINISTRACIÓN Y PRESUPUESTO');		
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 'fontSize'=>9, // Tamaño de Letras
						 'showLines'=>1, // Mostrar Líneas
						 'titleFontSize'=>7,
						 ...
						 'justification'=>'center', // Ancho de la tabla						 
						 'maxWidth'=>548,
						 'cols'=>array('firma'=>array('justification'=>'left','width'=>548))); // Ancho Máximo de la tabla
		$io_pdf->ezTable($la_datafir,$la_columna,'',$la_config);
		unset($la_data);
		unset($la_columna);
		unset($la_config);
		      ...
		      if ($li_numrows>0)
		         {
				   ...
				   $io_pdf->ezSetCmMargins(5,1,3,3); // Configuración de los margenes en centímetros
				   ...
				   while (($row=$io_sql->fetch_row($rs_data)) && $lb_valido)
						 {
					       ...
					       $io_pdf->ezSetCmMargins(5,8,3,3); // Configuración de los margenes en centímetros
