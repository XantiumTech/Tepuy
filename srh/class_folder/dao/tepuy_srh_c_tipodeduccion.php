...
class tepuy_srh_c_tipodeduccion
{
	var $obj="";
	var $io_sql;
	var $siginc;
	...
	function tepuy_srh_c_tipodeduccion($path)
	...
	function uf_srh_getProximoCodigo()
  {
        /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_srh_getProximoCodigo
		//         Access: public (tepuy_srh_d_concurso)
		//      Argumento: 
		  ...
		  if ($num!=0) {
			...
			while($row=$this->io_sql->fetch_row($rs_data))
			{
			   ...
			   switch($ls_nexo)
				...
				$ao_object[$ai_totrows][2]="<input name=txtsueldo".$ai_totrows." type=text id=txtsueldo".$ai_totrows." class=sin-borde size=14 onKeyPress='return(ue_formatonumero(this,".$punto.", ".$coma.",event))' style='text-align:center'  value='".$ls_sueldo."'>";
				...
				<option value='S' ".$la_hcm[0].">Si</option>
				<option value='N' ".$la_hcm[1]." >No</option></select> ";
			...
			$ao_object[$ai_totrows][8]="<input name=txtprima".$ai_totrows." type=text id=txtprima".$ai_totrows." class=sin-borde size=8 onKeyPress='return(ue_formatonumero(this,".$punto.", ".$coma.",event))'  value='".$li_prima."'>";
			$ao_object[$ai_totrows][9]="<input name=txtaporempre".$ai_totrows." type=text id=txtaporempre".$ai_totrows." class=sin-borde size=8 onKeyPress='return(ue_formatonumero(this,".$punto.", ".$coma.",event))' value='".$li_aporempre."' >";
			$ao_object[$ai_totrows][10]="<input name=txtaporemple".$ai_totrows." type=text id=txtaporemple".$ai_totrows." class=sin-borde size=8 onKeyPress='return(ue_formatonumero(this,".$punto.", ".$coma.",event))' value='".$li_aporemple."'>";
			$ao_object[$ai_totrows][11]="<a href=javascript:uf_agregar_dt(".$ai_totrows.");  align=center><img src=../../../../shared/imagebank/tools15/aprobado.png alt=Aceptar width=15 height=15 border=0 align=center></a>";
			$ao_object[$ai_totrows][12]="<a href=javascript:uf_delete_dt(".$ai_totrows.");   align=center><img src=../../../../shared/imagebank/tools15/eliminar.png alt=Eliminar width=15 height=15 border=0 align=center></a>";				
...
function uf_srh_eliminar_dt_configuracion_deduccion($as_codtipded, $aa_seguridad)
  {
		...
		//        access:  public (tepuy_srh_dt_tipodeduccion)														
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql=" SELECT sno_personaldeduccion.codper, srh_dt_tipodeduccion.*, sno_personal.hcmper ". 
				" FROM sno_personaldeduccion, srh_dt_tipodeduccion, sno_personal ".
...
//-------------------------------------------------------------------------------------------------------------------------------
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			$lb_valido=true;
			$ls_sql=" SELECT srh_dt_tipodeduccion.*, sno_familiardeduccion.codper, sno_familiardeduccion.cedfam, ".
					...
					"		   AND sno_familiar.cedfam=sno_familiardeduccion.cedfam) as nexofam, ".
					...
					"        WHERE  sno_familiar.codemp= sno_familiardeduccion.codemp ".
					"		   AND sno_familiar.codper=sno_familiardeduccion.codper   ".
					"		   AND sno_familiar.cedfam=sno_familiardeduccion.cedfam) as hcfam, ".
					"      (SELECT sno_familiar.hcmfam from sno_familiar              ".
					"        WHERE sno_familiar.codemp= sno_familiardeduccion.codemp  ".
					"		   AND sno_familiar.codper=sno_familiardeduccion.codper   ".
					"		   AND sno_familiar.cedfam=sno_familiardeduccion.cedfam) as hcmfam, ".
					"      (SELECT sno_familiar.sexfam from sno_familiar              ".
					"        WHERE sno_familiar.codemp= sno_familiardeduccion.codemp  ".
					"		   AND sno_familiar.codper=sno_familiardeduccion.codper   ".
					"		   AND sno_familiar.cedfam=sno_familiardeduccion.cedfam) as sexofam ".
				 ...
				 while($row=$this->io_sql->fetch_row($rs_data))
					{
				      ...
				      if ($ls_hcm!="")
				      {	
						if (($as_sueldo>=$ls_sueldobene)&&($edad_familiar>=$ls_edadmin)&&($edad_familiar<=$ls_edadmax)&&($ls_sexoben==$sexo_familiar)&&($ls_nexo==$nexo_familiar)&&($ls_sexoben==$sexo_familiar)&&($ls_hcm==$hcm_familiar))
						{
						 	switch($as_tipo)
							  {
								case "1":
									 $as_valor=  round ($ls_valorprima * $apor_empleado)/100;
									break;
								case "2":
									 $as_valor=  round ($ls_valorprima * $apor_empresa)/100;
									break;
							  }
						 }
				       }
					   ...
					   {
					     if (($as_sueldo>=$ls_sueldobene)&&($edad_familiar>=$ls_edadmin)&&($edad_familiar<=$ls_edadmax)&&($ls_sexoben==$sexo_familiar)&&($ls_nexo==$nexo_familiar)&&($ls_sexoben==$sexo_familiar))
						  {
						 	switch($as_tipo)
							  {
								case "1":
...
//---------------------------------------------------------------------------------------------------------------------------------
}// end   class tepuy_srh_c_tipodeduccion
?>