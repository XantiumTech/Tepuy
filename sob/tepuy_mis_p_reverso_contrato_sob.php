<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("SOB","tepuy_mis_p_reverso_contrato_sob.php",$ls_permisos,$la_seguridad,$la_permisos);
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_reverso_contrato($ls_codcon,$ls_codasi,$ld_fechaconta,$la_seguridad);
					if($lb_valido)
					{
						$in_class->io_msg->message("Contrato  ".$ls_codcon." fue reversada la contabilizacion.");
					}
					else
					{
						$in_class->io_msg->message("No se pudo reversar la Contabilización el Contrato ".$ls_codcon);
					}		
				}
			}
			$li_totrows=1;
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="tepuy_mis_p_reverso_contrato_sob.php";
			f.submit();
		}
		else
		{
