<?php
   session_start();
	...
	$io_fun_nomina->uf_load_seguridad_nomina("SNO","tepuy_sno_d_persxconce.php",$ls_codnom,$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
   ...
   function uf_agregarlineablanca(&$aa_object,$ai_totrows)
   {
		...
		$aa_object[$ai_totrows][3]="<input name=chk".$ai_totrows." type=checkbox id=chk".$ai_totrows." value=1 class=sin-borde size=20 style=text-align:center readonly>";
   }
   //--------------------------------------------------------------
?>
	...
	switch ($ls_operacion) 
	{
		...
		case "GUARDAR":
			...
			if($lb_valido)
			{
				...
				$lb_valido=$io_conceptopersonal->uf_load_conceptos_x_personal($ls_codper,$li_totrows,$lo_object);
			}
			else
			{
...
function ue_guardar()
{
	...
	{		
		...
		if(li_cambiar==1)
		{
			...
			f.action="tepuy_sno_d_persxconce.php";
			f.submit();
		}
		else
		{
...
function ue_buscar()
{
	...
	if (li_leer==1)
   	{
		window.open("tepuy_sno_cat_persxconce.php","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=530,height=400,left=50,top=50,location=no,resizable=no");
   	}
	else
   	{
 		alert("No tiene permiso para realizar esta operacion");
