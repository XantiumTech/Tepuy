<?php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","tepuy_mis_p_reverso_depreciacion_saf.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
...
<?php
	...
	require_once("class_folder/class_tepuy_saf_integracion.php");  	
	$in_class = new class_tepuy_saf_integracion();  
	uf_limpiarvariables();
	switch ($ls_operacion) 
	{
		case "NUEVO":
		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					if($lb_valido)
					{
						$in_class->io_msg->message("La Depreciación del Comprobante ".$ls_comprobante." fue Revesada.");
					}
					else
					{
		...
		case "BUSCAR":
			uf_load_variables();
	    	$in_class_contabiliza->uf_select_activos_contabilizar("",$ls_ano,"1",$lo_object,$li_totrows);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
	...
	if(li_ejecutar==1)
	{
		if(f.txtano.value!="")
		{
			...
			if(lb_valido)
			{
				...
				f.action="tepuy_mis_p_reverso_depreciacion_saf.php";
				f.submit();
			}
			else
			{
...
function uf_verdetalle(mes,ano)
{
	...
	window.open("tepuy_mis_pdt_saf.php?mes="+mes+"&ano="+ano+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
