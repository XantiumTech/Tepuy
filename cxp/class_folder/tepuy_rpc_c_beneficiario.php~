<?php
class tepuy_sep_c_beneficiario
 {
var $ls_sql="";
var $la_emp;
var $is_msg_error;
 	
	function tepuy_sep_c_beneficiario()
	{
  ...
  if ($rs_data===false)
   {
	 ...
	 $this->is_msg_error="CLASE->tepuy_sep_c_beneficiario; METODO->uf_insert_beneficiario; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message);
	 $lb_valido=false;
   }
  else
   {   
...
//////////////////////////////////////////////////////////////////////////////
 ...
 if ($rs_data===false)
   {                              
	 $this->io_sql->rollback();
	 $this->is_msg_error="CLASE->tepuy_sep_c_beneficiario; METODO->uf_update_beneficiario; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message);
	 $lb_valido=false;
   }
 else
   {   
...
//////////////////////////////////////////////////////////////////////////////
	...
	if ($rs_data===false)
	   {
		 $this->io_msg_message("CLASE->tepuy_sep_c_beneficiario; METODO->uf_update_beneficiario; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		 $lb_valido=false;
	   }
	else
	   {			
	...
	if(($lb_existe)&&(!$lb_relacion))
	  {
	    ...
	    if ($rs_data===false)
	       { 
		     $this->io_sql->rollback();
		     $this->is_msg_error="CLASE->tepuy_sep_c_beneficiario; METODO->uf_delete_beneficiario; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message);
...
{
	$ls_sql  = "SELECT numsol FROM sep_solicitud WHERE codemp='".$as_codemp."' AND trim(ced_bene)='".trim($as_cedben)."'";
	$rs_data = $this->io_sql->select($ls_sql);
	if ($rs_data===false)
	   {
		 $lb_valido=false;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
     ...
     if ($rs_data===false)
	    {
		  $lb_valido = false;
		  $this->io_msg->message("CLASE->tepuy_sep_c_beneficiario; METODO->uf_load_personal; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
     else
	    {
		  $li_numrows = $this->io_sql->num_rows($rs_data);
...
//print $ls_sql;
  ...
  if ($rs_data===false)
     {
	   $lb_valido = false;
	   $this->is_msg_error="CLASE->tepuy_sep_c_beneficiario; METODO->uf_load_datos_personal; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message);
	 }
  return $rs_data;
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if ($rs_data===false)
     {
	   $this->is_msg_error="CLASE->tepuy_sep_c_beneficiario; METODO->uf_insert_personal; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message);
	   $lb_valido=false;
     }
  else
     {   
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if ($rs_data===false)
     {
	   $this->io_msg->message("CLASE->tepuy_sep_c_beneficiario; METODO->uf_load_and_insert_codbansig(SELECT); ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	   $lb_valido=false;
	 }
  else
     {
	   ...
	   if ($li_numrows<=0)
	      {
			...
			if ($rs_datos===false)
			   {
	             $this->io_msg->message("CLASE->tepuy_sep_c_beneficiario; METODO->uf_load_and_insert_codbansig(INSERT); ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	             $lb_valido=false;
			   }
		  }
	 } 
