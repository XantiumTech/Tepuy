<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$as_titulo1,$as_nivel,&$io_pdf)
	{
		...
		$io_pdf->addText(40,570,6,"CÓDIGO PRESUPUESTARIO DEL ENTE: "); // Agregar el título
		$io_pdf->addText(40,560,6,"DENOMINACIÓN DEL ENTE: ".$_SESSION["la_empresa"]["nombre"]); // Agregar el título
		$io_pdf->addText(40,550,6,"ORGANO DE ADSCRIPCIÓN: ".strtoupper($_SESSION["la_empresa"]["nomorgads"])); // Agregar el título
		$io_pdf->addText(40,540,6,"PERIODO PRESUPUESTARIO: ".substr($_SESSION["la_empresa"]["periodo"],0,4)); // Agregar el título
		$li_tm=$io_pdf->getTextWidth(9,$as_titulo);
		$tm=500-($li_tm/2);
		$io_pdf->addText($tm,515,9,$as_titulo); // Agregar el título
		
		$li_tm=$io_pdf->getTextWidth(9,$as_titulo1);
		$tm=500-($li_tm/2);
		$io_pdf->addText($tm,505,7,$as_titulo1); // Agregar el título
		
		$io_pdf->rectangle(35,420,940,80); // segundo cuadro del encabezado		
		
		$io_pdf->line(120,500,120,420);//  Vertical
		$io_pdf->addText(60,430,7,"CÓDIGO"); // Agregar el título
		
		$io_pdf->line(450,500,450,420);//  Vertical
		$io_pdf->addText(250,430,7,"DENOMINACIÓN"); // Agregar el título
		$io_pdf->line(520,500,520,420);//  Vertical
		
		$io_pdf->addText(470,460,7,"SALDO"); // Agregar el título
		$io_pdf->addText(460,450,7,"PRESUPUESTO"); // Agregar el título
		$io_pdf->addText(470,440,7,"REAL AÑO"); // Agregar el título
		$io_pdf->addText(470,430,7,"ANTERIOR"); // Agregar el título
		
		$io_pdf->line(590,500,590,420);//  Vertical
		$io_pdf->addText(530,450,7,"SALDO"); // Agregar el título
		$io_pdf->addText(530,440,7,"PRESUPUESTO"); // Agregar el título
		$io_pdf->addText(530,430,7,"APROBADO"); // Agregar el título
		
		$io_pdf->line(660,500,660,420);//  Vertical
		$io_pdf->addText(600,450,7,"SALDO"); // Agregar el título
		$io_pdf->addText(600,440,7,"PRESUPUESTO"); // Agregar el título
		$io_pdf->addText(600,430,7,"MODIFICADO"); // Agregar el título
		
		$io_pdf->line(660,455,900,455);// Horizontal
		$io_pdf->addText(665,440,7,"SALDO"); // Agregar el título
		$io_pdf->addText(665,430,7,"PROGRAMADO"); // Agregar el título		
		$io_pdf->line(720,455,720,420);//  Vertical
		
		$io_pdf->addText(730,440,7,"SALDO"); // Agregar el título
		$io_pdf->addText(730,430,7,"EJECUTADO"); // Agregar el título		
		$io_pdf->line(780,500,780,420);//  Vertical		
		
		$io_pdf->addText(690,460,7,"TRIMESTRE NRO.  ".$as_nivel); // Agregar el título
		$io_pdf->addText(790,480,6.5,"VARIACIÓN SALDO EJECUTADO - "); // Agregar el título
		$io_pdf->addText(790,470,6.5,"SALDO PROGRAMADO EN EL"); // Agregar el título
		$io_pdf->addText(790,460,6.5,"TRIMESTRE NRO.  ". $as_nivel); // Agregar el título		
		$io_pdf->line(840,455,840,420);//  Vertical
		
		$io_pdf->addText(790,430,7,"ABSOLUTA"); // Agregar el título
		$io_pdf->addText(845,430,7,"PORCENTUAL"); // Agregar el título
		$io_pdf->line(900,500,900,420);//  Vertical
		
		$io_pdf->addText(910,460,5.5,"VARIACIÓN SALDO"); // Agregar el título
		$io_pdf->addText(910,450,5.5,"EJECUTADO PERIODO"); // Agregar el título
		$io_pdf->addText(910,440,5.5,"N, MESES SALDO"); // Agregar el título
		$io_pdf->addText(910,430,5.5,"PERIODO N-1"); // Agregar el título
    ...
    function uf_print_detalle($la_data,&$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'xPos'=>506,
						 'cols'=>array('cuentas'=>array('justification'=>'center','width'=>85), // Justificación y ancho de la columna
...
//------------------------------------------------------------------------------------------------------------------------------------
	...
	{   
		...
		else// Imprimimos el reporte
		{
			...
			uf_print_encabezado_pagina($ls_titulo,$ls_titulo1,$ls_nivel,$io_pdf); // Imprimimos el encabezado de la página
			$io_pdf->ezStartPageNumbers(980,50,10,'','',1); // Insertar el número de página	
