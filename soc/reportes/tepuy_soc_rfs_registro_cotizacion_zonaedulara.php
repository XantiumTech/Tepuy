<?php
	...
	function uf_insert_seguridad($as_titulo)
	{
		...
		$lb_valido=$io_fun_soc->uf_load_seguridad_reporte("SOC","tepuy_soc_p_registro_cotizacion.php",$ls_descripcion);
		return $lb_valido;
	}
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numcot,$as_feccot,$as_dentipcot,$as_obscot,$as_numsolcot,&$io_pdf)
	{
		...
		$io_pdf->saveState();
		$io_pdf->rectangle(140,925,450,40);
		$io_pdf->line(450,965,450,925);
		$io_pdf->line(450,945,590,945);
		$io_pdf->addJpegFromFile('../../shared/imagebank/'.$_SESSION["ls_logo"],40,926.5,$_SESSION["ls_width"],$_SESSION["ls_height"]); // Agregar Logo
		$li_tm=$io_pdf->getTextWidth(10,$as_titulo);		
		$io_pdf->addText(200,940,14,"<b>".$as_titulo."</b>"); // Agregar el título
		$io_pdf->addText(460,950,10,"<b>   No.:</b>");      // Agregar texto
		$io_pdf->addText(495,950,10,$as_numcot); // Agregar Numero de la solicitud
		$io_pdf->addText(450,930,10,"<b>  Fecha:</b>"); // Agregar texto
		$io_pdf->addText(495,930,10,$as_feccot); // Agregar la Fecha
