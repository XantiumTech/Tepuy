...
class tepuy_scb_c_cmp_ret_iva
{
	...
	var $lb_firsttime;
	var $ds_det_ret;
	function tepuy_scb_c_cmp_ret_iva($as_retid)
	{
	...
	{	
		...
		//////////////////////////////////////////////////////////////////////////////
		/*	print "entro<br>";
			print "tipo $ls_tipo <br>";
			print "codigo $ls_codproben<br>";
			print_r($arr_proben);/*/
			...
			{				
				...
				if(($ldec_ivaretenido == 0)OR($this->lb_ivacxp))
				{
				   $ls_sql="SELECT NPR.monret as retenido 
							FROM   cxp_rd_deducciones NPR, tepuy_deducciones NPS 
		...
		else
		{
			if($row=$this->io_sql->fetch_row($rs_result))
			{
				...
				for($li_i=1;$li_i<=$li_total_rows;$li_i++)
				{
					...
					$object[$li_i][8] = "<input type=text name=txtporiva".$li_i." value='$ldec_porimp'      style=text-align:right class=sin-borde size=15 onKeyPress=return(currencyFormat(this,'.',',',event)) <a href=javascript:uf_iva(".$li_i.");><img src=../shared/imagebank/tools15/buscar.png width=15 height=15 border=0></a>";						
					...
					$object[$li_i][12]= "<a href=javascript:uf_delete_dt('$ls_numope','$ls_numfac');><img src=../shared/imagebank/tools15/eliminar.png alt='Eliminar Retencion' width=15 height=10 border=0></a><input type=hidden name=hidforcar".$li_i."  value=''><input type=hidden name=hidcodded".$li_i."  value=''><input type=hidden name=hidforded".$li_i."  value=''>";	
				}
			}
		}		
	}		
			 ...
			 $aa_seguridad)
	{
		...
		else
		{
			if($li_result>=0)
			{
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				 $ls_evento="UPDATE";
				 $ls_descripcion ="Actualiz�en CXP en la tabla scb_dt_cmp_ret del Comprobante $ls_numcom el detalle ".$ls_numope;
				 $ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
				 $aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
				 $aa_seguridad["ventanas"],$ls_descripcion);
				 /////////////////////////////////         SEGURIDAD               /////////////////////////// 			
			...
			{
				$lb_valido=false;
				$this->msg->message("La Factura no Existe para ser Actualizada");
			}
		}	
	return $lb_valido;
	}	
		...
		else
		{
			...
			else
			{
				...
				else
				{
					$lb_valido=false;
					$this->io_sql->rollback();
					$this->msg->message("La Factura no Existe para ser Eliminada ");
				}
			}			
		}	
		return $lb_valido;
	...
	function uf_delete_dt_cmp_ret($ls_codret,$ls_numcom,$ls_numope,$ls_numfac,$aa_seguridad)
	{
		...
		//	Returns:		 
		//	Description:  M�odo encarga de eliminar el detalle de un comprobante  
		//////////////////////////////////////////////////////////////////////////////
