...
<?php
	...
	if($ls_operacion=="BUSCAR")
	{
		$ls_sql="SELECT siv_articulo.*,".
				...
				"      (SELECT denmil FROM tepuy_catalogo_milco".
				"        WHERE tepuy_catalogo_milco.codmil=siv_articulo.codmil) as denmil".
				"  FROM siv_articulo".
		...
		if($li_num>0)
		{
			while($row=$io_sql->fetch_row($rs_cta))
			{
				...
				print "'$li_catalogo','$ls_sccuenta','$ls_scdencuenta','$li_reoart','$ls_codmil','$ls_denmil','$ls_serart','$ls_fabart','$ls_ubiart','$ls_docart');\">".$ls_codart."</a></td>";
...
<script language="JavaScript">
	function aceptar(ls_codart,ls_denart,ls_codtipart,ls_dentipart,ls_codunimed,ls_denunimed,ld_feccreart,ls_obsart,li_exiart,
					 ...
					 li_catalogo,ls_sccuenta,ls_scdencuenta,li_reoart,ls_codmil,ls_denmil,ls_serart,ls_fabart,ls_ubiart,ls_docart)
	{
	...
	function ue_search()
  	{
		...
		f.action="tepuy_siv_cat_articulo.php";
		f.submit();
	}
</script>
</html>