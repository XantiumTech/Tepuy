...
class tepuy_scb_c_cmp_ret
{
	...
	var $ds_dt_cmpret;
	var $is_msg_error;
	function tepuy_scb_c_cmp_ret($as_retid)
	{
	...
	function uf_crear_comprobante($ld_fecdesde,$ld_fechasta, $ls_tipo, $ls_codproben, &$ls_mensaje)
	{
		...
		{
		   $ldec_total_iva=0;///////agregado el 20/12/2007
			...
			{				
				...
				if($lb_continue)
				{
					...
					{
						if($this->uf_rep_cur_libro_compra( $ls_numsol,$ldec_porc, &$ls_Concepto, $arr_datos_proben))
						{
		...
		$ls_sql ="SELECT d.numope, d.numfac, d.iva_ret
					  FROM	scb_dt_cmp_ret d, scb_cmp_ret c
					  WHERE d.numcom = '".$ls_nrocomp."'  
					  AND d.codret ='".$this->ls_retid."'
					  AND d.numcom=c.numcom 
					  AND d.codret=c.codret 
					  AND c.estcmpret<>2 
					  GROUP by d.numope, d.numfac, d.iva_ret 
					  ORDER by d.numope,d.numfac,d.iva_ret ";
					  
		    ///print $ls_sql."<br> entro por aqui<br>";
			///print $ldec_total_iva."monto total de iva del comprobante".$ls_nrocomp."<br>";
			
			$rs_data=$this->io_sql->select($ls_sql);
			if($rs_data===false)
			{
				$this->is_msg_error=$this->io_funciones->uf_convetirmsg( $this->io_sql->message);
				print $this->is_msg_error;
				return false;
			}
			else
			...
			{	
					...
					///print $ls_sql."<br> iva retenido es mayor al total retenido";/////////////////////////////////////////////////////////////	
					
					$li_result=$this->io_sql->execute($ls_sql);
					if($li_result === false)
					{	
						$this->is_msg_error=$this->io_funciones->uf_convertirmsg( $this->io_sql->message);
						print $this->is_msg_error;
						return false;
					}
			}
			elseif($ldec_ivaret<$ldec_total_iva)
			{
					...
					$ls_sql = "UPDATE scb_dt_cmp_ret
						...
						////print print $ls_sql."<br> iva retenido es menor al total retenido";	//////////////////////////////////////////////////////
	...
	function uf_rep_cur_pagos($ld_fecdesde, $ld_fechasta, $ls_tipo, $ls_codproben)
	{
		...
		if($ls_tipo=='B')
		{
			$ls_sql=" SELECT BRH.numdoc as numdoc, BRH.fecmov as fecmov, XSC.numsol as numsol, XSC.monto as monto,BRH.codban as codban,BRH.ctaban as ctaban,BRH.codope as codope
				      ...
				      WHERE (BRH.codemp='".$ls_codemp."' AND BRH.codope='CH') AND (BRH.estmov='C') AND BRH.ced_bene='".$ls_codproben."'  AND (BRH.fecmov between '".$ld_fecdesde."' AND '".$ld_fechasta."') AND  
				     		  BRH.numdoc not IN ( SELECT d.numdoc FROM scb_dt_cmp_ret d, scb_cmp_ret c WHERE d.codret='".$this->ls_retid."' 
							   AND d.codemp='".$ls_codemp."' and d.codret=c.codret and d.codemp=c.codemp and c.numcom= d.numcom and
							    c.estcmpret<>2) ".$ls_filt;
		...
		elseif($ls_tipo=='P')
		{
			$ls_sql=" SELECT BRH.numdoc as numdoc, BRH.fecmov as fecmov, XSC.numsol as numsol, XSC.monto as monto,".
				    ...
				    "   WHERE ( BRH.codemp='".$ls_codemp."' AND BRH.codope='CH' OR BRH.codope='ND') ".
					"   AND (BRH.estmov='C') ".
	...
	{		
		...
		if($this->ls_retid=='0000000003')
		{
			$ls_sql=" SELECT BRC.codded as codded, XMC.porded as porded, sum(BRC.monto) as retenido, sum(BRC.monobjret) as moret 
			  		  FROM scb_movbco_scg BRC,tepuy_deducciones XMC 
		...
		else
		{
		   $ls_sql=" SELECT XRC.codded as codded, XMC.porded as porded, sum(XRC.monret) as retenido, sum(XRC.monobjret) as moret
			  		 FROM cxp_dt_solicitudes XDS,tepuy_deducciones XMC, cxp_rd_deducciones XRC 
	...
	function uf_ccr_get_nro($ls_periodofiscal,&$ls_nrocomp)
	{
		...
		$ls_sql="SELECT substr(numcom,7,8) as numcom
				 ...
				 codret = '".$this->ls_retid."' AND perfiscal = '".$ls_periodofiscal."'
				 ORDER  by numcom desc ";
		//print "<br> substring 1 $ls_sql";
		$rs_result=$this->io_sql->select($ls_sql);
	...
	function uf_rep_cur_libro_compra( $ls_numsol,$ldec_porc, $ls_Concepto, $arr_datos_proben)
	{
		$ls_concepto ="";
		$li_total=0;		
		$ls_codemp=$this->dat_emp["codemp"];
		$ls_im=" (XRD.estimpmun = 1) AND ";		
		$ls_codigo=$arr_datos_proben["codigo"];
		$ls_tipo=$arr_datos_proben["tipo"];
		
		if ($ls_tipo=="P")
		{
			$ls_criterio="  AND xrd.cod_pro='".$ls_codigo."' ".
                         "  AND xrd.ced_bene='----------'    ".
                         "  AND xrd.codemp='".$ls_codemp."'  ";
		}
		else
		{
			$ls_criterio="  AND xrd.cod_pro='----------'      ".
                         "  AND xrd.ced_bene='".$ls_codigo."' ".
                         "  AND xrd.codemp='".$ls_codemp."'   ";		
		}
		
				
		if(($this->ls_gestorbd=="MYSQL")||($this->ls_gestorbd=="ORACLE") || ($this->ls_gestorbd=="POSTGRE"))
		{
			$ls_sql = " SELECT XDS.numrecdoc as numrecdoc,XRD.codtipdoc as codtipdoc,XRD.cod_pro as cod_pro,XRD.ced_bene
						 as ced_bene,XRD.fecemidoc AS fecha, XRD.numrecdoc AS factura, ".
					  "               XRD.mondeddoc as mondeddoc,XRD.numref AS nro_control,XRD.montotdoc as total_p_isv, 
					  					XRD.montotdoc - ((XRD.montotdoc - XRD.moncardoc) + XRD.moncardoc) as csdc,
										  (XRD.montotdoc - XRD.moncardoc + XRD.mondeddoc) as base_imponible,
										  XRD.moncardoc as total_impuesto,0 as porc_impuesto,XSP.consol  as consol".
					  "        FROM cxp_rd XRD,cxp_dt_solicitudes XDS,cxp_solicitudes XSP ".
					  "        WHERE (XRD.codemp=XDS.codemp AND XRD.codemp=XSP.codemp AND XRD.codemp='".$ls_codemp."') 
					  			AND ".
					  ...
					  "  	         (XSP.estprosol='C' OR XSP.estprosol='S' OR XSP.estprosol='P') AND XSP.numsol='".$ls_numsol."'   ".$ls_criterio.
					  "  ORDER BY  XRD.fecemidoc, XDS.numrecdoc "; 					  
		}
		 else
		{		
			$ls_sql=" SELECT XDS.numrecdoc as numrecdoc,XRD.codtipdoc as codtipdoc,XRD.cod_pro as cod_pro, XRD.ced_bene as ced_bene,  ".
					...
					"		(XDC.codcar IN (SELECT codcar FROM tepuy_cargos where  and estlibcom=1))  ".
					"		 AND XSP.numsol='".$ls_numsol."'".$ls_criterio.
					"  ORDER  by XRD.fecemidoc, XRD.numrecdoc ";
		}
		$rs_result = $this->io_sql->select($ls_sql);
		//print "<br> order by 1 $ls_sql";
		//print "------------------------------------------------------------".$ls_sql."----------------------------------------------   <br>";
		if($rs_result===false)
		{
			...
			for($li_i=1;$li_i<=$li_total;$li_i++)
			{
				...
				$ldec_ivaretenido = $ldec_totimp * ($ldec_porc/100);
				   ...
				   $ls_sql="SELECT b.porded
							FROM scb_movbco_scg a, tepuy_deducciones b
		...
		else
		{
			if($row=$this->io_sql->fetch_row($rs_result))
			{
				...
				for($li_i=1;$li_i<=$li_total_rows;$li_i++)
				{
					...
					$object[$li_i][11]= "<a href=javascript:uf_delete_dt('$ls_numope','$ls_numfac');><img src=../shared/imagebank/tools15/eliminar.png alt='Eliminar Retención' width=15 height=15 border=0></a>";
