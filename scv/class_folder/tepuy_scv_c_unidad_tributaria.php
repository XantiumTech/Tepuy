<?php
class tepuy_scv_c_unidad_tributaria
 {
  	var $ls_sql="";
	var $io_msg_error;
	
	
function tepuy_scv_c_unidad_tributaria()//Constructor de la Clase.
	{
...
{	
	$ls_cadena = "SELECT * FROM tepuy_unidad_tributaria WHERE codunitri='".$as_codigo."' ";
	$rs_data   = $this->io_sql->select($ls_cadena);
	if ($rs_data===false)
	   {
		$this->io_msg->message("CLASE->tepuy_scv_c_unidad_tributaria; METODO->uf_select_unidad_tributaria;ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));   
  ...
  if ($lb_val_fecha)
   {
	...
	$as_valunitri=str_replace(",",".",$as_valunitri);
   	if(!$lb_existe)
		{
			$ls_cadena= " INSERT INTO tepuy_unidad_tributaria(codunitri,anno,fecentvig,gacofi,fecpubgac,decnro,fecdec,valunitri) VALUES('".$as_codigo."', '".$as_anno."' ,'".$ld_fecentvig."','".$as_gacofi."','".$ld_fecpubgac."','".$as_decnro."','".$ld_fecdec."','".$as_valunitri."') ";
			$ls_evento="INSERT";
		...
		{  
		  // if($ls_status=='C')
			//{
			$ls_cadena= " UPDATE tepuy_unidad_tributaria SET anno='".$as_anno."',fecentvig='".$ld_fecentvig."',gacofi='".$as_gacofi."',fecpubgac='".$ld_fecpubgac."',decnro='".$as_decnro."',fecdec='".$ld_fecdec."',valunitri='".$as_valunitri."' WHERE  codunitri='".$as_codigo."'";
		...
		{
			$lb_valido=false;
		    $this->io_msg->message("CLASE->tepuy_scv_c_unidad_tributaria; METODO->uf_guardar_unidad_tributaria;ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));   
...
function uf_delete_unidad_tributaria($as_codigo,$aa_seguridad)
{
	...
	{
	$ls_sql    = "DELETE FROM tepuy_unidad_tributaria WHERE codunitri='".$as_codigo."' ";
	$this->io_sql->begin_transaction();
	...
	if ($rs_data===false)
	   {
		 $lb_valido=false;
	     $this->io_msg_error->message("CLASE->tepuy_scv_C_UNIDAD_TRIBUTARIA; METODO->uf_select_unidad_tributaria;ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));   
	   }
	else
	   {
	   	 $ls_evento="DELETE";
...
function AgregarUno(&$cod)
 {
  ...
  function retornar_ultimo()
	{
	 $ls_sql= "select max(codunitri) as max from tepuy_unidad_tributaria "; 
	/* if ($ls_sql==="")
	  {
	   $codigo='0001'
	  }
