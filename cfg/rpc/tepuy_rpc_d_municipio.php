...
require_once("class_folder/tepuy_rpc_c_municipio.php");
...
$io_municipio = new tepuy_rpc_c_municipio($conn);//Instanciando la Clase tepuy Definiciones.
...
//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
	...
	$ls_sistema  = "CFG";
	$ls_ventanas = "tepuy_rpc_d_municipio.php";
...
if ($ls_operacion=="NUEVO") 
   {
	 $ls_codmun  = $io_municipio->uf_generar_codigo($lb_empresa,$ls_codemp,'tepuy_municipio','codmun',$ls_pais,$ls_estado);
	 if(empty($ls_codmun))
	   {
   ...
   { 
	 ...
	 if ($lb_existe)
        { 
	      if ($ls_estatus=="NUEVO")
		     {
			   $io_msg->message("Este Código de Municipio ya existe !!!");  
			   $lb_valido=false;
			 }
      ...
      if ($lb_existe)
	     {
	       ...
	       $lb_tiene     = $io_chkrel->uf_check_relaciones($ls_empresa,$ls_condicion,'tepuy_municipio',$ls_codmun,$ls_mensaje);//Verifica los movimientos asociados a la cuenta
           if (!$lb_tiene)
		      {
        ...
        <td width="522" height="206"><div align="center">
	     ...
	     ?>
            ...
            <tr>
              ...
              <td height="22"><input name="txtdenominacion" id="txtdenominacion" value="<?php print $ls_denmun ?>" type="text" size="60" maxlength="60" onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnñopqrstuvwxyz '+',.-');"></td>
            </tr>
            <tr>
              <td height="22">&nbsp;</td>
              <td height="22">&nbsp;</td>
...
function ue_nuevo()
{
  ...
  if (li_incluir==1)
	 {	
	   ...
	   f.action="tepuy_rpc_d_municipio.php";
	   f.submit();
	 }
  else
	 {
...
if (((lb_status=="GRABADO")&&(li_cambiar==1))||(lb_status=="NUEVO")&&(li_incluir==1))
   {
     with (document.form1)
	      {
	        if (campo_requerido(txtcodigo,"El Código del Municipio debe estar lleno !!")==false)
			   {
			     txtcodigo.focus();
			   }
			else
			   { 
				 ...
				 if (campo_requerido(txtdenominacion,"La Denominación del Municipio debe estar llena !!")==false)
					{
	...
	if (li_leer==1)
	   {
 	     if (f.cmbpais.value=="s1")
	        {
		      alert("Debe seleccionar un Pais y un Estado para ver el Catálogo de Municipios");
	        }
	     else
	        {
			  if (f.cmbestado.value=="s1")
		         ...
		         {   
				   ...
				   pagina="tepuy_cfg_cat_municipios.php?hidestado="+ls_estado+"&hidpais="+ls_pais;
