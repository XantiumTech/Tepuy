<?php
	...
	$io_fun_cxp->uf_load_seguridad("CXP","tepuy_cxp_r_retencionesunificadas.php",$ls_permisos,$la_seguridad,$la_permisos);
	$ls_reporte=$io_fun_cxp->uf_select_config("CXP","REPORTE","FORMATO_RETMUN","tepuy_cxp_rpp_retencionesunificadas.php","C");
	$ls_activo          = "checked";
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
?>
...
<table width="600" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    ...
    <td height="22" colspan="3" align="center"><input name="chkincluir" type="checkbox" class="sin-borde" id="chkincluir" value="1" checked <?php print $ls_activo; ?>> 
      Incluir Logos/Numeracion de Comprobante Unificado</td>
  </tr>
        </table>
      </div></td>
    </tr>
...
function ue_print()
{
	...
	{
		ls_proveedores="";
		totrow=ue_calcular_total_fila_local("txtfecha");
		//alert(totrow);
		...
		for(li_i=1;li_i<=totrow;li_i++)
		{
			...
			{
				ls_documento=eval("f.txtrif"+li_i+".value");
				ls_numsol=eval("f.txtordenpago"+li_i+".value");
				//Agregado el 22-09-2016 por Ing. Arnaldo Paredes para enviar la fecha de la Retención
				ls_fecsol=eval("f.txtfecha"+li_i+".value");
				//ls_codret=eval("f.txtcodret"+li_i+".value");
				//alert(ls_codret);
				if(ls_proveedores.length>0)
				{
					ls_proveedores = ls_proveedores+"."+ls_documento;
					//ls_codretenciones = ls_codretenciones+"-"+ls_codret;
				}
				else
				{
					ls_proveedores = ls_documento;
					//ls_codretenciones = ls_codret;
				}
				//alert(ls_proveedores);
			}
		}
		if(ls_proveedores!="")
		{
			...
			formato="tepuy_cxp_rpp_retencionesunificadas.php";
			//pagina="reportes/"+formato+"?proveedores="+ls_proveedores+"&mes="+ls_mes+"&anio="+ls_anio+"&tiporeporte="+tiporeporte+"&ls_numsol="+ls_numsol+"&encabezado="+incluir;
			//Modificado el 22-09-2016 por Ing. Arnaldo Paredes para enviar la fecha de la Retención
			pagina="reportes/"+formato+"?proveedores="+ls_proveedores+"&mes="+ls_mes+"&anio="+ls_anio+"&tiporeporte="+tiporeporte+"&ls_numsol="+ls_numsol+"&ls_fecsol="+ls_fecsol+"&encabezado="+incluir;
			//alert(pagina);
...
function ue_search()
{
	...
	ajax.send("catalogo=RETENCIONESUNIFICADAS&tipproben="+tipproben+"&codprobendes="+codprobendes+"&codprobenhas="+codprobenhas+
