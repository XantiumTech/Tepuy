var url= "../../php/tepuy_srh_a_aspectos.php";
var metodo='get';
  ...
  if(lb_valido)
  {
	  ...
	  function onGuardar(respuesta)
	  {
	 	if (trim(respuesta.responseText)=="El Aspecto de Evaluacion fue Registrado")
		 {
			   ...
			   f.action="tepuy_srh_d_aspectos.php";
			   f.submit();	
		 }
...
function ue_buscar()
{
	...
	if(li_leer==1)
	{
		...
		window.open("../catalogos/tepuy_srh_cat_aspectos.php?valor_cat=1","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=518,height=400,left=50,top=50,location=no,resizable=yes");
	}
	else
	{
