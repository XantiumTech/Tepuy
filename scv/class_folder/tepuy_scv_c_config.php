<?php
class tepuy_scv_c_config
{
	var $ls_sql;
	var $is_msg_error;
		
	function tepuy_scv_c_config($conn)
	{
	...
	function uf_insert_scv_config($as_codemp,$as_codsis,$as_seccion,$as_entry,$as_type,$as_value,$aa_seguridad) 
	{
		...
		if ($rs_data===false)		     
		{
			$this->io_msg->message("CLASE->tepuy_scv_c_config METODO->uf_insert_scv_config ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
	...
	function uf_update_scv_config($as_codemp,$as_codsis,$as_seccion,$as_entry,$as_type,$as_value,$aa_seguridad) 
	{
		...
		////////////////////////////////////////////////////////////////////////////// 
		$lb_valido=false;
		$ls_sql=" UPDATE tepuy_config SET value='".$as_value."'".
		...
		if ($rs_data===false)
		{
			$this->io_msg->message("CLASE->tepuy_scv_c_config METODO->uf_update_scv_config ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$lb_valido=true;
	...
	function uf_scv_load_config($as_codemp,$as_codsis,$as_seccion,$as_entry,&$as_spgcuenta,&$as_denspgcuenta) 
	{
		...
		if(($as_entry=="NACIONALES")||($as_entry=="INTERNACIONALES"))
		{
			$ls_sql=" SELECT value,".
					...
					"          WHERE spg_cuentas.spg_cuenta=tepuy_config.value".
		...
		else
		{
			$ls_sql=" SELECT value,".
					...
					"          WHERE scg_cuentas.sc_cuenta=tepuy_config.value".
		...
		if ($rs_data===false)
		{
			$this->io_msg->message("CLASE->tepuy_scv_c_config METODO->uf_scv_load_config ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
			{
				$as_spgcuenta=trim($row["value"]);
	...
	function uf_scv_select_config($as_codemp,$as_codsis,$as_seccion,$as_entry) 
	{
		...
		if ($rs_data===false)
		{
			$this->io_msg->message("CLASE->tepuy_scv_c_config METODO->uf_scv_select_config ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
