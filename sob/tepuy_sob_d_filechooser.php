<?Php
	...
	else
	{
		...
		if($ls_documento=="ACTA")
		{
			...
			require_once("class_folder/tepuy_sob_c_supervisores.php");
			$io_supervisores=new tepuy_sob_c_supervisores();
			...
			$io_funsob->uf_ventanasimple_js("tepuy_sob_d_documentos_acta.php?contrato=".$ls_contrato."&acta=".$ls_acta."&obra=".$ls_obra."&inspector=".$ls_inspector."&residente=".$ls_residente."&ruta=".$ls_ruta."&documento=".$ls_documento);				
		...
		elseif($ls_documento=="CONTRATO")
		{
			...
			$io_funsob->uf_ventanasimple_js("tepuy_sob_d_documentos_contrato.php?documento=".$ls_documento."&ruta=".$ls_ruta."&contrato=".$ls_contrato."&condiciones=".$ls_condiciones."&retenciones=".$ls_retenciones."&garantias=".$ls_garantias."&obra=".$ls_obra."&asignacion=".$ls_asignacion."&inspector=".$ls_inspector."&contratista=".$ls_contratista."&unidadmulta=".$ls_unidadmulta."&unidadgarantia=".$ls_unidadgarantia);			
		}	
		elseif($ls_documento="PUNTODECUENTA")
		{
			//----------------Clases y Objetos--------------------------------------//
			...
			$io_funsob->uf_ventanasimple_js("tepuy_sob_d_documentos_puntodecuenta.php?documento=".$ls_documento."&ruta=".$ls_ruta."&puntodecuenta=".$ls_puntodecuenta."&codigopresupuestariomonto=".$ls_codigopresupuestariomonto."&cuentamonto=".$ls_cuentamonto."&codigopresupuestarioiva=".$ls_codigopresupuestarioiva."&cuentaiva=".$ls_cuentaiva);			
		}
	}
?>
...
<table width="356" border="0" align="center" cellpadding="0" cellspacing="0" class="titulo-celdanew">
  ...
  <table width="356" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    ...
    <tr>
      ...
      <td width="247"><a href="javascript:uf_aceptar();"><img src="../shared/imagebank/aprobado.png" alt="Aceptar" width="15" height="15" border="0" ></a><a href="javascript:uf_cancelar();"><img src="../shared/imagebank/eliminar.png" border="0" alt="Cancelar" width="15" height="15" ></a></td>
    </tr>
  </table>
  <input type="hidden" name="codcon" id="codcon" value="<? print $ls_codcon;?>">
  <input type="hidden" name="codact" id="codact" value="<? print $ls_codact;?>">
  <input type="hidden" name="tipact" id="tipact" value="<? print $ls_tipact;?>">
  <input type="hidden" name="codpuncue" id="codpuncue" value="<? print $ls_codpuncue;?>">
  <input type="hidden" name="codobr" id="codobr" value="<? print $ls_codobr;?>">
  <input type="hidden" name="documento" id="documento" value="<? print $ls_documento;?>">
...
function uf_aceptar()
{
	...
	if (f.txtfile.value!="")
	{
		...
		if(documento=="ACTA")
		{
			pagina="tepuy_sob_documentos.php?ruta="+ls_ruta+"&codcon="+codcon+"&tipact="+tipact+"&codact="+codact+"&documento="+documento;
		}
		else
		{
