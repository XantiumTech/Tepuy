...
ini_set('precision','15');
 ////////////////////////////////////////////////////////////////////////////////////////////////////
 //           CLASS:  class_tepuy_int_scg
 //	         Access:  public
 //     Programador:  Ing. Nelson Barraez  e Ing. Juniors Fraga
 ...
 ////////////////////////////////////////////////////////////////////////////////////////////////////
 class class_tepuy_int_scg extends class_tepuy_int
 {
    var $io_function;
	var $io_fecha;
	var $is_fecha;
	...
	} // end function class_tepuy_int_scg
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_scg_select_movimiento($as_cuenta,$as_procede_doc,$as_documento,$as_debhab,&$adec_monto,&$ai_orden)
	{
		...
		if($rs_mov===false)
		{
			$this->is_msg_error="CLASE->tepuy_int_scg MÉTODO->uf_scg_select_movimiento ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
		}
		else
		{
	...
	function uf_scg_insert_movimiento($as_cuenta,$as_procede_doc,$as_documento,$as_debhab,$as_descripcion,$adec_monto)
	{
		...
		if($li_result===false)
		{
			...
			{   
				$this->is_msg_error="CLASE->tepuy_int_scg MÉTODO->uf_scg_insert_movimiento ERROR->Fallo alguna clave foranea";
			}
			else
			{
				$this->is_msg_error="CLASE->tepuy_int_scg MÉTODO->uf_scg_insert_movimiento ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			}
		   	$lb_valido=false;
		}
		return $lb_valido;
	...
	function uf_scg_obtener_orden_movimiento()
	{
		...
		if($rs_saldos===false)
		{
			$this->is_msg_error="CLASE->tepuy_int_scg MÉTODO->uf_scg_obtener_orden_movimiento ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			$lb_valido=false;
		}
		else
		{
	...
	function uf_scg_select_cuenta($as_codemp,$as_sc_cuenta,&$as_status,&$as_denominacion)
	{
		...
		if ($rs_cuentas===false)
		{
			$this->is_msg_error = "CLASE->tepuy_int_scg MÉTODO->uf_scg_select_cuenta ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			return false;
		}
		else
		{
	...
	function uf_scg_select_cuenta_sin_cero($as_codemp,$as_cuenta)
	{
		...
		if ($rs_data===false)
		{
			$this->is_msg_error = "CLASE->tepuy_int_scg MÉTODO->uf_scg_select_cuenta_sin_cero ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			return false;
		}
		else
		{
	...
	function uf_scg_select_cuenta_movimiento($as_codemp,$as_sc_cuenta)
	{
		...
		if ($rs_data===false)
		{
			$this->is_msg_error = "CLASE->tepuy_int_scg MÉTODO->uf_scg_select_cuenta_movimiento ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			return false;
		}
		else
		{
	...
	function uf_select_plan_unico($as_cuenta)
	{
		...
		{
		   $lb_valido=false;
		   $this->is_msg_error="CLASE->tepuy_int_scg MÉTODO->uf_select_plan_unico ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
		}
		else
		{
	...
	function uf_scg_valida_cuenta($as_codemp,$as_cuenta)
	{
		...
		{
			$lb_existe=false; 
			$this->is_msg_error="CLASE->tepuy_int_scg MÉTODO->uf_scg_valida_cuenta ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
	...
	function uf_scg_select_saldo($as_cuenta,$as_fecha)
	{
		...
		if($rs_data===false)
		{
			 $this->is_msg_error = "CLASE->tepuy_int_scg MÉTODO->uf_scg_select_saldo ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			 $lb_existe = false;		
		}
		else
		{
	...
	function uf_scg_insert_saldo($as_cuenta,$as_fecha,$adec_monto_actual,$as_debhab)
	{
		...
		if($li_exec===false)
		{
			$this->is_msg_error = "CLASE->tepuy_int_scg MÉTODO->uf_scg_insert_saldo ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			$lb_valido=false;
		}
		return $lb_valido;
	...
	function uf_select_plan_unico_cuenta($as_sc_cuenta,$as_denominacion)
	{
		...
		if($rs_data===false)
		{
			$this->is_msg_error = "CLASE->tepuy_int_scg MÉTODO->uf_select_plan_unico_cuenta ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			return false;
		}
		else
		{
    ...
    {	
		...
		if($rs_data===false)
		{
			$this->is_msg_error="CLASE->tepuy_int_scg MÉTODO->uf_select_plan_unico_cuenta_recurso ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			return false;
		}
		else
		{
	...
	function  uf_scg_insert_cuenta($as_codemp,$as_cuenta,$as_denominacion,$as_status,$ai_nivel,$as_cuenta_ref,$as_mensaje)
	{
		...
		if($li_result===false)
		{
			$this->is_msg_error = "CLASE->tepuy_int_scg MÉTODO->uf_scg_insert_cuenta ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			$lb_valido=false;
		}
		return $lb_valido;
	...
	function uf_cargar_datastore_plan_cuentas($ads_cuentas)
	{
		...
		{
			$lb_valido=false;
			$this->is_msg_error="CLASE->tepuy_int_scg MÉTODO->uf_cargar_datastore_plan_cuentas ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
		}
		else
		{
	...
	function uf_cargar_comprobante_cierre($as_codemp,$as_procede,$as_comprobante,$as_fecha)
	{
		...
		{
			$lb_valido=false;
			$this->is_msg_error="CLASE->tepuy_int_scg MÉTODO->uf_cargar_comprobante_cierre ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
		}
		else
		{
			while ($row=$this->io_sql->fetch_row($rs_data)) 
	...
	function uf_cargar_comprobante_cierre_delete($as_codemp,$as_procede,$as_comprobante,$as_fecha)
	{
		...
		{
			$lb_valido=false;
			$this->is_msg_error="CLASE->tepuy_int_scg MÉTODO->uf_cargar_comprobante_cierre_delete ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
		}
		else
		{
			while ($row=$this->io_sql->fetch_row($rs_data)) 
	...
	function uf_cargar_datastore_cuentas($ads_cuentas,$as_cuentas)
	{
		...
		{
			$lb_valido=false;
			$this->is_msg_error="CLASE->tepuy_int_scg MÉTODO->uf_cargar_datastore_cuentas ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
		}
		else
		{
			while ($row=$this->io_sql->fetch_row($rs_data)) 
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
     ...
     {	 
		...
		if($rs_data==false)
		{
			$this->is_msg_error="CLASE->tepuy_int_scg MÉTODO->uf_scg_cargar_detalle_comprobante ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			$lb_valido=false;
		}
		return $rs_data;
	...
	function uf_scg_saldo($as_cuenta,&$adec_saldo,$adt_fec_cierre)
	{
		...
		if($rs_saldo===false)
		{
			$this->is_msg_error = "CLASE->tepuy_int_scg MÉTODO->uf_scg_saldo ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			return false;
		}
		else
		{
	...
	function uf_scg_comprobante_update($li_monto)
	{
		...
		if($li_exec===false)
		{
			$this->is_msg_error="CLASE->tepuy_int_scg MÉTODO->uf_scg_comprobante_update ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			$lb_valido=false;
		}	   
		return $lb_valido;
	...
	function uf_scg_comprobante_update_cero()
	{
		...
		if($li_exec===false)
		{
			$this->is_msg_error="CLASE->tepuy_int_scg MÉTODO->uf_scg_comprobante_update_cero ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			$lb_valido=false;
		}	   
		return $lb_valido;
	...
	function uf_insert_plan_unico_cuenta($as_cuenta,$as_denominacion,$as_status)
	{
		...
		if($this->uf_select_plan_unico($as_cuenta))
		{
			if($as_status=='C')
			{
				...
				if($li_exec===false)
				{
					$this->is_msg_error="CLASE->tepuy_int_scg MÉTODO->uf_insert_plan_unico_cuenta ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
	...
	{	 
		...
		if($li_result===false)
		{
			$this->is_msg_error = "CLASE->tepuy_int_scg MÉTODO->uf_scg_update_cuenta ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			$this->io_sql->rollback();		  			
			return false;
		}
		else 
	...
	{	
		...
		if($li_result===false)
		{
			$this->is_msg_error = "CLASE->tepuy_int_scg MÉTODO->uf_scg_delete_cuenta ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message);
			$this->io_sql->rollback();					
			$lb_valido = false;
		}
		else
