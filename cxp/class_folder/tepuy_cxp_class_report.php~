...
class tepuy_cxp_class_report
{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------	
	function uf_select_contribuyentes_libro_timbrefiscal($as_mes,$as_anio)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql="SELECT  scb_cmp_ret.nomsujret, scb_cmp_ret.rif, scb_cmp_ret.numcom, ".
			    " scb_dt_cmp_ret.basimp, scb_dt_cmp_ret.iva_ret, scb_dt_cmp_ret.fecfac, scb_dt_cmp_ret.numsop, scb_dt_cmp_ret.totcmp_con_iva, cxp_solicitudes.fecaprosol".
...
//print $ls_sql;
	...
	function uf_select_solicitud($as_numsol)
	{
		...
		$ls_sql="SELECT cxp_solicitudes.numsol, cxp_solicitudes.cod_pro, cxp_solicitudes.ced_bene, cxp_solicitudes.codfuefin,".
				...
				"       cxp_solicitudes.monsol, cxp_solicitudes.obssol,sss_usuarios.nomusu as nomusu,sss_usuarios.apeusu as apeusu,cxp_solicitudes.estaprosol,cxp_solicitudes.monsolaux,".
...
//nuevo
				"       (CASE tipproben WHEN 'P' THEN (SELECT rpc_proveedor.nacpro ".
				"                                        FROM rpc_proveedor ".
				"                                       WHERE rpc_proveedor.codemp=cxp_solicitudes.codemp ".
				"                                         AND rpc_proveedor.cod_pro=cxp_solicitudes.cod_pro) ".
...
//
				...
				"  FROM cxp_solicitudes, sss_usuarios ".	
				" WHERE cxp_solicitudes.codemp='".$this->ls_codemp."' ".
				"   AND cxp_solicitudes.numsol='".$as_numsol."' ".
				"   AND cxp_solicitudes.usuaprosol=sss_usuarios.codusu "; //print $ls_sql;
								   ...
								   $ai_registrada,$ai_anulada,$ai_procesada,$as_orden)
	{
		...
		$ls_sql="SELECT cxp_rd.numrecdoc,cxp_rd.fecemidoc,cxp_rd.fecregdoc,MAX(cxp_rd.montotdoc) AS montotdoc,".
				...
				"		MAX(cxp_documento.dentipdoc) AS dentipdoc, MAX(cxp_rd_spg.numdoccom) AS numdoccom,".
				"       MAX(cxp_rd_spg.procede_doc) AS procede_doc,".
				...
				"                                         AND rpc_beneficiario.ced_bene=cxp_rd.ced_bene) ". 
				"                       ELSE 'NINGUNO' END ) AS nombre ".
				"  FROM cxp_rd,cxp_documento,cxp_rd_spg ".	
				" WHERE cxp_rd.codemp='".$this->ls_codemp."' ".
				"   ".$ls_criterio." ".
				"   AND cxp_rd.codtipdoc=cxp_documento.codtipdoc".
...
*/
	...
	function uf_retencionesislr_cxp($as_numsol)
	{
	   ...
	   $ls_sql="SELECT cxp_rd.numrecdoc AS numdoc,cxp_rd.numref, cxp_rd.fecemidoc, cxp_rd.tipproben, rpc_proveedor.nitpro AS nit, ".
	   		   "	   rpc_proveedor.nompro AS proveedor, rpc_proveedor.telpro, rpc_proveedor.dirpro, rpc_proveedor.rifpro AS rif, ".
			   $ls_cadena." AS beneficiario, rpc_beneficiario.dirbene, rpc_beneficiario.rifben, rpc_beneficiario.telbene, ".
			   "	   rpc_proveedor.cod_pro, rpc_beneficiario.ced_bene,cxp_solicitudes.numsol,".
			   "	   cxp_solicitudes.consol, cxp_rd.montotdoc, cxp_rd_deducciones.monret AS retenido, ".
			   "	   COALESCE(cxp_rd_deducciones.monobjret, cxp_solicitudes.monsol) AS monobjret,cxp_dt_solicitudes.codtipdoc, ".
			   "	   tepuy_deducciones.porded AS porcentaje,tepuy_deducciones.dended AS dended,".
               "      tepuy_deducciones.monded, cxp_rd.mondeddoc,cxp_rd.moncardoc,	".
               "	   (SELECT cxp_sol_banco.numdoc".
			   "		  FROM cxp_sol_banco".
			   "		 WHERE cxp_sol_banco.estmov<>'A' 
			               AND cxp_sol_banco.estmov<>'O' 
			               AND cxp_solicitudes.codemp=cxp_sol_banco.codemp".
			   "           AND cxp_solicitudes.numsol=cxp_sol_banco.numsol) AS cheque,".
			   "	   (SELECT cxp_sol_banco.ctaban".
			   "		  FROM cxp_sol_banco".
			   "		 WHERE cxp_sol_banco.estmov<>'A' 
			               AND cxp_sol_banco.estmov<>'O' 
			               AND cxp_solicitudes.codemp=cxp_sol_banco.codemp".
			   "           AND cxp_solicitudes.numsol=cxp_sol_banco.numsol) AS ctaban,".
		...
		{
			$this->io_mensajes->message("CLASE->Report MÉTODO->uf_retencionesislr_cxp ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			//$this->io_mensajes->message("ERROR:".$ls_sql);
			$lb_valido=false;
		}
		else
		{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_select_retenciones($as_codded,$as_coddedhas)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_retencionesespecifico($as_codded,$as_coddedhas,$as_tipproben,$as_codprobenhas,$as_codprobendes,$ad_fecdes,$ad_fechas)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql="SELECT cxp_solicitudes.numsol, MAX(rpc_proveedor.nitpro) AS nitpro, MAX(cxp_solicitudes.tipproben) AS tipproben, MAX(cxp_solicitudes.fecemisol) AS fecemisol, ".
			   "       MAX(rtrim(cxp_solicitudes.consol)) AS concepto, MAX(cxp_solicitudes.monsol) AS monsol, MAX(cxp_solicitudes.estprosol) AS estprosol, ".
			   "	   MAX(CAST(SUBSTRING(COALESCE(cxp_solicitudes.obssol,' '),1,250) AS CHAR(250))) AS observaciones, ".
			   "	   MAX(cxp_rd_deducciones.monobjret) AS mon_obj_ret, SUM(cxp_rd_deducciones.monret) AS monret, ".
			   "       MAX(cxp_rd_deducciones.codded) as codded,MAX(tepuy_deducciones.dended) as dended,".
			   "       (CASE MAX(cxp_solicitudes.tipproben) WHEN 'P' THEN MAX(rpc_proveedor.nompro) ".
			   "								       WHEN 'B' THEN ".$ls_cadena.
	...
	function uf_retencionesiva_detalle($as_numcom)
	{
		...
		$ls_sql="SELECT max(codret) as codret, max(numcom) as numcom, max(numope) as numope, max(fecfac) as fecfac, ".
		    	...
		    	/*" (select cxp_sol_banco.numdoc from cxp_sol_banco,scb_movbco where  cxp_sol_banco.numsol=scb_dt_cmp_ret.numsop and cxp_sol_banco.numdoc=scb_movbco.numdoc) as numche, ".
				" (select fecmov from cxp_sol_banco,scb_movbco where cxp_sol_banco.numsol=scb_dt_cmp_ret.numsop and cxp_sol_banco.numdoc=scb_movbco.numdoc ) as fecche ".*/
				"  FROM scb_dt_cmp_ret ".
				" WHERE codemp='".$this->ls_codemp."' ".
				"   AND numcom='".$as_numcom."' ".
				"	AND codret='0000000001' ".
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
function uf_declaracioninformativaislr($as_mes,$as_anio,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_declaracioninformativa
		//         Access: public  
		...
		$lb_valido=true;
		$ld_fechadesde=$as_anio."-".$as_mes."-01";
		$ld_fechahasta=$as_anio."-".$as_mes."-31";
	    	$ls_periodofiscal=substr($ld_fechadesde,0,4).substr($ld_fechadesde,5,2);
		$ls_criterio="";
		$ls_archivo="declaracioninformativa/Retencion_ISLR_".date("Y_m_d_H_i").".txt";
		$lo_archivo=fopen("$ls_archivo","a+");	
		$ld_fechahasta=$as_anio."-".$as_mes."-31";
		$ls_criterio=$ls_criterio." AND fecrep >='".$ld_fechadesde."'".
										  " AND fecrep <='".$ld_fechahasta."'";
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report MÉTODO->uf_declaracioninformativaislr ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			...
			if($lb_valido)
			{
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="PROCESS";
				$ls_descripcion ="Genero el txt de Declaración Informativa ISLR Para el Año ".$as_anio." Mes ".$as_mes." Archivo ".$ls_archivo." Asociado a la empresa ".$this->ls_codemp;
				$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////					
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_retencionestimbrefiscal_proveedor($as_numcom,$as_mes,$as_anio)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_retencionestimbrefiscal_proveedor
		//         Access: public  
		//	    Arguments: as_numcom // Numero de comprobante municipal
		//	    		   as_mes    // mes del comprobante
		//	    		   as_anio   // año del comprobante
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report MÉTODO->uf_retencionestimbrefiscal_proveedor ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
		...
		return $lb_valido;
	}// end function uf_retencionestimbrefiscal_proveedor
	//-----------------------------------------------------------------------------------------------------------------------------------



//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_retencionesislr_proveedor($as_numcom,$as_mes,$as_anio)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_retencionesmunicipales_proveedor
		//         Access: public  
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report MÉTODO->uf_retencionesislr_proveedor ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
		...
		return $lb_valido;
	}// end function uf_retencionesislr_proveedor
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_retencionesmunicipales_detalles($as_numcom)
	{
		...
		$ls_sql="SELECT max(codret) as codret, max(numsop) as numsop, max(fecfac) as fecfac,  max(numfac) as numfac, max(numcon) as numcon, ".
				"		SUM(basimp) as basimp,  porimp, SUM(iva_ret) as totimp ".
				"  FROM scb_dt_cmp_ret ".
				" WHERE codemp='".$this->ls_codemp."' ".
				"   AND numcom='".$as_numcom."' ".
				"	AND codret='0000000002' ".
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_retencionestimbrefiscal_detalles($as_numcom)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$lb_valido=true;
		$ls_sql="SELECT max(codret) as codret, max(numsop) as numsop, max(fecfac) as fecfac,  max(numfac) as numfac, max(numcon) as numcon, ".
				"		SUM(totcmp_con_iva) as montototal, SUM(basimp) as basimp,  porimp, SUM(iva_ret) as totimp ".
				"  FROM scb_dt_cmp_ret ".
				" WHERE codemp='".$this->ls_codemp."' ".
				"   AND numcom='".$as_numcom."' ".
				"	AND codret='0000000005' ".
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report MÉTODO->uf_retencionestimbrefiscal_detalles ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
...
//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_retencionesislr_detalles($as_numcom)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_retencionesmunicipales_detalles
		//         Access: public  
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report MÉTODO->uf_retencionesislr_detalles ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
...
//				mysql_data_seek( $rs_data,0);//Devuelvo el puntero al comienzo
	...
	function uf_select_informacionpagoscxp($as_tipproben,$as_cedbene,$as_codpro,$ad_fecregdes,$ad_fecreghas)
	{
		...
		$ls_sql=" SELECT cxp_solicitudes.numsol,cxp_sol_banco.monto,cxp_sol_banco.codope,scb_movbco.fecmov,scb_movbco.conmov,".
				"        (SELECT nomban FROM scb_banco WHERE cxp_sol_banco.codemp=scb_banco.codemp AND cxp_sol_banco.codban=scb_banco.codban) AS nomban,".
				"         cxp_sol_banco.ctaban,cxp_sol_banco.numdoc".
				"   FROM cxp_solicitudes, cxp_historico_solicitud,cxp_sol_banco,scb_movbco".
				"  WHERE cxp_solicitudes.codemp='".$this->ls_codemp."' ".
				"	 AND cxp_solicitudes.tipproben='".$as_tipproben."'".
				"    AND trim(cxp_solicitudes.ced_bene)=trim('".$as_cedbene."')".
				"    AND cxp_solicitudes.cod_pro='".$as_codpro."'".
				"    AND cxp_historico_solicitud.estprodoc='P'".
				"    AND cxp_solicitudes.estprosol<>'A'".
		...
		//	print_r($this->io_sql->obtener_datos($rs_data));
			}
			$this->io_sql->free_result($rs_data);
		}		
//print_r ($this->ds_pagactuales);
		return $lb_valido;
	...
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_select_sol_cargos
		//         Access: public  
		//	    Arguments: as_codemp     // Codigo de empresa
		//                 as_numero     // Numero de Solicitud
		//	      Returns: lb_valido True si se creo el Data stored correctamente ó False si no se creo
		//    Description: 
		//	   Creado Por: 
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		 ...
		 $ls_sql=" SELECT cxp_dt_solicitudes.numsol as numsol,cxp_dt_solicitudes.numrecdoc as numrecdoc,".
				 "        cxp_rd_cargos.codcar as codcar,SUM(cxp_rd_cargos.monobjret) as monobjretcar,".
				 "		  SUM(cxp_rd_cargos.monret) as objretcar, ".
				 " 		  max(tepuy_cargos.dencar) as dencar,MAX(cxp_rd_cargos.porcar) AS porcar".
				 "  FROM cxp_dt_solicitudes, cxp_rd_cargos , tepuy_cargos".
				 ...
				 "	 AND cxp_rd_cargos.codemp=tepuy_cargos.codemp".
				 "	 AND cxp_rd_cargos.codcar=tepuy_cargos.codcar".
				 " GROUP BY cxp_rd_cargos.codcar,cxp_dt_solicitudes.numsol,cxp_dt_solicitudes.numrecdoc";
		 $this->ds_car_dt->resetds("numsol");			
		 $rs=$this->io_sql->select($ls_sql);
		 if ($rs===false)
		 {		 
			 $this->io_mensajes->message("CLASE->tepuy_CXP_CLASS_REPORT; METODO->uf_select_sol_cargos; ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			 $lb_valido=false;
		 }
		 else
		 {
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_retencionesislr_cxp_iva($as_numrecdoc,$as_codpro,$as_cedbene,$as_codtipdoc,&$ai_monret)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//       Function: uf_retencionesislr_cxp_iva
		//         Access: public  
		//	    Arguments: as_numrecdoc     // Numero recepcion de documentos
		//                 as_codpro     // Codigo de proveedor
		//                 as_cedben     // Cedula de Beneficiario
		//                 as_codtipdoc     // codigo de tipo de documento
		//                 ai_monret     // Monto retenido por concepto de IVA
		//	      Returns: lb_valido True si se creo el Data stored correctamente ó False si no se creo
		//    Description: Función que busca la información el monto retenido por iva
		//	   Creado Por: Ing. Gusmary Balza / Ing. Juniors Fraga          
		// Fecha Creación: 12/03/2008									Fecha Última Modificación :  
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ai_monret=0;
		$ls_sql="SELECT monret".
	   			"  FROM cxp_rd_deducciones,tepuy_deducciones".
				...
				"   AND tepuy_deducciones.islr=0".
				"   AND tepuy_deducciones.iva=1".
				"   AND cxp_rd_deducciones.codemp=tepuy_deducciones.codemp".
				"   AND cxp_rd_deducciones.codded=tepuy_deducciones.codded";
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->Report MÉTODO->uf_retencionesislr_cxp_iva ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
			{
				$ai_monret=$row["monret"];
			}
			$this->io_sql->free_result($rs_data);
		}		
		return $lb_valido;
	}// end function uf_retencionesislr_cxp_iva
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_select_deducciones_recepcion($as_codemp,$as_numrecdoc,$as_codtipdoc,$as_codpro,$as_cedben)
	{
	 ...
	 $ls_sql=" SELECT cxp_rd_deducciones.monret,
                ...
                WHERE cxp_rd.codemp='".$as_codemp."'  
				  ...
				  AND cxp_rd.ced_bene=cxp_rd_deducciones.ced_bene
				  AND tepuy_deducciones.codemp=cxp_rd_deducciones.codemp
				  AND tepuy_deducciones.codded=cxp_rd_deducciones.codded";//print $ls_sql;
