<?php
class tepuy_soc_c_tiposer
{
var $ls_sql;
var $is_msg_error;
	
	function tepuy_soc_c_tiposer($conn)
	{
...
function uf_insert_tiposervicio($as_codtip,$as_dentip,$aa_seguridad,$as_codmil) 
{
	...
	if ($rs_data===false)
	   {
         $lb_valido=false;
		 $this->io_msg->message("CLASE->tepuy_SOC_C_TIPOSER; METODO->uf_insert_tiposervicio; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	   }
	else
	 {
	   $lb_valido=true;
...
function uf_update_tiposervicio($as_codtip,$as_dentip,$aa_seguridad,$as_codmil) 
{
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////  
	  ...
	  if ($rs_data===false)
		 {
		   $lb_valido=false;
		   $this->io_msg->message("CLASE->tepuy_SOC_C_TIPOSER; METODO->uf_update_tiposervicio; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		 }
	  else
		 {
		   $lb_valido=true;
...
{          		 
   ...
   if ($rs_data===false)
      {
	    $lb_valido=false;
	    $this->io_msg->message("CLASE->tepuy_SOC_C_TIPOSER; METODO->uf_delete_tiposervicio; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	  }
   else
      {
	    $lb_valido=true;
...
////////////////////////////////////////////////////////////////////////////// 
  ...
  if ($rs_data===false)
	 {
	   $lb_valido=false;
	   $this->io_msg->message("CLASE->tepuy_SOC_C_TIPOSER; METODO->uf_select_tiposervicio; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	 }
  else
	 {
