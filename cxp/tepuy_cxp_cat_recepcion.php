<?php
	...
	$ls_tipodestino=$io_fun_cxp->uf_obtenervalor("cmbtipdes","-");
	$ls_codprovben=$io_fun_cxp->uf_obtenervalor("txtcodigo","");
	$ls_nomprovben=$io_fun_cxp->uf_obtenervalor("txtnombre","");
	unset($io_fun_cxp);
...
?>		
        ...
        <td width="262" height="22"><div align="left">
          <input name="txtnombre2" type="text" class="sin-borde2" id="txtnombre2" value="<?php print $ls_nombre; ?>" size="30" readonly>
...
<?php
	...
	{
?>
          <td height="22"><input name="txtnombre2" type="hidden" class="sin-borde2" id="txtnombre2">
...
?>
      ...
      <tr>
        <td width="136" height="22"><div align="right">N&uacute;mero de Ficha </div></td>
        <td width="262" height="22"><div align="left">
	  ...
	  <tr>
        ...
        <td colspan="4"><div align="left">
          ...
          <input name="txtnombre" type="text" class="sin-borde" id="txtnombre" value="<?php print $ls_nomprovben;?>" size="30" readonly>
        </div></td>
      </tr>
	  <?php }?>
	  <tr>
        <td colspan="3"><div align="right"><?php  if($ls_tipo!="SOLICITUDPAGO"){?><a href="javascript: ue_search_recepcion();"><?php }else{?><a href="javascript: ue_search();"><?php }?><img src="../shared/imagebank/tools20/buscar.png" alt="Buscar" width="20" height="20" border="0"> Buscar</a></div></td>
	  </tr>
	</table> 
	<p>
...
function ue_cambiardestino()
{
	...
	if(tipdes!="-")
	{
		if(tipdes=="P")
		{
          window.open("tepuy_cxp_cat_proveedor.php?tipo=SOLICITUDPAGO","_blank","menubar=no,toolbar=no,scrollbars=no,width=600,height=400,resizable=yes,location=no,left=50,top=50");
		}
		else
		{
            window.open("tepuy_cxp_cat_beneficiario.php?tipo=SOLICITUDPAGO","_blank","menubar=no,toolbar=no,scrollbars=no,width=600,height=400,resizable=yes,location=no,left=50,top=50");
		}	
	}
}
function ue_aceptar(numrecdoc,numfactura,codtipdoc,cedbene,codpro,codcla,fecemidoc,fecregdoc,fecvendoc,montotdoc,
					mondeddoc,moncardoc,tipproben,numref,estprodoc,procede,estlibcom,estaprord,estimpmun,nombre,rif,
					sccuenta,tipocont,estcon,estpre,li_i,ls_estatus,codfuefin,denfuefin)
{
	...
	else
	{
		...
		opener.document.formulario.txtnumfactura.value=numfactura;
		//alert(numfactura);
		opener.document.formulario.txtnumrecdoc.readOnly=true;
		// PERMITE CAMBIAR LA FICHA DEL DOCUMENTO //
		//opener.document.formulario.txtnumrecdoc.readOnly=false;
		...
		opener.document.formulario.recepcionexiste.value="true";
...
function ue_aceptar_solicitud(numrecdoc,codtipdoc,dentipdoc,montotdoc,li_i)
{
	//---------------------------------------------------------------------------------
	...
	if(consol=="")
	{
		opener.document.formulario.txtconsol.value=eval("f.txtdencondoc"+li_i+".value;");
//		opener.document.formulario.txtconsol.value=dencondoc;
	}
	else
	{
		//consol= consol+", "+dencondoc;
		opener.document.formulario.txtconsol.value=consol+", "+eval("f.txtdencondoc"+li_i+".value;");
		//opener.document.formulario.txtconsol.value=dencondoc;
	}
