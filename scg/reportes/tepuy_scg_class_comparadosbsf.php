<?php
class tepuy_scg_class_comparadosbsf
{
	var $la_empresa;
	var $io_fun;
	var $io_sql;
	...
	var $ds_currep;
	var $ds_reporte2;
	function tepuy_scg_class_comparadosbsf()
	{
 ...
 function  uf_scg_reporte_select_saldo_ingreso_BG($adt_fecdesde,$adt_fechasta,$ai_ingreso,&$ad_saldo) 
   {				 
	 ...
	 $ls_sql=" SELECT COALESCE(sum(SD.debe_mesaux-SD.haber_mesaux),0) as saldo ".
			 ...
			 "        SD.fecsal <= '".$this->io_fun->uf_convertirdatetobd($adt_fechasta)."' AND (SC.sc_cuenta like '".$ai_ingreso."%') AND SC.nivel=1
...
/****************************************************************************************************************************************/	
	...
	function uf_procesar_cuenta($as_cuenta,$ld_fecsalini,$ld_fecsalfin,$li_sel)
	{
		...
		else
		{
			...
			{			
				...
				$ls_sql    = "SELECT (debe_mesaux) AS debe_mes, (haber_mesaux) as haber_mes
						      FROM   scg_saldos
	...
	{
		$ls_sql    = "SELECT SUM(debe_mesaux-haber_mesaux) as saldo
					  FROM   scg_saldos
					  WHERE  codemp='".$ls_codemp."' AND sc_cuenta like '".$ls_cuenta_aux."%' 
	...
	{
		$ls_sql="SELECT SUM(debe_mesaux) as debe, SUM(haber_mesaux) as haber 
				 FROM   scg_saldos 
	...
	{
		$ls_sql = "SELECT (sum(debe_mesaux) - sum(haber_mesaux)) as saldo 
				   FROM scg_saldos 
	...
	function uf_calcular_resultado($ldt_feclimit,$li_maxnivel)
	{
		...
		$ls_codemp =$this->la_empresa["codemp"];
		$ls_cuenta_resultado = $this->la_empresa["c_resultad"];
		if(($this->ls_gestor=="MYSQL")||($this->ls_gestor=="ORACLE"))
		{
			$ls_sql=" SELECT SC.sc_cuenta,SC.denominacion,SC.status,curSaldo.T_Debe,curSaldo.T_Haber, 0 as Nivel 
					  FROM scg_cuentas SC
					  LEFT OUTER JOIN ( SELECT  sc_cuenta, sum(debe_mesaux)as T_Debe, sum(haber_mesaux) as T_Haber 
					                       FROM scg_saldos 
					                       WHERE codemp='".$ls_codemp."' AND fecsal<='".$this->io_fun->uf_convertirdatetobd($ldt_feclimit)."'
					                       GROUP BY sc_cuenta) curSaldo 
					  ON SC.sc_cuenta=curSaldo.sc_cuenta 	
					  WHERE codemp='".$ls_codemp."' AND (SC.sc_cuenta like '".$li_activo."%' OR SC.sc_cuenta like '".$li_pasivo."%' OR SC.sc_cuenta like '".substr($ls_cuenta_resultado,0,1)."%' OR SC.sc_cuenta like '".$li_capital."%' OR SC.sc_cuenta like '".$li_orden_d."%' OR SC.sc_cuenta like '".$li_orden_h."%') 
					  ORDER BY  SC.sc_cuenta ";
		}
		else
		{
			$ls_sql =" SELECT  SC.sc_cuenta, SC.denominacion, SC.status, curSaldo.T_Debe, curSaldo.T_Haber, cast(0 as int) as Nivel 
					   FROM  scg_cuentas SC,(SELECT sc_cuenta, sum(debe_mesaux)as T_Debe, sum(haber_mesaux) as T_Haber 
											 FROM   scg_saldos 
											 WHERE  codemp='".$ls_codemp."' AND fecsal<= '".$this->io_fun->uf_convertirdatetobd($ldt_feclimit)."'
					  GROUP BY sc_cuenta) as curSaldo 
					  WHERE codemp='".$ls_codemp."' AND ( SC.sc_cuenta *= curSaldo.sc_cuenta) AND (SC.sc_cuenta like '".$ls_activo."%' 
					  OR SC.sc_cuenta like '".$ls_pasivo."%' OR SC.sc_cuenta like '".substr($ls_cuenta_resultado,0,1)."%' 
					  OR SC.sc_cuenta like '".substr($ls_capital,0,1)."%' OR SC.sc_cuenta like '".$ls_orden_d."%' OR SC.sc_cuenta like '".$ls_orden_h."%')
					  ORDER BY by SC.sc_cuenta ";
		}
	 $rs_data=$this->io_sql->select($ls_sql);
	 if($rs_data===false)
	 {// error interno sql
	 ...
	 else
	 {
		...
		$ldec_ingreso=0;
		$ldec_gasto=0;
		$ls_sql=" SELECT sum(B.debe_mesaux-B.haber_mesaux) as Saldo 
				  ...
				  WHERE  SC.codemp='".$ls_codemp."' AND SC.codemp=B.codemp AND (SC.sc_cuenta = B.sc_cuenta) AND SC.status='C' AND 
					     B.fecsal <= '".$this->io_fun->uf_convertirdatetobd($ldt_feclimit)."' AND SC.sc_cuenta like '".$li_ingreso."%'";
		$rs_data= $this->io_sql->select($ls_sql);
		if($rs_data===false)
		{
...
//		omitir=thisform.chkOmitir.value
	...
	{
		$lb_existe=false;
		$ls_sql="SELECT codemp, cod_report, spi_cuenta, denominacion, status, sc_cuenta, distribuir, nivel, referencia, (previstoaux) as previsto, (devengadoaux) As devengado, ".
				"		(cobradoaux) AS cobrado, (cobrado_anticipadoaux) AS cobrado_anticipado, (aumentoaux) AS aumento, (disminucionaux) AS disminucion, ".
				"		(eneroaux) AS enero, (febreroaux) AS febrero, (marzoaux) AS marzo, (abrilaux) AS abril, (mayoaux) AS mayo, (junioaux) AS junio, ".
				"		(julioaux) AS julio, (agostoaux) AS agosto, (septiembreaux) AS septiembre, (octubreaux) AS octubre, (noviembreaux) AS noviembre, ".
	...
	function uf_scg_select_saldo($as_cuenta,$as_fecdesde,$as_fechasta,$adec_debe,$adec_haber)
	{
	   ...
	   $ls_sql = " SELECT SUM(debe_mesaux) as debe_mes,SUM(haber_mesaux) as haber_mes FROM scg_saldos ".
	...
	function uf_saldo_csm($ls_cuenta,$ld_fechacierre,$ldec_saldo)
	{
		...
		$ls_sql="SELECT sum(debe_mesaux) as total_debe, sum(haber_mesaux) as total_haber
				 FROM   scg_saldos
	...
	{
		$ls_sql="SELECT codemp, codrep, codestpro1, codestpro2, codestpro3, codestpro4, codestpro5, spg_cuenta, denominacion, ".
				"       status, sc_cuenta, distribuir, nivel, referencia, modrep, (asignadoaux) AS asignado, ".
				"		(precomprometidoaux) as precomprometido, (comprometidoaux) as comprometido, (causadoaux) AS causado, ".
				"		(pagadoaux) as pagado, (aumentoaux) as aumento, (disminucionaux) as disminucion, (eneroaux) as enero, ".
				"		(febreroaux) as febrero, (marzoaux) as marzo, (abrilaux) as abril, (mayoaux) as mayo, (junioaux) as junio, ".
				"		(julioaux) as julio, (agostoaux) as agosto, (septiembreaux) as septiembre, (octubreaux) as octubre, ".
				"		(noviembreaux) as noviembre, (diciembreaux) as diciembre
	...
	{		
		...
		$ls_sql="SELECT	a.fecha,(a.montoaux) as monto, b.aumento,b.disminucion,b.precomprometer,b.comprometer,b.causar,b.pagar
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	{
		$ls_sql="SELECT SUM(PMV.montoaux) as total 
	...
	function uf_oaf_dism_res_ganancia($ld_fecini,$ld_fecfin,$ldec_ganancia)
	{
		...
		$ldec_ganancia = 0;
		$ls_sql=" SELECT SUM(SA.debe_mesaux-SA.haber_mesaux) as saldo 
	...
	function uf_calcular_total_cta_ref($ls_cuenta,$ld_fecini,$ld_fecfin,$ldec_saldo)
	{
		...
		//--SALDO ANTERIOR		
		//---Total Debe
		$ls_sql = "SELECT SUM(montoaux) as total 
				   FROM scg_dt_cmp
