...
class tepuy_sno_c_impexpdato
{
							  ...
							  $as_concpreper,$as_concpreesp,$as_concmontepio,$as_concfianza,$as_concprehip,$aa_seguridad)
	{
		...
		if($lb_valido)
		{
			...
			if($rs_data===false)
			{
				$this->io_mensajes->message("CLASE->Importar/Exportar Datos MÉTODO->uf_exportardatos ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
				$lb_valido=false;
			}
			else
			{
			...
			fclose($lo_archivo); //cerramos la conexión y liberamos la memoria
			unset($lo_archivo);
		}
		if($lb_valido)
		{
