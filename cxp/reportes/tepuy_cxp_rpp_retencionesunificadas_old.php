<?php
	...
	function uf_insert_seguridad($as_titulo)
	{
		...
		$lb_valido=$io_fun_cxp->uf_load_seguridad_reporte("CXP","tepuy_cxp_r_retencionesmunicipales.php",$ls_descripcion);
		return $lb_valido;
	}
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_print_cabecera($as_numcon,$ad_fecrep,$as_perfiscal,$as_licagenteret,$as_diragenteret,
							   $as_dirsujret,$as_nomsujret,$as_rif,$as_numlic,$ai_estcmpret,$as_fecfac,$as_numficha,$as_numfac,$as_totcmp_con_iva,$as_numcom,$as_encabezado,$as_numsol,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		unset($la_config);
///////////////// FACTURA /////////////
		$la_data[1]=array('name'=>'<b>DATOS DE LA FACTURA SEGUN </b>');				
		$la_columna=array('name'=>'');		
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'fontSize' => 10, // Tamaño de Letras
						 'showLines'=>0, // Mostrar Lieas
...
////////////////////////////////////////////////FIN DE FUNCIONES //////////////////////////////////////////
	...
	$ls_imprimencabezado=$io_fun_cxp->uf_obtenervalor_get("encabezado",0);
	...
	$ls_proveedores=$io_fun_cxp->uf_obtenervalor_get("proveedores","");
	//$ls_codretenciones=$io_fun_cxp->uf_obtenervalor_get("codretenciones","");
	$ls_mes=$io_fun_cxp->uf_obtenervalor_get("mes","");
	$ls_anio=$io_fun_cxp->uf_obtenervalor_get("anio","");
	$ls_numsol1=$io_fun_cxp->uf_obtenervalor_get("ls_numsol","");
	...
	if($lb_valido)
	{
		$la_proveedores=split("\.",$ls_proveedores);
		//$la_codretenciones=split('-',$ls_codretenciones);
		$la_datos=array_unique($la_proveedores);
		...
		else
		{
			...
			for ($li_z=0;($li_z<$li_totrow);$li_z++)
			{
				...
				$lb_valido=$io_report->uf_retenciones_del_proveedor_unificadas($ld_rif,$ls_mes,$ls_anio,$ls_numsol1);
				...
				if($lb_valido)
				{
					...
					uf_print_cabecera($ls_documento,$ls_fecrep,$ls_perfiscal,$ls_licagenteret,$ls_diragenteret,$ls_dirsujret,$ls_nomsujret,$ls_rif,$ls_numlic,$li_estcmpret,$ls_fecfac,$ls_numficha,$ls_numfac,$totcmp_con_iva,$ls_numcom,$ls_imprimencabezado,$ls_numsol,&$io_pdf);
					$total_retenciones=0;
					...
					switch(true)
					{
						case ($ls_codret==$ls_timbrefiscal):
							$lb_valido=$io_report->uf_retencionestimbrefiscal_detalles($ls_numcom,$ls_timbrefiscal);
						...
						case ($ls_codret==$ls_municipal):
							$lb_valido=$io_report->uf_retencionesmunicipales_detalles($ls_numcom,$ls_municipal);
						...
						case ($ls_codret==$ls_islr):
							$lb_valido=$io_report->uf_retencionesislr_detalles($ls_numcom,$ls_islr);
						...
						case ($ls_codret==$ls_iva):
							$lb_valido=$io_report->uf_retencionesiva_detalle_unificadas($ls_numcom,$ls_iva);
					...
					if($lb_valido)
					{
						...
						for($la_i=1;$la_i<=$la_total;$la_i++)
						{
							...
							{
								//if($ls_codret==$ls_iva){	print "% Imp".$li_porimp; die();}
								$li_porimpreal=number_format($li_porimp,2,",",".");
								//if($ls_codret==$ls_iva){	$li_totivaret=$porcentaje;}
								//if($ls_codret==$ls_iva){	print "% Imp".$li_porimpreal; die();}
							}			
