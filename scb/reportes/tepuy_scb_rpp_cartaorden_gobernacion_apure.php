<?php
	...
	function uf_print_encabezado_pagina($as_numdoc,$as_fecha,&$io_pdf)
	{
		...
		$io_pdf->line(360,730,585,730);		
		$io_pdf->line(360,750,360,710);		
		$io_pdf->addText(370,732,11,"<b>Nro.:</b>     ".$as_numdoc); // Nmero de Orden de compra
		$io_pdf->addText(220,725,13,"<b>CARTA ORDEN</b>"); // Agregar el t?ulo
		$io_pdf->restoreState();
		$io_pdf->closeObject();
		$io_pdf->addObject($io_encabezado,'all');
		//Fecha actual
		$io_pdf->add_texto(110,4.5,10,"<b>".$as_fecha."</b>");
		
		$io_pdf->add_texto(-17,-8,7,"<b>Republica Bolivariana de Venezuela</b>");
		$io_pdf->add_texto(-14,-5,7,"<b>Gobernacion del Estado Apure</b>");
		$io_pdf->add_texto(-12,17,7,"<b>Despacho del Gobernador</b>");
		...
		if ($as_tipproben=='B')
		   {
			 $la_config=array('showHeadings'=>1, // Mostrar encabezados
							  ...
							  'cols'=>array('cedbene'=>array('justification'=>'center','width'=>50),// Justificación y ancho de la columna
										    'nombene'=>array('justification'=>'left','width'=>170),// Justificación y ancho de la columna
										    'numsolpag'=>array('justification'=>'center','width'=>80), // Justificación y ancho de la columna
										    'consolpag'=>array('justification'=>'left','width'=>210), // Justificación y ancho de la columna
			 ...
			 $la_columnas=array('cedbene'=>'<b>CEDULA</b>',
							    ...
							    'numsolpag'=>'<b>NO. DE ORDEN DE PAGO</b>',
								'consolpag'=>'<b>CONCEPTO DE ORDEN DE PAGO</b>',
							    'monto'=>'<b>MONTO</b>');
		   }
	...
	if(rtrim($la_data["archrtf"][1])!="")
	{
		...
		header('Location: tepuy_scb_rpp_cartaorden_word.php?codigo='.$ls_codigo.'&codban='.$ls_codban.'&ctaban='.$ls_ctaban.'&numdoc='.$ls_numdoc.'&chevau=&codope=ND&tipproben='.$ls_tipproben);
	}
	...
	$la_data_dt=$class_report->uf_select_dt_cartaorden($ls_numdoc,$ls_codban,$ls_ctaban,$ls_tipproben);//print_r($la_data_dt);
