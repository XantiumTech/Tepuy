<?php
	...
	function uf_insert_seguridad($as_nota)
	{
		...
		$lb_valido=$io_fun_cxp->uf_load_seguridad_reporte("CXP","tepuy_cxp_p_ncnd.php",$ls_descripcion);
		return $lb_valido;
	}
	//--------------------------------------------------------------------------------------------------------------------------------
	...
	if(count($la_nota)>0) 
	{
	    ...
	    $ls_estatus    =$la_nota["estnotadc"];
		 ...
		 if($ls_tipproben=='P')
		 {
			$la_provben=$io_report->uf_select_rowdata($io_sql,"SELECT * FROM rpc_proveedor WHERE codemp='".$ls_codemp."' AND cod_pro='".$ls_codproben."'");
		 ...
		 {
			$la_provben=$io_report->uf_select_rowdata($io_sql,"SELECT * FROM rpc_beneficiario WHERE codemp='".$ls_codemp."' AND  ced_bene='".$ls_codproben."'");
