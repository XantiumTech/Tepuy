...
$io_funobr= new tepuy_sob_c_funciones_sob(); 
  ...
  <p align="center">
    <input name="operacion" type="hidden" id="operacion">
	 ...
	 <table width="646" border="0" cellpadding="0" cellspacing="0" class="formato-blanco" align="center">
      ...
      <tr>
        <td colspan="2"></td>
        <td colspan="4"><div align="right"><a href="javascript: ue_search();"><img src="../shared/imagebank/tools20/buscar.png" alt="Buscar" width="20" height="20" border="0"> Buscar</a></div></td>
      </tr>
    </table>
...
{
	$ls_sql="SELECT a.codcon,a.codant,a.fecant,a.fecintant,a.porant,a.monto,a.fecant,a.numrecdoc,
			a.conant,a.montotant,a.sc_cuenta as cuenta,a.estant,o.desobr,o.codobr,a.estspgscg,c.monto as montocontrato,a.estgenrd, b.denominacion as cuentaden
			FROM sob_anticipo a,sob_obra o,sob_contrato c,sob_asignacion ag, scg_cuentas b
			WHERE a.codemp='".$ls_codemp."' AND a.codemp=o.codemp AND o.codemp=c.codemp AND a.sc_cuenta=b.sc_cuenta AND c.codemp=ag.codemp AND a.codcon=c.codcon AND c.codasi=ag.codasi
	...
	}else
	{
		if($row=$io_sql->fetch_row($rs_data))
		{
			...
			print "<td><a href=javascript:ue_ordenar('a.numrecdoc','BUSCAR');><font color=#FFFFFF>N. de Ficha</font></a></td>";
			...
			for($li_z=1;$li_z<=$li_totrow;$li_z++)
			{
				...
				$ls_estado=$io_funobr->uf_convertir_numeroestado($data["estant"][$li_z]);
				$ls_codest=$data["estant"][$li_z];
				$ls_codobr=$data["codobr"][$li_z];
				$ld_monto=$data["monto"][$li_z];
				$ls_codant=$data["codant"][$li_z];
				$ls_fecintant=$io_funcion->uf_convertirfecmostrar($data["fecintant"][$li_z]);
				$ld_porant=$data["porant"][$li_z];
				$ls_conant=$data["conant"][$li_z];
				$ld_montotant=$data["montotant"][$li_z];
				$ls_cuenta=$data["cuenta"][$li_z];
				$ls_estspgscg=$data["estspgscg"][$li_z];
				$ld_montocontrato=$data["montocontrato"][$li_z];
				$ls_fecant=$io_funcion->uf_convertirfecmostrar($data["fecant"][$li_z]);
				$ls_estgenrd=$data["estgenrd"][$li_z];
				$ls_cuentaden=$data["cuentaden"][$li_z];
				print "<td><a href=\"javascript: aceptar('$ls_codcon','$ls_desobr','$ls_numrecdoc','$ls_estado','$ls_codest','$ls_codobr','$ld_monto',
				'$ls_codant','$ls_fecintant','$ld_porant','$ls_conant','$ld_montotant','$ls_cuenta','$ls_fecant','$ld_montocontrato','$ls_estgenrd','$ls_cuentaden');\">".$ls_codant."</a></td>";
				print "<td>".$ls_codcon."</td>";
				print "<td>".$ls_numrecdoc."</td>";
				...
				if	($ls_estspgscg==0)
					...
					print "<td><img src=../shared/imagebank/aprobado.png width=15 height=15 border=0></td>";	
				print "</tr>";			
			}
			print "</table>";
		}
...
function aceptar(ls_codcon,ls_desobr,ls_numrecdoc,ls_estado,ls_codest,ls_codobr,$ld_monto,ls_codant,ls_fecintant,ld_porant,ls_conant,
				 ld_montotant,ls_cuenta,ls_fecant,ld_montocontrato,ls_estgenrd,ls_cuentaden)
  {
    opener.ue_cargaranticipo(ls_codcon,ls_desobr,ls_numrecdoc,ls_estado,ls_codest,ls_codobr,$ld_monto,ls_codant,ls_fecintant,ld_porant,
							 ls_conant,ld_montotant,ls_cuenta,ls_fecant,ld_montocontrato,ls_estgenrd,ls_cuentaden);
	close();
  }
  ...
  f.action="tepuy_cat_anticipo.php";
  f.submit();
  }
</script>
