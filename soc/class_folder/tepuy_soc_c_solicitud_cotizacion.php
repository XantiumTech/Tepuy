<?php
class tepuy_soc_c_solicitud_cotizacion
{
  function tepuy_soc_c_solicitud_cotizacion($as_path)
  {
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//	     Function: tepuy_soc_c_solicitud_cotizacion
	//		   Access: public 
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_guardar($as_existe,&$as_numsolcot,$as_tipsolcot,$as_obssolcot,$as_consolcot,$as_uniejeaso,$ad_fecregsolcot,$as_codunieje,$as_cedpersol,$as_codcarper,
						$ai_totrowbienes,$ai_totrowservicios,$ai_totrowsep,$ai_totrowproveedores,$as_telpersol,$as_faxpersol,$aa_seguridad)
	{		
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_guardar
		//		   Access: public (tepuy_soc_p_solicitud_cotizacion.php)
		...
		switch ($as_existe)
		{
			case "FALSE":
					...
					$lb_valido=$this->uf_insert_solicitud_cotizacion($ad_fecregsolcot,$as_numsolcot,$as_codunieje,$as_cedpersol,$as_codcarper,$as_telpersol,
					                                                 $as_faxpersol,$as_tipsolcot,$as_obssolcot,$as_consolcot,$as_uniejeaso,$ai_totrowbienes,
																	 $ai_totrowservicios,$ai_totrowsep,$ai_totrowproveedores,$ls_tabla,$ls_campo,$aa_seguridad);
			...
			case "TRUE":
				if($lb_encontrado)
				{
					$lb_valido=$this->uf_update_solicitud_cotizacion($ad_fecregsolcot,$as_numsolcot,$as_codunieje,$as_cedpersol,$as_codcarper,$as_telpersol,
					                                                 $as_faxpersol,$as_tipsolcot,$as_obssolcot,$as_consolcot,$as_uniejeaso,$ai_totrowbienes,
																	 $ai_totrowservicios,$ai_totrowsep,$ai_totrowproveedores,$ls_tabla,$ls_campo,$aa_seguridad);
				}
				else
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_insert_solicitud_cotizacion($ad_fecregsolcot,&$as_numsolcot,$as_codunieje,$as_cedpersol,$as_codcarper,$as_telpersol,$as_faxpersol,
					                        $as_tipsolcot,$as_obssolcot,$as_consolcot,$as_uniejeaso,$ai_totrowbienes,$ai_totrowservicios,
											$ai_totrowsep,$ai_totrowproveedores,$as_tabla,$as_campo,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($lb_valido)
		{
			$ls_sql="INSERT INTO soc_sol_cotizacion (codemp,numsolcot,fecsol,obssol,estcot,codusu,cedper,codcar,soltel,solfax,coduniadm,tipsolcot,consolcot,uniejeaso)".
					"	  VALUES ('".$this->ls_codemp."','".$as_numsolcot."','".$ad_fecregsolcot."','".$as_obssolcot."','R','".$ls_codusu."','".$as_cedpersol."',".
					"             '".$as_codcarper."','".$as_telpersol."','".$as_faxpersol."','".$as_codunieje."','".$as_tipsolcot."','".$as_consolcot."','".$as_uniejeaso."')";
//print $ls_sql;
//return false;
			...
			if($li_row===false)
			{
				...
				{
					$lb_valido=$this->uf_insert_solicitud_cotizacion($ad_fecregsolcot,$as_numsolcot,$as_codunieje,$as_cedpersol,$as_codcarper,$as_telpersol,$as_faxpersol,
					                        			             $as_tipsolcot,$as_obssolcot,$as_consolcot,$as_uniejeaso,$ai_totrowbienes,$ai_totrowservicios,
														             $ai_totrowsep,$ai_totrowproveedores,$as_tabla,$as_campo,$aa_seguridad);
				}
				else
				{
				  $lb_valido=false;
				  $this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_insert_solicitud_cotizacion ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_update_solicitud_cotizacion($ad_fecregsolcot,$as_numsolcot,$as_codunieje,$as_cedpersol,$as_codcarper,$as_telpersol,$as_faxpersol,
					                        $as_tipsolcot,$as_obssolcot,$as_consolcot,$as_uniejeaso,$ai_totrowbienes,$ai_totrowservicios,
											$ai_totrowsep,$ai_totrowproveedores,$as_tabla,$ls_campo,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//print $ls_sql;			
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_update_solicitud_cotizacion ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$this->io_sql->rollback();
		}
		else
		{
					   ...
					   for ($i=1;($i<$ai_totrowbienes)&&($lb_valido);$i++)
						   {
							 ...
							 $ld_codcar = $_POST["txtcodiva".$i];
							 $ld_canart = str_replace('.','',$ld_canart);
							 $ld_canart = str_replace(',','.',$ld_canart);
							 $ls_uniart = "D";
							 $ls_numsep = $_POST["hidnumsep".$i];
							 $ls_sql    = " INSERT INTO soc_dtsc_bienes (codemp, numsolcot, codart, cod_pro, codcar, unidad, canart, orden, numsep) ".
										  " VALUES ('".$this->ls_codemp."','".$as_numsolcot."','".$ls_codart."','".$ls_codpro."','".$ld_codcar."','".$ls_uniart."',".$ld_canart.",".$i.",'".$ls_numsep."')";
			//print $ls_sql;
							 $rs_data = $this->io_sql->execute($ls_sql);
							 if ($rs_data===false)
								{
									$lb_valido=false;
									$this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_insert_bienes ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
								}
								else
								{
									//$as_numsolcot=$as_numsolcot."', codcar='".$ld_codcar;
					   ...
					   for ($i=1;($i<$ai_totrowservicios)&&($lb_valido);$i++)
						   {
							 ...
							 $ld_canser = str_replace(',','.',$ld_canser);
							 $ls_numsep = $_POST["hidnumsep".$i];
							 $ls_sql    = " INSERT INTO soc_dtsc_servicios (codemp, numsolcot, codser, cod_pro, codcar, canser, orden, numsep) ".
										  " VALUES ('".$this->ls_codemp."','".$as_numsolcot."','".$ls_codser."','".$ls_codpro."','".$ld_codcar."',".$ld_canser.",".$i.",'".$ls_numsep."')";
							 $rs_data = $this->io_sql->execute($ls_sql);
							 if ($rs_data===false)
								{
									$lb_valido=false;
									$this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_insert_servicios ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message));
		...
		for ($i=1;$i<$ai_totrowsep;$i++)
			{
			  ...
			  if (!empty($ls_numsep))
				 {
				   $ls_sql  = " INSERT INTO soc_solcotsep (codemp, numsolcot, numsol, codunieje) VALUES ('".$this->ls_codemp."','".$as_numsolcot."','".$ls_numsep."','".$ls_unieje."')";
				   $rs_data = $this->io_sql->execute($ls_sql);
				   if ($rs_data===false)
					  {
						$lb_valido=false;
						$this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_insert_solicitudes_presupuestarias;ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
						echo $this->io_sql->message;
					  }
				   else
					  {
	...
	function uf_select_solicitud_cotizacion($as_numsolcot)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_select_solicitud_cotizacion ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_existe=false;
		}
		else
		{
	...
	function uf_validar_fecha_solicitud_cotizacion($ad_fecregsolcot)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion->MÉTODO->uf_validar_fecha_solicitud_cotizacion ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			$lb_valido=false;
		}
		else
		{
	...
	function uf_load_bienes($as_numsolcot)
	{
		...
		$ls_sql ="SELECT soc_dtsc_bienes.numsolcot,soc_dtsc_bienes.codart,soc_dtsc_bienes.codcar,               ".
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_load_bienes ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		return $rs_data;
	...
	function uf_load_servicios($as_numsolcot)
	{
		...
		//	  Description: Función que busca los bienes asociados a una solicitud de cotizacion
		//	   Creado Por: Ing. Miguel Palencia.
		// Fecha Creación: 12/05/2007						Fecha Última Modificación : 09/02/2016
		//////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		
		$ls_sql ="SELECT soc_dtsc_servicios.numsolcot,soc_dtsc_servicios.codser,soc_dtsc_servicios.codcar,	 ".
				 ...
				 " ORDER BY soc_dtsc_servicios.orden ASC                                     ";//print $ls_sql;
		
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_load_servicios ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		return $rs_data;
	...
	function uf_load_proveedores($as_numsolcot,$as_tipsolcot)
	{
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_load_proveedores.ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		return $rs_data;
	...
	function uf_delete_solicitud_cotizacion($as_numsolcot,$as_tipsolcot,$ai_totrowsep,$aa_seguridad)
	{
		...
		if($lb_valido)
		{
			...
			{
				$lb_valido=false;
				$this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_delete_solicitud_cotizacion.php->ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
				$this->io_sql->rollback();
			}
			else
			{
		...
		if ($rs_data===false)
		   {
		     $this->io_sql->rollback();
			 $lb_valido = false;
			 $this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_delete_detalles(Bienes)->ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		   }
		if ($lb_valido)
		   {
			 ...
			 if ($rs_data===false)
			    {
				  $lb_valido=false;
			      $this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_delete_detalles ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		  	    }
		   }
		if ($lb_valido)
		   {
	...
	function uf_load_bienes_sep($as_numsol)
	{
		...
		$ls_sql = "SELECT sep_solicitud.numsol,sep_dt_articulos.codart, sep_dt_articulos.canart,
					      sep_dt_articulos.unidad, siv_articulo.denart, siv_unidadmedida.unidad as uniart
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_load_bienes_sep.ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		return $rs_data;
	...
	function uf_load_servicios_sep($as_numsol)
	{
		...
		$ls_sql ="SELECT sep_solicitud.numsol,sep_dt_servicio.codser, sep_dt_servicio.canser, soc_servicios.denser
		...
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_load_servicios_sep.ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		return $rs_data;
	...
	function uf_load_sep_solcot($as_numsolcot)
	{
		...
		$ls_sql = "SELECT soc_solcotsep.numsol as numsep, sep_solicitud.consol as densep, sep_solicitud.monto as monsep, 
		                  soc_solcotsep.codunieje, spg_unidadadministrativa.denuniadm
					...
					WHERE soc_solcotsep.codemp='".$this->ls_codemp."'
					  ...
					  AND soc_solcotsep.codunieje=spg_unidadadministrativa.coduniadm"; //print $ls_sql;
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		{
			$this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_load_sep_solcot.ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			return false;
		}
		return $rs_data;
	...
	//////////////////////////////////////////////////////////////////////////////
		...
		if ($rs_recordset===false)
		   {
			 $lb_valido = false;
			 $this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_update_estatus_incorporacion->ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			 $this->io_sql->rollback();
		   }
		return $lb_valido;
      ...
      if ($as_desope=='INSERT')
	     {
		   ...
		   if ($rs_data===false)
		      {
			    $lb_valido = false;
			    $this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_update_estatus_sep(INSERT)->ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			  }
		 } 
      else
	     {
		   ...
		   if ($rs_data===false)
		      {
			    $lb_valido = false;
			    $this->io_mensajes->message("CLASE->tepuy_soc_c_solicitud_cotizacion.php->MÉTODO->uf_update_estatus_sep->ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
			  }
		   else
		      {
