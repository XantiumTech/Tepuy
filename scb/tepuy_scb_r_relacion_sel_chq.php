...
$io_fun_banco->uf_load_seguridad("SCB","tepuy_scb_r_relacion_sel_chq.php",$ls_permisos,$la_seguridad,$la_permisos);
$ls_reporte   = $io_fun_banco->uf_select_config("SCB","REPORTE","RELACION_CHEQUE","tepuy_scb_rpp_relacion_chq.php","C");
$li_diasem = date('w');
switch ($li_diasem){
	  ...
	  $ls_sql="SELECT   codban, numdoc, codope, ctaban, fecmov, MAX(conmov) AS conmov, SUM(monto-monret) AS total, MAX(nomproben) AS nomproben
	   ...
	   else
	   {
		   while($row=$io_sql->fetch_row($rs_bancos))
		   {
				...
				$ld_fecmov    = $io_fun->uf_convertirfecmostrar($row["fecmov"]);
...
?>
  <table width="524" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    ...
    <tr class="titulo-ventana">
      <td height="22" colspan="2" align="center">Relacion Selectiva de Cheques <span style="text-align:left">
...
f = document.form1;
	  ...
	  if (li_imprimir=='1')
	     {
			  ...
			  if(ls_documentos.length>0)
			  {
					pagina="reportes/tepuy_scb_rpp_relacion_chq_excel.php?codban="+ls_codban+"&ctaban="+ls_ctaban+"&hidnomban="+ls_denban+"&dencta="+ls_dencta+"&documentos="+ls_documentos+"&fechas="+ls_fechas+"&operaciones="+ls_operaciones+"&tiporeporte="+ls_tiporeporte+"&fecdesde="+ld_fecdes+"&fechasta="+ld_fechas;    	
					  window.open(pagina,"catalogo","menubar=no,toolbar=no,scrollbars=yes,width=800,height=600,resizable=yes,location=no");		  
			  }
			  else
			  {
	...
	function uf_cargar_dt()
	{
		...
		if((ls_codban!="")&&(ls_ctaban!="")&&(ld_fecdesde!="")&&(ld_fechasta!=""))
		{
			...
			f.action='tepuy_scb_r_relacion_sel_chq.php';	
			f.submit();
		}
		else
		{
