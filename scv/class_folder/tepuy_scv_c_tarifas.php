<?php
class tepuy_scv_c_tarifas
{
var $ls_sql;
var $is_msg_error;
	
	function tepuy_scv_c_tarifas()
	{
							  ...
							  $ai_monali,$ai_monmov,$ai_exterior,$as_codcat,$as_codnom,$aa_seguridad) 
	{
		...
		if ($rs_data===false)		     
		{
			...
			$this->io_msg->message("CLASE->tepuy_scv_c_tarifas METODO->uf_insert_tarifa ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$lb_valido=true;
							  ...
							  $ai_monhos,$ai_monali,$ai_monmov,$ai_exterior,$as_codcat,$as_codnom,$aa_seguridad) 
	{
		...
		{
			$this->io_sql->rollback();
			$this->io_msg->message("CLASE->tepuy_scv_c_tarifas METODO->uf_update_tarifa; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$lb_valido=true;
	...
	function uf_delete_tarifa($as_codemp,$as_codtar,$aa_seguridad)
	{
		...
		if (!$lb_relacion)
		{
			...
			{
				$this->io_sql->rollback();
				$this->io_msg->message("CLASE->tepuy_scv_c_tarifas METODO->uf_delete_tarifas ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			}
			else
			{
				$lb_valido=true;
	...
	function uf_select_tarifa($as_codemp,$as_codtar) 
	{
		...
		if ($rs_data===false)
		{
			$this->io_msg->message("CLASE->tepuy_scv_c_tarifas METODO->uf_select_tarifa ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
	...
	function uf_load_tarifa($as_codemp,$as_codtar) 
	{
		...
		{
		  	return false;
			$this->io_msg->message("CLASE->tepuy_scv_c_tarifas METODO->uf_load_tarifa ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$li_numrows = $this->io_sql->num_rows($rs_data);	    
	...
	function uf_check_relaciones($as_codemp,$as_codtar)
	{
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->tepuy_scv_c_tarifas METODO->uf_check_relaciones; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
