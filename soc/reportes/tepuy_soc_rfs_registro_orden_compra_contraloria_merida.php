...
//  ORGANISMO: Contraloria del Estado Mérida.
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    session_start();   
									   ...
									   $ls_forpagcom,&$io_pdf)
	{
		...
		if($as_estcondat=="B") 
        {
			 ...
			 $ls_tipordcom = 'COMPRA';
			 $ls_texto     = 'JUNTO CON LA MERCANCÍA';
        }
        else
        {
             $ls_titulo="Orden de Servicio";
			 $ls_titulo_grid="Servicios";
			 $ls_tipordcom = 'SERVICIO';
			 $ls_texto     = 'AL MOMENTO DE EJECUTARLO';
        }
		...
		/////////cuadro inferior
		$io_pdf->Rectangle(15,130,570,60);
		$io_pdf->addText(17,180,8,"INSTRUCCIÓN PARA EL PROVEEDOR"); // Agregar el título
		$io_pdf->line(15,175,585,175);	//HORIZONTAL
		$io_pdf->addText(17,165,8,"LA FACTURA ORIGINAL DEBERÁ ANEXARSE A LA ORDEN DE ".$ls_tipordcom); // Agregar el título
		$io_pdf->addText(17,155,8,"LA ORDEN DE".$ls_tipordcom." DEBE SER PRESENTADA ".$ls_texto); // Agregar el título
		$io_pdf->addText(17,145,8,"NO SE ACEPTAN FACTURAS CON TACHADURA NI ENMENDADURAS."); // Agregar el título
		$io_pdf->addText(17,135,8,"SE AGRADECE AL PROVEEDOR DESPACHAR LA MERCANCÍA COTIZADA CON LAS CARACTERÍSTICAS ESPECIFICADAS."); // Agregar el título
        $io_pdf->Rectangle(15,60,570,70);
		///////////// 
		$io_pdf->line(15,117,585,117);	//HORIZONTAL	
		$io_pdf->addText(45,121,7,"ELABORADO POR"); // Agregar el título
		$io_pdf->addText(20,64,7,"AREA DE COMPRAS Y SERVICIOS"); // Agregar el título
		$io_pdf->line(140,60,140,130);	//VERTICAL	
		$io_pdf->addText(200,121,7,"APROBADO POR"); // Agregar el título
		$io_pdf->addText(145,64,7,"DIRECCIÓN DE ADMINISTRACIÓN Y PRESUPUESTO"); // Agregar el título
		$io_pdf->line(325,60,325,130);	//VERTICAL		
		$io_pdf->addText(360,121,7,"REVISADO POR"); // Agregar el título
		$io_pdf->line(450,60,450,130);	//VERTICAL		
		$io_pdf->addText(490,121,7,"PROVEEDOR"); // Agregar el título
		$io_pdf->addText(475,64,7,"FIRMA / SELLO / FECHA"); // Agregar el título
	...
	else  // Imprimimos el reporte
	{
		...
		if($lb_valido)	
		{
			...
			$io_pdf->ezSetCmMargins(7.5,8.5,3,3); // Configuración de los margenes en centímetros
			$io_pdf->ezStartPageNumbers(570,42,8,'','',1); // Insertar el número de página
			if ($row=$io_sql->fetch_row($rs_data))
			{
			   ...
			   if ($lb_valido)
			   {
				 ...
				 if ($li_totrows>0)
				 {
					...
					$io_pdf->ezSetCmMargins(5.8,8.4,3,3); // Configuración de los margenes en centímetros
	...
	unset($io_report,$io_funciones,$io_fun_soc);
?>