...
$ls_tipo=$io_fun_facturacion->uf_obtenertipo();
$ls_operacion=$io_fun_facturacion->uf_obteneroperacion();
$ls_codigo=$io_fun_facturacion->uf_obtenervalor("codigo","");
$ls_denominacion=$io_fun_facturacion->uf_obtenervalor("nombre","");
$ls_cuentaspg=$io_fun_facturacion->uf_obtenervalor("txtcuentascg","");
//$ls_codigo="3".$ls_codigo;

if($ls_operacion=="BUSCAR")
{
	$ls_tipo=$io_fun_facturacion->uf_obtenervalor("tipo","");
}
?>
...
<html>
<head>
<title>Cat&aacute;logo de Cuentas de Ingresos</title>
...
<form name="form1" method="post" action="">
  ...
  <table width="511" border="0" align="center" cellpadding="1" cellspacing="1">
    <tr>
      <td width="507" height="22" colspan="2" class="titulo-celda" style="text-align:center">Cat&aacute;logo de Cuentas de Ingresos</td>
    </tr>
  </table>
  <br>
  <div align="center">
...
if($ls_operacion=="BUSCAR")
{
   ...
   else
   {
	   /* $ls_sql = " SELECT  DISTINCT spg_cuenta, denominacion, codestpro1,codestpro2, codestpro3,sc_cuenta,status   ".
	    ...
	    $ls_sql = " SELECT  DISTINCT spi_cuenta, MAX(denominacion) as denominacion, MAX(sc_cuenta) as sc_cuenta, MAX(status)as status   ".
	              " FROM    spi_cuentas                                                                             ".
	...
	//print $ls_sql;
		...
		for($li_i=0;$li_i<$li_total;$li_i++)
		{
			if($li_i==0)
			{
				$ls_sql=$ls_sql."   AND (spi_cuenta like '".$la_spg_cuenta[$li_i]."%'";
			}
			else
			{
				$ls_sql=$ls_sql."    OR spi_cuenta like '".$la_spg_cuenta[$li_i]."%'";
			}
		
		}						
		
		/*$ls_sql= $ls_sql." ) GROUP BY spg_cuenta, denominacion,sc_cuenta,status".
						 "   ORDER BY spg_cuenta"; */
	    $ls_sql= $ls_sql." ) GROUP BY spi_cuenta ".
						 "   ORDER BY spi_cuenta"; 
		//print $ls_sql;
		...
		{  
			...
			if($li_row>0)
			{
				...
				{
					$ls_spgcuenta=$row["spi_cuenta"];
					$ls_denominacion=$row["denominacion"];
					$ls_sccuenta=$row["sc_cuenta"];
					$status=$row["status"];
		                    $ls_spgcuenta=trim($ls_spgcuenta);
					if ($status=="S")
				    {
...
<script language="JavaScript">
  ...
  function ue_search()
  {
	  ...
	  f.action="tepuy_sfa_cat_ctasspg.php";
	  f.submit();
  }
</script>
</html>
