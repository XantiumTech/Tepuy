...
class tepuy_scv_class_report
{
	...
	function tepuy_scv_class_report()
	{
										 ...
										 $as_coduniadm,$as_codper,$ai_orden)	
	{
		...
		$ls_sql="SELECT scv_solicitudviatico.codsolvia, MAX(scv_solicitudviatico.codmis) AS codmis, MAX(scv_solicitudviatico.codrut) AS codrut, ".
				...
				"   AND scv_solicitudviatico.estsolvia='P'".
		...
		$ls_sql=$ls_sql." GROUP BY scv_solicitudviatico.codsolvia, scv_solicitudviatico.codemp, ".
                        ...
                        "           scv_solicitudviatico.coduniadm";
		if(!empty($ai_orden))
		{
			if($ai_orden==1)
	...
	function uf_select_dt_spg($as_codemp,$as_codsolvia)	
	{
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		$ls_sql= "SELECT codemp,codsolvia, codestpro1, codestpro2, codestpro3, codestpro4, codestpro5, monto, spg_cuenta".
				 "  FROM scv_dt_spg ".
	...
	function uf_select_solicitudpago_personal($as_codemp,$as_codsoldes,$as_codsolhas,$ad_fecdes,$ad_fechas,
	                                          $ai_orden,$as_codsolvia,$ai_rdnomina)	
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//  			       $ai_orden      // parametro por el cual vamos a ordenar los resultados
		//  			       $as_codsolvia  // codigo de solicitud de viaticos
		//                     $ai_rbnomina   // Tipo de nomina (especial o normal)
		//	         Returns : $lb_valido True si se creo el Data stored correctamente ó False si no se creo
		//	      Description: Función que se encarga de la busqueda del personal asociado a una solicitud de viatico
		//         Creado por: Ing. Miguel Palencia  
		//     Modificado Por: Ing. Juniors Fraga  (Modificacion Realizada por Solicittud de Foncrei)       
		//   Fecha de Cracion: 29/11/2006							Fecha de Ultima Modificación: 19/02/2008  
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
		$ls_criterio="";
		if((!empty($ad_fecdes))&&(!empty($ad_fechas)))
		...
		$ls_sql="SELECT DISTINCT(sno_personal.cedper) as cedper, sno_personal.nomper, sno_personal.apeper, sno_unidadadmin.desuniadm, ".
				...
				"		scv_solicitudviatico.codsolvia,scv_solicitudviatico.solviaext,scv_solicitudviatico.obssolvia, sno_nomina.racnom,scv_solicitudviatico.coduniadm, scv_solicitudviatico.porcentaje, scv_solicitudviatico.dentro_fuera, ".
				...
				" WHERE scv_solicitudviatico.codemp='".$as_codemp."' ".
				$ls_criterio.
				"   AND sno_nomina.espnom = ".$ai_rdnomina." ".
		...
		if($li_numrows==0)
		{
			...
			$ls_sql="SELECT rpc_beneficiario.ced_bene AS cedper, MAX(rpc_beneficiario.nombene) AS nomper, ".
                    ...
                    "       MAX(scv_solicitudviatico.obssolvia) AS obssolvia, '' AS racnom,MAX(scv_solicitudviatico.coduniadm) AS coduniadm,  ".
					"       (SELECT COUNT(codper) FROM scv_dt_personal ".
					"		  WHERE scv_dt_personal.codemp = scv_solicitudviatico.codemp  ".
					"			AND scv_dt_personal.codsolvia = scv_solicitudviatico.codsolvia ) AS acompanante,  ".
					"       '' AS cargo".
	...
	function uf_select_monto_viatico($as_codemp,$as_codtar,$as_garantia,$dentro_fuera,$as_fecha_viatico)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql="SELECT a.utfuera,a.utdentro,b.valunitri".
				...
				"   AND b.fecentvig<='". $as_fecha_viatico ."'".
				"   AND a.tipoviatico='". $as_codtar ."'";
		//print $ls_sql;
		$rs_data=$this->io_sql->select($ls_sql);
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->solicitud_tablaviaticos MÉTODO->uf_scv_select_tarifas ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
		...
		return $as_montar;
	}  // end function uf_select_monto_viatico

	function uf_select_unidadsolicitante($as_codemp,$as_coduniadm,&$as_denuniadm)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_select_unidadsolicitante
		//         Access: public 
		//      Argumento: $as_codemp    // codigo de empresa
		//  			   $ls_coduniadm // codigo de unidad administrativa
		//  			   $as_denuniadm // denominacion de unidad administrativa
		//	      Returns: Retorna un Booleano
		//    Description: Funcion que obtiene la denominacion de la unidad solicitante
		//	   Creado Por: Ing. Miguel Palencia
		// Fecha Creación: 09/11/2006 								Fecha Última Modificación :  
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=false;
	...
	function uf_select_solicitudpago_asignaciones($as_codemp,$as_codsolvia)	
	{
		...
		$ls_sql= "SELECT scv_dt_asignaciones.codemp, scv_dt_asignaciones.codsolvia, scv_dt_asignaciones.codasi,".
				...
				// "                          WHERE scv_dt_asignaciones.codemp=scv_tarifas.codemp".
				// " 							  AND scv_dt_asignaciones.codasi=scv_tarifas.codtar)".
				 "        WHEN 'TVS' THEN (SELECT concat_ws(' - ','CALCULADO SEGUN RANGO DE SALARIOS MINIMOS ENTRE',scv_tablaviatico.cantsalarioini,scv_tablaviatico.cantsalariofin)".
	...
	function uf_select_solicitud_personal_spg($as_codemp,$as_codsolvia,$as_ced_bene)	
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$lb_valido=false;
		$ls_monto=0;
		$ls_sql= "SELECT monto FROM scv_dt_spg ".
				 " WHERE codemp='".$as_codemp."'".
				 "   AND codsolvia='".$as_codsolvia."'".
				 "   AND ced_bene='".$as_ced_bene."'";
				// " GROUP BY spg_cuenta, codestpro1, codestpro2  ";
		//print $ls_sql;
	    	$rs_data=$this->io_sql->select($ls_sql);
	    	//$li_numrows=$this->io_sql->num_rows($rs_data);	
		if($rs_data===false)
