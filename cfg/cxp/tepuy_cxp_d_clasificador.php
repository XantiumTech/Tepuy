...
require_once("class_folder/tepuy_cxp_c_clasificaci.php");
...
$io_clasificador = new tepuy_cxp_c_clasificaci($conn);//Instanciando la Clase tepuy Definiciones.
...
//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
	...
	$ls_sistema  = "CFG";
	$ls_ventanas = "tepuy_cxp_d_clasificador.php";
	 ...
	 if ($lb_existe)
        { 
		  ...
		  elseif($ls_estatus=="GRABADO")
		     {
	           $lb_valido=$io_clasificador->uf_update_clasificador($ls_codigo,$ls_denclas,$la_seguridad);
	           if ($lb_valido)
		          {
	    	        $io_sql->commit();
         ...
         {   
	       $lb_valido=$io_clasificador->uf_insert_clasificador($ls_codigo,$ls_denclas,$la_seguridad);
	       if ($lb_valido)
		      {
		        $io_sql->commit();
...
function ue_nuevo()
{
  ...
  if (li_incluir==1)
	 {	
	   ...
	   f.action="tepuy_cxp_d_clasificador.php";
       f.submit();
	}
	else
	{
...
function ue_buscar()
{
	...
	if (li_leer==1)
	   {
         f.operacion.value="";			
	     pagina="tepuy_cxp_cat_clasificador.php";
