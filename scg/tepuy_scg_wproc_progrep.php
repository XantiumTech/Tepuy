...
if(!array_key_exists("la_logusr",$_SESSION))
{
	...
	$ls_sistema="SCG";
	$ls_ventanas="tepuy_scg_wproc_progrep.php";
...
require_once("tepuy_scg_procesos.php");
$iscg_procesos= new tepuy_scg_procesos();
...
if (array_key_exists("txtsaldoreal",$_POST))
{
  $ld_saldoreal=$_POST["txtsaldoreal"];
}
else
{
  $ld_saldoreal="";
...
if (array_key_exists("txtsaldoaprob",$_POST))
{
  $ls_saldoaprob=$_POST["txtsaldoaprob"];
}
else
{
  $ls_saldoaprob="";
}
if (array_key_exists("txtsaldomod",$_POST))
{
  $ld_saldomod=$_POST["txtsaldomod"];
}
else
{
  $ld_saldomod="";
...
?>
        ...
        <table width="644" border="0" align="center" cellpadding="1" cellspacing="1" class="formato-blanco">
          <tr class="titulo-ventana">
            <td colspan="5" class="titulo-celda"><div align="center" class="titulo-ventana">Programaci&oacute;n de Reporte </div></td>
          </tr>
          <tr class="contorno">
            <td colspan="5"></td>
          </tr>
	...
	?>
            ...
            <td colspan="4"><input name="operacion" type="hidden" id="operacion" value="<?php $_POST["operacion"]?>">
                <select name="select" id="select">
                  ...
                  <option value="0405" <?php  print $inv ?> o>Inversiones</option>
                ...
                <span class="cd-titulo"> <a href="javascript: selec();"><img src="../shared/imagebank/tools15/aprobado.png" alt="Aceptar" width="15" height="15" border="0"></a> </span></td>
          </tr>
          <tr class="formato-blanco">
          ...
          <tr>
            <td class="fd-blanco"><div align="right">Saldo Anterio Real </div></td>
            <td class="fd-blanco"><span class="cd-titulo">
              <input name="txtsaldoreal" type="text" class="fd-blanco" id="txtsaldoreal" onBlur="uf_format(this)"  onKeyPress="return(currencyFormat(this,'.',',',event))" value="<?php print $ld_saldoreal?>" size="23">
            </span></td>
            <td class="fd-blanco"><div align="right">Saldo Anterior Aprobado </div></td>            
			<td class="fd-blanco"><span class="cd-titulo">
              <input name="txtsaldoaprob" type="text" class="fd-blanco" id="txtsaldoaprob" onBlur="uf_format(this)"  onKeyPress="return(currencyFormat(this,'.',',',event))" value="<?php print $ls_saldoaprob?>" size="23">
            </span></td>
          </tr>
          <tr>
            <td class="fd-blanco"><div align="right">Saldo Anterio Modificado </div></td>
            <td class="fd-blanco"><span class="cd-titulo">
              <input name="txtsaldomod" type="text" class="fd-blanco" id="txtsaldomod" onBlur="uf_format(this)"  onKeyPress="return(currencyFormat(this,'.',',',event))" value="<?php print $ld_saldomod?>" size="23">
            </span></td>
            <td class="fd-blanco">&nbsp;</td>
  ...
  <p>&nbsp;</p>
		...
		<td width="62"><div align="center">Saldo Real Anterior</div></td>
	    <td width="62"><div align="center">Saldo Aprobado</div></td>
	    <td width="62"><div align="center">Saldo Modificado</div></td>
...
if($ls_operacion=="SELECT")
{
					 ...
					 { 
								...
								if($ls_status=="S")
								{
									...
									<tr class="celdas-azules">
									  <td ><a href="javascript: aceptar('<?php print $ds_prorep->getValue("asignado",$i);?>','<?php print $ds_prorep->getValue("sc_cuenta",$i);?>','<?php print $i?>','<?php print $li_num?>','<?php print $ls_status ?>','','<?php print $ds_prorep->getValue("saldo_real_ant",$i) ?>','<?php print $ds_prorep->getValue("saldo_apro",$i) ?>','<?php print $ds_prorep->getValue("saldo_mod",$i) ?>');"><?php print $ds_prorep->getValue("sc_cuenta",$i);?></a></td>
									<?php
								}
								elseif($ls_status=="C")
								{
								...
								elseif($ls_status=="I")
								{
								 ...
								 print"<td>".number_format($ds_prorep->getValue("saldo_aprob",$i),2,",",".")."</td>";
								 print"<td>".number_format($ds_prorep->getValue("sald_mod",$i),2,",",".")."</td>";
				...
				if($lb_valido)
				{
					 ...
					 { 
							...
							if($ls_status=="S")
							{
								?>
    <tr class="celdas-azules">
      <td ><a href="javascript: aceptar('<?php print $ds_prorep->getValue("asignado",$i);?>','<?php print $ds_prorep->getValue("sc_cuenta",$i);?>','<?php print $i?>','<?php print $li_num?>','<?php print $ls_status ?>','','<?php print $ld_saldoreal ?>','<?php print $ls_saldoaprob ?>','<?php print $ld_saldomod ?>');"><?php print $ds_prorep->getValue("sc_cuenta",$i);?></a></td>
      <?php
							}
							elseif($ls_status=="C")
							{
	  ...
	  if ($li_rtn>0) //tiene registro
	  {
		 ...
		 { 
			...
			if($ls_status=="S")
			{
				?>
    <tr class="celdas-azules">
      <td ><a href="javascript: aceptar('<?php print $ds_prorep->getValue("asignado",$i);?>','<?php print $ds_prorep->getValue("sc_cuenta",$i);?>','<?php print $i?>','<?php print $li_num?>','<?php print $ls_status ?>','<?php print $ds_prorep->getValue("enero",$i);?>','<?php print $ds_prorep->getValue("saldo_real_ant",$i) ?>','<?php print $ds_prorep->getValue("saldo_apro",$i) ?>','<?php print $ds_prorep->getValue("saldo_mod",$i) ?>');"><?php print $ds_prorep->getValue("sc_cuenta",$i);?></a></td>
      <?php
		    }
		    elseif($ls_status=="C")
		    {
  ...
  if ($ls_operacion=="DISTRIBUIR")
  {
	 ...
	 $ld_total_saldo_real=$ld_saldoreal;
	 $ld_total_saldo_apro= $ls_saldoaprob;
	 $ld_total_saldo_mod=$ld_saldomod;
	...
	{   
		...
		$ds_prorep->updateRow("saldo_real_ant",$ld_total_saldo_real,$i);
		$ds_prorep->updateRow("saldo_apro",$ld_total_saldo_apro,$i);
		$ds_prorep->updateRow("saldo_mod",$ld_total_saldo_mod,$i);
		$ds_prorep->updateRow("asignado",$ld_asignado,$i);
		...
		while($li_nivel_cta>=1)
		{
			...
			$ds_prorep->updateRow("saldo_real_ant",$ld_total_saldo_real,$pos);
			$ds_prorep->updateRow("saldo_apro",$ld_total_saldo_apro,$pos);
...
{	
	 ...
	 for ($i=1; $i<= $li_num; $i++)
	 {
		...
		//saldos
		$la_datos[6]  = $ds_prorep->getValue("saldo_real_ant",$i);
		$la_datos[7]  = $ds_prorep->getValue("saldo_apro",$i);
		$la_datos[8]  = $ds_prorep->getValue("saldo_mod",$i);
		//meses
		$la_datos[9]  = $ds_prorep->getValue("enero",$i);
		$la_datos[10]  = $ds_prorep->getValue("febrero",$i);
		$la_datos[11]  = $ds_prorep->getValue("marzo",$i);
		$la_datos[12]  = $ds_prorep->getValue("abril",$i);
		$la_datos[13] = $ds_prorep->getValue("mayo",$i);
		$la_datos[14] = $ds_prorep->getValue("junio",$i);
		$la_datos[15] = $ds_prorep->getValue("julio",$i);
		$la_datos[16] = $ds_prorep->getValue("agosto",$i);
		$la_datos[17] = $ds_prorep->getValue("septiembre",$i);
		$la_datos[18] = $ds_prorep->getValue("octubre",$i);
		$la_datos[19] = $ds_prorep->getValue("noviembre",$i);
		$la_datos[20] = $ds_prorep->getValue("diciembre",$i);
		
		$la_datos[21] = $ds_prorep->getValue("nivel",$i);
		$la_datos[22] = $ds_prorep->getValue("referencia",$i);
		$la_datos[23] = $ds_prorep->getValue("no_fila",$i);
		$la_datos[24] = $ds_prorep->getValue("tipo",$i);
		$la_datos[25] = $ds_prorep->getValue("cta_res",$i);
		$la_datos[26] = $ds_prorep->getValue("modrep",$i);
 ...
 if ($ls_operacion=="")
 {
		...
		{
			$col=$arr[$i];
			if(($col!="ls_width")&&($col!="ls_height")&&($col!="ls_logo")&&($col!="ls_hostname")&&($col!="ls_login")&&($col!="ls_password")&&($col!="ls_database")&&($col!="ls_gestor")&&($col!="con")&&($col!="gestor")&&($col!="la_empresa")&&($col!="la_logusr")&&($col!="la_ususeg")&&($col!="la_sistema")&&($col!="ls_port"))
			{
				unset($_SESSION["$col"]);			
			}
		}
    ...
    <tr class="celdas-azules">
      ...
      <?php
								...
								elseif($ls_status=="C")
								{
								   ?>
    <tr class="celdas-blancas">
      <td ><a href="javascript: aceptar('<?php print $ds_prorep->getValue("asignado",$i);?>','<?php print $ds_prorep->getValue("sc_cuenta",$i);?>','<?php print $i?>','<?php print $li_num?>','<?php print $ls_status ?>','<?php print $ld_saldoreal ?>','<?php print $ld_saldoaprob ?>','<?php print $ld_saldomod ?>');"><?php print $ds_prorep->getValue("sc_cuenta",$i);?></a></td>
      <?php
								}
								elseif($ls_status=="I")
								{
...
function ue_distribuir()
{
   ...
   else
   {
    ...
    document.opcion = f.radiobutton[i].value; 
	 ...
	 if  (document.opcion=="M" ) 
     {
		 ...
		 {
		     pagina="tepuy_scg_wdist_manual.php?fila="+i+"&txtAsignado="+f.txtAsignado.value+"&txtsaldoreal="+f.txtsaldoreal.value+"&txtsaldoapro="+f.txtsaldoaprob.value+"&txtsaldomod="+f.txtsaldomod.value;
			 window.open(pagina,"Asignación","menubar=no,toolbar=no,scrollbars=no,width=500,height=500,left=50,top=50,resizable=yes,location=no");
   ...
   if (document.opcion=="A")
   {
	 ...
	 {
	   f.operacion.value="DISTRIBUIR";
	   f.action="tepuy_scg_wproc_progrep.php";
	   f.submit();
	 } 
   }
...
function aceptar(c,d,i,li_row,s,e,saldo1,saldo2,saldo3)
{

f=document.form1;
f.txtAsignado.value=c;
f.txtsaldoreal.value=saldo1;
f.txtsaldoaprob.value=saldo2;
f.txtsaldomod.value=saldo3;
f.txtCuenta.value=d;
f.fila.value=i;
