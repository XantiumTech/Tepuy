<?php
class tepuy_scv_c_misiones
{
	var $ls_sql;
	var $is_msg_error;
		
	function tepuy_scv_c_misiones($conn)
	{
	...
	function uf_insert_mision($as_codemp,$as_codmis,$as_denmis,$aa_seguridad) 
	{
		...
		if ($rs_data===false)		     
		{
			...
			$this->io_msg->message("CLASE->tepuy_SCV_C_MISIONES; METODO->uf_insert_mision; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
	...
	function uf_update_mision($as_codemp,$as_codmis,$as_denmis,$aa_seguridad) 
	{
		...
		{
			$this->io_sql->rollback();
			$this->io_msg->message("CLASE->tepuy_SCV_C_MISIONES; METODO->uf_update_mision; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$lb_valido=true;
	...
	function uf_delete_mision($as_codemp,$as_codmis,$aa_seguridad)
	{
		...
		if (!$lb_relacion)
		{
			...
			{
				$this->io_sql->rollback();
				$this->io_msg->message("CLASE->tepuy_SCV_C_MISIONES; METODO->uf_delete_mision; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			}
			else
			{
				$lb_valido=true;
	...
	function uf_load_mision($as_codemp,$as_codmis) 
	{
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->tepuy_SCV_C_MISIONES; METODO->uf_load_mision; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
	...
	function uf_check_relaciones($as_codemp,$as_codmis)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->tepuy_SCV_C_MISIONES; METODO->uf_check_relaciones; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
