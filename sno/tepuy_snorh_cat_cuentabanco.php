   ...
   function uf_print($as_codban, $as_tipo)
   {
		...
		else
		{
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				switch ($as_tipo)
				{
					case "replisban": // el llamado se hace desde  tepuy_sno_r_listadopersonalcheque.php
					...
					case "replisben": // el llamado se hace desde  tepuy_sno_r_listadopersonalcheque.php
						print "<tr class=celdas-blancas>";
...
function ue_search(existe)
{
	f=document.form1;
  	...
  	f.action="tepuy_snorh_cat_cuentabanco.php?codban<?PHP print $ls_codban;?>&tipo=<?PHP print $ls_tipo;?>";
  	f.submit();
}
</script>
</html>
