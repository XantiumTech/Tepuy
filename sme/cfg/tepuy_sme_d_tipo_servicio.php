...
<html>
<head>
<title>Registro de Tipo de Servicios Médicos</title>
...
<table width="780" border="0" align="center" cellpadding="0" cellspacing="0" class="contorno">
  ...
  <tr>
    <td height="40" class="cd-menu">
	<table width="776" border="0" align="center" cellpadding="0" cellspacing="0">
	
		<td width="423" height="20" bgcolor="#E7E7E7" class="descripcion_sistema">Sistema Control de Servicios Médicos -><b> Tipos </b></td>
...
$io_serviciomedico  = new tepuy_sme_c_tipo_servicio($conn);//Instanciando la Clase tepuy Definiciones.
...
//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
	...
	$ls_sistema  = "SME";
	$ls_ventanas = "tepuy_sme_d_tipo_servicio.php";
...
if (array_key_exists("operacion",$_POST))
   {
     ...
     $ls_codtipservicio    = $_POST["txtcodigo"];
     $ls_denserviciomedico= $_POST["txtdenominacion"];
...
if ($ls_operacion=="NUEVO")
   {
	 $ls_codtipservicio=$io_funciondb->uf_generar_codigo($ls_empresa,$ls_codemp,'sme_tiposervicio','codtipservicio');
	 if (empty($ls_codtipservicio))
	    {
...
if ($ls_operacion=="ue_guardar")
   { 
	 $lb_existe=$io_serviciomedico->uf_select_serviciomedico($ls_codemp,$ls_codtipservicio);
	 if ($lb_existe)
        { 
	      if ($ls_estatus=="NUEVO")
		     {
			   $io_msg->message("Este Código de Tipo de Servicio Médico Ya Existe !!!");  
			   $lb_valido=false;
			 }
		  elseif($ls_estatus=="GRABADO")
		     {
		       $lb_valido=$io_serviciomedico->uf_update_serviciomedico($ls_codemp,$ls_codtipservicio,$ls_denserviciomedico,$la_seguridad);
  	           if ($lb_valido)
		          {
				    $io_sql->commit();
				    $io_msg->message("Registro Actualizado !!!");
				    $ls_codtipservicio=$io_funciondb->uf_generar_codigo($lb_empresa,$ls_codemp,'sme_tiposervicio','codtipservicio');
				    $ls_denserviciomedico="";
					$ls_estatus="NUEVO";
			      }
		       else
	    ...
	    {  
		  $lb_valido=$io_serviciomedico->uf_insert_serviciomedico($ls_codtipservicio,$ls_denserviciomedico,$la_seguridad);
	      if($lb_valido)
		    {
		      $io_sql->commit();
...
if ($ls_operacion=="ELIMINAR")
   {
	  ...
	  if ($lb_existe)
	     {
		   $ls_condicion = " AND (column_name='codtipservicio')";//Nombre del o los campos que deseamos buscar.
	       $ls_mensaje   = "";                              //Mensaje que será enviado al usuario si se encuentran relaciones a asociadas al campo.
	       $lb_tiene     = $io_chkrel->uf_check_relaciones($ls_codemp,$ls_condicion,'sme_tiposervicio',$ls_codtipservicio,$ls_mensaje);//Verifica los movimientos asociados a la cuenta  
		   if (!$lb_tiene)
		      {
			    $lb_valido=$io_serviciomedico->uf_delete_serviciomedico($ls_codemp,$ls_codtipservicio,$la_seguridad);
			    if ($lb_valido)
				   {
					 $io_sql->commit();
    ...
    <tr>
      <td width="517" height="170"><div align="center">
        <table  border="0" cellspacing="0" cellpadding="0" class="formato-blanco" align="center">
          <tr>
            <td height="22" colspan="2" class="titulo-ventana">Registro de Tipo de Servicios Médicos</td>
          </tr>
          <tr>
            <td height="22" >&nbsp;</td>
          ...
          <tr>
            ...
            <td width="334" height="22" ><input name="txtcodigo" type="text" id="txtcodigo" value="<?php print  $ls_codtipservicio ?>" size="2" maxlength="2" onKeyPress="return keyRestrict(event,'1234567890');" style="text-align:center "  onBlur="javascript:rellenar_cadena(this.value,2);">
                <input name="operacion" type="hidden" id="operacion"  value="<?php print $ls_operacion ?>"></td>
          </tr>
          <tr>
            <td height="22"><div align="right">Denominaci&oacute;n</div></td>
            <td height="22"><input name="txtdenominacion" id="txtdenominacion" value="<?php print $ls_denserviciomedico ?>" type="text" size="60" maxlength="60" onKeyPress="return keyRestrict(event,'1234567890'+'abcdefghijklmnñopqrstuvwxyz '+'.,-');"></td>
          </tr>
          <tr>
            <td height="22">&nbsp;</td>
            <td height="22">&nbsp;</td>
...
<?php
	$io_serviciomedico->uf_destructor();
	unset($io_serviciomedico);
?> 
</body>
...
function ue_nuevo()
{
	...
	if (li_incluir==1)
	   {	
	     ...
	     f.action="tepuy_sme_d_tipo_servicio.php";
	     f.submit();
	   }
    else
	   {
...
if (((lb_status=="GRABADO")&&(li_cambiar==1))||(lb_status=="NUEVO")&&(li_incluir==1))
   {
     with (document.form1)
	      {
	        if (campo_requerido(txtcodigo,"El codigo del Tipo de Servicio Médico debe estar lleno !!!")==false)
			   {
		 	     txtcodigo.focus();
			   }
			else
			   {
			     ...
			     if (campo_requerido(txtdenominacion,"La denominacion del Tipo de Servicio Médico debe estar llena !!")==false)
				    {
...
function ue_buscar()
{
	...
	if (li_leer==1)
	   {
	     f.operacion.value="";			
	     pagina="tepuy_sme_cat_tipo_servicio.php";
