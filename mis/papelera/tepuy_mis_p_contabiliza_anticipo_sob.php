<?Php
   ...
   $in_class_contabiliza->uf_load_seguridad("MIS","tepuy_mis_p_contabiliza_anticipo_sob.php",$ls_permisos,$la_seguridad,$la_permisos);
?>
  ...
  {  
		...
		for($li_i=1;$li_i<=$li_total_record;$li_i++)
		{
			if(array_key_exists("chksel".$li_i,$_POST))
			{
				...
				$lb_valido = $in_class_int->uf_procesar_contabilizar_anticipo( $ls_codcon, $ls_codant , $ls_fecha);
				if ($lb_valido) { $in_class_int->io_msg->message("El Anticipo Nº ".$ls_codant." fue contabilizado"); }
				else { $in_class_int->io_msg->message("Error en la contabilización del Anticipo Nº ".$ls_codcon ); }		
			}
		}
  }
  ...
  <table width="502" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    ...
    <tr>
	  ...
	  <?php
	     ...
	     $in_class_contabiliza->uf_select_sob_anticipos_contabilizar( $arr_object ,$li_total_record,0);
		 $io_grid->makegrid($li_total_record,$title,$arr_object,500,"Anticipos Emitidos","grdsep" );
	  ?>
	  </div></td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		f.action="tepuy_mis_p_contabiliza_anticipo_sob.php";
		f.submit();
	}
	else
	{
