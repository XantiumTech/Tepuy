<?php
	...
	function uf_insert_seguridad($as_titulo)
	{
		...
		$lb_valido=$io_fun_scg->uf_load_seguridad_reporte("SCG","tepuy_scg_r_comprobante_formato2.php",$ls_descripcion);
		return $lb_valido;
	}
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	if($lb_valido)
	{
		 $lb_valido=$io_report->uf_scg_reporte_select_comprobante_formato2($ls_cuentadesde,$ls_cuentahasta,$ldt_fecdes,
		                                                                   $ldt_fechas,$ls_orden,&$rs_data);
	}
	//---------------------------------------------------------------------------------------------------------------------------
	...
	else // Imprimimos el reporte
	{
	    ...
	    while($row=$io_sql->fetch_row($rs_data))
		{
			...
			$ldt_feccom=$row["fecha"];
			$ldt_fecha=$io_funciones->uf_convertirfecmostrar($row["fecha"]);
			...
			$lb_valido=$io_report->uf_scg_reporte_comprobante_formato2($ls_comprobante,$ls_procede,$ldt_feccom,$ls_orden,&$rs_detalle);
            if($lb_valido)
			{
				$li_row=$li_row+1;
