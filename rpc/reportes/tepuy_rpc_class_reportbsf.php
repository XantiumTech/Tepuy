<?php
class tepuy_rpc_class_reportbsf
{
	//-----------------------------------------------------------------------------------------------------------------------------------
	function tepuy_rpc_class_reportbsf($conn)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: tepuy_rpc_class_reportbsf
		//		   Access: public 
	...
	}// end function tepuy_rpc_class_reportbsf
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_select_proveedores($as_codemp,$ai_orden,$as_codprov1,$as_codprov2,&$lb_valido) 
	{
		...
		$ls_sql="SELECT P.codemp, P.cod_pro, P.nompro, P.dirpro, P.telpro, P.faxpro, P.nacpro, P.rifpro, P.nitpro, P.fecreg, ".
				...
				"       C.despai as pais, tepuy_estados.desest AS estado ".
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->tepuy_rpc_class_reportbsf; METODO->uf_select_proveedores; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_select_documentosproveedores($as_codemp,$as_codprov,$aa_documentos,$as_estatus,&$lb_valido) 
	{
		...
		if($li_total>1)
		{
			...
			if ($rs_data===false)
			{
				$this->io_msg->message("CLASE->tepuy_rpc_class_reportbsf; METODO->uf_select_documentosproveedores; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
				$lb_valido=false;
			}
			else
			{
