...
function tepuy_sob_c_anticipo()
{
	...
	require_once ("tepuy_sob_c_funciones_sob.php");
	$this->io_funnum= new tepuy_sob_c_funciones_sob(); 
	require_once("tepuy_sob_c_contrato.php");
	$this->io_contrato=new tepuy_sob_c_contrato();
...
function uf_select_cuentacontable($as_codcon,&$ls_cuenta,&$ls_dencuenta)
{
	...
	$ls_sql="SELECT p.sc_ctaant as cuenta, b.denominacion as dencuenta
			FROM rpc_proveedor p,sob_contrato c, sob_asignacion a, scg_cuentas b
			WHERE p.sc_ctaant=b.sc_cuenta AND p.codemp='".$ls_codemp."' AND c.codemp='".$ls_codemp."' AND a.codemp='".$ls_codemp."'
	...
	else
	{
		if($la_row=$this->io_sql->fetch_row($rs_data))
		{
			...
			$ls_dencuenta=$la_data["dencuenta"][1];
		}			
	}	
	return $lb_valido;
...
function uf_buscar_valuacion($as_codcon)
	{
		/***************************************************************************************/
...
function uf_guardar_anticipo ($as_codcon ,$as_codant ,$af_fecant,$af_fecintant,$ad_porant,$ad_monto,$as_conant,$ad_montotant,$as_sc_cuenta,$aa_seguridad,$as_numrecdoc)      
{ 
	...
	$lb_valido=false;
	$ls_codemp=$this->la_empresa["codemp"];
	$ls_sql="INSERT INTO sob_anticipo (codemp,codcon ,codant,numrecdoc,fecant,fecintant,porant,monto,conant,montotant,sc_cuenta,estant)
	         VALUES ('".$ls_codemp."','".$as_codcon."','".$as_codant."','".$as_numrecdoc."','".$af_fecant."','".$af_fecintant."','".$ad_porant."','".$ad_monto."','".$as_conant."','".$ad_montotant."','".$as_sc_cuenta."',1)";
...
function uf_update_anticipo($as_codcon, $as_codant,$af_fecant,$af_fecintant,$ad_porant,$ad_monto,$as_conant,$ad_montotant,$as_sc_cuenta,$aa_seguridad,$as_numrecdoc)
{
	//////////////////////////////////////////////////////////////////////////////
	...
	$ls_sql="UPDATE sob_anticipo
				 SET codcon='".$as_codcon."',fecant='".$af_fecant."',numrecdoc='".$as_numrecdoc."',
...
/*function uf_select_retenciones ($as_codcon,$as_codant,&$aa_data,&$ai_rows)
{
	...
	$ls_sql="SELECT r.codcon,r.codant, r.codded,d.dended as dended,d.sc_cuenta as cuenta, d.monded as deducible, r.codemp,d.formula
			FROM sob_retencionanticipo r, tepuy_deducciones d
...
function uf_select_retenciones ($as_codcon,$as_codant,&$aa_data,&$ai_rows)
{
	...
	$lb_valido=false;
	$ls_codemp=$this->la_empresa["codemp"];
	$ls_sql="SELECT r.codcon,r.codant, r.codded,r.codemp,d.dended,d.sc_cuenta as cuenta,d.monded as deducible,d.formula,r.monret,r.montotret 
			FROM sob_retencionanticipo r,tepuy_deducciones d
			WHERE r.codemp='".$ls_codemp."' AND  r.codcon='".$as_codcon."' AND r.codant='".$as_codant."' AND r.codded=d.codded";
	$rs_data=$this->io_sql->select($ls_sql);
	if($rs_data===false)
	{
		$this->is_msg_error="Error en select retencionesanticipo".$this->io_function->uf_convertirmsg($this->io_sql->message);
...
function uf_update_retenciones($as_codcon,$as_codant,$aa_retencionesnuevas,$ai_totalfilas,$aa_seguridad)
	{
		...
		for ($li_i=1;$li_i<$li_totalnuevas;$li_i++)
		{
			...
			if (!$lb_existe)
			{
				$this->uf_guardar_retenciones($as_codcon,$as_codant,$aa_retencionesnuevas["codret"][$li_i],$aa_retencionesnuevas["monret"][$li_i],$aa_retencionesnuevas["montotret"][$li_i],$aa_seguridad);
			}
			if ($lb_update)
			{
		...
		for ($li_j=1;$li_j<=$li_totalviejas;$li_j++)
		{
			...
			if (!$lb_existe)
			{
				$this->uf_delete_retenciones($as_codcon,$as_codant,$la_retencionesviejas["codded"][$li_j],$aa_seguridad);
			}
		}			
	}
  ...
  return $ls_codigo;	
	...
	function uf_procesar_recepcion_documentos($as_codemp,$as_numrecdoc,$as_codtipdoc,$as_conant,$ad_fecant,$ai_montotant,$ai_totreten,$as_codcon,$as_codant,$aa_seguridad)
	{
		...
		if(!$lb_existe)
		{
			...
			$ls_sql="INSERT INTO cxp_rd (codemp,numrecdoc,codtipdoc,ced_bene,cod_pro,dencondoc,fecemidoc, fecregdoc, fecvendoc,".
					...
					"               .$ai_montotant.",".$ai_totreten.",0,'".$ls_tipodestino."','".$as_factura."','R','SOBCON',0,0,'1900-01-01','',0,'--')";
			//print $ls_sql;
	...
	function uf_insert_recepcion_documento_contable($as_comprobante,$as_codtipdoc,$as_cedbene,$as_codpro,$ai_montotant,$as_codcon,$as_codant,$ai_totreten)
	{
		...
		if($lb_valido)
		{
			...
			if($lb_valido)
			{
				$ls_sql="SELECT tepuy_deducciones.sc_cuenta,sob_retencionanticipo.montotret,sob_retencionanticipo.codded".
						"  FROM sob_retencionanticipo,tepuy_deducciones ".
						...
						"   AND sob_retencionanticipo.codemp=tepuy_deducciones.codemp ".
						"   AND sob_retencionanticipo.codded=tepuy_deducciones.codded ";
				$rs_data=$this->io_sql->select($ls_sql);
				if($rs_data===false)
				{   
	...
	function uf_update_estatus_generacion_rd($as_codcon, $as_conant,$aa_seguridad)
	{
		...
		else
		{
			if($li_row>0)
			{
				/////////////////////////////////         SEGURIDAD               /////////////////////////////		
				$ls_evento="UPDATE";
				$ls_descripcion ="Actualizó el estatus de generacion de R.D. del Anticipo ".$as_codant." del Contrato ".$as_codcon." Asociado a la Empresa ".$this->ls_codemp;
				$lb_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
				/////////////////////////////////         SEGURIDAD               /////////////////////////////	
