...
if($ls_operacion=="GUARDARCON")
{
	...
	$lb_valido=$in_classcmp->uf_guardar_automatico($ls_comprobante,$ld_fecha,$ls_proccomp,$ls_desccomp,&$ls_prov,&$ls_bene,
	                                               $ls_tipo,2,0,$ls_codfuefin,$ls_coduniadm);
	if(!$lb_valido)
	{
...
/*		$lb_valido=$in_classcmp->uf_update_bsf_tepuycmpmd(0,$ls_codemp,$ls_proccomp,$ls_comprobante,
	...
	if($lb_valido)
	{
		...
		$lb_valido=$in_classcmp->uf_guardar_movimientos_contable($arr_cmp,$ls_cuenta,$ls_procede,$ls_descripcion,
		                                                         $ls_documento,$ls_operacioncon,$ldec_monto);
		if($lb_valido)
		{
...
/*			      $lb_valido=$in_classcmp->uf_update_bsf_scgdtcmp($ldec_monto,$ls_codemp,$ls_procede,$ls_comprobante,$ld_fecdb,
				                                                  $ls_cuenta,$ls_procede,$ls_documento,$ls_operacioncon,
                                                                  $la_security);*/
			}
			if($lb_valido){ $in_classcmp->io_sql->commit();}
			else{ $in_classcmp->io_sql->rollback();}
		}
		else
		{
			$in_classcmp->io_sql->rollback();
...
<table width="567" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
  ...
  <tr>
    ...
    <td><input name="txtcuenta" type="text" id="txtcuenta" readonly="true" value="<?php print $ls_cuentaplan ?>" size="20" style="text-align:center"> 
    <a href="javascript:catalogo_cuentasSCG();"><img src="../shared/imagebank/tools15/buscar.png" width="15" height="15" border="0" alt="Catálogo de Cuentas de Gastos"></a>	 <input name="txtdenominacion" type="text" class="sin-borde" id="txtdenominacion3" style="text-align:left" value="<?php print $ls_denominacion ?>" size="50" maxlength="254"></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td><input name="txtmonto" type="text" id="txtmonto" style="text-align:right" size="20" onKeyPress="return(currency_Format(this,'.',',',event))"  onBlur="javascript:uf_format(this);"> 
      <a href="javascript:aceptar_presupuestario();"><img src="../shared/imagebank/tools15/aprobado.png" alt="Agregar Detalle Presupuestario" width="15" height="15" border="0"></a> <a href="javascript:uf_close();"><img src="../shared/imagebank/tools15/eliminar.png" alt="Cancelar Registro de Detalle Presupuestario" width="15" height="15" border="0"></a></td>
  </tr>
  <tr>
    <td height="22">&nbsp;</td>
...
<script language="JavaScript">
  function aceptar_presupuestario()
  {
	...
	if((ls_cuenta!="")&&(ls_procedencia!="")&&(ls_documento!="")&&(ldec_monto!="")&&(ls_debhab!="")&&(ls_descripcion!=""))
	{
		...
		f.action="tepuy_w_regdt_adicionalscg.php";
		f.submit();
	}
	else
	{
