...
<?php
  ...
  $ls_sistema = "MIS";
  $ls_ventanas= "tepuy_mis_p_contabiliza_presoc.php";
  ...
  {  
		...
		for($li_i=1;$li_i<=$li_total_record;$li_i++)
		{
			if(array_key_exists("chksel".$li_i,$_POST))
			{
				...
				$lb_valido = $in_class->uf_contabilizar_precompromiso_compra( $ls_numordcom , $ls_fecha);
				if($lb_valido)		
				{
					$in_class->io_msg->message("La Compra fue Pre-Comprometida");
				}
				else
				{
  	 			    $in_class->io_msg->message("Error en el pre-compromiso de la compra.");
				}		
			}
		}
  }
 ...
 <p>&nbsp;</p>
  <table width="502" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
    <tr class="titulo-ventana">
      <td colspan="2">Precomprometer Ordenes de Compra </td>
    </tr>
    <tr>
    ...
    <tr>
	  ...
	  <?php
	     $title[1]="";$title[2]="Nº Orden";$title[3]="Fecha Orden de Compra";$title[4]="Concepto"; 
	     $in_class_contabiliza->uf_select_ordencompra_contabilizar( $arr_object ,$li_total_record,1);
		 $io_grid->makegrid($li_total_record,$title,$arr_object,500,"Compras Emitidas","grdsep" );
	  ?>
	  </div></td>
    </tr>
    <tr>
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		f.operacion.value ="PROCESAR";
  	    f.action="tepuy_mis_p_contabiliza_presoc.php";
		f.submit();
	}
	else
	{
