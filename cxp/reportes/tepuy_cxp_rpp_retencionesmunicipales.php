...
// pie de pagina articulo de retencion de Impuesto Municipal
	...
	$ls_tiporeten=$io_report->uf_obtengo_retencion("SELECT codcmp FROM cxp_contador WHERE codemp='$ls_codemp' AND tipo='M'",'C');
		...
		else
		{
			...
			for ($li_z=0;($li_z<$li_totrow)&&($lb_valido);$li_z++)
			{
				...
				if($lb_valido)
				{
					...
					$lb_valido=$io_report->uf_retencionesmunicipales_detalles($ls_numcom,$ls_tiporeten,2);
					if($lb_valido)
					{
						$li_totalbaseimp=0;
						$li_totalmontoimp=0;
