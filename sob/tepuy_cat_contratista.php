...
$io_casig=new tepuy_sob_class_asignacion();
...
{
 $ls_cadena=" SELECT cod_pro,nompro,nomreppro ".
			"   FROM rpc_proveedor ".
			...
			else
			{
				if($row=$io_sql->fetch_row($rs_datauni))
				{
					...
					for($z=1;$z<=$totrow;$z++)
					{
						...
						{
							case "":
								print "<td><a href=\"javascript: aceptar('$codpro','$nompro','$ls_nomreppro');\">".$codpro."</a></td>";
...
<script language="JavaScript">
	...
	function aceptar(codp,nomp,representante)
	{
		opener.ue_cargarcontratista(codp,nomp,representante);
		close();
	}
	function ue_search()
	{
		...
		f.action="tepuy_cat_contratista.php?tipo=<?php print $ls_tipo;?>";
		f.submit();
	}
