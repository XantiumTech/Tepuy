<?php
	...
	$io_fun_nomina->uf_load_seguridad("SNR","tepuy_snorh_d_proyecto.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
...
<?php 
	...
	switch ($ls_operacion) 
	{
		case "GUARDAR":
			uf_load_variables();
			$lb_valido=$io_proyecto->uf_guardar($lb_existe,$ls_codproy,$ls_nomproy,$ls_codpro,$la_seguridad);
			if($lb_valido)
			{
				uf_limpiarvariables();
				$lb_existe="FALSE";
...
function ue_nuevo()
{
	...
	{	
		...
		f.action="tepuy_snorh_d_proyecto.php";
		f.submit();
   	}
	else
   	{
...
function ue_buscar()
{
	...
	if (li_leer==1)
   	{
		window.open("tepuy_snorh_cat_proyecto.php","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=530,height=400,left=50,top=50,location=no,resizable=yes");
   	}
	else
   	{
 		alert("No tiene permiso para realizar esta operacion");
