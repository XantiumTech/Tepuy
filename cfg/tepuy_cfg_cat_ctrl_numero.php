...
if ($ls_operacion=="BUSCAR")
   {
		$ls_sql= " SELECT codsis,procede,id,prefijo,nro_inicial,nro_final,maxlen,nro_actual  ".
		         " FROM tepuy_ctrl_numero ".
		        // " WHERE id like '".$ls_codigo."' AND codusu='".$ls_codusu."' order by id asc";
				 " WHERE id like '".$ls_codigo."' ".
				 "group by codsis,procede,id,prefijo,nro_inicial,nro_final,maxlen,nro_actual ".
				 "order by id asc"; 
			 ...
			 for ($z=1;$z<=$totrow;$z++)
				 {
				  ...
				  // $ls_descripcion=$data["desproc"][$z];
				   print "<td><a href=\"javascript: aceptar('$ls_codsis','$ls_procede','$ls_codigo','$ls_prefijo','$ls_numini','$ls_numfin','$ls_maxlen','$ls_numact');\">".$ls_codigo."</a></td>";
...
<script language="JavaScript">
  function aceptar(sistema,procede,codigo,prefijo, numini,numfin,maxlen,numact)
  {
  ...
  function ue_search()
  {
    ...
    f.action="tepuy_cfg_cat_ctrl_numero.php";
    f.submit();
  }
</script>
</html>