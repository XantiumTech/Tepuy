<?php
class tepuy_sno_c_cierre_periodo3
{
	var $io_sql;
	var $io_mensajes;
	var $io_funciones;
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function tepuy_sno_c_cierre_periodo3()
	{	
		//////////////////////////////////////////////////////////////////////////////
		//	     Function: tepuy_sno_c_cierre_periodo3
		...
		require_once("tepuy_sno_c_prestamocuotas.php");
        $this->io_cuota=new tepuy_sno_c_prestamocuotas();		
		$this->ls_codemp=$_SESSION["la_empresa"]["codemp"];
        $this->ls_codnom=$_SESSION["la_nomina"]["codnom"];
	}// end function tepuy_sno_c_cierre_periodo3
	//-----------------------------------------------------------------------------------------------------------------------------------
	...
	function uf_delete_prestamoamortizado($as_codperi)
	{
	    ...
	    //     Creado por: Ing. Miguel Palencia
	    // Fecha Creación: 17/02/2006     
		// Modificado Por: Ing. Miguel Palencia						Fecha Última Modificación : 01/12/2006
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql="DELETE ".
	...
	function uf_insert_unidadadmin($adt_anocurnom,$as_codperi_abrir)
	{
		...
		$ls_sql=" INSERT INTO sno_unidadadmin (codemp,minorguniadm,ofiuniadm,uniuniadm,depuniadm,prouniadm,desuniadm,codprouniadm) ".
                "      SELECT codemp,minorguniadm,ofiuniadm,uniuniadm,depuniadm,prouniadm,desuniadm,codprouniadm ".
	...
	function uf_insert_proyecto($adt_anocurnom,$as_codperi_abrir)
	{
		...
		$ls_sql=" INSERT INTO sno_proyecto (codemp, codproy, nomproy, estproproy) ".
                "      SELECT codemp, codproy, nomproy, estproproy ".
                "        FROM sno_hproyecto ".
                "       WHERE codemp='".$this->ls_codemp."' ".
				"		  AND codnom='".$this->ls_codnom."' ".
				"         AND anocur='".$adt_anocurnom."' ".
	...
	function uf_insert_personalnomina($adt_anocurnom,$as_codperi_abrir)
	{
		...
		$ls_sql= "INSERT INTO sno_personalnomina (codemp, codnom, codper,codsubnom, codtab, codasicar, ".
				 ...
				 "            fecsusper,cauegrper,codescdoc,codcladoc,codubifis,tipcestic,conjub,catjub,codclavia, codunirac, pagtaqper, fecascper, grado ".
                 "       FROM sno_hpersonalnomina ".
                 " 		WHERE codemp='".$this->ls_codemp."' ".
				 "		  AND codnom='".$this->ls_codnom."' ".
				 "        AND anocur='".$adt_anocurnom."' ".
	...
	function uf_insert_asignacioncargo($adt_anocurnom,$as_codperi_abrir)
	{
		...
		$ls_sql= "INSERT INTO sno_asignacioncargo (codemp, codnom, codasicar, denasicar, claasicar, codtab, codpas, codgra, ".
				 ...
				 "                                 ofiuniadm, uniuniadm, depuniadm, prouniadm) ".
	...
	function uf_insert_vacacpersonal($adt_anocurnom,$as_codperi_abrir)
	{
		...
		$ls_sql="INSERT INTO sno_vacacpersonal(codemp, codper, codvac, fecvenvac, fecdisvac, fecreivac, diavac, ".
                ...
                "            Monto_3,periodo_4,cod_4,nro_dias_4,Monto_4,periodo_5,cod_5,nro_dias_5,Monto_5 ".
	...
	function uf_insert_concepto($adt_anocurnom,$as_codperi_abrir)
	{
		...
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql= "INSERT INTO sno_concepto (codemp, codnom, codconc, nomcon, conprep, titcon, sigcon, forcon, ".
                 ...
                 "     SELECT codemp,codnom,codconc,nomcon,conprep,titcon,sigcon,forcon,glocon,acumaxcon,valmincon,valmaxcon,concon, ".
				 "           cueprecon,cueconcon,aplisrcon,sueintcon,intprocon,codpro,codpro1,forpatcon,cueprepatcon,cueconpatcon, ".
	...
	function uf_insert_constante($adt_anocurnom,$as_codperi_abrir)
	{
		...
		$ls_sql="INSERT INTO sno_constante (codemp, codnom, codcons, nomcon, unicon, equcon,topcon, valcon, reicon, tipnumcon) ".
                "     SELECT codemp,codnom,codcons,nomcon,unicon,equcon,topcon,valcon,reicon,tipnumcon ".
                "       FROM sno_hconstante ".
                "      WHERE codemp='".$this->ls_codemp."' ".
				"		 AND codnom='".$this->ls_codnom."' ".
				"        AND anocur='".$adt_anocurnom."' ".
	...
	function uf_insert_constantepersonal($adt_anocurnom,$as_codperi_abrir)
	{
		...
		$ls_sql= "INSERT INTO sno_constantepersonal (codemp, codnom, codper, codcons, moncon) ".
                 "     SELECT codemp,codnom,codper,codcons,moncon ".
