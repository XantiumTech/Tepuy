<?php
class tepuy_saf_c_activoanexos
{
	var $obj="";
	var $io_sql;
	var $siginc;
	...
	function tepuy_saf_c_activoanexos()
	{
	...
	function uf_saf_select_activomantenimiento($as_codemp,$as_codact,&$as_numconman,&$as_codproman,&$as_denproman,&$ad_feciniman,&$ad_fecfinman)
	{
		...
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$ad_feciniman=$row["feciniman"];
				$ad_fecfinman=$row["fecfinman"];
			}
		}
		$this->io_sql->free_result($rs_data);
		return $lb_valido;
	...
	function uf_saf_select_activorotulacion($as_codemp,$as_codact,&$as_codrot,&$as_denrot,&$as_codprorot,&$as_denprorot,&$ad_fecrot)
	{
		...
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				$as_codprorot=$row["codproman"];
				$as_denprorot=$row["denproman"];
				$ad_fecrot=$row["fecrot"];
			}
		}
		$this->io_sql->free_result($rs_data);
		return $lb_valido;
...
}//fin de la class tepuy_saf_c_activosanexos
?>
