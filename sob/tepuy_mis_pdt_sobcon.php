<?php
   ...
   function uf_imprimirresultados($as_codcom)
   {
		...
		$ls_group="";
		$ls_criterio="";
		$ls_sql="SELECT sob_contrato.obscon, sob_asignacion.cod_pro, sob_contrato.codasi, sob_asignacion.obsasi, ".
		...
		else
		{
			...
			{
				$ls_descripcion=$row["obscon"];
				$ls_codasi=$row["codasi"];
				$ls_desasi=$row["codasi"]." - ".$row["obsasi"];
...
<?php
	...
	uf_imprimirresultados($ls_codcon);
?>
</div>
</form>
</body>
