			  ...
			  while($row=$io_sql->fetch_row($rs_data))
			       {
					 ...
					 $ls_codtipcol = $row["codtipcol"];
					 $ld_feccol    = $io_funcion->uf_convertirfecmostrar($row["feccol"]);
					 $ld_fecvencol = $io_funcion->uf_convertirfecmostrar($row["fecvencol"]);
					 $ld_monto	   = $row["monto"];
					 $ld_tascol	   = $row["tascol"];
					 $ls_scgcta	   = $row["sc_cuenta"];
					 $li_diacol	   = $row["diacol"];
...
function aceptar(ls_numcol,ls_dencol,ls_ctaban,ls_dencta,ls_scgcta,ld_tascol,ld_monto)
{
  ...
  fop.txtmonto.value		= uf_convertir(ld_monto);
  close();
}
  ...
  f.action="tepuy_cat_colocaciones.php";
  f.submit();
  }
</script>
</html>