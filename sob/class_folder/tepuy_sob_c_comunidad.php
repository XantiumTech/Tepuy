<?php
class tepuy_sob_c_comunidad
{

var $ls_sql;
var $is_msg_error;
	
	function tepuy_sob_c_comunidad($conn)
	{
...
function uf_insert_comunidad($as_codpais,$as_codest,$as_codmun,$as_codpar,$as_codcom,$as_nomcom,$aa_seguridad) 
{
//////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////
	...
	$ls_sql = " INSERT INTO tepuy_comunidad                                                                                   ".
			  " (codpai,codest,codmun,codpar,codcom,nomcom)                                                                    ".
			  " VALUES ('".$as_codpais."','".$as_codest."','".$as_codmun."','".$as_codpar."','".$as_codcom."','".$as_nomcom."')";
	$this->io_sql->begin_transaction();
	$rs_data=$this->io_sql->execute($ls_sql);
	if ($rs_data===false)
	   {
	     $lb_valido=false;
		 $this->io_msg->message("CLASE->tepuy_sob_C_COMUNIDAD; METODO->uf_insert_comunidad; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	   }
	else
	   {
		 /////////////////////////////////         SEGURIDAD               /////////////////////////////		
		 $ls_evento      = "INSERT";
		 $ls_descripcion ="Insertó en CFG la Comunidad ".$as_nomcom." con código ".$as_codcom;
...
function uf_update_comunidad($as_codpais,$as_codest,$as_codmun,$as_codpar,$as_codcom,$as_nomcom,$aa_seguridad) 
{
//////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////
	...
	$ls_sql=" UPDATE tepuy_comunidad                                                                       ".
			" SET  nomcom='".$as_nomcom."'                                                                  ".
			" WHERE codpai='".$as_codpais."' AND codest ='".$as_codest."'                                    ".
			" AND codmun='".$as_codmun."'    AND codpar ='" .$as_codpar. "' AND codcom = '" .$as_codcom. "'";
	$this->io_sql->begin_transaction();
	$rs_data=$this->io_sql->execute($ls_sql);
	if ($rs_data===false)
	   {
	     $lb_valido=false;
		 $this->io_msg->message("CLASE->tepuy_sob_C_COMUNIDAD; METODO->uf_update_comunidad; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	   }
	else
	   {
		 $lb_valido=true;
		 /////////////////////////////////         SEGURIDAD               /////////////////////////////		
		 $ls_evento      = "UPDATE";
		 $ls_descripcion = "Actualizó en CFG la Comunidad con código ".$as_codcom.
...
function uf_delete_comunidad($as_codemp,$as_codpais,$as_codest,$as_codmun,$as_codpar,$as_codcom,$as_nomcom,$aa_seguridad)
{
///////////////////////////////////////////////////////////////////////////////////////////////
//	    Metodo:  uf_delete_comunidad
//	    Access:  public
//	 Arguments:  $as_codpais,$as_codest,$as_codmun,$as_codpar,$as_codcom,$as_nomcom,$aa_seguridad
//	   Returns:	 $lb_valido	
// Description: Funcion que se encarga de eliminar una parroquia de un municipio especifico . 
////////////////////////////////////////////////////////////////////////////////////////////////

  $lb_valido = false;
  $ls_sql    = "DELETE FROM tepuy_comunidad                                                            ".
		       " WHERE codpai='".$as_codpais."' AND codest='".$as_codest."' AND                         ". 
               "       codmun='".$as_codmun."'  AND codpar='".$as_codpar."' AND codcom='".$as_codcom. "'";
  $this->io_sql->begin_transaction();
  $rs_data=$this->io_sql->execute($ls_sql);
  if ($rs_data===false)
	 {
	   $lb_valido=false;
	   $this->io_msg->message("CLASE->tepuy_sob_C_COMUNIDAD; METODO->uf_delete_comunidad; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	 }
  else
	 {
	   $lb_valido=true;
	   /////////////////////////////////         SEGURIDAD               /////////////////////////////		
	   $ls_evento="DELETE";
	   $ls_descripcion ="Eliminó en CFG la Comunidad ".$as_nomcom." con código ".$as_codcom;
	   $ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
	   $aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
	   $aa_seguridad["ventanas"],$ls_descripcion);
	   /////////////////////////////////         SEGURIDAD               ///////////////////////////
...
function uf_select_comunidad($as_codpais,$as_codest,$as_codmun,$as_codpar,$as_codcom) 
{
//////////////////////////////////////////////////////////////////////////////
//	     Metodo:  uf_select_comunidad
//	     Access:  public
//	  Arguments:  $as_codpais,$as_codest,$as_codmun,$as_codpar,$as_codcom
//	    Returns:  $lb_valido		
...
//////////////////////////////////////////////////////////////////////////////
	...
	$lb_valido = false;
	$ls_sql    = " SELECT * FROM tepuy_comunidad                                                        ".
			     " WHERE codpai='".$as_codpais."' AND codest='".$as_codest."' AND                        ".
			     "       codmun='".$as_codmun."'  AND codpar='".$as_codpar."' AND codcom='".$as_codcom."'";
	...
	if ($rs_data===false)
	   {
		  $lb_valido=false;
     	  $this->io_msg->message("CLASE->tepuy_sob_C_COMUNIDAD; METODO->uf_select_comunidad; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	   }
	else
	   {
...
//	    Access:  public
//	   Returns:	 $lb_valido.	
// Description:  Funcion que se encarga de extraer todos aquellos registro de la tabla tepuy_pais
//               para ser cargados en un objeto de tipo combobox/list menu. 
//////////////////////////////////////////////////////////////////////////////

	$ls_sql=" SELECT * FROM tepuy_pais WHERE codpai='058' ORDER BY codpai ASC";
	$rs_data=$this->io_sql->select($ls_sql);
	if ($rs_data===false)
	   {
		  $lb_valido=false;
     	  $this->io_msg->message("CLASE->tepuy_sob_C_PARROQUIA; METODO->uf_llenarcombo_pais; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	   }
	else
	   {
         $li_numrows=$this->io_sql->num_rows($rs_data);	   
...
//	    Access:  public
//	   Returns:	 $lb_valido.	
// Description:  Funcion que se encarga de extraer todos aquellos registro de la tabla tepuy_estado
//               para ser cargados en un objeto de tipo combobox/list menu. 
//////////////////////////////////////////////////////////////////////////////

	$ls_sql=" SELECT * FROM tepuy_estados WHERE codpai='".$as_codpais."' AND codest <>'---' ORDER BY desest ASC ";
	$rs_data=$this->io_sql->select($ls_sql);
	if ($rs_data===false)
	   {
		  $lb_valido=false;
     	  $this->io_msg->message("CLASE->tepuy_sob_C_COMUNIDAD; METODO->uf_llenarcombo_estado; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	   }
	else
	   {
         $li_numrows=$this->io_sql->num_rows($rs_data);	   
...
//	    Access:  public
//	   Returns:	 $lb_valido.	
// Description:  Funcion que se encarga de extraer todos aquellos registro de la tabla tepuy_municipio
//               para ser cargados en un objeto de tipo combobox/list menu. 
//////////////////////////////////////////////////////////////////////////////

	$ls_sql=" SELECT * FROM tepuy_municipio WHERE codpai='".$as_codpai."' AND codest='".$as_codest."' AND codmun <>'---' ORDER BY denmun";
	$rs_data=$this->io_sql->select($ls_sql);
	if ($rs_data===false)
	   {
		  $lb_valido=false;
     	  $this->io_msg->message("CLASE->tepuy_sob_C_PARROQUIA; METODO->uf_llenarcombo_municipio; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
	   }
	else
	   {
         $li_numrows=$this->io_sql->num_rows($rs_data);	   
...
function uf_select_parroquia($as_codpai,$as_codest,$as_codmun)
{
 ...
 //////////////////////////////////////////////////////////////////////////////
   ...
   $ls_sql  = " SELECT * FROM tepuy_parroquia WHERE codpai='".$as_codpai."' AND codest='".$as_codest."' AND codmun='".$as_codmun."' ORDER BY codpar ";
   $rs_data = $this->io_sql->select($ls_sql);
   if ($rs_data===false)
	  {
	    $lb_valido = false;
...
function uf_generar_codigo($ab_empresa,$as_codemp,$as_tabla,$as_columna,$as_columna2,$as_columna3,$as_columna4,$as_columna5)
{ 
//////////////////////////////////////////////////////////////////////////////
			   ...
			   {	
				...
				else
				{
					  $ls_sql=" SELECT ".$as_columna." FROM ".$as_tabla." WHERE codpai='".$as_columna2."' AND      ".
					          " codest='".$as_columna3."' AND codmun='".$as_columna4."' AND codpar='".$as_columna5."'".
...
// Fecha de Creación:  02/08/2014      
//	  Modificado Por:  Ing. Miguel Palencia
// 		Fecha Modif.:  02/10/2014
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// 
      $lb_existe = false;
	   switch ($this->io_gestor)
	   {
